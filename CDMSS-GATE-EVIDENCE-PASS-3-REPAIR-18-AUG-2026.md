# CDMSS gate evidence — pass 3 REPAIR (proofs 35, 45, 47, 56 under Saul review 36), 18 August 2026
Authority: Saul review 36 (closed 46 and 49; held 35, 45, 47, 56). Addendum v25, AUTHORIZED by the orchestrator on V's
delegation, ratified by Saul review 35; supersedes v23. There is no digest and no signature script; stop condition 3 —
the v25 status line reads AUTHORIZED by the orchestrator — was checked instead.
The repair commit (commit 15): f391973045c70f40edc77ceeedb053310aaa1f52 — four paths: lib/retrieval-telemetry-core.ts
(the two validator repairs, v25 §3.2 and §3.3), lib/__tests__/retrieval-telemetry-core.test.ts (proof 35),
...
(proof 56). Forward-only from a4ef66e; commits 13 and 14 stand.
Gate run: 2026-08-18T16:09:54Z to 2026-08-18T16:11:28Z UTC, from commit 15.
Mutation table (NINE rows) run BEFORE the gate, completed before commit 15 was created; the gate then ran once, from
commit 15. Upstream throughout: 01c2375; nothing pushed.

Raw capture directory: $HOME/cdmss-pass3-repair-gate-18-aug-2026
...
⚠️ THE IGNORED CHECK SHOWS FOUR LINES, NOT THREE. Besides v25, review 35 and review 36, addendum v24 sits in the worktree
root as an ignored, untracked file. v25 §6 says v24 is not tracked and will not be; the builder neither tracked nor
deleted it. It appears in every ignored listing below and it is why the post-commit-16 no-ignored-CDMSS check cannot exit
zero (Part 3). Reported, not worked around.
...
## Part 1 — the mutation table: nine rows, BEFORE the gate (kickoff §4; v25 §3)

The kickoff's six required rows are rows 1 to 6, in its order. Rows 7 to 9 add one row per further load-bearing claim
(kickoff §4: "Add rows wherever a repair has more than one load-bearing claim"). Sandbox per the established shape
...
`lib/__tests__` present with all three pass 3 test files, `.git` absent, `.next` absent, `node_modules` a symlink; the four
changed files AND lib/rerank.ts, lib/opd-note-audit.ts, lib/retrieval-capture.ts `cmp`-identical to the worktree); deleted
after the table. No git command inside the sandbox. Each row applies ONE exact string replacement to the sandbox copy of
the named file, records `diff -u` of the worktree original against the mutated sandbox copy, runs ONLY its named test
file, and restores the file with `cp`, verified with `cmp`.

⚠️ THIS ROUND EVERY ROW RUNS WITH THE SANDBOX AS cwd (`cd <sandbox> && node --test --import tsx <test file>`), not the
worktree as in the previous rounds: proof 47.6 is a comment-stripped SOURCE PIN that reads `lib/opd-note-audit.ts`
relative to cwd, and 35.8 reads `lib/rerank.ts` for JUDGE_BATCH the same way, so the sandbox must be the cwd for a
sandbox mutation of either file to be what the test sees. `node_modules` resolves through the sandbox's symlink.

⚠️ RECORDER GUARDRAIL (review 34's standing condition; kickoff §4). NO ROW alters judge request lifecycle or recorder
counting. Rows 1 and 7 mutate the soft-failure record SYNTHESIS in `recordSoftFailure` in the sandbox copy of
`lib/rerank.ts` — code that runs after a backend has thrown, issues no request and touches no recorder; row 5 mutates the
context assembly in the sandbox copy of `lib/opd-note-audit.ts`; row 8 the sandbox copy of `lib/retrieval-capture.ts`;
rows 2, 3, 4, 6, 9 `lib/retrieval-telemetry-core.ts`. Only the three pass 3 test files ran, none of which imports
`lib/__tests__/judge-server-stub.ts`. (Proof 35.8 imports `lib/rerank.ts` dynamically with injected throwers and a no-op
health check; no socket is opened. Proof 56 opens a loopback socket to its own disposable PostgreSQL only.)

### The byte-equality proof (kickoff §4) — four files, four hashes each

Hash 1 (worktree, before the table) · Hash 2 (sandbox baseline) · Hash 3 (worktree, after the table) · Hash 4
(`git show HEAD:<path> | shasum -a 256` after commit 15):
...
fc1e311888a65dded8889489c4bc7ab55f6449f6a93591180e53f6ad2c321e79  lib/__tests__/retrieval-telemetry-validation.test.ts
0151d53fd591bda751e57a0a272a84f355d466f1552c873233112d3ae535c15e  lib/__tests__/retrieval-telemetry-canonicalization.test.ts
# hash 4 — git show HEAD:<path> after commit 15
803ad74ff38c3a6829d3450ba591aa8b063185d751b08fd7c670a08ce0107080  lib/retrieval-telemetry-core.ts
72f78341cd43c871e09d884dc3c108be2a939c4a66f16fed2699b06b83cad6a3  lib/__tests__/retrieval-telemetry-core.test.ts
fc1e311888a65dded8889489c4bc7ab55f6449f6a93591180e53f6ad2c321e79  lib/__tests__/retrieval-telemetry-validation.test.ts
0151d53fd591bda751e57a0a272a84f355d466f1552c873233112d3ae535c15e  lib/__tests__/retrieval-telemetry-canonicalization.test.ts
...
All four agree for every file. The tested bytes are the committed bytes. The nine rows were run ONCE, against these bytes.

### Row 1 — synthesise exactly ONE soft-failure record regardless of the planned boundary count (recordSoftFailure in the SANDBOX copy of lib/rerank.ts — synthesis after a throw; no request is issued and no recorder is touched)

mutated file: lib/rerank.ts (sandbox copy). test file: lib/__tests__/retrieval-telemetry-core.test.ts (run with the sandbox as cwd). must fail by name: 35.8.
...
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/rerank.ts	2026-08-18 21:38:56
@@ -382,9 +382,7 @@
...
+    const boundaries = [{ start: 0, end: candidates.length }];
...
# fail 1
# cancelled 0
# duration_ms 365.735459
```

### Row 2 — drop the prompt_tokens validation

mutated file: lib/retrieval-telemetry-core.ts (sandbox copy). test file: lib/__tests__/retrieval-telemetry-validation.test.ts (run with the sandbox as cwd). must fail by name: batch.prompt_tokens: missing, batch.prompt_tokens: invalid, 45.1.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-18 21:32:44
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-18 21:38:56
@@ -793,7 +793,7 @@
...
+      const usageFields: Array<'prompt_tokens' | 'completion_tokens'> = ['completion_tokens'];
...
exit=1
STARTED: 2026-08-18T16:08:56Z
ENDED: 2026-08-18T16:08:57Z
```

Failing tests, by name:

```text
not ok 76 - 45.76 — batch.prompt_tokens: missing (null is permitted) — v25 §3.2, unvalidated until the pass 3 repair → batch_prompt_tokens_field_absent
not ok 77 - 45.77 — batch.prompt_tokens: invalid: a numeric STRING → batch_prompt_tokens_invalid
not ok 78 - 45.78 — batch.prompt_tokens: invalid number: negative → batch_prompt_tokens_invalid
not ok 79 - 45.79 — batch.prompt_tokens: invalid number: NaN → batch_prompt_tokens_invalid
not ok 110 - 45.1 — OWN-PROPERTY CHECKS: missing, explicit null, empty array, empty string and invalid number are FIVE different answers, and the validator tells them apart
```
...
# fail 5
# cancelled 0
# duration_ms 1019.414709
```

### Row 3 — drop the completion_tokens validation

mutated file: lib/retrieval-telemetry-core.ts (sandbox copy). test file: lib/__tests__/retrieval-telemetry-validation.test.ts (run with the sandbox as cwd). must fail by name: batch.completion_tokens: missing, batch.completion_tokens: invalid, 45.1.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-18 21:32:44
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-18 21:38:57
@@ -793,7 +793,7 @@
       // carry a string, and the manifest still validated clean — a malformed manifest classified as
       // complete. That defect shipped in commit 13 and no test asked. Same pattern as
       // `rerank_temperature` and `active_backfill_state` above and below: `has`, then null, then type.
-      const usageFields: Array<'prompt_tokens' | 'completion_tokens'> = ['prompt_tokens', 'completion_tokens'];
+      const usageFields: Array<'prompt_tokens' | 'completion_tokens'> = ['prompt_tokens'];
       for (const usage of usageFields) {
         if (!has(b, usage)) { v.push(`batch_${usage}_field_absent`); continue; }
         const t = b[usage];
```
...
cd /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo && node --test --import tsx lib/__tests__/retrieval-telemetry-validation.test.ts
exit=1
STARTED: 2026-08-18T16:08:57Z
ENDED: 2026-08-18T16:08:58Z
```

Failing tests, by name:

```text
not ok 80 - 45.80 — batch.completion_tokens: missing (null is permitted) — v25 §3.2, unvalidated until the pass 3 repair → batch_completion_tokens_field_absent
not ok 81 - 45.81 — batch.completion_tokens: invalid: a numeric STRING → batch_completion_tokens_invalid
not ok 82 - 45.82 — batch.completion_tokens: invalid number: Infinity → batch_completion_tokens_invalid
not ok 83 - 45.83 — batch.completion_tokens: invalid: an object → batch_completion_tokens_invalid
not ok 110 - 45.1 — OWN-PROPERTY CHECKS: missing, explicit null, empty array, empty string and invalid number are FIVE different answers, and the validator tells them apart
```
...
# fail 5
# cancelled 0
# duration_ms 494.116916
```

### Row 4 — accept a non-null scorer-context HMAC on a non-primary role (the v25 §3.3 rejection removed)

mutated file: lib/retrieval-telemetry-core.ts (sandbox copy). test file: lib/__tests__/retrieval-telemetry-validation.test.ts (run with the sandbox as cwd). must fail by name: 47.4.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-18 21:32:44
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-18 21:38:58
@@ -849,9 +849,6 @@
...
```text
not ok 117 - 47.4 — a NON-NULL scorer-context HMAC on any of the four non-primary roles is REJECTED (v25 §3.3): scorer_context_hmac_on_non_primary_role — and primary is untouched
```
...
# fail 1
# cancelled 0
# duration_ms 495.934791
```

### Row 5 — bypass assembleAuditContext in the production caller path (the SANDBOX copy of lib/opd-note-audit.ts assembles sources and citedContext directly, so the exact-bytes handoff is gone)

mutated file: lib/opd-note-audit.ts (sandbox copy). test file: lib/__tests__/retrieval-telemetry-validation.test.ts (run with the sandbox as cwd). must fail by name: 47.6.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/opd-note-audit.ts	2026-08-15 19:44:51
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/opd-note-audit.ts	2026-08-18 21:38:58
@@ -1601,7 +1601,8 @@
...
# fail 1
# cancelled 0
# duration_ms 495.991792
```

### Row 6 — reorder an array and treat it as equal: canonicalize SORTS arrays, so [12,11,13] and [11,12,13] canonicalize identically

mutated file: lib/retrieval-telemetry-core.ts (sandbox copy). test file: lib/__tests__/retrieval-telemetry-canonicalization.test.ts (run with the sandbox as cwd). must fail by name: 56.4, 56.1.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-18 21:32:44
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-18 21:38:59
@@ -1126,7 +1126,7 @@
   }
   if (typeof value !== 'object') return value;
   if (Array.isArray(value)) {
-    return value.map((el) => {
+    return [...value].sort().map((el) => {
...
exit=1
STARTED: 2026-08-18T16:08:59Z
ENDED: 2026-08-18T16:09:00Z
```

Failing tests, by name:

```text
not ok 2 - 56.1 — RECURSIVE canonicalization: keys are sorted at EVERY depth, inside objects and inside objects nested in arrays, and array order is preserved
not ok 5 - 56.4 — ARRAY REORDER IS NOT EQUAL: array order is content, at the top level and nested — for canonicalJson AND for the jsonb PostgreSQL holds; a reordered array round-tripped through the database is a DIFFERENT document
```
...
# fail 2
# cancelled 0
# duration_ms 1337.6255
```

### Row 7 — the synthesised records keep their COUNT but lose their exact boundaries (every record starts at 0) — proves 35.8 asserts WHICH boundaries, not how many
...
```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/rerank.ts	2026-08-15 20:58:10
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/rerank.ts	2026-08-18 21:39:00
@@ -422,7 +422,7 @@
     };
...
cd /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo && node --test --import tsx lib/__tests__/retrieval-telemetry-core.test.ts
exit=1
STARTED: 2026-08-18T16:09:00Z
ENDED: 2026-08-18T16:09:01Z
```
...
# fail 1
# cancelled 0
# duration_ms 227.720375
```

### Row 8 — the empty-string citedContext yields null instead of the HMAC of the empty string (the SANDBOX copy of lib/retrieval-capture.ts; the empty case proof 47 names)

mutated file: lib/retrieval-capture.ts (sandbox copy). test file: lib/__tests__/retrieval-telemetry-validation.test.ts (run with the sandbox as cwd). must fail by name: 47.5, 47.2.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-capture.ts	2026-08-18 19:16:52
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-capture.ts	2026-08-18 21:39:01
@@ -361,7 +361,7 @@
...
```text
not ok 115 - 47.2 — the EMPTY-STRING case: zero candidates render an empty citedContext, and the HMAC of the empty string is a DEFINED value — never null because reranking was skipped
not ok 118 - 47.5 — through the REAL assembleAuditContext: the primary HMAC is the keyed HMAC of the EXACT rendered citedContext (literature only, and literature plus the normative block), and with zero hits the rendered context is the EMPTY STRING whose HMAC is a defined value
not ok 120 - 49.1 — EMPTY FUSION: retrieve() returns before the rerank block exists — fused 0, hydrated 0, expected 0, recorded 0, batches [], backend and model none
not ok 121 - 49.2 — HYDRATE EMPTIED: fused > 0, hydrated 0 — the same shape, and the TWO COUNTS DIFFER, which is the point
```
...
# fail 4
# cancelled 0
# duration_ms 495.483583
```

### Row 9 — the usage TYPE check dropped: a present, non-null prompt/completion value of any type validates clean

mutated file: lib/retrieval-telemetry-core.ts (sandbox copy). test file: lib/__tests__/retrieval-telemetry-validation.test.ts (run with the sandbox as cwd). must fail by name: 45.77, 45.78, 45.79 (prompt_tokens invalid), 45.81, 45.82, 45.83 (completion_tokens invalid)  (the runner's initial expectation also listed 45.1, which asserts absence, null and zero — not type — so it is untouched by this mutation; corrected here, not hidden).

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-18 21:32:44
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-18 21:39:01
@@ -797,7 +797,7 @@
       for (const usage of usageFields) {
         if (!has(b, usage)) { v.push(`batch_${usage}_field_absent`); continue; }
         const t = b[usage];
-        if (t !== null && !(isFiniteNum(t) && t >= 0)) v.push(`batch_${usage}_invalid`);
+        if (t !== null && false) v.push(`batch_${usage}_invalid`);
       }
...
# fail 6
# cancelled 0
# duration_ms 496.947208
```

Every row failed its named test by name; every run completed in under two seconds; no row was accepted on a file-level
timeout. Row 9's description file records a corrected expectation (45.1 was over-listed by the runner; its six named
failures are the invalid-value rows) — stated there, not hidden.
...
used anywhere in the gate. ⚠️ `lib/retrieval-telemetry-core.ts` is in the import graph and changed in this commit; the map
did NOT move — both generations wrote 90492 "bytes" (UTF-16 code units; `wc -c` 90494) and `git diff --exit-code` exited 0.
...
!! .next/
!! .vercel/
!! CDMSS-RERANK-TELEMETRY-ADDENDUM-v24-18-AUG-2026.md
...
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 81ffcf4b-2154-45b0-93dd-454a33a7243f
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall bd6bc493-f6aa-4023-a144-e58d18245e3c
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 85ab63e3-8678-4b59-babd-310c7a4d3d16
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall da0bd5b1-f2c7-4737-8433-dd1a9ed5863d
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 9236cf2b-43e7-4476-a553-38fe59f5215a
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall ce0131d7-7859-4939-92d4-b3d823248b40
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 2c1f0e37-0174-4ee5-a949-5460e498d5cb
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 5b844e54-dff9-442f-b2c9-b8e0e0909d5a
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall b25a8905-9e46-453c-af7d-3c30a5059a03
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 789f800b-b56b-4806-b818-1a006197fa0b
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
...
ok 2275 - promResponsesToEncounter: deterministic (twice → deep-equal)
  ---
  duration_ms: 1.130542
  type: 'test'
  ...
# Subtest: versions
ok 2276 - versions
  ---
  duration_ms: 0.597833
  type: 'test'
  ...
...
ok 2678 - 56.6 — NON-FINITE numbers are rejected at any depth, and finite ones pass — and survive the real round trip
  ---
  duration_ms: 83.809417
  type: 'test'
  ...
# Subtest: the runtime states and the state CHECK are the same list, in the same order
ok 2679 - the runtime states and the state CHECK are the same list, in the same order
  ---
  duration_ms: 1.216375
  type: 'test'
  ...
# Subtest: the outcome CHECK pins its two state lists the same way, and neither slice is empty
ok 2680 - the outcome CHECK pins its two state lists the same way, and neither slice is empty
  ---
  duration_ms: 0.32075
  type: 'test'
  ...
# Subtest: the three outcome-CHECK sets PARTITION all fourteen states — no overlap, none omitted
ok 2681 - the three outcome-CHECK sets PARTITION all fourteen states — no overlap, none omitted
  ---
  duration_ms: 0.098375
  type: 'test'
  ...
# Subtest: two states are non-terminal, and a window cannot close on either
ok 2682 - two states are non-terminal, and a window cannot close on either
  ---
  duration_ms: 0.0795
  type: 'test'
  ...
# Subtest: the migration still declares its retention, access and deletion controls (§4.2)
ok 2683 - the migration still declares its retention, access and deletion controls (§4.2)
  ---
  duration_ms: 0.830459
  type: 'test'
  ...
# Subtest: the HMAC is keyed, versioned, and unreproducible with an unkeyed hash
ok 2684 - the HMAC is keyed, versioned, and unreproducible with an unkeyed hash
  ---
  duration_ms: 3.614417
  type: 'test'
  ...
# Subtest: key version travels with the value, so a rotation is visible rather than inferred
ok 2685 - key version travels with the value, so a rotation is visible rather than inferred
  ---
  duration_ms: 0.167041
  type: 'test'
  ...
# Subtest: a missing secret THROWS, and a whitespace-only key counts as missing (D8, test 71)
ok 2686 - a missing secret THROWS, and a whitespace-only key counts as missing (D8, test 71)
  ---
...
ok 2687 - 35.1 — every served class increments its OWN counter, and a null increments none (D16 rows: provider success → that route counter; proven non-delivery → not_served; attribution gap → unattributed; a stage-level null → nothing)
  ---
  duration_ms: 0.358958
  type: 'test'
  ...
# Subtest: counters derive from the manifest, so row and payload cannot disagree
ok 2688 - counters derive from the manifest, so row and payload cannot disagree
  ---
  duration_ms: 0.362542
  type: 'test'
  ...
# Subtest: rerank_429_attempts is the count of http_429 attempts, wherever they happened (test 13)
ok 2689 - rerank_429_attempts is the count of http_429 attempts, wherever they happened (test 13)
  ---
  duration_ms: 0.071166
  type: 'test'
  ...
# Subtest: batch order is a property of candidate boundaries, never of completion order (constraint 7)
ok 2690 - batch order is a property of candidate boundaries, never of completion order (constraint 7)
  ---
  duration_ms: 0.374208
  type: 'test'
  ...
# Subtest: neither field-bearing manifest declaration has a field that could carry clinical text
ok 2691 - neither field-bearing manifest declaration has a field that could carry clinical text
  ---
  duration_ms: 0.5305
  type: 'test'
  ...
# Subtest: the ban loop really bans — it fails when a banned field is added
ok 2692 - the ban loop really bans — it fails when a banned field is added
  ---
  duration_ms: 0.11525
  type: 'test'
  ...
# Subtest: TelemetryCapture is not declared in the core — the raw bytes live elsewhere (D5)
ok 2693 - TelemetryCapture is not declared in the core — the raw bytes live elsewhere (D5)
  ---
  duration_ms: 0.313833
  type: 'test'
  ...
# Subtest: StampedRetrievalManifest is EXACTLY the intersection, so nothing can be smuggled through it
ok 2694 - StampedRetrievalManifest is EXACTLY the intersection, so nothing can be smuggled through it
  ---
  duration_ms: 2.590667
  type: 'test'
  ...
# Subtest: every route maps to a class, and an unknown caller is never assigned to the nearest match
ok 2695 - every route maps to a class, and an unknown caller is never assigned to the nearest match
  ---
  duration_ms: 0.2105
  type: 'test'
  ...
# Subtest: the reconciler is an INVOCATION route and never a retrieval route (D17)
ok 2696 - the reconciler is an INVOCATION route and never a retrieval route (D17)
  ---
  duration_ms: 0.110708
  type: 'test'
  ...
# Subtest: the five roles are closed, and the appropriateness exclusion is by ROUTE not by role
ok 2697 - the five roles are closed, and the appropriateness exclusion is by ROUTE not by role
  ---
  duration_ms: 0.104167
  type: 'test'
  ...
# Subtest: usage aggregates by served provider/model, not by intended
ok 2698 - usage aggregates by served provider/model, not by intended
  ---
  duration_ms: 10.901625
  type: 'test'
  ...
# Subtest: a bucket with no usage reports null tokens and counts the unknowns — never zero (§4.6)
ok 2699 - a bucket with no usage reports null tokens and counts the unknowns — never zero (§4.6)
  ---
  duration_ms: 0.110958
  type: 'test'
  ...
# Subtest: partial usage is summed without inventing the missing half
ok 2700 - partial usage is summed without inventing the missing half
  ---
  duration_ms: 0.067708
  type: 'test'
  ...
# Subtest: local, not-served and skipped stages are UNPRICED; unattributed and parse failures are not
ok 2701 - local, not-served and skipped stages are UNPRICED; unattributed and parse failures are not
  ---
  duration_ms: 0.166625
  type: 'test'
  ...
# Subtest: this module prices nothing — money has ONE source of truth
ok 2702 - this module prices nothing — money has ONE source of truth
  ---
  duration_ms: 0.427125
  type: 'test'
  ...
# Subtest: the row contract and the manifest contract version independently (§4.3)
ok 2703 - the row contract and the manifest contract version independently (§4.3)
  ---
...
ok 2704 - 35.2 — ROW 1, provider success: vertex → vertex, openrouter → openrouter, ollama → local; that route counter += 1, failed unchanged, served model and attempts PRESERVED
  ---
  duration_ms: 0.576208
  type: 'test'
  ...
...
ok 2705 - 35.3 — ROW 2, terminal failure PROVEN to have returned no completion: not_served += 1, failed += 1, served_route_class not_served, served_model null
  ---
  duration_ms: 0.102042
  type: 'test'
  ...
...
ok 2706 - 35.4 — ROW 3, a completion may have arrived and attribution is unavailable: unattributed += 1 (rerank_unattributed_batches), failed FOLLOWS the batch outcome, served_route_class unattributed
  ---
  duration_ms: 0.103666
  type: 'test'
  ...
...
ok 2707 - 35.5 — ROW 4, timeout: the attempt records outcome timeout, the batch outcome is timeout, and the served class follows the PROOF rule — not_served with proof, unattributed without
  ---
...
ok 2708 - 35.6 — ROW 5 and the v11 §6.1 CORRECTION: a SKIPPED expansion stage records served_route_class null (NOT not_served), no route counter, no not_served counter — and attempts: [] , never null
  ---
...
ok 2709 - 35.7 — the v11 §6.1 CORRECTION at the other two sites: ABSENT evidence records attempts [] on a batch and on variant generation, while a transport that did NOT COLLECT (attempts: null on real evidence) keeps null
  ---
  duration_ms: 0.2065
  type: 'test'
  ...
...
ok 2710 - 35.8 — ROW 6 AS AMENDED (v7 §6, v8 §2): a soft failure without transport proof synthesises one terminal_failure record per PLANNED boundary — MORE THAN ONE boundary on the judge arm, each with its exact start and end — served_route_class UNATTRIBUTED (not the not_served D16 wrote), rerank_soft_failed true; not_served stands only where proof exists
  ---
  duration_ms: 81.393958
  type: 'test'
  ...
# Subtest: 35.9 — ROW 7, intended local request: exactly one ollama attempt; local on success, not_served on PROVEN failure
ok 2711 - 35.9 — ROW 7, intended local request: exactly one ollama attempt; local on success, not_served on PROVEN failure
  ---
...
ok 2712 - 35.10 — ROW 8, variant parse_failure: served provider, model, attempts and BOTH token counts are PRESERVED, status parse_failure, never not_served
  ---
  duration_ms: 0.304166
  type: 'test'
  ...
# Subtest: 35.11 — ROW 9, variant parsed_empty / all_invalid / not_an_array: served provider, model and usage preserved
ok 2713 - 35.11 — ROW 9, variant parsed_empty / all_invalid / not_an_array: served provider, model and usage preserved
  ---
  duration_ms: 0.108167
  type: 'test'
  ...
...
ok 2714 - 35.12 — ROW 10 and the v11 §6.2 CORRECTION: a variant-generation stage that RAN and failed_open records not_served ONLY with proof, otherwise UNATTRIBUTED — and only a stage that did NOT run records the stage-level null
  ---
...
ok 2715 - 35.13 — D16, Bedrock defensively: a Bedrock completion on this path is unattributed, never quietly mapped to a plausible class
  ---
...
ok 2716 - 19 — the predeclared run id is the one every later write targets, and no second row is inserted
  ---
  duration_ms: 4.889542
  type: 'test'
  ...
# Subtest: 20 — declare returns 0, the terminal write returns 1, and a stale handle is REJECTED
ok 2717 - 20 — declare returns 0, the terminal write returns 1, and a stale handle is REJECTED
  ---
  duration_ms: 0.688333
  type: 'test'
  ...
# Subtest: 20 — revisions advance PER ROLE: a normative write cannot invalidate the primary's handle
ok 2718 - 20 — revisions advance PER ROLE: a normative write cannot invalidate the primary's handle
  ---
  duration_ms: 2.133
  type: 'test'
  ...
# Subtest: 20 — a terminal write that matches nothing is NOT retried, and does not advance the handle
ok 2719 - 20 — a terminal write that matches nothing is NOT retried, and does not advance the handle
  ---
  duration_ms: 1.870541
  type: 'test'
  ...
# Subtest: 31 — the reuse guard returns BEFORE any telemetry statement exists
ok 2720 - 31 — the reuse guard returns BEFORE any telemetry statement exists
  ---
  duration_ms: 0.326833
  type: 'test'
  ...
# Subtest: 32 — the attached handle is absent from JSON.stringify, from the keys, and from a spread
ok 2721 - 32 — the attached handle is absent from JSON.stringify, from the keys, and from a spread
  ---
  duration_ms: 0.107292
  type: 'test'
  ...
# Subtest: 34 — terminal write fails, failure row fails: the invocation counter is the only evidence left
ok 2722 - 34 — terminal write fails, failure row fails: the invocation counter is the only evidence left
  ---
  duration_ms: 0.352959
  type: 'test'
  ...
# Subtest: 34 — and when the counter ALSO fails: a log line, nothing else, still no propagation
ok 2723 - 34 — and when the counter ALSO fails: a log line, nothing else, still no propagation
  ---
  duration_ms: 0.307417
  type: 'test'
  ...
# Subtest: 34 — NO MODULE-LEVEL COUNTER EXISTS ANYWHERE, asserted by source search
ok 2724 - 34 — NO MODULE-LEVEL COUNTER EXISTS ANYWHERE, asserted by source search
  ---
  duration_ms: 1.052166
  type: 'test'
  ...
# Subtest: 52 — every owner in the D9 matrix settles, including both scripts and both MCP paths
ok 2725 - 52 — every owner in the D9 matrix settles, including both scripts and both MCP paths
  ---
  duration_ms: 1.023084
  type: 'test'
  ...
# Subtest: 53 — the callback carries the audit id, and its failure never changes the save result
ok 2726 - 53 — the callback carries the audit id, and its failure never changes the save result
  ---
  duration_ms: 0.268
  type: 'test'
  ...
...
ok 2727 - CANARY-GATE HAZARD — primary rejected, normative lands: the audit persisted and the Stage 0b primary-link gate fails
  ---
  duration_ms: 9.17925
  type: 'test'
  ...
# Subtest: CANARY-GATE HAZARD — the mirror: primary lands, normative rejected
ok 2728 - CANARY-GATE HAZARD — the mirror: primary lands, normative rejected
  ---
  duration_ms: 7.461917
  type: 'test'
  ...
# Subtest: 54 — fourteen states, two of them non-terminal, and the two sets partition the whole
ok 2729 - 54 — fourteen states, two of them non-terminal, and the two sets partition the whole
  ---
  duration_ms: 1.065666
  type: 'test'
  ...
# Subtest: 54 — the implemented table IS D12's table, in both directions
ok 2730 - 54 — the implemented table IS D12's table, in both directions
  ---
  duration_ms: 0.820917
  type: 'test'
  ...
# Subtest: 54 — every one of the 196 ordered pairs answers the way D12 says
ok 2731 - 54 — every one of the 196 ordered pairs answers the way D12 says
  ---
  duration_ms: 0.447375
  type: 'test'
  ...
# Subtest: 54 — TERMINAL STATES NEVER TRANSITION, to anything, including themselves
ok 2732 - 54 — TERMINAL STATES NEVER TRANSITION, to anything, including themselves
  ---
  duration_ms: 0.208584
  type: 'test'
  ...
# Subtest: 54 — the two deliberate asymmetries are both present, and are not accidents
ok 2733 - 54 — the two deliberate asymmetries are both present, and are not accidents
  ---
  duration_ms: 0.20875
  type: 'test'
  ...
...
ok 2734 - 54 — every settlement outcome lands on a state, and the settlement table names none of the three reconciler-mapped states
  ---
  duration_ms: 0.200708
  type: 'test'
  ...
# Subtest: 54 — every reconciler-assigned state is a LEGAL transition from the state it is assigned from
ok 2735 - 54 — every reconciler-assigned state is a LEGAL transition from the state it is assigned from
  ---
  duration_ms: 0.24475
  type: 'test'
  ...
...
ok 2736 - 45.0 — the fixture is CLEAN: the valid manifest returns no code, so every code below is caused by the one mutation named beside it
  ---
  duration_ms: 4.606375
  type: 'test'
  ...
# Subtest: 45.2 — manifest_schema_version: null → manifest_version_unrecognized
ok 2737 - 45.2 — manifest_schema_version: null → manifest_version_unrecognized
  ---
  duration_ms: 0.226583
  type: 'test'
  ...
# Subtest: 45.3 — manifest_schema_version: wrong version 2 → manifest_version_unrecognized
ok 2738 - 45.3 — manifest_schema_version: wrong version 2 → manifest_version_unrecognized
  ---
  duration_ms: 0.103542
  type: 'test'
  ...
# Subtest: 45.4 — hmac_key_version: null WITHOUT hmac_key_absent → hmac_key_version_absent
ok 2739 - 45.4 — hmac_key_version: null WITHOUT hmac_key_absent → hmac_key_version_absent
  ---
  duration_ms: 0.075125
  type: 'test'
  ...
# Subtest: 45.5 — hmac_key_version: empty string → hmac_key_version_absent
ok 2740 - 45.5 — hmac_key_version: empty string → hmac_key_version_absent
  ---
  duration_ms: 0.335084
  type: 'test'
  ...
# Subtest: 45.6 — operational: missing → operational_absent
ok 2741 - 45.6 — operational: missing → operational_absent
  ---
  duration_ms: 0.088291
  type: 'test'
  ...
# Subtest: 45.7 — operational.route: missing → route_absent_or_invalid
ok 2742 - 45.7 — operational.route: missing → route_absent_or_invalid
  ---
  duration_ms: 0.136042
  type: 'test'
  ...
# Subtest: 45.8 — operational.route: invalid (not in RETRIEVAL_ROUTES) → route_absent_or_invalid
ok 2743 - 45.8 — operational.route: invalid (not in RETRIEVAL_ROUTES) → route_absent_or_invalid
  ---
  duration_ms: 0.074333
  type: 'test'
  ...
# Subtest: 45.9 — operational.route_class: null → route_class_absent
ok 2744 - 45.9 — operational.route_class: null → route_class_absent
  ---
  duration_ms: 0.185292
  type: 'test'
  ...
# Subtest: 45.10 — operational.retrieval_role: invalid → retrieval_role_absent_or_invalid
ok 2745 - 45.10 — operational.retrieval_role: invalid → retrieval_role_absent_or_invalid
  ---
  duration_ms: 0.333875
  type: 'test'
  ...
# Subtest: 45.11 — operational.started_at: null → started_at_absent
ok 2746 - 45.11 — operational.started_at: null → started_at_absent
  ---
  duration_ms: 0.065583
  type: 'test'
  ...
# Subtest: 45.12 — operational.completed_at: empty string → completed_at_absent
ok 2747 - 45.12 — operational.completed_at: empty string → completed_at_absent
  ---
  duration_ms: 0.05
  type: 'test'
  ...
# Subtest: 45.13 — operational.invocation_id: empty string → invocation_id_absent
ok 2748 - 45.13 — operational.invocation_id: empty string → invocation_id_absent
  ---
  duration_ms: 0.046167
  type: 'test'
  ...
# Subtest: 45.14 — operational.trace_id: missing (null is permitted) → trace_id_field_absent
ok 2749 - 45.14 — operational.trace_id: missing (null is permitted) → trace_id_field_absent
  ---
  duration_ms: 0.045959
  type: 'test'
  ...
# Subtest: 45.15 — operational.deployment_sha: missing (null is permitted) → deployment_sha_field_absent
ok 2750 - 45.15 — operational.deployment_sha: missing (null is permitted) → deployment_sha_field_absent
  ---
  duration_ms: 0.048083
  type: 'test'
  ...
# Subtest: 45.16 — operational.routing_flags: null ({} is permitted) → routing_flags_absent
ok 2751 - 45.16 — operational.routing_flags: null ({} is permitted) → routing_flags_absent
  ---
  duration_ms: 0.04525
  type: 'test'
  ...
# Subtest: 45.17 — operational.routing_flags: an array is not a flags object → routing_flags_absent
ok 2752 - 45.17 — operational.routing_flags: an array is not a flags object → routing_flags_absent
  ---
  duration_ms: 0.045375
  type: 'test'
  ...
# Subtest: 45.18 — operational.active_backfill_run_id: missing (null is permitted) → active_backfill_run_id_field_absent
ok 2753 - 45.18 — operational.active_backfill_run_id: missing (null is permitted) → active_backfill_run_id_field_absent
  ---
  duration_ms: 0.0435
  type: 'test'
  ...
# Subtest: 45.19 — operational.active_backfill_target: missing (null is permitted) → active_backfill_target_field_absent
ok 2754 - 45.19 — operational.active_backfill_target: missing (null is permitted) → active_backfill_target_field_absent
  ---
...
ok 2755 - 45.20 — operational.active_backfill_state: missing (null is permitted) → active_backfill_state_field_absent
  ---
...
ok 2756 - 45.21 — operational.active_backfill_state: invalid (not active/idle) → active_backfill_state_invalid
  ---
  duration_ms: 0.04375
  type: 'test'
  ...
# Subtest: 45.22 — operational.active_lab_experiment_id: missing (null is permitted) → active_lab_experiment_id_field_absent
ok 2757 - 45.22 — operational.active_lab_experiment_id: missing (null is permitted) → active_lab_experiment_id_field_absent
  ---
  duration_ms: 0.043958
  type: 'test'
  ...
# Subtest: 45.23 — retrieval_outcome: null → retrieval_outcome_absent_or_invalid
ok 2758 - 45.23 — retrieval_outcome: null → retrieval_outcome_absent_or_invalid
  ---
  duration_ms: 0.0565
  type: 'test'
  ...
# Subtest: 45.24 — retrieval_outcome: invalid → retrieval_outcome_absent_or_invalid
ok 2759 - 45.24 — retrieval_outcome: invalid → retrieval_outcome_absent_or_invalid
  ---
...
ok 2760 - 45.25 — retrieval_error_class: missing (null is permitted on success) → retrieval_error_class_field_absent
  ---
...
ok 2761 - 45.26 — retrieval_error_class: null when the outcome is retrieval_failure → retrieval_error_class_absent_on_failure
  ---
  duration_ms: 0.043875
  type: 'test'
  ...
# Subtest: 45.27 — expansion: missing → expansion_absent
ok 2762 - 45.27 — expansion: missing → expansion_absent
  ---
  duration_ms: 0.044042
  type: 'test'
  ...
# Subtest: 45.28 — expansion.status: null → expansion_status_absent_or_invalid
ok 2763 - 45.28 — expansion.status: null → expansion_status_absent_or_invalid
  ---
  duration_ms: 0.042625
  type: 'test'
  ...
# Subtest: 45.29 — expansion.input_hmac: missing → expansion_input_hmac_field_absent
ok 2764 - 45.29 — expansion.input_hmac: missing → expansion_input_hmac_field_absent
  ---
  duration_ms: 0.045292
  type: 'test'
  ...
# Subtest: 45.30 — expansion.input_hmac: null on an EXPANDED stage without hmac_key_absent → expansion_input_hmac_absent
ok 2765 - 45.30 — expansion.input_hmac: null on an EXPANDED stage without hmac_key_absent → expansion_input_hmac_absent
  ---
  duration_ms: 0.04225
  type: 'test'
  ...
# Subtest: 45.31 — expansion.served_route_class: missing → expansion_served_route_class_field_absent
ok 2766 - 45.31 — expansion.served_route_class: missing → expansion_served_route_class_field_absent
  ---
...
ok 2804 - 45.69 — batch.served_route_class: null — the type permits it, the contract does not (A6) → batch_served_route_class_absent
  ---
  duration_ms: 0.045209
  type: 'test'
  ...
# Subtest: 45.70 — batch.served_route_class: invalid → batch_served_route_class_invalid
ok 2805 - 45.70 — batch.served_route_class: invalid → batch_served_route_class_invalid
  ---
  duration_ms: 0.08875
  type: 'test'
  ...
# Subtest: 45.71 — batch.served_model: missing (null is permitted) → batch_served_model_field_absent
ok 2806 - 45.71 — batch.served_model: missing (null is permitted) → batch_served_model_field_absent
  ---
  duration_ms: 0.0495
  type: 'test'
  ...
# Subtest: 45.72 — batch.served_model: non-null on an unattributed batch (§10) → unattributed_with_model
ok 2807 - 45.72 — batch.served_model: non-null on an unattributed batch (§10) → unattributed_with_model
  ---
  duration_ms: 0.044209
  type: 'test'
  ...
# Subtest: 45.73 — batch.served_model: non-null on a not_served batch (§10) → not_served_with_model
ok 2808 - 45.73 — batch.served_model: non-null on a not_served batch (§10) → not_served_with_model
  ---
  duration_ms: 0.0415
  type: 'test'
  ...
# Subtest: 45.74 — batch.attempts: missing (null is permitted) → batch_attempts_field_absent
ok 2809 - 45.74 — batch.attempts: missing (null is permitted) → batch_attempts_field_absent
  ---
  duration_ms: 0.043625
  type: 'test'
  ...
# Subtest: 45.75 — batch.attempts: a member with an outcome outside the six → attempt_outcome_absent_or_invalid
ok 2810 - 45.75 — batch.attempts: a member with an outcome outside the six → attempt_outcome_absent_or_invalid
  ---
  duration_ms: 0.046208
  type: 'test'
  ...
# Subtest: 45.76 — batch.prompt_tokens: missing (null is permitted) — v25 §3.2, unvalidated until the pass 3 repair → batch_prompt_tokens_field_absent
ok 2811 - 45.76 — batch.prompt_tokens: missing (null is permitted) — v25 §3.2, unvalidated until the pass 3 repair → batch_prompt_tokens_field_absent
  ---
  duration_ms: 0.04375
  type: 'test'
  ...
# Subtest: 45.77 — batch.prompt_tokens: invalid: a numeric STRING → batch_prompt_tokens_invalid
ok 2812 - 45.77 — batch.prompt_tokens: invalid: a numeric STRING → batch_prompt_tokens_invalid
  ---
  duration_ms: 0.045291
  type: 'test'
  ...
# Subtest: 45.78 — batch.prompt_tokens: invalid number: negative → batch_prompt_tokens_invalid
ok 2813 - 45.78 — batch.prompt_tokens: invalid number: negative → batch_prompt_tokens_invalid
  ---
  duration_ms: 0.041667
  type: 'test'
  ...
# Subtest: 45.79 — batch.prompt_tokens: invalid number: NaN → batch_prompt_tokens_invalid
ok 2814 - 45.79 — batch.prompt_tokens: invalid number: NaN → batch_prompt_tokens_invalid
  ---
  duration_ms: 0.042
  type: 'test'
  ...
# Subtest: 45.80 — batch.completion_tokens: missing (null is permitted) — v25 §3.2, unvalidated until the pass 3 repair → batch_completion_tokens_field_absent
ok 2815 - 45.80 — batch.completion_tokens: missing (null is permitted) — v25 §3.2, unvalidated until the pass 3 repair → batch_completion_tokens_field_absent
  ---
  duration_ms: 0.0425
  type: 'test'
  ...
# Subtest: 45.81 — batch.completion_tokens: invalid: a numeric STRING → batch_completion_tokens_invalid
ok 2816 - 45.81 — batch.completion_tokens: invalid: a numeric STRING → batch_completion_tokens_invalid
  ---
  duration_ms: 0.0415
  type: 'test'
  ...
# Subtest: 45.82 — batch.completion_tokens: invalid number: Infinity → batch_completion_tokens_invalid
ok 2817 - 45.82 — batch.completion_tokens: invalid number: Infinity → batch_completion_tokens_invalid
  ---
  duration_ms: 0.041458
  type: 'test'
  ...
# Subtest: 45.83 — batch.completion_tokens: invalid: an object → batch_completion_tokens_invalid
ok 2818 - 45.83 — batch.completion_tokens: invalid: an object → batch_completion_tokens_invalid
  ---
  duration_ms: 0.046667
  type: 'test'
  ...
# Subtest: 45.84 — batch.outcome: invalid → batch_outcome_absent_or_invalid
ok 2819 - 45.84 — batch.outcome: invalid → batch_outcome_absent_or_invalid
  ---
  duration_ms: 0.050334
  type: 'test'
  ...
# Subtest: 45.85 — batch.expected_score_keys: null → expected_score_keys_absent
ok 2820 - 45.85 — batch.expected_score_keys: null → expected_score_keys_absent
  ---
  duration_ms: 0.044708
  type: 'test'
  ...
# Subtest: 45.86 — batch.finite_score_keys: invalid number: negative → finite_score_keys_absent
ok 2821 - 45.86 — batch.finite_score_keys: invalid number: negative → finite_score_keys_absent
  ---
  duration_ms: 0.047709
  type: 'test'
  ...
# Subtest: 45.87 — batch.finite_score_keys: more finite keys than expected → score_keys_exceed_expected
ok 2822 - 45.87 — batch.finite_score_keys: more finite keys than expected → score_keys_exceed_expected
  ---
  duration_ms: 0.041459
  type: 'test'
  ...
# Subtest: 45.88 — ordered_final_candidate_ids: null ([] is permitted) → ordered_final_candidate_ids_absent
ok 2823 - 45.88 — ordered_final_candidate_ids: null ([] is permitted) → ordered_final_candidate_ids_absent
  ---
  duration_ms: 0.040917
  type: 'test'
  ...
# Subtest: 45.89 — retrieval_config: null → retrieval_config_absent
ok 2824 - 45.89 — retrieval_config: null → retrieval_config_absent
  ---
  duration_ms: 0.040958
  type: 'test'
  ...
# Subtest: 45.90 — retrieval_config: an array is not a config object → retrieval_config_absent
ok 2825 - 45.90 — retrieval_config: an array is not a config object → retrieval_config_absent
  ---
  duration_ms: 0.040333
  type: 'test'
  ...
# Subtest: 45.91 — retrieval_config.rerank_temperature: missing (v7 §10: required as of manifest v3; null is permitted) → rerank_temperature_field_absent
ok 2826 - 45.91 — retrieval_config.rerank_temperature: missing (v7 §10: required as of manifest v3; null is permitted) → rerank_temperature_field_absent
  ---
  duration_ms: 0.044584
  type: 'test'
  ...
# Subtest: 45.92 — retrieval_config.rerank_temperature: invalid number: NaN → rerank_temperature_invalid
ok 2827 - 45.92 — retrieval_config.rerank_temperature: invalid number: NaN → rerank_temperature_invalid
  ---
  duration_ms: 0.04525
  type: 'test'
  ...
# Subtest: 45.93 — retrieval_config.rerank_seed_status: missing (v7 §10) → rerank_seed_status_field_absent
ok 2828 - 45.93 — retrieval_config.rerank_seed_status: missing (v7 §10) → rerank_seed_status_field_absent
  ---
  duration_ms: 0.042875
  type: 'test'
  ...
# Subtest: 45.94 — retrieval_config.rerank_seed_status: null — never null, not_applicable is the value for no decode → rerank_seed_status_invalid
ok 2829 - 45.94 — retrieval_config.rerank_seed_status: null — never null, not_applicable is the value for no decode → rerank_seed_status_invalid
  ---
  duration_ms: 0.041541
  type: 'test'
  ...
# Subtest: 45.95 — corpus_version: missing (null is permitted) → corpus_version_field_absent
ok 2830 - 45.95 — corpus_version: missing (null is permitted) → corpus_version_field_absent
  ---
  duration_ms: 0.042375
  type: 'test'
  ...
# Subtest: 45.96 — index_version: null → index_version_absent
ok 2831 - 45.96 — index_version: null → index_version_absent
  ---
  duration_ms: 0.040209
  type: 'test'
  ...
# Subtest: 45.97 — index_version: empty string → index_version_absent
ok 2832 - 45.97 — index_version: empty string → index_version_absent
  ---
  duration_ms: 0.042125
  type: 'test'
  ...
# Subtest: 45.98 — scorer_context_hmac: missing → scorer_context_hmac_field_absent
ok 2833 - 45.98 — scorer_context_hmac: missing → scorer_context_hmac_field_absent
  ---
  duration_ms: 0.041625
  type: 'test'
  ...
# Subtest: 45.99 — scorer_context_hmac: null on role primary WITHOUT hmac_key_absent → scorer_context_hmac_absent
ok 2834 - 45.99 — scorer_context_hmac: null on role primary WITHOUT hmac_key_absent → scorer_context_hmac_absent
  ---
  duration_ms: 0.04125
  type: 'test'
  ...
# Subtest: 45.100 — multi_query: present on a role that is not lab_multi_query → multi_query_on_non_multi_query_role
ok 2835 - 45.100 — multi_query: present on a role that is not lab_multi_query → multi_query_on_non_multi_query_role
  ---
  duration_ms: 0.057417
  type: 'test'
  ...
# Subtest: 45.101 — the lab_multi_query fixture is CLEAN before its rows run
ok 2836 - 45.101 — the lab_multi_query fixture is CLEAN before its rows run
  ---
  duration_ms: 0.0795
  type: 'test'
  ...
# Subtest: 45.102 — multi_query: missing on lab_multi_query → multi_query_absent
ok 2837 - 45.102 — multi_query: missing on lab_multi_query → multi_query_absent
  ---
  duration_ms: 0.059708
  type: 'test'
  ...
# Subtest: 45.103 — multi_query.variant_generation: null → variant_generation_absent
ok 2838 - 45.103 — multi_query.variant_generation: null → variant_generation_absent
  ---
  duration_ms: 0.044666
  type: 'test'
  ...
# Subtest: 45.104 — multi_query.variant_generation.status: invalid → variant_generation_status_absent_or_invalid
ok 2839 - 45.104 — multi_query.variant_generation.status: invalid → variant_generation_status_absent_or_invalid
  ---
  duration_ms: 0.048542
  type: 'test'
  ...
# Subtest: 45.105 — multi_query.variant_generation.served_route_class: missing (null is permitted for a stage that did not run) → variant_generation_served_route_class_field_absent
ok 2840 - 45.105 — multi_query.variant_generation.served_route_class: missing (null is permitted for a stage that did not run) → variant_generation_served_route_class_field_absent
  ---
  duration_ms: 0.047042
  type: 'test'
  ...
# Subtest: 45.106 — multi_query.variant_generation.generated_variant_count: null → generated_variant_count_absent
ok 2841 - 45.106 — multi_query.variant_generation.generated_variant_count: null → generated_variant_count_absent
  ---
  duration_ms: 0.044958
  type: 'test'
  ...
# Subtest: 45.107 — multi_query.variant_generation.attempts: a member with an outcome outside the six → attempt_outcome_absent_or_invalid
ok 2842 - 45.107 — multi_query.variant_generation.attempts: a member with an outcome outside the six → attempt_outcome_absent_or_invalid
  ---
  duration_ms: 0.047667
  type: 'test'
  ...
# Subtest: 45.108 — multi_query.variants: null → variants_absent
ok 2843 - 45.108 — multi_query.variants: null → variants_absent
  ---
  duration_ms: 0.044542
  type: 'test'
  ...
# Subtest: 45.109 — multi_query.variants: length ≠ generated_variant_count + 1 → variant_arity_mismatch
ok 2844 - 45.109 — multi_query.variants: length ≠ generated_variant_count + 1 → variant_arity_mismatch
  ---
...
ok 2845 - 45.1 — OWN-PROPERTY CHECKS: missing, explicit null, empty array, empty string and invalid number are FIVE different answers, and the validator tells them apart
  ---
  duration_ms: 0.359958
  type: 'test'
  ...
# Subtest: 45.110 — the HMAC-absent licence covers EXACTLY the four D8 fields, and only when telemetry_error declares it
ok 2846 - 45.110 — the HMAC-absent licence covers EXACTLY the four D8 fields, and only when telemetry_error declares it
  ---
  duration_ms: 0.56625
  type: 'test'
  ...
...
ok 2847 - 46.1 — expansion.served_route_class null with status skipped is valid: no expansion code, so a normative_channel row is not partial by construction
  ---
  duration_ms: 0.758417
  type: 'test'
  ...
...
ok 2848 - 46.2 — through the REAL builder: a normative_channel capture (expansion never set — the leg sets skipExpand unconditionally) produces a payload that validates clean
  ---
  duration_ms: 0.529667
  type: 'test'
  ...
...
ok 2849 - 47.1 — on role primary the scorer-context HMAC is REQUIRED: computed over the EXACT citedContext bytes, and it changes when one byte does
  ---
  duration_ms: 4.059917
  type: 'test'
  ...
...
ok 2850 - 47.2 — the EMPTY-STRING case: zero candidates render an empty citedContext, and the HMAC of the empty string is a DEFINED value — never null because reranking was skipped
  ---
  duration_ms: 0.152416
  type: 'test'
  ...
# Subtest: 47.3 — on the other FOUR roles the HMAC is null, and those nulls are NOT partial
ok 2851 - 47.3 — on the other FOUR roles the HMAC is null, and those nulls are NOT partial
  ---
...
ok 2852 - 47.4 — a NON-NULL scorer-context HMAC on any of the four non-primary roles is REJECTED (v25 §3.3): scorer_context_hmac_on_non_primary_role — and primary is untouched
  ---
...
ok 2853 - 47.5 — through the REAL assembleAuditContext: the primary HMAC is the keyed HMAC of the EXACT rendered citedContext (literature only, and literature plus the normative block), and with zero hits the rendered context is the EMPTY STRING whose HMAC is a defined value
  ---
  duration_ms: 0.445167
  type: 'test'
  ...
# Subtest: 47.6 — the PRODUCTION CALLER handoff, pinned in comment-stripped source: lib/opd-note-audit.ts destructures citedContext from assembleAuditContext(hits, normHits), passes it into writeRetrievalTerminals, keys the PRIMARY payload with scorerContext: citedContext, the NORMATIVE payload with scorerContext: null, and validates the stamped primary manifest
ok 2854 - 47.6 — the PRODUCTION CALLER handoff, pinned in comment-stripped source: lib/opd-note-audit.ts destructures citedContext from assembleAuditContext(hits, normHits), passes it into writeRetrievalTerminals, keys the PRIMARY payload with scorerContext: citedContext, the NORMATIVE payload with scorerContext: null, and validates the stamped primary manifest
  ---
  duration_ms: 11.934875
  type: 'test'
  ...
...
ok 2855 - 49.1 — EMPTY FUSION: retrieve() returns before the rerank block exists — fused 0, hydrated 0, expected 0, recorded 0, batches [], backend and model none
  ---
...
ok 2856 - 49.2 — HYDRATE EMPTIED: fused > 0, hydrated 0 — the same shape, and the TWO COUNTS DIFFER, which is the point
  ---
  duration_ms: 1.458375
  type: 'test'
  ...
...
ok 2857 - 49.3 — the two zero-candidate shapes are DISTINGUISHABLE: empty fusion (0/0) and hydrate emptied (3/0) differ in fused_candidate_count and agree in hydrated_candidate_count
  ---
...
ok 2858 - 49.4 — ONE HYDRATED CANDIDATE: rerank() is never entered — hydrated 1, expected 0, recorded 0, batches [], backend and model none
  ---
  duration_ms: 0.076625
  type: 'test'
  ...
...
ok 2859 - 49.5 — RERANKER DISABLED (normative_channel always; lab_direct when the caller sets it): several hydrated candidates, the same shape as the one-candidate case
  ---
...
ok 2860 - default path: filter clause array is byte-identical to production, no params
  ---
  duration_ms: 0.780459
  type: 'test'
  ...
# Subtest: default path with structural filters keeps the base guards and remaps $FP offsets per leg
ok 2861 - default path with structural filters keeps the base guards and remaps $FP offsets per leg
  ---
  duration_ms: 0.951875
  type: 'test'
  ...
# Subtest: relaxed path: both quarantine guards gain a bound OR arm on both legs
ok 2862 - relaxed path: both quarantine guards gain a bound OR arm on both legs
  ---
  duration_ms: 0.233416
  type: 'test'
  ...
# Subtest: relaxed path ordering: the quarantine label takes $FP_0, structural filters follow
ok 2863 - relaxed path ordering: the quarantine label takes $FP_0, structural filters follow
  ---
  duration_ms: 0.094625
  type: 'test'
  ...
# Subtest: hostile labels are slugged by labLabel and cannot widen the filter
ok 2864 - hostile labels are slugged by labLabel and cannot widen the filter
  ---
  duration_ms: 0.235167
  type: 'test'
  ...
# Subtest: empty/whitespace includeQuarantined is treated as omitted (byte-identical default path)
ok 2865 - empty/whitespace includeQuarantined is treated as omitted (byte-identical default path)
  ---
  duration_ms: 0.099708
  type: 'test'
  ...
# Subtest: clampLabRetrieveTopK clamps to [1,20], default 8
ok 2866 - clampLabRetrieveTopK clamps to [1,20], default 8
  ---
...
ok 2867 - restrictSources omitted ⇒ default clauses + no params (byte-identical)
  ---
  duration_ms: 0.777958
  type: 'test'
  ...
# Subtest: restrictSources: [choosing-wisely] ⇒ both legs source = ANY, bound array param
ok 2868 - restrictSources: [choosing-wisely] ⇒ both legs source = ANY, bound array param
  ---
  duration_ms: 0.13
  type: 'test'
  ...
# Subtest: a named labq: source is admitted through the quarantine guard; un-named stays excluded
ok 2869 - a named labq: source is admitted through the quarantine guard; un-named stays excluded
  ---
  duration_ms: 0.107792
  type: 'test'
  ...
# Subtest: empty or all-blank restrictSources falls back to the default filter (no restriction)
ok 2870 - empty or all-blank restrictSources falls back to the default filter (no restriction)
  ---
  duration_ms: 0.136125
  type: 'test'
  ...
# Subtest: restrictSources stacks with book/chunk filters and supersedes the single-source filter
ok 2871 - restrictSources stacks with book/chunk filters and supersedes the single-source filter
  ---
  duration_ms: 0.0535
  type: 'test'
  ...
# Subtest: group key exactness: same (subject, signal_type) groups; different signal splits
ok 2872 - group key exactness: same (subject, signal_type) groups; different signal splits
  ---
  duration_ms: 1.708083
  type: 'test'
  ...
# Subtest: '' signal folds like stewardship (empty signal groups together)
ok 2873 - '' signal folds like stewardship (empty signal groups together)
  ---
  duration_ms: 0.150041
  type: 'test'
  ...
# Subtest: ≥2 threshold: a pair groups, a singleton does not
ok 2874 - ≥2 threshold: a pair groups, a singleton does not
  ---
  duration_ms: 0.440208
  type: 'test'
  ...
# Subtest: section order + group sort (size desc, newest tie-break) + singles in queue order
ok 2875 - section order + group sort (size desc, newest tie-break) + singles in queue order
  ---
  duration_ms: 0.29925
  type: 'test'
  ...
# Subtest: disagreement pinning: disagreements section leads the order, groups/singles below
ok 2876 - disagreement pinning: disagreements section leads the order, groups/singles below
  ---
  duration_ms: 0.185833
  type: 'test'
  ...
# Subtest: traversal: within-group → next group → singles; wrap; exhausted → null
ok 2877 - traversal: within-group → next group → singles; wrap; exhausted → null
  ---
  duration_ms: 0.109417
  type: 'test'
  ...
# Subtest: skip sinks within its section and traversal passes it over
ok 2878 - skip sinks within its section and traversal passes it over
  ---
  duration_ms: 0.138792
  type: 'test'
  ...
# Subtest: labeled stays in place (not sunk) with its status; k/n progress counts labeled+skipped
ok 2879 - labeled stays in place (not sunk) with its status; k/n progress counts labeled+skipped
  ---
  duration_ms: 0.064625
  type: 'test'
  ...
# Subtest: determinism: same input → identical output
ok 2880 - determinism: same input → identical output
  ---
  duration_ms: 0.208833
  type: 'test'
  ...
# Subtest: hashBucket is deterministic and in 0..99
ok 2881 - hashBucket is deterministic and in 0..99
  ---
  duration_ms: 0.9655
  type: 'test'
  ...
# Subtest: overlap is ~20% and buckets < 20
ok 2882 - overlap is ~20% and buckets < 20
  ---
  duration_ms: 0.502917
  type: 'test'
  ...
# Subtest: overlap findings are served to EVERY reviewer; partitioned to exactly one
ok 2883 - overlap findings are served to EVERY reviewer; partitioned to exactly one
  ---
  duration_ms: 0.449792
  type: 'test'
  ...
# Subtest: a reviewer not on the roster still gets the overlap set (only)
ok 2884 - a reviewer not on the roster still gets the overlap set (only)
  ---
  duration_ms: 0.126666
  type: 'test'
  ...
# Subtest: partition is roughly even across the roster
ok 2885 - partition is roughly even across the roster
  ---
  duration_ms: 0.158208
  type: 'test'
  ...
# Subtest: balanceBySignalType interleaves types and is newest-first within a type
ok 2886 - balanceBySignalType interleaves types and is newest-first within a type
  ---
  duration_ms: 0.428917
  type: 'test'
  ...
# Subtest: disagreement items come first, then fresh; limit respected
ok 2887 - disagreement items come first, then fresh; limit respected
  ---
  duration_ms: 0.295666
  type: 'test'
  ...
# Subtest: passthrough: optional uid + prescription_url survive buildReviewQueue onto emitted items
ok 2888 - passthrough: optional uid + prescription_url survive buildReviewQueue onto emitted items
  ---
  duration_ms: 0.078791
  type: 'test'
  ...
# Subtest: excludes labeled-by-this-reviewer, informational, unassigned, and filtered-out findings
ok 2889 - excludes labeled-by-this-reviewer, informational, unassigned, and filtered-out findings
  ---
  duration_ms: 0.4115
  type: 'test'
  ...
# Subtest: parseGoal: valid / missing / garbage → exact defaults; personal ceil
ok 2890 - parseGoal: valid / missing / garbage → exact defaults; personal ceil
  ---
  duration_ms: 1.630333
  type: 'test'
  ...
# Subtest: prevDay + istWeekStart (Monday-start)
ok 2891 - prevDay + istWeekStart (Monday-start)
  ---
  duration_ms: 3.229958
  type: 'test'
  ...
# Subtest: countedLabels: impact excluded, missed included, roster filter, finding current-state (later wins)
ok 2892 - countedLabels: impact excluded, missed included, roster filter, finding current-state (later wins)
  ---
  duration_ms: 0.433834
  type: 'test'
  ...
# Subtest: streak: threshold exactly 15, consecutive, yesterday-grace, today-only, gap → 0
ok 2893 - streak: threshold exactly 15, consecutive, yesterday-grace, today-only, gap → 0
  ---
  duration_ms: 0.39425
  type: 'test'
  ...
# Subtest: agreement: pair construction (2 & 3 reviewers), tier match/mismatch, overlap-only
ok 2894 - agreement: pair construction (2 & 3 reviewers), tier match/mismatch, overlap-only
  ---
  duration_ms: 0.251166
  type: 'test'
  ...
# Subtest: agreement: current-state dedup feeds pairs (later verdict wins), then match recomputed
ok 2895 - agreement: current-state dedup feeds pairs (later verdict wins), then match recomputed
  ---
  duration_ms: 0.122834
  type: 'test'
  ...
# Subtest: computeReviewStats: ≥20-pair display boundary, week total, badges shape
ok 2896 - computeReviewStats: ≥20-pair display boundary, week total, badges shape
  ---
  duration_ms: 0.426125
  type: 'test'
  ...
# Subtest: the committed gold artifact is frozen, ratified, and catalog-consistent
ok 2897 - the committed gold artifact is frozen, ratified, and catalog-consistent
  ---
  duration_ms: 5.34525
  type: 'test'
  ...
# Subtest: loadCheckGold rejects drift: wrong version, unratified, polarity/target mismatch
ok 2898 - loadCheckGold rejects drift: wrong version, unratified, polarity/target mismatch
  ---
  duration_ms: 6.051834
  type: 'test'
  ...
# Subtest: the committed 2.0 artifact is frozen, ratified, family-split, and catalog-consistent
ok 2899 - the committed 2.0 artifact is frozen, ratified, family-split, and catalog-consistent
  ---
  duration_ms: 3.209291
  type: 'test'
  ...
# Subtest: loadCheckGold2: accepts the delivered shape — empty targets legal, L carries annex/memberHistory
ok 2900 - loadCheckGold2: accepts the delivered shape — empty targets legal, L carries annex/memberHistory
  ---
  duration_ms: 0.502084
  type: 'test'
  ...
# Subtest: loadCheckGold2 rejects drift: version, status, dup ids, missing verdict fields, annex misuse
ok 2901 - loadCheckGold2 rejects drift: version, status, dup ids, missing verdict fields, annex misuse
  ---
  duration_ms: 1.320209
  type: 'test'
  ...
# Subtest: splitCheckGold2: P/N/C form the scored floor, L is the annex — never folded together
ok 2902 - splitCheckGold2: P/N/C form the scored floor, L is the annex — never folded together
  ---
  duration_ms: 1.031416
  type: 'test'
  ...
# Subtest: checkGold2CatalogGaps: unbound polarity-side targets are flagged, bound ones are not
ok 2903 - checkGold2CatalogGaps: unbound polarity-side targets are flagged, bound ones are not
  ---
  duration_ms: 2.196416
  type: 'test'
  ...
# Subtest: scoreCheckAgainstGold: per-target-rec, deterministic, ignores non-target firings
ok 2904 - scoreCheckAgainstGold: per-target-rec, deterministic, ignores non-target firings
  ---
  duration_ms: 0.554083
  type: 'test'
  ...
# Subtest: aggregateCheckGold: hand-computed recall / specificity / precision / F1
ok 2905 - aggregateCheckGold: hand-computed recall / specificity / precision / F1
  ---
  duration_ms: 0.694291
  type: 'test'
  ...
# Subtest: Fix A: ANALYZE_SYSTEM carries the verdict discipline (uncertain = equipoise only)
ok 2906 - Fix A: ANALYZE_SYSTEM carries the verdict discipline (uncertain = equipoise only)
  ---
  duration_ms: 0.456416
  type: 'test'
  ...
# Subtest: Fix A: normNetValue contract unchanged, but the parse fallback is now visible
ok 2907 - Fix A: normNetValue contract unchanged, but the parse fallback is now visible
  ---
  duration_ms: 0.18425
  type: 'test'
  ...
# Subtest: Fix B: the two syncope recs are in the seed, verified, unique, well-formed
ok 2908 - Fix B: the two syncope recs are in the seed, verified, unique, well-formed
  ---
  duration_ms: 0.385667
  type: 'test'
  ...
# Subtest: Fix B gold: deterministic recall hits C04 with BOTH new recs, and no other check case
ok 2909 - Fix B gold: deterministic recall hits C04 with BOTH new recs, and no other check case
  ---
  duration_ms: 1.751333
  type: 'test'
  ...
# Subtest: flag-off byte-identical: every grounded builder without the param equals Slice 1 exactly
ok 2910 - flag-off byte-identical: every grounded builder without the param equals Slice 1 exactly
  ---
  duration_ms: 0.813208
  type: 'test'
  ...
# Subtest: grounded: the picture lands between the input and the downstream sections, verbatim
ok 2911 - grounded: the picture lands between the input and the downstream sections, verbatim
  ---
  duration_ms: 0.140875
  type: 'test'
  ...
# Subtest: patientPictureBlock: formatClinicalState content + the two prompt rules
ok 2912 - patientPictureBlock: formatClinicalState content + the two prompt rules
  ---
  duration_ms: 1.347333
  type: 'test'
  ...
# Subtest: grounding flag is double-gated on the master flag
ok 2913 - grounding flag is double-gated on the master flag
  ---
  duration_ms: 0.80275
  type: 'test'
  ...
# Subtest: frozen bank right-care-eval/1.0: pinned, unique ids, per-mode shape
ok 2914 - frozen bank right-care-eval/1.0: pinned, unique ids, per-mode shape
  ---
  duration_ms: 8.932959
  type: 'test'
  ...
# Subtest: pair-judge parser: defensive on directions, safety classes, fences
ok 2915 - pair-judge parser: defensive on directions, safety classes, fences
  ---
  duration_ms: 1.0135
  type: 'test'
  ...
# Subtest: deterministic check diff: added / removed / kept by rec id
ok 2916 - deterministic check diff: added / removed / kept by rec id
  ---
  duration_ms: 0.4725
  type: 'test'
  ...
# Subtest: scorecard gates: FAIL_SAFETY dominates; PASS needs net improvement clearing noise
ok 2917 - scorecard gates: FAIL_SAFETY dominates; PASS needs net improvement clearing noise
  ---
  duration_ms: 2.187042
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: Order check constructs a ClinicalState from the provided input with counts > 0
ok 2918 - Order check constructs a ClinicalState from the provided input with counts > 0
  ---
  duration_ms: 3.079583
  type: 'test'
  ...
# Subtest: Care pathway constructs from the presentation field with counts > 0
ok 2919 - Care pathway constructs from the presentation field with counts > 0
  ---
  duration_ms: 0.394708
  type: 'test'
  ...
# Subtest: Record audit adapts the existing ExtractedCase and round-trips on the shared fields
ok 2920 - Record audit adapts the existing ExtractedCase and round-trips on the shared fields
  ---
  duration_ms: 0.753875
  type: 'test'
  ...
# Subtest: fail-open: a throwing LLM stage keeps the deterministic state; junk input never throws
ok 2921 - fail-open: a throwing LLM stage keeps the deterministic state; junk input never throws
  ---
  duration_ms: 0.345
  type: 'test'
  ...
# Subtest: flag-off neutrality: no gate flag → feature inert; UI field off → {}
ok 2922 - flag-off neutrality: no gate flag → feature inert; UI field off → {}
  ---
  duration_ms: 0.509458
  type: 'test'
  ...
# Subtest: save-run reconstruction: same pure builders, schema-valid, per mode
ok 2923 - save-run reconstruction: same pure builders, schema-valid, per mode
  ---
  duration_ms: 0.685833
  type: 'test'
  ...
# Subtest: member link: strict validation, and identity stays OUT of the state
ok 2924 - member link: strict validation, and identity stays OUT of the state
  ---
  duration_ms: 0.20275
  type: 'test'
  ...
...
ok 2925 - 1 — a PRIMARY defect leaves the NORMATIVE run clean
  ---
  duration_ms: 1.81625
  type: 'test'
  ...
# Subtest: 2 — a NORMATIVE defect leaves the PRIMARY run clean
ok 2926 - 2 — a NORMATIVE defect leaves the PRIMARY run clean
  ---
  duration_ms: 0.316625
  type: 'test'
  ...
# Subtest: 3 — both roles dirty: both settle dirty
ok 2927 - 3 — both roles dirty: both settle dirty
  ---
  duration_ms: 0.442542
  type: 'test'
  ...
# Subtest: 4 — neither dirty: both settle clean
ok 2928 - 4 — neither dirty: both settle clean
  ---
  duration_ms: 0.211709
  type: 'test'
  ...
# Subtest: 4b — an EMPTY array is clean; an ABSENT key on a linkable run is NOT
ok 2929 - 4b — an EMPTY array is clean; an ABSENT key on a linkable run is NOT
  ---
  duration_ms: 0.2275
  type: 'test'
  ...
# Subtest: 5 — no map at all: a single-role save behaves exactly as before
ok 2930 - 5 — no map at all: a single-role save behaves exactly as before
  ---
  duration_ms: 3.409917
  type: 'test'
  ...
# Subtest: 5b — only the CLEAN branch is upgraded; a losing race or a skip is never made partial
ok 2931 - 5b — only the CLEAN branch is upgraded; a losing race or a skip is never made partial
  ---
  duration_ms: 0.092125
  type: 'test'
  ...
# Subtest: 5c — a revision-0 role is still not linked, and the per-run upgrade did not disturb that
ok 2932 - 5c — a revision-0 role is still not linked, and the per-run upgrade did not disturb that
  ---
...
ok 2933 - 6 — every persistence owner passes the role map, and none passes an empty one where it holds defects
  ---
  duration_ms: 1.175833
  type: 'test'
  ...
# Subtest: 6b — the upgrade is applied in settlement, not by the owners
ok 2934 - 6b — the upgrade is applied in settlement, not by the owners
  ---
  duration_ms: 0.8815
  type: 'test'
  ...
# Subtest: 6c — settleOwned still takes ONE base outcome and makes ONE settlement call
ok 2935 - 6c — settleOwned still takes ONE base outcome and makes ONE settlement call
  ---
...
ok 2936 - 6d — settlement is fail-safe: a role map on a handle with no runs settles nothing and throws nothing
  ---
  duration_ms: 0.053708
  type: 'test'
  ...
# Subtest: 7 — the three cases of `verdictForRun`, stated directly
ok 2937 - 7 — the three cases of `verdictForRun`, stated directly
  ---
  duration_ms: 0.381209
  type: 'test'
  ...
# Subtest: 7b — requirement 10: an INHERITED key is not a verdict
ok 2938 - 7b — requirement 10: an INHERITED key is not a verdict
  ---
  duration_ms: 0.159584
  type: 'test'
  ...
# Subtest: 7c — an inherited key does not rescue a run from partial, through settlement
ok 2939 - 7c — an inherited key does not rescue a run from partial, through settlement
  ---
  duration_ms: 0.158625
  type: 'test'
  ...
# Subtest: 7d — THE PLACEMENT TEST: the rule reaches a linkable run and stops at a revision-zero one
ok 2940 - 7d — THE PLACEMENT TEST: the rule reaches a linkable run and stops at a revision-zero one
  ---
  duration_ms: 0.236
  type: 'test'
  ...
# Subtest: 7e — requirement 3 in the only place it is observable: settlement never mutates the map
ok 2941 - 7e — requirement 3 in the only place it is observable: settlement never mutates the map
  ---
  duration_ms: 0.169833
  type: 'test'
  ...
# Subtest: 7f — the base outcome still decides: only a CLEAN run is made partial by a missing key
ok 2942 - 7f — the base outcome still decides: only a CLEAN run is made partial by a missing key
  ---
  duration_ms: 0.0705
  type: 'test'
  ...
# Subtest: 7g — no new settlement outcome value was added, and nothing writes the synthetic code
ok 2943 - 7g — no new settlement outcome value was added, and nothing writes the synthetic code
  ---
  duration_ms: 0.318708
  type: 'test'
  ...
# Subtest: matchRoomCategory prefers the longest alias and falls back
ok 2944 - matchRoomCategory prefers the longest alias and falls back
  ---
  duration_ms: 0.563625
  type: 'test'
  ...
# Subtest: excessBedDays = LOS − benchmark, floored at 0
ok 2945 - excessBedDays = LOS − benchmark, floored at 0
  ---
  duration_ms: 0.08225
  type: 'test'
  ...
# Subtest: computeBedDayCost: 8-day single room over-stay = 7 × 6500 = 45,500 (est.)
ok 2946 - computeBedDayCost: 8-day single room over-stay = 7 × 6500 = 45,500 (est.)
  ---
  duration_ms: 10.114542
  type: 'test'
  ...
# Subtest: computeBedDayCost returns 0 when not flagged, day-care, or single-day
ok 2947 - computeBedDayCost returns 0 when not flagged, day-care, or single-day
  ---
  duration_ms: 0.092708
  type: 'test'
  ...
# Subtest: tariff-status table drops the (est.) label
ok 2948 - tariff-status table drops the (est.) label
  ---
...
ok 2949 - the routes still declare the numbers this guard reads
  ---
  duration_ms: 1.402042
  type: 'test'
  ...
# Subtest: the body extractor is not vacuous — it finds real code, not an empty default param
ok 2950 - the body extractor is not vacuous — it finds real code, not an empty default param
  ---
  duration_ms: 1.328958
  type: 'test'
  ...
# Subtest: IPD_ANALYZE_LEGS equals the analyze call sites in lib/doc-audit.ts — a FOURTH leg fails here
ok 2951 - IPD_ANALYZE_LEGS equals the analyze call sites in lib/doc-audit.ts — a FOURTH leg fails here
  ---
  duration_ms: 0.531334
  type: 'test'
  ...
# Subtest: OPD_AUDIT_LEGS equals the EXECUTABLE legs in auditOpdNote — a third fails here
ok 2952 - OPD_AUDIT_LEGS equals the EXECUTABLE legs in auditOpdNote — a third fails here
  ---
  duration_ms: 0.830834
  type: 'test'
  ...
# Subtest: THE IPD WORKER FITS ITS BOX, for every provider that can serve it
ok 2953 - THE IPD WORKER FITS ITS BOX, for every provider that can serve it
  ---
  duration_ms: 0.390917
  type: 'test'
  ...
# Subtest: ipd-audit-now fits the same box on the same basis (DEC-B5)
ok 2954 - ipd-audit-now fits the same box on the same basis (DEC-B5)
  ---
  duration_ms: 0.107208
  type: 'test'
  ...
# Subtest: THE OPD WORKER FITS ITS BOX, for every provider that can serve it
ok 2955 - THE OPD WORKER FITS ITS BOX, for every provider that can serve it
  ---
  duration_ms: 0.362208
  type: 'test'
  ...
# Subtest: the OPD call site sends a per-attempt ceiling that matches its budget (DEC-B9)
ok 2956 - the OPD call site sends a per-attempt ceiling that matches its budget (DEC-B9)
  ---
  duration_ms: 0.758167
  type: 'test'
  ...
# Subtest: raising max, lowering the box, or adding a retry FAILS the guard
ok 2957 - raising max, lowering the box, or adding a retry FAILS the guard
  ---
  duration_ms: 0.273
  type: 'test'
  ...
# Subtest: a null budget means REFUSE, never substitute a default
ok 2958 - a null budget means REFUSE, never substitute a default
  ---
  duration_ms: 0.574375
  type: 'test'
  ...
# Subtest: the IPD cron interval clears the IPD box — and this is NOT extended to OPD
ok 2959 - the IPD cron interval clears the IPD box — and this is NOT extended to OPD
  ---
  duration_ms: 0.501541
  type: 'test'
  ...
# Subtest: OPENROUTER_TIMEOUT_MS and OPENROUTER_MAX_TRIES still read 110,000 and 3
ok 2960 - OPENROUTER_TIMEOUT_MS and OPENROUTER_MAX_TRIES still read 110,000 and 3
  ---
  duration_ms: 0.146
  type: 'test'
  ...
# Subtest: check mode → Runs + Interventions + CW_Flags + Citations, normalized by run_id
ok 2961 - check mode → Runs + Interventions + CW_Flags + Citations, normalized by run_id
  ---
  duration_ms: 2.397333
  type: 'test'
  ...
# Subtest: pathway mode → PathwayStages merges skeleton+enrichment by id, ordered
ok 2962 - pathway mode → PathwayStages merges skeleton+enrichment by id, ordered
  ---
  duration_ms: 0.18425
  type: 'test'
  ...
# Subtest: audit mode → findings/completeness/diff/suggestions/idealised/extracted/citations
ok 2963 - audit mode → findings/completeness/diff/suggestions/idealised/extracted/citations
  ---
  duration_ms: 0.234375
  type: 'test'
  ...
# Subtest: mergeRunSheets stacks rows across runs by sheet name
ok 2964 - mergeRunSheets stacks rows across runs by sheet name
  ---
  duration_ms: 0.153125
  type: 'test'
  ...
# Subtest: THE INVARIANT: all-Standard weighting reproduces legacy completeness EXACTLY
ok 2965 - THE INVARIANT: all-Standard weighting reproduces legacy completeness EXACTLY
  ---
  duration_ms: 2.050958
  type: 'test'
  ...
# Subtest: THE INVARIANT holds for the null vector too (PRD §8.1 fallback = legacy behaviour)
ok 2966 - THE INVARIANT holds for the null vector too (PRD §8.1 fallback = legacy behaviour)
  ---
  duration_ms: 0.151584
  type: 'test'
  ...
# Subtest: THE INVARIANT holds at all-Minor as well (PRD §8.4: mathematically identical to all-Standard)
ok 2967 - THE INVARIANT holds at all-Minor as well (PRD §8.4: mathematically identical to all-Standard)
  ---
  duration_ms: 0.201208
  type: 'test'
  ...
# Subtest: the fixture set actually exercises the hard cases (guards against a vacuous invariant)
ok 2968 - the fixture set actually exercises the hard cases (guards against a vacuous invariant)
  ---
  duration_ms: 0.135917
  type: 'test'
  ...
# Subtest: the na-policy divergence is REAL and this build takes the legacy branch (flagged deviation)
ok 2969 - the na-policy divergence is REAL and this build takes the legacy branch (flagged deviation)
  ---
  duration_ms: 0.121334
  type: 'test'
  ...
# Subtest: a CONDITIONAL na leaves both sides under BOTH policies (mandatoryTotal 20 vs 21, PRD §2.9)
ok 2970 - a CONDITIONAL na leaves both sides under BOTH policies (mandatoryTotal 20 vs 21, PRD §2.9)
  ---
  duration_ms: 0.090375
  type: 'test'
  ...
# Subtest: tier points are exactly Critical 8 · Important 4 · Standard 2 · Minor 1, and none is zero
ok 2971 - tier points are exactly Critical 8 · Important 4 · Standard 2 · Minor 1, and none is zero
  ---
  duration_ms: 0.0975
  type: 'test'
  ...
# Subtest: normalised weights sum to 100.0 ± 0.05 for every combination (PRD §10)
ok 2972 - normalised weights sum to 100.0 ± 0.05 for every combination (PRD §10)
  ---
  duration_ms: 0.560666
  type: 'test'
  ...
# Subtest: weighting actually MOVES the score when tiers differ (the change is not a no-op)
ok 2973 - weighting actually MOVES the score when tiers differ (the change is not a no-op)
  ---
  duration_ms: 1.164834
  type: 'test'
  ...
# Subtest: partial is exactly 0.5, and na is not partial
ok 2974 - partial is exactly 0.5, and na is not partial
  ---
  duration_ms: 0.331334
  type: 'test'
  ...
# Subtest: all-na document returns 100 without dividing by zero (PRD §8.5)
ok 2975 - all-na document returns 100 without dividing by zero (PRD §8.5)
  ---
  duration_ms: 0.068459
  type: 'test'
  ...
# Subtest: unknown key defaults to Standard; empty/garbage vector falls back to equal weights (PRD §8.2)
ok 2976 - unknown key defaults to Standard; empty/garbage vector falls back to equal weights (PRD §8.2)
  ---
  duration_ms: 0.062
  type: 'test'
  ...
# Subtest: malformed input never throws and never produces a wrong-looking score
ok 2977 - malformed input never throws and never produces a wrong-looking score
  ---
  duration_ms: 0.121334
  type: 'test'
  ...
# Subtest: rounding is half-up, applied via legacy's DOUBLE round
ok 2978 - rounding is half-up, applied via legacy's DOUBLE round
  ---
  duration_ms: 0.066
  type: 'test'
  ...
# Subtest: missingMandatory lists applicable missing fields by label (the unweighted gap count)
ok 2979 - missingMandatory lists applicable missing fields by label (the unweighted gap count)
  ---
  duration_ms: 0.039958
  type: 'test'
  ...
# Subtest: legacyCompleteness (the independent path) agrees with the null-vector weighted path
ok 2980 - legacyCompleteness (the independent path) agrees with the null-vector weighted path
  ---
  duration_ms: 0.245167
  type: 'test'
  ...
# Subtest: the re-stated domain weights match the closed cores VERBATIM (drift guard)
ok 2981 - the re-stated domain weights match the closed cores VERBATIM (drift guard)
  ---
  duration_ms: 0.185917
  type: 'test'
  ...
# Subtest: OPD index reproduces the core formula on a worked case
ok 2982 - OPD index reproduces the core formula on a worked case
  ---
  duration_ms: 0.089666
  type: 'test'
  ...
# Subtest: PDQI-9 absent ⇒ note_quality drops and the divisor is 0.75 (PRD §2.6)
ok 2983 - PDQI-9 absent ⇒ note_quality drops and the divisor is 0.75 (PRD §2.6)
  ---
  duration_ms: 0.049166
  type: 'test'
  ...
# Subtest: Care-Value Index reproduces the six-domain formula
ok 2984 - Care-Value Index reproduces the six-domain formula
  ---
  duration_ms: 0.052667
  type: 'test'
  ...
# Subtest: substituting a new documentation score moves the index and can re-band
ok 2985 - substituting a new documentation score moves the index and can re-band
  ---
  duration_ms: 0.051792
  type: 'test'
  ...
# Subtest: band boundaries at 39/40, 54/55, 69/70, 84/85
ok 2986 - band boundaries at 39/40, 54/55, 69/70, 84/85
  ---
  duration_ms: 0.045166
  type: 'test'
  ...
# Subtest: no domain scores at all ⇒ index 0, not NaN
ok 2987 - no domain scores at all ⇒ index 0, not NaN
  ---
  duration_ms: 0.0355
  type: 'test'
  ...
# Subtest: the weights-version label is exact (PRD §2.8, §8.3)
ok 2988 - the weights-version label is exact (PRD §2.8, §8.3)
  ---
  duration_ms: 0.048625
  type: 'test'
  ...
# Subtest: preview: an unchanged candidate moves nothing
ok 2989 - preview: an unchanged candidate moves nothing
  ---
  duration_ms: 0.368584
  type: 'test'
  ...
# Subtest: preview: making a widely-missing field Critical moves the mean and reports movers
ok 2990 - preview: making a widely-missing field Critical moves the mean and reports movers
  ---
  duration_ms: 6.604417
  type: 'test'
  ...
# Subtest: preview: empty cohort yields zeroed stats, no throw (the OPD empty state)
ok 2991 - preview: empty cohort yields zeroed stats, no throw (the OPD empty state)
  ---
  duration_ms: 0.076958
  type: 'test'
  ...
# Subtest: preview: SD is population SD and a single row has SD 0
ok 2992 - preview: SD is population SD and a single row has SD 0
  ---
  duration_ms: 0.054
  type: 'test'
  ...
# Subtest: missingPrevalence excludes `na` from the base, and reports a percentage
ok 2993 - missingPrevalence excludes `na` from the base, and reports a percentage
  ---
  duration_ms: 0.179333
  type: 'test'
  ...
# Subtest: systemic-defect warning fires only above 50% missing AND only at Critical; it never blocks
ok 2994 - systemic-defect warning fires only above 50% missing AND only at Critical; it never blocks
  ---
  duration_ms: 0.103125
  type: 'test'
  ...
# Subtest: the systemic-defect copy is verbatim per PRD §5.3
ok 2995 - the systemic-defect copy is verbatim per PRD §5.3
  ---
  duration_ms: 0.039542
  type: 'test'
  ...
# Subtest: scoreRow routes IPD and OPD to different index formulas
ok 2996 - scoreRow routes IPD and OPD to different index formulas
  ---
  duration_ms: 0.074417
  type: 'test'
  ...
# Subtest: the 21 discharge_summary fields match data/nabh-rubric.json EXACTLY (key, label, section)
ok 2997 - the 21 discharge_summary fields match data/nabh-rubric.json EXACTLY (key, label, section)
  ---
  duration_ms: 0.478417
  type: 'test'
  ...
# Subtest: cause_of_death is the ONE conditional key, read from the rubric
ok 2998 - cause_of_death is the ONE conditional key, read from the rubric
  ---
  duration_ms: 0.143625
  type: 'test'
  ...
# Subtest: the OPD label→key mapping covers every live-observed label (companion spec §4.7)
ok 2999 - the OPD label→key mapping covers every live-observed label (companion spec §4.7)
  ---
  duration_ms: 0.156125
  type: 'test'
  ...
# Subtest: the OPD engine's ACTUAL emitted keys are all in the catalogue (no orphan can appear)
ok 3000 - the OPD engine's ACTUAL emitted keys are all in the catalogue (no orphan can appear)
  ---
  duration_ms: 0.273959
  type: 'test'
  ...
# Subtest: the OPD structured emission is ADDITIVE: status/section added, present/mandatory preserved
ok 3001 - the OPD structured emission is ADDITIVE: status/section added, present/mandatory preserved
  ---
  duration_ms: 37.361667
  type: 'test'
  ...
# Subtest: the OPD engine emits the structured shape from BOTH completeness paths (GP and obstetric)
ok 3002 - the OPD engine emits the structured shape from BOTH completeness paths (GP and obstetric)
  ---
...
ok 3036 - E-1 (finding 36): a time-critical cardiac pattern in the finding text promotes to tier 1 — from any kind
  ---
...
ok 3037 - E-2 (finding 49): persistent swelling ≥ 4 weeks with no follow-through promotes; with follow-through it does not
  ---
  duration_ms: 1.488708
  type: 'test'
  ...
# Subtest: praise never escalates: a high-value finding praising an appropriate ACS referral stays praise
ok 3038 - praise never escalates: a high-value finding praising an appropriate ACS referral stays praise
  ---
  duration_ms: 0.07725
  type: 'test'
  ...
# Subtest: bucketByTier: buckets are disjoint, complete, and count unlisted kinds
ok 3039 - bucketByTier: buckets are disjoint, complete, and count unlisted kinds
  ---
  duration_ms: 0.09175
  type: 'test'
  ...
...
ok 3040 - dedupeTwins: same (finding_ref, doctor_uid, day) collapses with an occurrence count; different notes same day still collapse; different days do not
  ---
  duration_ms: 0.398667
  type: 'test'
  ...
# Subtest: dedupeTwins: unkeyable rows (no ref / no doctor / no date) never merge
ok 3041 - dedupeTwins: unkeyable rows (no ref / no doctor / no date) never merge
  ---
  duration_ms: 0.039042
  type: 'test'
  ...
# Subtest: allows SELECT / WITH and auto-adds LIMIT
ok 3042 - allows SELECT / WITH and auto-adds LIMIT
  ---
  duration_ms: 1.034125
  type: 'test'
  ...
# Subtest: rejects writes, DDL, multiple statements, non-SELECT, over-cap LIMIT, system fns
ok 3043 - rejects writes, DDL, multiple statements, non-SELECT, over-cap LIMIT, system fns
  ---
  duration_ms: 0.098417
  type: 'test'
  ...
# Subtest: blocks PHI-bearing relations anywhere in the query
ok 3044 - blocks PHI-bearing relations anywhere in the query
  ---
  duration_ms: 0.408084
  type: 'test'
  ...
# Subtest: honors a smaller caller cap
ok 3045 - honors a smaller caller cap
  ---
...
ok 3046 - coverage floor (PRD §4): ≥5 dose-ceiling, ≥3 SOS, ≥3 banned-FDC, ≥4 interaction, ≥4 incomplete-dosing positives; exactly 6 negatives
  ---
  duration_ms: 0.630542
  type: 'test'
  ...
# Subtest: fixtures carry no PHI, no db13 uid, and banned-FDC fixtures use placeholder molecules only
ok 3047 - fixtures carry no PHI, no db13 uid, and banned-FDC fixtures use placeholder molecules only
  ---
...
ok 3048 - positive POS-DOSE-1 fires dose_ceiling_exceeded — paracetamol stacked across two products: 650 QID + 500 TDS = 4100 mg/day > 4000
  ---
...
ok 3049 - positive POS-DOSE-2 fires dose_ceiling_exceeded — ibuprofen 800 QID = 3200 mg/day > 2400 (single product)
  ---
  duration_ms: 0.033
  type: 'test'
  ...
# Subtest: positive POS-DOSE-3 fires dose_ceiling_exceeded — diclofenac 75 TDS = 225 mg/day > 150
ok 3050 - positive POS-DOSE-3 fires dose_ceiling_exceeded — diclofenac 75 TDS = 225 mg/day > 150
  ---
  duration_ms: 0.036083
  type: 'test'
  ...
...
ok 3051 - positive POS-DOSE-4 fires dose_ceiling_exceeded — etoricoxib 120 OD with NO documented gout > the 90 mg/day default ceiling (Decision 6)
  ---
  duration_ms: 0.028
  type: 'test'
  ...
# Subtest: positive POS-DOSE-5 fires dose_ceiling_exceeded — mefenamic acid 500 QID = 2000 mg/day > 1500
ok 3052 - positive POS-DOSE-5 fires dose_ceiling_exceeded — mefenamic acid 500 QID = 2000 mg/day > 1500
  ---
  duration_ms: 0.088291
  type: 'test'
  ...
...
ok 3053 - positive POS-SOS-1 fires dose_ceiling_sos — paracetamol 1000 TDS scheduled + 650 SOS uncapped (default cap 3) → 4950 potential > 4000
  ---
  duration_ms: 0.0315
  type: 'test'
  ...
# Subtest: positive POS-SOS-2 fires dose_ceiling_sos — etoricoxib 90 SOS with an EXPLICIT max 2/day → 180 potential > 90
ok 3054 - positive POS-SOS-2 fires dose_ceiling_sos — etoricoxib 90 SOS with an EXPLICIT max 2/day → 180 potential > 90
  ---
...
ok 3055 - positive POS-SOS-3 fires dose_ceiling_sos — ibuprofen 600 grid 1-0-1 + 600 SOS uncapped → 3000 potential > 2400
  ---
  duration_ms: 0.26875
  type: 'test'
  ...
# Subtest: positive POS-FDC-1 fires banned_fdc — exact two-molecule banned set (placeholders mol-a + mol-b)
ok 3056 - positive POS-FDC-1 fires banned_fdc — exact two-molecule banned set (placeholders mol-a + mol-b)
  ---
...
ok 3057 - positive POS-FDC-2 fires banned_fdc — exact three-molecule banned set (placeholders mol-c/mol-d/mol-e)
  ---
...
ok 3058 - positive POS-FDC-3 fires banned_fdc — order-swapped banned pair (mol-b + mol-a) still matches the stored set
  ---
  duration_ms: 0.023375
  type: 'test'
  ...
# Subtest: positive POS-DDI-1 fires drug_interaction — warfarin + ibuprofen — anticoagulant + NSAID (major)
ok 3059 - positive POS-DDI-1 fires drug_interaction — warfarin + ibuprofen — anticoagulant + NSAID (major)
  ---
  duration_ms: 0.022375
  type: 'test'
  ...
# Subtest: positive POS-DDI-2 fires drug_interaction — atorvastatin + clarithromycin — statin + macrolide (major)
ok 3060 - positive POS-DDI-2 fires drug_interaction — atorvastatin + clarithromycin — statin + macrolide (major)
  ---
  duration_ms: 0.126709
  type: 'test'
  ...
# Subtest: positive POS-DDI-3 fires drug_interaction — sertraline + tramadol — two serotonergic drugs (major)
ok 3061 - positive POS-DDI-3 fires drug_interaction — sertraline + tramadol — two serotonergic drugs (major)
  ---
  duration_ms: 0.099917
  type: 'test'
  ...
# Subtest: positive POS-DDI-4 fires drug_interaction — telmisartan + spironolactone — ACE-I/ARB + potassium-sparing diuretic (major)
ok 3062 - positive POS-DDI-4 fires drug_interaction — telmisartan + spironolactone — ACE-I/ARB + potassium-sparing diuretic (major)
  ---
  duration_ms: 0.076792
  type: 'test'
  ...
# Subtest: positive POS-DOSING-1 fires incomplete_dosing — dose/strength blanked (no dose field, no strength, none in the name)
ok 3063 - positive POS-DOSING-1 fires incomplete_dosing — dose/strength blanked (no dose field, no strength, none in the name)
  ---
  duration_ms: 0.062625
  type: 'test'
  ...
# Subtest: positive POS-DOSING-2 fires incomplete_dosing — frequency blanked
ok 3064 - positive POS-DOSING-2 fires incomplete_dosing — frequency blanked
  ---
  duration_ms: 0.057583
  type: 'test'
  ...
# Subtest: positive POS-DOSING-3 fires incomplete_dosing — duration blanked
ok 3065 - positive POS-DOSING-3 fires incomplete_dosing — duration blanked
  ---
...
ok 3066 - positive POS-DOSING-4 fires incomplete_dosing — route blanked and not inferable (no dosage-form word anywhere on the line)
  ---
  duration_ms: 0.057208
  type: 'test'
  ...
# Subtest: negative NEG-1 stays silent — ibuprofen 800 TDS = exactly 2400 mg/day — AT the ceiling, not over it
ok 3067 - negative NEG-1 stays silent — ibuprofen 800 TDS = exactly 2400 mg/day — AT the ceiling, not over it
  ---
...
ok 3068 - negative NEG-2 stays silent — etoricoxib 120 OD WITH a documented gout diagnosis — the conditional 120 ceiling applies
  ---
  duration_ms: 0.059792
  type: 'test'
  ...
# Subtest: negative NEG-3 stays silent — amoxicillin + paracetamol — just OUTSIDE every interaction pair (no shared mechanism tag)
ok 3069 - negative NEG-3 stays silent — amoxicillin + paracetamol — just OUTSIDE every interaction pair (no shared mechanism tag)
  ---
  duration_ms: 0.058417
  type: 'test'
  ...
# Subtest: negative NEG-4 stays silent — a COMPLETE prescription — dose, frequency, duration and route all present
ok 3070 - negative NEG-4 stays silent — a COMPLETE prescription — dose, frequency, duration and route all present
  ---
...
ok 3071 - negative NEG-5 stays silent — banned core + one extra molecule (mol-a + mol-b + mol-z) — the C5 superset boundary
  ---
  duration_ms: 0.056958
  type: 'test'
  ...
# Subtest: negative NEG-6 stays silent — paracetamol 500 SOS max 3/day = 1500 mg potential — well inside the 4000 ceiling
ok 3072 - negative NEG-6 stays silent — paracetamol 500 SOS max 3/day = 1500 mg potential — well inside the 4000 ceiling
  ---
  duration_ms: 0.051875
  type: 'test'
  ...
# Subtest: recall_det = fired / planted, over the deterministic leg only (no LLM recall claim — PRD §6)
ok 3073 - recall_det = fired / planted, over the deterministic leg only (no LLM recall claim — PRD §6)
  ---
  duration_ms: 0.084667
  type: 'test'
  ...
# Subtest: 57 case 1 — production Vercel build with no key at all: MISSING
ok 3074 - 57 case 1 — production Vercel build with no key at all: MISSING
  ---
  duration_ms: 0.519166
  type: 'test'
  ...
# Subtest: 57 case 2 — production Vercel build with an unusable key (empty or whitespace): MISSING
ok 3075 - 57 case 2 — production Vercel build with an unusable key (empty or whitespace): MISSING
  ---
  duration_ms: 0.068125
  type: 'test'
  ...
# Subtest: 57 case 3 — production Vercel build with a usable key: NOT missing
ok 3076 - 57 case 3 — production Vercel build with a usable key: NOT missing
  ---
  duration_ms: 0.048625
  type: 'test'
  ...
# Subtest: 57 case 4 — a Vercel build that is not production: NOT missing, at any key value
ok 3077 - 57 case 4 — a Vercel build that is not production: NOT missing, at any key value
  ---
  duration_ms: 0.395833
  type: 'test'
  ...
# Subtest: 57 case 5 — not a Vercel build, even when the environment says production: NOT missing
ok 3078 - 57 case 5 — not a Vercel build, even when the environment says production: NOT missing
  ---
  duration_ms: 0.18025
  type: 'test'
  ...
# Subtest: 57 EXECUTED — importing the config in production with no key throws, and names the variable
ok 3079 - 57 EXECUTED — importing the config in production with no key throws, and names the variable
  ---
  duration_ms: 44.730333
  type: 'test'
  ...
# Subtest: 57 EXECUTED — a production import WITH a key succeeds
ok 3080 - 57 EXECUTED — a production import WITH a key succeeds
  ---
  duration_ms: 41.562833
  type: 'test'
  ...
# Subtest: 57 EXECUTED — a non-production import with no key succeeds
ok 3081 - 57 EXECUTED — a non-production import with no key succeeds
  ---
  duration_ms: 44.538375
  type: 'test'
  ...
# Subtest: 57 whole file — it parses, and holds EXACTLY three top-level statements in order
ok 3082 - 57 whole file — it parses, and holds EXACTLY three top-level statements in order
  ---
  duration_ms: 7.331083
  type: 'test'
  ...
# Subtest: 57 whole file — the declaration and the export are exactly what they must be
ok 3083 - 57 whole file — the declaration and the export are exactly what they must be
  ---
  duration_ms: 0.459459
  type: 'test'
  ...
# Subtest: 57 whole file — nothing executable outside the guard
ok 3084 - 57 whole file — nothing executable outside the guard
  ---
  duration_ms: 0.140417
  type: 'test'
  ...
# Subtest: 57 pin — each copy is exactly D8's three clauses, and there is no fourth of any kind
ok 3085 - 57 pin — each copy is exactly D8's three clauses, and there is no fourth of any kind
  ---
  duration_ms: 0.384542
  type: 'test'
  ...
# Subtest: 57 pin — next.config.mjs and telemetry-key-guard.ts express the SAME condition
ok 3086 - 57 pin — next.config.mjs and telemetry-key-guard.ts express the SAME condition
  ---
  duration_ms: 1.246833
  type: 'test'
  ...
# Subtest: 57 pin — the guard THROWS one Error, and the message a reader sees names the variable
ok 3087 - 57 pin — the guard THROWS one Error, and the message a reader sees names the variable
  ---
  duration_ms: 0.377459
  type: 'test'
  ...
# Subtest: no surface outside the allow-list SELECTs from the telemetry tables
ok 3088 - no surface outside the allow-list SELECTs from the telemetry tables
  ---
  duration_ms: 151.350333
  type: 'test'
  ...
# Subtest: the scan can actually fail — it is not passing because the matcher never matches
ok 3089 - the scan can actually fail — it is not passing because the matcher never matches
  ---
  duration_ms: 0.1415
  type: 'test'
  ...
# Subtest: the allow-list is by EXACT path, and every entry is one this build owns
ok 3090 - the allow-list is by EXACT path, and every entry is one this build owns
  ---
  duration_ms: 0.186083
  type: 'test'
  ...
# Subtest: no clinician-facing or patient-facing route names a telemetry table at all
ok 3091 - no clinician-facing or patient-facing route names a telemetry table at all
  ---
  duration_ms: 33.47925
  type: 'test'
  ...
# Subtest: GUARD 1 — admin: a set ADMIN_TOKEN with nothing presented is refused, and isAdminUnlocked is NOT consulted
ok 3092 - GUARD 1 — admin: a set ADMIN_TOKEN with nothing presented is refused, and isAdminUnlocked is NOT consulted
  ---
  duration_ms: 303.316084
  type: 'test'
  ...
# Subtest: GUARD 3 — preview: VERCEL_ENV=production is refused even with everything else correct
ok 3093 - GUARD 3 — preview: VERCEL_ENV=production is refused even with everything else correct
  ---
  duration_ms: 269.565167
  type: 'test'
  ...
# Subtest: GUARD 3 — preview: VERCEL_ENV=preview on a DIFFERENT branch is refused
ok 3094 - GUARD 3 — preview: VERCEL_ENV=preview on a DIFFERENT branch is refused
  ---
  duration_ms: 274.757166
  type: 'test'
  ...
# Subtest: GUARD 4 — arming: an unset CDMSS_OVERHEAD_MEASURE is refused
ok 3095 - GUARD 4 — arming: an unset CDMSS_OVERHEAD_MEASURE is refused
  ---
  duration_ms: 236.279166
  type: 'test'
  ...
# Subtest: GUARD 5 — THE ONE THAT MATTERS: a production endpoint id is refused
ok 3096 - GUARD 5 — THE ONE THAT MATTERS: a production endpoint id is refused
  ---
  duration_ms: 165.955125
  type: 'test'
  ...
# Subtest: GUARD 5 — an UNSET expectation refuses, it does not pass
ok 3097 - GUARD 5 — an UNSET expectation refuses, it does not pass
  ---
  duration_ms: 147.591542
  type: 'test'
  ...
# Subtest: GUARD 5 — an unparseable DATABASE_URL refuses
ok 3098 - GUARD 5 — an unparseable DATABASE_URL refuses
  ---
  duration_ms: 142.704291
  type: 'test'
  ...
# Subtest: GUARD 5 — a password containing @ cannot shift the parsed host
ok 3099 - GUARD 5 — a password containing @ cannot shift the parsed host
  ---
  duration_ms: 144.778042
  type: 'test'
  ...
# Subtest: GUARD 2 — expiry: past the hard UTC date every request is 410
ok 3100 - GUARD 2 — expiry: past the hard UTC date every request is 410
  ---
  duration_ms: 143.447209
  type: 'test'
  ...
# Subtest: GUARD 2 — before the expiry the route still runs
ok 3101 - GUARD 2 — before the expiry the route still runs
  ---
  duration_ms: 150.694791
  type: 'test'
  ...
# Subtest: ALL FIVE PASS — the route runs, writes route=script, and reports the first sample separately
ok 3102 - ALL FIVE PASS — the route runs, writes route=script, and reports the first sample separately
  ---
  duration_ms: 152.103208
  type: 'test'
  ...
# Subtest: FIX 2 + FIX 1 — a REAL 500, driven by an unparseable URL that still satisfies guard 5
ok 3103 - FIX 2 + FIX 1 — a REAL 500, driven by an unparseable URL that still satisfies guard 5
  ---
  duration_ms: 149.29225
  type: 'test'
  ...
# Subtest: v5 FIX 1 — two of the three leaking shapes are refused BEFORE the driver ever sees them
ok 3104 - v5 FIX 1 — two of the three leaking shapes are refused BEFORE the driver ever sees them
  ---
  duration_ms: 292.604333
  type: 'test'
  ...
# Subtest: FIX 3 — a query parameter cannot move the parsed host away from the one the driver uses
ok 3105 - FIX 3 — a query parameter cannot move the parsed host away from the one the driver uses
  ---
  duration_ms: 149.390083
  type: 'test'
  ...
# Subtest: FIX 4 — the denylist refuses production even when the expected value also names it
ok 3106 - FIX 4 — the denylist refuses production even when the expected value also names it
  ---
  duration_ms: 143.66075
  type: 'test'
  ...
# Subtest: FIX 4 — an ABSENT denylist refuses: a denylist that is not there is not a denylist
ok 3107 - FIX 4 — an ABSENT denylist refuses: a denylist that is not there is not a denylist
  ---
  duration_ms: 142.543958
  type: 'test'
  ...
# Subtest: FIX 5 — p95 and p99 are withheld below their floors, never a maximum in disguise
ok 3108 - FIX 5 — p95 and p99 are withheld below their floors, never a maximum in disguise
  ---
  duration_ms: 296.580167
  type: 'test'
  ...
# Subtest: FIX 5 — and p95 IS emitted once its floor is met, so the floor is not a blanket refusal
ok 3109 - FIX 5 — and p95 IS emitted once its floor is met, so the floor is not a blanket refusal
  ---
  duration_ms: 148.9595
  type: 'test'
  ...
# Subtest: FIX 6 — the invocation insert has its own cell
ok 3110 - FIX 6 — the invocation insert has its own cell
  ---
  duration_ms: 295.505792
  type: 'test'
  ...
# Subtest: FIX 7 — the shape is true per cell, and conc is gone
ok 3111 - FIX 7 — the shape is true per cell, and conc is gone
  ---
  duration_ms: 291.978916
  type: 'test'
  ...
# Subtest: FIX 8 — the real-audit arm refuses rather than silently becoming the null arm
ok 3112 - FIX 8 — the real-audit arm refuses rather than silently becoming the null arm
  ---
  duration_ms: 445.940417
  type: 'test'
  ...
# Subtest: v6 FIX 1 — the branch POOLED host passes against the bare expected id
ok 3113 - v6 FIX 1 — the branch POOLED host passes against the bare expected id
  ---
  duration_ms: 145.739083
  type: 'test'
  ...
# Subtest: v6 FIX 1 — the branch DIRECT host passes against the bare expected id
ok 3114 - v6 FIX 1 — the branch DIRECT host passes against the bare expected id
  ---
  duration_ms: 147.699708
  type: 'test'
  ...
# Subtest: v6 FIX 1 — the branch POOLED host passes against a POOLED expected id
ok 3115 - v6 FIX 1 — the branch POOLED host passes against a POOLED expected id
  ---
  duration_ms: 144.698334
  type: 'test'
  ...
# Subtest: v6 FIX 2 — production on its POOLED host refuses forbidden_endpoint, NOT endpoint_mismatch
ok 3116 - v6 FIX 2 — production on its POOLED host refuses forbidden_endpoint, NOT endpoint_mismatch
  ---
  duration_ms: 142.187125
  type: 'test'
  ...
# Subtest: v6 FIX 2 — production on its DIRECT host also refuses forbidden_endpoint
ok 3117 - v6 FIX 2 — production on its DIRECT host also refuses forbidden_endpoint
  ---
  duration_ms: 146.139916
  type: 'test'
  ...
# Subtest: v6 — `pooler` in the MIDDLE of a label is part of the id and is never truncated
ok 3118 - v6 — `pooler` in the MIDDLE of a label is part of the id and is never truncated
  ---
  duration_ms: 298.001375
  type: 'test'
  ...
# Subtest: v6 — a doubled `-pooler-pooler` strips exactly ONE, and that is the stated rule
ok 3119 - v6 — a doubled `-pooler-pooler` strips exactly ONE, and that is the stated rule
  ---
  duration_ms: 445.363875
  type: 'test'
  ...
# Subtest: v6 — normalisation cannot make two DIFFERENT endpoints compare equal
ok 3120 - v6 — normalisation cannot make two DIFFERENT endpoints compare equal
  ---
  duration_ms: 575.625542
  type: 'test'
  ...
# Subtest: NO OUTPUT ANYWHERE CARRIES A DATABASE_URL SUBSTRING — every response shape, stdout and stderr
ok 3121 - NO OUTPUT ANYWHERE CARRIES A DATABASE_URL SUBSTRING — every response shape, stdout and stderr
  ---
  duration_ms: 1597.567291
  type: 'test'
  ...
# Subtest: the route is POST-only, and carries its own expiry and deletion notice in source
ok 3122 - the route is POST-only, and carries its own expiry and deletion notice in source
  ---
...
ok 3123 - DEFAULT-OFF: unset ⇒ no cap (the shipped path is uncapped and must stay so)
  ---
  duration_ms: 0.439916
  type: 'test'
  ...
# Subtest: DEFAULT-OFF: 0, negative, junk and empty all mean "no cap", never a cap of 0
ok 3124 - DEFAULT-OFF: 0, negative, junk and empty all mean "no cap", never a cap of 0
  ---
  duration_ms: 0.095291
  type: 'test'
  ...
# Subtest: a set budget is honored, floored to an integer (the arms: 1647 / 823 / 128)
ok 3125 - a set budget is honored, floored to an integer (the arms: 1647 / 823 / 128)
  ---
  duration_ms: 0.577583
  type: 'test'
  ...
# Subtest: the cap rides the SL0-verified wire format (top-level google.thinking_config)
ok 3126 - the cap rides the SL0-verified wire format (top-level google.thinking_config)
  ---
  duration_ms: 0.727167
  type: 'test'
  ...
# Subtest: the cap is Gemini-only and cannot leak onto the Ollama fallback path
ok 3127 - the cap is Gemini-only and cannot leak onto the Ollama fallback path
  ---
  duration_ms: 0.209375
  type: 'test'
  ...
# Subtest: gen_params records the budget ONLY when capped — an uncapped trace is unchanged
ok 3128 - gen_params records the budget ONLY when capped — an uncapped trace is unchanged
  ---
  duration_ms: 0.148666
  type: 'test'
  ...
# Subtest: note-audit row → ClinicalFinding: verbatim vocab in the audit ext, valid core
ok 3129 - note-audit row → ClinicalFinding: verbatim vocab in the audit ext, valid core
  ---
  duration_ms: 1.845208
  type: 'test'
  ...
# Subtest: LOSSLESS: note-audit row round-trips byte-for-byte, incl. unmapped engine fields
ok 3130 - LOSSLESS: note-audit row round-trips byte-for-byte, incl. unmapped engine fields
  ---
  duration_ms: 0.460459
  type: 'test'
  ...
# Subtest: note-audit round-trip preserves absence: a minimal row gains no keys
ok 3131 - note-audit round-trip preserves absence: a minimal row gains no keys
  ---
  duration_ms: 0.116917
  type: 'test'
  ...
# Subtest: deterministic-source row maps to extractionMethod deterministic
ok 3132 - deterministic-source row maps to extractionMethod deterministic
  ---
  duration_ms: 0.05625
  type: 'test'
  ...
# Subtest: doc-audit AuditFinding → ClinicalFinding: verdict rides in ext.netValue (verbatim, separate slot)
ok 3133 - doc-audit AuditFinding → ClinicalFinding: verdict rides in ext.netValue (verbatim, separate slot)
  ---
  duration_ms: 0.397041
  type: 'test'
  ...
# Subtest: LOSSLESS: doc-audit AuditFinding round-trips byte-for-byte
ok 3134 - LOSSLESS: doc-audit AuditFinding round-trips byte-for-byte
  ---
  duration_ms: 0.1455
  type: 'test'
  ...
# Subtest: ExtractedCase → ClinicalState: clinical content in the core, metadata in surfaceExtras
ok 3135 - ExtractedCase → ClinicalState: clinical content in the core, metadata in surfaceExtras
  ---
  duration_ms: 0.6005
  type: 'test'
  ...
# Subtest: LOSSLESS: ExtractedCase round-trips byte-for-byte (full PX discharge shape)
ok 3136 - LOSSLESS: ExtractedCase round-trips byte-for-byte (full PX discharge shape)
  ---
  duration_ms: 0.129834
  type: 'test'
  ...
# Subtest: LOSSLESS: a sparse pre-PX ExtractedCase (no riskFactors/aftercare/completeness) round-trips
ok 3137 - LOSSLESS: a sparse pre-PX ExtractedCase (no riskFactors/aftercare/completeness) round-trips
  ---
  duration_ms: 0.254
  type: 'test'
  ...
...
ok 3153 - a total transport failure records no served provider, and says so explicitly
  ---
  duration_ms: 0.569709
  type: 'test'
  ...
# Subtest: a null attempt list means NOT COLLECTED, and is distinguishable from an empty one
ok 3154 - a null attempt list means NOT COLLECTED, and is distinguishable from an empty one
  ---
  duration_ms: 0.322459
  type: 'test'
  ...
# Subtest: every terminal phase is a stable NAME — never a message, never an interpolated value
ok 3155 - every terminal phase is a stable NAME — never a message, never an interpolated value
  ---
  duration_ms: 0.589958
  type: 'test'
  ...
# Subtest: the intended-local path records ONE attempt rather than an empty list
ok 3156 - the intended-local path records ONE attempt rather than an empty list
  ---
  duration_ms: 0.193875
  type: 'test'
  ...
# Subtest: both local arms record their attempt — the by-design one and the substitution
ok 3157 - both local arms record their attempt — the by-design one and the substitution
  ---
  duration_ms: 0.214083
  type: 'test'
  ...
# Subtest: the local success attempt is well-formed, and is at most one per invocation
ok 3158 - the local success attempt is well-formed, and is at most one per invocation
  ---
  duration_ms: 0.240667
  type: 'test'
  ...
# Subtest: classifyLocalAttempt reads what the SDK declared, and guesses nothing
ok 3159 - classifyLocalAttempt reads what the SDK declared, and guesses nothing
  ---
  duration_ms: 0.237125
  type: 'test'
  ...
# Subtest: the 429 rule has exactly one home — classifyLocalAttempt delegates, never duplicates
ok 3160 - the 429 rule has exactly one home — classifyLocalAttempt delegates, never duplicates
  ---
  duration_ms: 0.272375
  type: 'test'
  ...
# Subtest: the two terminal throws are BYTE-IDENTICAL to before this build
ok 3161 - the two terminal throws are BYTE-IDENTICAL to before this build
  ---
  duration_ms: 0.3375
  type: 'test'
  ...
# Subtest: the phase selector cannot drift from the throws it describes
ok 3162 - the phase selector cannot drift from the throws it describes
  ---
  duration_ms: 0.511583
  type: 'test'
  ...
# Subtest: the failure attach is a statement, not a control-flow change
ok 3163 - the failure attach is a statement, not a control-flow change
  ---
  duration_ms: 0.263
  type: 'test'
  ...
# Subtest: nothing in the failure path touches the outbound request object
ok 3164 - nothing in the failure path touches the outbound request object
  ---
  duration_ms: 0.236375
  type: 'test'
  ...
# Subtest: the local calls are wrapped, and the SDK call expression itself is unchanged
ok 3165 - the local calls are wrapped, and the SDK call expression itself is unchanged
  ---
  duration_ms: 0.193584
  type: 'test'
  ...
# Subtest: failure evidence is a SEPARATE property, invisible to every success-attribution reader
ok 3166 - failure evidence is a SEPARATE property, invisible to every success-attribution reader
  ---
  duration_ms: 0.080541
  type: 'test'
  ...
# Subtest: the low-value-care judge reader is untouched — it reads two fields of the success shape
ok 3167 - the low-value-care judge reader is untouched — it reads two fields of the success shape
  ---
  duration_ms: 0.190625
  type: 'test'
  ...
# Subtest: tracedChat is not touched — D14 is scoped to the traceless arm
ok 3168 - tracedChat is not touched — D14 is scoped to the traceless arm
  ---
  duration_ms: 0.29425
  type: 'test'
  ...
# Subtest: every new name also resolves through lib/trace.ts, so no importer has to know the core path
ok 3169 - every new name also resolves through lib/trace.ts, so no importer has to know the core path
  ---
  duration_ms: 0.049833
  type: 'test'
  ...
# Subtest: failure evidence is IMMUTABLE — a later frame cannot rewrite what failed
ok 3170 - failure evidence is IMMUTABLE — a later frame cannot rewrite what failed
  ---
  duration_ms: 0.084209
  type: 'test'
  ...
# Subtest: a hostile or exotic error cannot break the transport
ok 3171 - a hostile or exotic error cannot break the transport
  ---
  duration_ms: 0.112875
  type: 'test'
  ...
# Subtest: failure evidence carries enums and counts only — no message, no body, no identifier
ok 3172 - failure evidence carries enums and counts only — no message, no body, no identifier
  ---
  duration_ms: 0.087
  type: 'test'
  ...
# Subtest: the attempt shape admits the local provider, and nothing else new
ok 3173 - the attempt shape admits the local provider, and nothing else new
  ---
  duration_ms: 0.20975
  type: 'test'
  ...
# Subtest: a finish_reason defect is NOT retryable; an empty 200 still is
ok 3174 - a finish_reason defect is NOT retryable; an empty 200 still is
  ---
  duration_ms: 0.6275
  type: 'test'
  ...
# Subtest: THE 54 SECONDS: a truncating call is attempted ONCE, not three times
ok 3175 - THE 54 SECONDS: a truncating call is attempted ONCE, not three times
  ---
  duration_ms: 0.466709
  type: 'test'
  ...
# Subtest: …and the empty-200 retry budget is spent in full, exactly as before
ok 3176 - …and the empty-200 retry budget is spent in full, exactly as before
  ---
  duration_ms: 0.309292
  type: 'test'
  ...
# Subtest: the terminal error still names the truncation, so the sizing bug is readable
ok 3177 - the terminal error still names the truncation, so the sizing bug is readable
  ---
  duration_ms: 0.118958
  type: 'test'
  ...
# Subtest: transport failures are untouched by this rule — only BODY verdicts changed
ok 3178 - transport failures are untouched by this rule — only BODY verdicts changed
  ---
  duration_ms: 0.397209
  type: 'test'
  ...
# Subtest: the mini-sized cap is raised to a FLOOR on the bedrock path
ok 3179 - the mini-sized cap is raised to a FLOOR on the bedrock path
  ---
  duration_ms: 0.157458
  type: 'test'
  ...
# Subtest: ⚠️ BYTE-IDENTITY: the floor is the BEDROCK transport’s, and reaches no other provider
ok 3180 - ⚠️ BYTE-IDENTITY: the floor is the BEDROCK transport’s, and reaches no other provider
  ---
  duration_ms: 0.831958
  type: 'test'
  ...
# Subtest: a critique that never completed is recorded as UNAUDITED, not as clean
ok 3181 - a critique that never completed is recorded as UNAUDITED, not as clean
  ---
  duration_ms: 0.925
  type: 'test'
  ...
# Subtest: the probe reducers carry critic_ran, so a lab row can tell the two apart
ok 3182 - the probe reducers carry critic_ran, so a lab row can tell the two apart
  ---
  duration_ms: 0.489291
  type: 'test'
  ...
# Subtest: unknown_finding + missing_critical + instability_input derive from a ClinicalState
ok 3183 - unknown_finding + missing_critical + instability_input derive from a ClinicalState
  ---
  duration_ms: 8.037625
  type: 'test'
  ...
# Subtest: med_contradiction derives from an open medication conflict (member stateRef)
ok 3184 - med_contradiction derives from an open medication conflict (member stateRef)
  ---
  duration_ms: 0.269584
  type: 'test'
  ...
# Subtest: med_contradiction: conflict on an episode HIGH-ALERT med is safety-critical
ok 3185 - med_contradiction: conflict on an episode HIGH-ALERT med is safety-critical
  ---
  duration_ms: 0.601959
  type: 'test'
  ...
# Subtest: med_contradiction also derives from reconciled status stopped/not_taking/unknown without a conflict row
ok 3186 - med_contradiction also derives from reconciled status stopped/not_taking/unknown without a conflict row
  ---
  duration_ms: 0.548292
  type: 'test'
  ...
# Subtest: new_medication (B5): derives ONLY for meds absent from a NON-EMPTY snapshot med list
ok 3187 - new_medication (B5): derives ONLY for meds absent from a NON-EMPTY snapshot med list
  ---
  duration_ms: 0.388292
  type: 'test'
  ...
# Subtest: new_medication (B5): a high-alert episode med is skipped (it wins rank 0 anyway)
ok 3188 - new_medication (B5): a high-alert episode med is skipped (it wins rank 0 anyway)
  ---
  duration_ms: 0.0845
  type: 'test'
  ...
# Subtest: care_gap derives from a stale mapped-range abnormal (detail verbatim, severity mapped)
ok 3189 - care_gap derives from a stale mapped-range abnormal (detail verbatim, severity mapped)
  ---
  duration_ms: 0.365584
  type: 'test'
  ...
# Subtest: followup_open derives from advice keywords; suppressed when a committed follow-up matches
ok 3190 - followup_open derives from advice keywords; suppressed when a committed follow-up matches
  ---
  duration_ms: 0.303417
  type: 'test'
  ...
# Subtest: allergy_unconfirmed only when the note allergy field is blank
ok 3191 - allergy_unconfirmed only when the note allergy field is blank
  ---
  duration_ms: 0.21625
  type: 'test'
  ...
# Subtest: snapshot absent ⇒ member-derived kinds simply absent (episode-only degradation, D14)
ok 3192 - snapshot absent ⇒ member-derived kinds simply absent (episode-only degradation, D14)
  ---
  duration_ms: 0.320917
  type: 'test'
  ...
# Subtest: determinism: identical inputs ⇒ deep-equal output (double run)
ok 3193 - determinism: identical inputs ⇒ deep-equal output (double run)
  ---
  duration_ms: 0.159333
  type: 'test'
  ...
# Subtest: every UnknownItem carries ≥1 sourceRef and a stateRef
ok 3194 - every UnknownItem carries ≥1 sourceRef and a stateRef
  ---
  duration_ms: 0.10775
  type: 'test'
  ...
# Subtest: stable ordering: safety before review before info, then kind, then subject
ok 3195 - stable ordering: safety before review before info, then kind, then subject
  ---
  duration_ms: 0.177209
  type: 'test'
  ...
# Subtest: bandFor thresholds
ok 3196 - bandFor thresholds
  ---
  duration_ms: 1.180292
  type: 'test'
  ...
# Subtest: findingPenalty scales with verdict severity and confidence
ok 3197 - findingPenalty scales with verdict severity and confidence
  ---
  duration_ms: 0.253417
  type: 'test'
  ...
# Subtest: a clean, complete episode scores high (band A)
ok 3198 - a clean, complete episode scores high (band A)
  ---
  duration_ms: 1.135416
  type: 'test'
  ...
# Subtest: domains route by tag; cost driven by low-value tariff spend; untagged → appropriateness
ok 3199 - domains route by tag; cost driven by low-value tariff spend; untagged → appropriateness
  ---
  duration_ms: 20.897125
  type: 'test'
  ...
# Subtest: estimated bed-day cost dents the cost domain even with no tariffed spend
ok 3200 - estimated bed-day cost dents the cost domain even with no tariffed spend
  ---
  duration_ms: 0.276125
  type: 'test'
  ...
# Subtest: weights are configurable and normalised
ok 3201 - weights are configurable and normalised
  ---
  duration_ms: 0.09225
  type: 'test'
  ...
...
ok 3202 - cloudLadder: Vertex first, OpenRouter second — and GEMINI_VIA_OPENROUTER=1 inverts it
  ---
  duration_ms: 114.298459
  type: 'test'
  ...
# Subtest: cloudLadder: a second tier exists ONLY with a leg budget, and only when it can serve
ok 3203 - cloudLadder: a second tier exists ONLY with a leg budget, and only when it can serve
  ---
  duration_ms: 0.187958
  type: 'test'
  ...
# Subtest: the tier-2 slug derivation is the same google/ prefixing, flag or no flag
ok 3204 - the tier-2 slug derivation is the same google/ prefixing, flag or no flag
  ---
  duration_ms: 0.07225
  type: 'test'
  ...
# Subtest: the flag itself is NOT touched by this unit — one code read, no default, no write
ok 3205 - the flag itself is NOT touched by this unit — one code read, no default, no write
  ---
  duration_ms: 0.251458
  type: 'test'
  ...
# Subtest: tierCeilingMs: tier 1 gets the full budget, tier 2 the remainder, a spent leg gets 0
ok 3206 - tierCeilingMs: tier 1 gets the full budget, tier 2 the remainder, a spent leg gets 0
  ---
  duration_ms: 0.085583
  type: 'test'
  ...
# Subtest: a leg never exceeds its budget across both tiers — the naive sum would blow the box
ok 3207 - a leg never exceeds its budget across both tiers — the naive sum would blow the box
  ---
  duration_ms: 0.04525
  type: 'test'
  ...
# Subtest: ladderSkipError names the skipped tier and carries the earlier failure, capped
ok 3208 - ladderSkipError names the skipped tier and carries the earlier failure, capped
  ---
  duration_ms: 0.141209
  type: 'test'
  ...
# Subtest: both transports run the SAME ladder mechanics — no second budget idiom
ok 3209 - both transports run the SAME ladder mechanics — no second budget idiom
  ---
...
ok 3210 - F1: Vertex tier fails → the OpenRouter tier serves the SAME leg (the hop is real)
  ---
  duration_ms: 52.445041
  type: 'test'
  ...
# [provider-fallback] gemini gemini-2.5-pro failed → openrouter: {"provider":"gemini","label":"chatWithFallback","feature":null,"fellBackTo":"openrouter","intended_model":"gemini-2.5-pro","fallback_model":null,"region":"asia-south1","sa_identity":null,"inFlightAtError":1,"in_flight_by_provider":{"gemini":1},"http_status":null,"error_status":null,"error_code":null,"message":"GCP_SA_KEY is not valid JSON (or base64 JSON)","details":null}
# Subtest: F2: with NO leg budget there is NO second tier — the utility path is byte-identical
ok 3211 - F2: with NO leg budget there is NO second tier — the utility path is byte-identical
  ---
  duration_ms: 14.429292
  type: 'test'
  ...
...
ok 3212 - F3: noLocalFallback=true → both tiers failing THROWS; Ollama is not called
  ---
  duration_ms: 7.5215
  type: 'test'
  ...
...
ok 3213 - F4: noLocalFallback absent → both tiers failing still falls back to Ollama (today's behaviour)
  ---
  duration_ms: 14.909667
  type: 'test'
  ...
...
ok 3214 - F5: GEMINI_VIA_OPENROUTER=1 makes OpenRouter tier 1 — the inversion is live, not just typed
  ---
  duration_ms: 788.063458
  type: 'test'
  ...
...
ok 3262 - a note with an ID absent from the vitals table is still no-vitals; one present is still covered
  ---
  duration_ms: 0.046084
  type: 'test'
  ...
# Subtest: the MEASURED window reproduces: 160 of 561 = 28.5%
ok 3263 - the MEASURED window reproduces: 160 of 561 = 28.5%
  ---
  duration_ms: 0.141042
  type: 'test'
  ...
# Subtest: rows outside the window are DROPPED — a boundary sliver is not a day
ok 3264 - rows outside the window are DROPPED — a boundary sliver is not a day
  ---
  duration_ms: 0.373875
  type: 'test'
  ...
# Subtest: Metabase type wobble is absorbed: string counts and ISO timestamps
ok 3265 - Metabase type wobble is absorbed: string counts and ISO timestamps
  ---
  duration_ms: 0.04575
  type: 'test'
  ...
# Subtest: junk never produces a number that looks real
ok 3266 - junk never produces a number that looks real
  ---
  duration_ms: 0.072958
  type: 'test'
  ...
# Subtest: the core is PURE and dependency-free — it must not reach the engine or any score
ok 3267 - the core is PURE and dependency-free — it must not reach the engine or any score
  ---
  duration_ms: 12.799916
  type: 'test'
  ...
# Subtest: GATE 2 — flag OFF: all three fetch SQL strings are byte-identical to today's
ok 3268 - GATE 2 — flag OFF: all three fetch SQL strings are byte-identical to today's
  ---
  duration_ms: 0.753667
  type: 'test'
  ...
# Subtest: flag ON: vitals LEFT JOIN present, DISTINCT ON newest _update_time, scan bounded, quoted table
ok 3269 - flag ON: vitals LEFT JOIN present, DISTINCT ON newest _update_time, scan bounded, quoted table
  ---
  duration_ms: 0.291125
  type: 'test'
  ...
# Subtest: SWEEP-1 (D2) — the day fetch deduplicates by uid; the single/bulk uid fetches do NOT change
ok 3270 - SWEEP-1 (D2) — the day fetch deduplicates by uid; the single/bulk uid fetches do NOT change
  ---
  duration_ms: 0.098333
  type: 'test'
  ...
# Subtest: GATE 5 — no selected column ends in _tag, flag on or off (R-11: numbers, not judgments)
ok 3271 - GATE 5 — no selected column ends in _tag, flag on or off (R-11: numbers, not judgments)
  ---
  duration_ms: 0.111
  type: 'test'
  ...
# Subtest: GATE 3 — synthetic control: a vitals row parses to the exact case shape
ok 3272 - GATE 3 — synthetic control: a vitals row parses to the exact case shape
  ---
  duration_ms: 0.934042
  type: 'test'
  ...
# Subtest: GATE 3 — no vitals row → vitalsRecorded false + vitals null (weight/height still mapped)
ok 3273 - GATE 3 — no vitals row → vitalsRecorded false + vitals null (weight/height still mapped)
  ---
...
ok 3274 - a record with every measurement blank is STILL vitalsRecorded true — a different finding from "no record"
  ---
  duration_ms: 0.099917
  type: 'test'
  ...
# Subtest: bp parse: null unless the string matches ^\\d+\\/\\d+$ (the raw string is kept as recorded)
ok 3275 - bp parse: null unless the string matches ^\\d+\\/\\d+$ (the raw string is kept as recorded)
  ---
  duration_ms: 0.101833
  type: 'test'
  ...
# Subtest: recordedAt: null when the note timestamp is missing (no wall clock ever leaks)
ok 3276 - recordedAt: null when the note timestamp is missing (no wall clock ever leaks)
  ---
...
ok 3277 - fail-safe: an error in the vitals leg resets to the safe state and leaves the rest of the case intact
  ---
  duration_ms: 0.351875
  type: 'test'
  ...
# Subtest: flag OFF: the A1 fields stay absent — every existing case literal and behaviour unchanged
ok 3278 - flag OFF: the A1 fields stay absent — every existing case literal and behaviour unchanged
  ---
  duration_ms: 0.076875
  type: 'test'
  ...
# Subtest: GATE 1 — opdCaseText is byte-identical with and without the vitals block (A1 is score-invariant)
ok 3279 - GATE 1 — opdCaseText is byte-identical with and without the vitals block (A1 is score-invariant)
  ---
  duration_ms: 0.537541
  type: 'test'
  ...
# Subtest: GATE 4 — OpdKeys carries no vitals field, no weight, no height
ok 3280 - GATE 4 — OpdKeys carries no vitals field, no weight, no height
  ---
...
# skipped 0
# todo 0
# duration_ms 19776.12
--- end output ---
EXIT: 0
ENDED: 2026-08-18T16:10:14Z
```
...
```text
COMMAND: cp lib/architecture/map.generated.ts /Users/vinaybhardwaj/cdmss-pass3-repair-gate-18-aug-2026/gen1.ts
STARTED: 2026-08-18T16:10:53Z
--- output (stdout+stderr merged) ---
--- end output ---
EXIT: 0
ENDED: 2026-08-18T16:10:53Z
...
```text
COMMAND: cmp lib/architecture/map.generated.ts /Users/vinaybhardwaj/cdmss-pass3-repair-gate-18-aug-2026/gen1.ts
STARTED: 2026-08-18T16:10:53Z
--- output (stdout+stderr merged) ---
--- end output ---
EXIT: 0
...
```text
GATE START 2026-08-18T16:09:54Z at commit f391973045c70f40edc77ceeedb053310aaa1f52
...
GATE END 2026-08-18T16:11:28Z
```

## Part 3 — evidence integrity, and the v24 condition

The NINE earlier evidence files, hashed at this pass — the six pinned by addendum v20 §6.1, the supplemental-3 file pinned
in commit f6e9188's message, the supplemental-4 file pinned in commit 01c2375's message, and the pass 3 file pinned in
commit a4ef66e's message — every one must match:
...
All nine digests match their pinned values (v20 §6.1 for the first six; `f80c7591…` from f6e9188's message; `a5ff9ff2…`
from 01c2375's message; `d4eb69da…` from a4ef66e's message). No earlier evidence file, report, addendum or review was
edited in this pass; addendum v19 stands unchanged.

**The post-commit-16 check.** The kickoff requires `! git status --porcelain --ignored | grep -q '^!! CDMSS-'` to exit
zero after commit 16. It cannot, for exactly one reason: `CDMSS-RERANK-TELEMETRY-ADDENDUM-v24-18-AUG-2026.md` is present
in the worktree root, is ignored by the root `/*.md` rule, and — per addendum v25 §6 and this kickoff §0 — must not be
tracked. The builder did not delete it (not the builder's document, and deletion is not authorized) and did not negate
it in `.gitignore`. The check's actual output is in the builder's report; every other CDMSS root document is tracked.
