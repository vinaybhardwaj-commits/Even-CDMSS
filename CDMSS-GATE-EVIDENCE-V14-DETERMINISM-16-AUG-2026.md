# CDMSS rerank telemetry — v14 determinism evidence at commit G

```text
Commit G       88026847cc646c8781139c23f68fdc64370375a1
Branch         exp/rerank-telemetry
Worktree       /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry
Run started    Sun Aug 16 02:38:23 UTC 2026
Run ended      Sun Aug 16 02:40:56 UTC 2026
Authority      Saul review 26; addendum v14 sections 5 and 6
```

## Guard-relevant state of `.env.local`

Three booleans. No value from that file is recorded, and no other name from it is
recorded.

```text
VERCEL===1: true
VERCEL_ENV===production: true
HMAC key name present: false
```

⚠️ Read these together with the build pair. `VERCEL` and `VERCEL_ENV` are set in
`.env.local`, so the guard's first two clauses are armed from that file, and the key name is
**absent** from it. Every build below therefore carries its own explicit environment rather than
relying on that file, and the pair differs in exactly one name.

## stdout and stderr

**Merged.** Every capture below was taken with `2>&1`, so stdout and stderr appear interleaved in
one stream, in the order the process emitted them. They are not separated.

## Raw output

Everything below is pasted from the captures written to `$HOME/cdmss-v14-gate-16-aug-2026`
outside the repository, so that writing the evidence could not make the tree dirty while the gate
was measuring that tree.


---

## B2. Starting state, before the gate

```console
── B2: starting state ──
$ git rev-parse HEAD
88026847cc646c8781139c23f68fdc64370375a1
...
$ date -u
Sun Aug 16 02:38:23 UTC 2026
```

---

## B3. Guard-relevant state of .env.local (three booleans)

```console
VERCEL===1: [33mtrue[39m
VERCEL_ENV===production: [33mtrue[39m
HMAC key name present: [33mfalse[39m
```

---

## Numbered command 1 — npm test
...
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 6ce91386-6eec-455b-b263-a858978a4b51
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 72151272-f029-4880-b2e5-2ba77cff4416
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall b78deeee-89fe-4353-a2fb-842a60b5575c
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall a96c1ef1-4cf4-4d2f-b771-0f87a3bf2c0a
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall df2a21f9-12b8-4057-a3b7-33be986ed4bc
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall ac7c2fbc-62ee-41b5-9847-7b26c4405358
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 2e52a7df-7b44-409d-adf5-a1c8e768c32f
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 0231190e-db0d-418f-ab4b-04660dc3609f
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 2f3f7871-7771-42c9-946a-63c43e370d91
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 30776a56-1bd6-43f4-b43b-044a2d512226
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
...
# skipped 0
# todo 0
# duration_ms 20426.149209
[exit status: 0]
```

---

## Numbered command 2 — npm run typecheck
...
## Numbered command 3 — production build, explicit keyed environment

```console
$ env VERCEL=1 VERCEL_ENV=production CDMSS_TELEMETRY_HMAC_KEY=local-gate-key-not-a-secret npm run build
...
> next build
 ...
 [32m[1m✓[22m[39m Compiled successfully in 9.3s
...
## Numbered command 4 — npm run architecture:check
...
## Numbered command 5 — npm run architecture:map
...
## Numbered command 6 — preconditions, and git status immediately before

```console
── git status --porcelain IMMEDIATELY BEFORE command 6 ──
(empty)

── PRECONDITION 1: git diff --exit-code -- lib/architecture/map.generated.ts ──
[exit status: 0]
── PRECONDITION 2: git diff --cached --exit-code -- lib/architecture/map.generated.ts ──
[exit status: 0]
```

---

## Numbered command 6 — the five lines, and git status immediately after

```console
══ COMMAND 6, five lines, each with its own exit status ══

── line 1: npm run architecture:map  (generation ONE) ──
...
── line 2: git add lib/architecture/map.generated.ts ──
[exit status: 0]

── line 3: npm run architecture:map  (generation TWO) ──
...
── line 4: git diff --exit-code lib/architecture/map.generated.ts ──
   (generation TWO vs staged generation ONE — proves the generator is deterministic)
[exit status: 0]

── line 5: git diff --cached --exit-code lib/architecture/map.generated.ts ──
   (staged generation ONE vs HEAD — proves the committed map was already current)
[exit status: 0]

── git status --porcelain IMMEDIATELY AFTER command 6 ──
(empty above = the map has no staged delta)
```

---

## Numbered command 7 — reasoning:registry, then git diff --exit-code
...
## Numbered command 8 — npm run reasoning:governance
...
## Numbered command 9 — npm run changelog:coverage
...
## Build pair, 1 of 2 — REFUSAL, key set to the empty string

```console
$ env VERCEL=1 VERCEL_ENV=production CDMSS_TELEMETRY_HMAC_KEY= npm run build
...
## Build pair, 2 of 2 — KEYED, repeating the command 3 form
...
> next build
 ...
 [37m[1m [22m[39m Creating an optimized production build ...
 [32m[1m✓[22m[39m Compiled successfully in 8.7s
 [37m[1m [22m[39m Linting and checking validity of types ...
 [37m[1m [22m[39m Collecting page data ...
The `fetchConnectionCache` option is deprecated (now always `true`)
The `fetchConnectionCache` option is deprecated (now always `true`)
...
## B7. Ending state, after the gate
...
The nine numbered commands, then the build pair.

| # | Command | Exit |
|---|---|---|
| 1 | `npm test` | 0 — 3178 tests, 3178 pass, 0 fail |
| 2 | `npm run typecheck` | 0 |
| 3 | `env VERCEL=1 VERCEL_ENV=production CDMSS_TELEMETRY_HMAC_KEY=local-gate-key-not-a-secret npm run build` | 0 |
| 4 | `npm run architecture:check` | 0 |
| 5 | `npm run architecture:map` | 0 |
| 6 | determinism and currency, five lines | 0 on all five |
| 7 | `npm run reasoning:registry && git diff --exit-code data/reasoning-registry/prompts.generated.json` | 0 |
| 8 | `npm run reasoning:governance` | 0 |
| 9 | `npm run changelog:coverage` | 0 |
| pair 1 | `env … CDMSS_TELEMETRY_HMAC_KEY= npm run build` | **1** — refused, naming the key |
| pair 2 | `env … CDMSS_TELEMETRY_HMAC_KEY=local-gate-key-not-a-secret npm run build` | 0 |

The build ran three times in total: numbered command 3 once, and the pair twice.

## Command 6, read out

| Line | Command | Exit |
|---|---|---|
| pre 1 | `git diff --exit-code -- lib/architecture/map.generated.ts` | 0 |
| pre 2 | `git diff --cached --exit-code -- lib/architecture/map.generated.ts` | 0 |
| 1 | `npm run architecture:map` (generation one) | 0 |
| 2 | `git add lib/architecture/map.generated.ts` | 0 |
| 3 | `npm run architecture:map` (generation two) | 0 |
| 4 | `git diff --exit-code lib/architecture/map.generated.ts` | 0 |
| 5 | `git diff --cached --exit-code lib/architecture/map.generated.ts` | 0 |

Line 4 compares generation two with staged generation one: **the generator is deterministic.**
Line 5 compares staged generation one with HEAD: **the committed map was already current.**

| Generation | Generator reports | `wc -c` reports |
|---|---|---|
| one | 90492 | 90494 |
| two | 90492 | 90494 |

**The two numbers are supposed to differ, and they differ by exactly two.** The generator prints
`source.length`, a count of UTF-16 code units, and labels it "bytes". The file holds one U+2014 EM
DASH, which is one UTF-16 code unit and three UTF-8 bytes. 90492 + 2 = 90494. Addendum v14 §3.3.
The mislabelled log line is recorded as owed work in §11 and is not fixed here.

The two generator runs report the same number as each other, which is the condition that matters.

## The build pair is a controlled comparison

The two commands differ in one name only. `VERCEL=1` and `VERCEL_ENV=production` are held constant
and set explicitly, so the guard's other two clauses are armed identically in both runs. A green
build alone would prove nothing about the key clause; the refusal is what carries the proof.

`env NAME= cmd` sets the name to an empty string rather than unsetting it. The guard tests
`!String(x ?? '').trim()`, so an empty string triggers the refusal. It throws while
`next.config.mjs` loads, before the build writes anything, so the refusal left no partial output.

## Tree state

`git status --porcelain` was empty before the gate, immediately before command 6, immediately after
command 6, and after the gate. `git add` of an unchanged file stages no delta, so the architecture
map has no staged change and appears in neither commit G nor commit E.
