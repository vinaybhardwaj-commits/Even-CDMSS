# CDMSS gate evidence — pass 2 SUPPLEMENTAL 2 (the recorder round), 16 August 2026

Authority: Saul review 30. Addendum v19, signed by V.
Recorder and comment repair commit (commit 7): 614da5425439264ffd8a56dea6f704a6b14463c7
Gate run: 2026-08-17T05:15:14Z to 2026-08-17T05:16:51Z UTC, from commit 7.
Mutation table (32 rows) run BEFORE the gate, completed before commit 7 was
created; the gate then ran once, from commit 7.

Raw capture directory: $HOME/cdmss-pass2-gate-recorder-16-aug-2026
...
⚠️ TIMESTAMP COMPLETENESS (v19 §3.6). EVERY command in Part 2 carries a
...
check, and both build-pair runs, the refusal included. The previous
supplemental evidence did not have this: its command 7 carried no timestamps
and its refusal build carried no ENDED line. That file is not edited; this one
claims only what it holds, and holds what it claims.
...
## Part 1 — the mutation table, run BEFORE the gate (addendum v19 §5, §7)

Thirty-two rows: the thirty of addendum v18 (all re-run in full — addendum v19 §3.1 and §3.2
change the recorder itself, and rows 1 to 14 all name recorder tests, so a row that
discriminated against the old recorder proves nothing about the new one) plus the two new rows
of addendum v19 §5. Sandbox built per kickoff v3 §9.1 (cp -a, .next/node_modules/.git removed,
node_modules symlinked), shape verified (dotfiles and lib/ present, .git absent), deleted after
the table completed.

Every run: cwd is the WORKTREE and the test file path is the SANDBOX copy, so module imports
resolve to the mutated sandbox code while source-text reads and `git show` stay on the
unmutated worktree — no git command runs inside the sandbox. Each row records the exact
unified diff, the exact command, the exit status, and the failing tests BY NAME. No row timed
out; every row exited 1 with named `not ok` lines. stdout and stderr were merged per row.
...
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/judge-server-stub.ts	2026-08-17 10:42:57
@@ -93,7 +93,7 @@
 }
...
```text
not ok 7 - 5.5a — the boundary: 1048576 bytes is ACCEPTED and recorded in full
not ok 8 - 5.5b — the boundary: 1048577 bytes is REJECTED with 413, an empty JSON body, an overflowed observation, and the SAME undestroyed socket carries the next request
```
...
### Row 3 — judge-server-stub.ts: snapshot returns the internal array (the descriptor clone becomes dead code). Must fail: the defensive-snapshot test (5.7).

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/__tests__/judge-server-stub.ts	2026-08-17 10:39:18
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/judge-server-stub.ts	2026-08-17 10:42:58
@@ -491,7 +491,7 @@
...
```text
not ok 11 - 5.7 — snapshot is DEFENSIVE: mutating the array, an object, or a Buffer leaves recorder state unchanged
not ok 17 - J1.1 — SUCCESS: explicit judge and env-default judge are byte-identical on the wire, in results, and in payload
not ok 18 - J1.2 — REAL BATCH PARSE FAILURE: both arms receive the same malformed completion and record it identically
not ok 19 - J1.3 — GENERIC OUTER JUDGE FAILURE, produced by CALL-THEN-THROW: nonempty and byte-identical wire observations on both arms
```
...
### Row 4 — judge-server-stub.ts: snapshot shares the same Buffer instead of copying it (clone.body = o.body). Must fail: the buffer-copy test (5.7).
...
### Row 5 — judge-server-stub.ts: assign seq at response end, not at acceptance. Must fail: the acceptance-sequencing test (5.4).
...
```text
not ok 6 - 5.4 — sequence numbers are assigned at ACCEPTANCE, monotonic from 0, and never reused
```
...
```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/__tests__/judge-server-stub.ts	2026-08-17 10:39:18
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/judge-server-stub.ts	2026-08-17 10:42:59
@@ -483,7 +483,6 @@
...
```text
not ok 9 - 5.6 — while a request is in flight, BOTH snapshot and resetObservations throw, naming the count
```
...
```text
not ok 15 - 5.10 — resetObservations clears observations and the counter and NOTHING in responder configuration
not ok 17 - J1.1 — SUCCESS: explicit judge and env-default judge are byte-identical on the wire, in results, and in payload
```
...
```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/__tests__/judge-server-stub.ts	2026-08-17 10:39:18
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/judge-server-stub.ts	2026-08-17 10:43:00
@@ -374,7 +374,7 @@
...
```text
not ok 12 - 5.8 — two identical requests produce two observations; the recorder never deduplicates
```
...
### Row 11 — judge-server-stub.ts: record an observation while recording is off (the sampled decision forced true). Must fail: the opt-in test (5.2).
...
### Row 12 — judge-server-stub.ts: store a header value on the observation (enumerable authHeader). Must fail: the no-headers test (5.3).

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/__tests__/judge-server-stub.ts	2026-08-17 10:39:18
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/judge-server-stub.ts	2026-08-17 10:43:01
@@ -375,7 +375,7 @@
       const raw = Buffer.concat(chunks);
       if (recordThisRequest) {
         observations.push({
...
```text
not ok 5 - 5.3 — one observation holds seq, socketId, method, path, and the exact body bytes; nothing else
```
...
```text
not ok 13 - 5.9 — comparison groups by marker SET, not arrival order
not ok 14 - 5.9b — the comparator keeps method, path and body as ONE tuple: a swap between marker groups is a difference
```
...
### Row 14 — judge-server-stub.ts: push a recorder field onto JudgeRequest and populate it (enumerable seq). Must fail: the parsed-API test (5.11).
...
```text
not ok 16 - 5.11 — the parsed `requests` API is unchanged: same fields, same push sites, same arrival order, and JudgeRequest gains no field
```
...
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/rerank.ts	2026-08-17 10:43:02
@@ -444,6 +444,7 @@
...
### Row 17 — rerank.ts (sandbox): in the env-default arm, call the judge before checkHealthy. Must fail: proof 70 order (70.1).
...
```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/rerank.ts	2026-08-15 20:58:10
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/rerank.ts	2026-08-17 10:43:03
@@ -565,7 +565,7 @@
...
### Row 19 — rerank.ts (sandbox): the per-batch containment inside rerankJudge is removed — the catch rethrows. Must fail: proof 17 (17.1).
...
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieve.ts	2026-08-17 10:43:03
@@ -586,7 +586,7 @@
       id: h.id,
       text: h.text,
       __orig: h,
...
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/multi-query.ts	2026-08-17 10:43:03
@@ -271,7 +271,7 @@
...
```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/multi-query.ts	2026-08-12 13:42:36
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/multi-query.ts	2026-08-17 10:43:04
@@ -360,6 +360,7 @@
...
### Row 23 — retrieval-telemetry-core.ts (sandbox): validateManifest returns [] unconditionally. Must fail: the DIRTY arms of proofs 16 and 70 (16.1, 70.3).

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-16 06:53:06
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-17 10:43:04
@@ -656,6 +656,7 @@
...
### Row 24 — judge-server-stub.ts: sameWireObservations drops path from the tuple. Must fail: the J1 path-swap test (5.9b).

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/__tests__/judge-server-stub.ts	2026-08-17 10:39:18
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/judge-server-stub.ts	2026-08-17 10:43:04
@@ -207,7 +207,7 @@
     for (const o of xs) {
       const text = o.body.toString('utf8');
       const key = markers.filter((mk) => text.includes(mk)).sort().join('|');
...
```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/rerank.ts	2026-08-15 20:58:10
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/rerank.ts	2026-08-17 10:43:05
@@ -252,7 +252,7 @@
...
```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/__tests__/judge-server-stub.ts	2026-08-17 10:39:18
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/judge-server-stub.ts	2026-08-17 10:43:06
@@ -345,7 +345,7 @@
     // The socket identity is assigned at ACCEPTANCE, beside `seq` (v18 §4.2). `req.socket` is live
     // at this hook point; the WeakMap hands the same identity back for every request the same
     // undestroyed socket carries.
-    const socketId = recordThisRequest ? socketIdentityOf(req.socket) : -1;
+    const socketId = recordThisRequest ? nextSocketId++ : -1;
...
### Row 29 — judge-server-stub.ts: an ENUMERABLE symbol-keyed field whose description matches the header regex. Must fail: the no-headers check (5.3).
...
```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/multi-query.ts	2026-08-12 13:42:36
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/multi-query.ts	2026-08-17 10:43:06
@@ -246,7 +246,7 @@
     };
   }
...
### Row 31 — judge-server-stub.ts: a NON-ENUMERABLE authorization field stored on the observation via Object.defineProperty. Must fail: the no-headers check (5.3), THROUGH the descriptor-faithful snapshot — this is the case row 29 misses; under the old spread-based snapshot the field never survived into the clone and the guard was blind to it (v19 §3.2, §5).

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/__tests__/judge-server-stub.ts	2026-08-17 10:39:18
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/judge-server-stub.ts	2026-08-17 10:43:31
@@ -374,10 +374,10 @@
       // themselves; `snapshot` copies them on the way out (§5.7).
       const raw = Buffer.concat(chunks);
       if (recordThisRequest) {
...
### Row 32 — judge-server-stub.ts: a hidden (non-enumerable) field stored on the PARSED JudgeRequest via Object.defineProperty. Must fail: the parsed-API guard (5.11, v15 §5.11) — judge.requests returns the stored objects directly, and Reflect.ownKeys sees the hidden key.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/__tests__/judge-server-stub.ts	2026-08-17 10:39:18
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/judge-server-stub.ts	2026-08-17 10:43:47
@@ -427,11 +427,11 @@
...
`$HOME/cdmss-pass2-gate-recorder-16-aug-2026/mutation/` (row-NN-output.txt).
...
APPROVED no-`git add` form (review 30; addendum v18 §4.1, adopted by v19 §6):
the five-line block proves determinism (`cmp` of generation two against
...
ok 873 - 5.2c — MID-FLIGHT TOGGLE on→off: a request accepted while recording was on produces a COMPLETE observation with a real seq, and the in-flight count returns to zero
  ---
  duration_ms: 0.682416
  type: 'test'
  ...
# Subtest: 5.3 — one observation holds seq, socketId, method, path, and the exact body bytes; nothing else
ok 874 - 5.3 — one observation holds seq, socketId, method, path, and the exact body bytes; nothing else
  ---
  duration_ms: 0.777208
  type: 'test'
  ...
# Subtest: 5.4 — sequence numbers are assigned at ACCEPTANCE, monotonic from 0, and never reused
ok 875 - 5.4 — sequence numbers are assigned at ACCEPTANCE, monotonic from 0, and never reused
  ---
  duration_ms: 9.9985
  type: 'test'
  ...
# Subtest: 5.5a — the boundary: 1048576 bytes is ACCEPTED and recorded in full
ok 876 - 5.5a — the boundary: 1048576 bytes is ACCEPTED and recorded in full
  ---
  duration_ms: 5.619416
  type: 'test'
  ...
...
ok 877 - 5.5b — the boundary: 1048577 bytes is REJECTED with 413, an empty JSON body, an overflowed observation, and the SAME undestroyed socket carries the next request
  ---
  duration_ms: 10.449334
  type: 'test'
  ...
# Subtest: 5.6 — while a request is in flight, BOTH snapshot and resetObservations throw, naming the count
ok 878 - 5.6 — while a request is in flight, BOTH snapshot and resetObservations throw, naming the count
  ---
  duration_ms: 3.161792
  type: 'test'
  ...
# Subtest: 5.6b — the in-flight counter decrements on a 413 response too
ok 879 - 5.6b — the in-flight counter decrements on a 413 response too
  ---
  duration_ms: 1.309042
  type: 'test'
  ...
# Subtest: 5.7 — snapshot is DEFENSIVE: mutating the array, an object, or a Buffer leaves recorder state unchanged
ok 880 - 5.7 — snapshot is DEFENSIVE: mutating the array, an object, or a Buffer leaves recorder state unchanged
  ---
  duration_ms: 0.479708
  type: 'test'
  ...
# Subtest: 5.8 — two identical requests produce two observations; the recorder never deduplicates
ok 881 - 5.8 — two identical requests produce two observations; the recorder never deduplicates
  ---
  duration_ms: 0.651084
  type: 'test'
  ...
# Subtest: 5.9 — comparison groups by marker SET, not arrival order
ok 882 - 5.9 — comparison groups by marker SET, not arrival order
  ---
...
ok 883 - 5.9b — the comparator keeps method, path and body as ONE tuple: a swap between marker groups is a difference
  ---
...
ok 884 - 5.10 — resetObservations clears observations and the counter and NOTHING in responder configuration
  ---
  duration_ms: 1.829708
  type: 'test'
  ...
...
ok 885 - 5.11 — the parsed `requests` API is unchanged: same fields, same push sites, same arrival order, and JudgeRequest gains no field
  ---
  duration_ms: 1.243625
  type: 'test'
  ...
# Subtest: J1.1 — SUCCESS: explicit judge and env-default judge are byte-identical on the wire, in results, and in payload
ok 886 - J1.1 — SUCCESS: explicit judge and env-default judge are byte-identical on the wire, in results, and in payload
  ---
  duration_ms: 22.632917
  type: 'test'
  ...
...
ok 887 - J1.2 — REAL BATCH PARSE FAILURE: both arms receive the same malformed completion and record it identically
  ---
  duration_ms: 2.895541
  type: 'test'
  ...
...
ok 888 - J1.3 — GENERIC OUTER JUDGE FAILURE, produced by CALL-THEN-THROW: nonempty and byte-identical wire observations on both arms
  ---
  duration_ms: 7.108792
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
...
ok 889 - J3.0 — the fixture is live: retrieve with the reranker OFF returns the fused order and dials no judge
  ---
  duration_ms: 155.9785
  type: 'test'
  ...
...
ok 890 - J3.1 — OMITTED-BACKEND CONTROL: the resilient arm runs; under a Cohere env default it shows Cohere intent AND downgrade
  ---
  duration_ms: 33.171167
  type: 'test'
  ...
...
ok 891 - J3.2 — EXPLICIT-JUDGE ARM: zero Cohere outbound requests, judge intent, judge service, no downgrade, nonzero batches, actual reordering
  ---
  duration_ms: 7.680458
  type: 'test'
  ...
...
ok 892 - J4.1 — every retrieval arm is called with useReranker OFF; exactly ONE fusion-level rerank; its third argument is the value passed as opts.rerankBackend
  ---
  duration_ms: 5.152666
  type: 'test'
  ...
...
ok 893 - J4.2 — the fusion rerank happens ONCE even when the arms return overlapping pools; no arm-level rerank stamps a batch
  ---
  duration_ms: 15.561375
  type: 'test'
  ...
# Subtest: §4.2 every read of opd_audit_feedback is study-filtered — three D12-allowlisted, commented
ok 894 - §4.2 every read of opd_audit_feedback is study-filtered — three D12-allowlisted, commented
  ---
  duration_ms: 156.347292
  type: 'test'
  ...
# Subtest: §4.2 the write paths: main INSERT names study; assertion_contest and doctor-response never set it
ok 895 - §4.2 the write paths: main INSERT names study; assertion_contest and doctor-response never set it
  ---
  duration_ms: 0.227667
  type: 'test'
  ...
# Subtest: 8.3 the predicate is parameterised IS NOT DISTINCT FROM — never = or a hardcoded IS NULL
ok 896 - 8.3 the predicate is parameterised IS NOT DISTINCT FROM — never = or a hardcoded IS NULL
  ---
...
ok 897 - buildFindingAuthorCurrentSql: DISTINCT ON (audit_id, finding_ref, author), order leads with the same three
  ---
  duration_ms: 0.176208
  type: 'test'
  ...
# Subtest: §8.5 rollup finding builder: study absent ⇒ SAME SQL text, param NULL — NULL matches NULL
ok 898 - §8.5 rollup finding builder: study absent ⇒ SAME SQL text, param NULL — NULL matches NULL
  ---
  duration_ms: 0.399333
  type: 'test'
  ...
# Subtest: §8.5 parseFeedbackBody: study absent ⇒ behaviour identical, study null; D8 author rule enforced
ok 899 - §8.5 parseFeedbackBody: study absent ⇒ behaviour identical, study null; D8 author rule enforced
  ---
  duration_ms: 0.255458
  type: 'test'
  ...
# Subtest: jaccard basics + empty-set guard
ok 900 - jaccard basics + empty-set guard
  ---
  duration_ms: 0.497334
  type: 'test'
  ...
# Subtest: exact finding_ref match when both stamped — regardless of subject
ok 901 - exact finding_ref match when both stamped — regardless of subject
  ---
  duration_ms: 0.16
  type: 'test'
  ...
# Subtest: fuzzy match needs signal_type equality AND Jaccard ≥ threshold
ok 902 - fuzzy match needs signal_type equality AND Jaccard ≥ threshold
  ---
  duration_ms: 0.180458
  type: 'test'
  ...
# Subtest: tie-break prefers the domain-equal student at equal Jaccard
ok 903 - tie-break prefers the domain-equal student at equal Jaccard
  ---
  duration_ms: 0.072333
  type: 'test'
  ...
# Subtest: disagreementsOf classifies tier-differs / teacher-only / student-only with reasons
ok 904 - disagreementsOf classifies tier-differs / teacher-only / student-only with reasons
  ---
  duration_ms: 0.393875
  type: 'test'
  ...
# Subtest: agreeing matched pairs are NOT disagreements
ok 905 - agreeing matched pairs are NOT disagreements
  ---
  duration_ms: 0.052334
  type: 'test'
  ...
# Subtest: every measured target resolves to a class containing Antibiotic, with the [0] invariant
ok 906 - every measured target resolves to a class containing Antibiotic, with the [0] invariant
  ---
  duration_ms: 2.959541
  type: 'test'
  ...
# Subtest: the cefpodoxime line: ester + salt variants both resolve — one entry per resolving fragment
ok 907 - the cefpodoxime line: ester + salt variants both resolve — one entry per resolving fragment
  ---
...
ok 927 - trap 1: a Gemini slug NEVER receives reasoning:{enabled:false} — the A-12 400 destroyed a diagnosis for 36h
  ---
  duration_ms: 0.146458
  type: 'test'
  ...
# Subtest: trap 1 control: a NON-Gemini slug reproduces the pre-bridge behaviour byte-for-byte
ok 928 - trap 1 control: a NON-Gemini slug reproduces the pre-bridge behaviour byte-for-byte
  ---
  duration_ms: 0.364
  type: 'test'
  ...
# Subtest: trap 2: a Gemini slug gets baseMax + 8192 — Pro spends output budget on reasoning FIRST
ok 929 - trap 2: a Gemini slug gets baseMax + 8192 — Pro spends output budget on reasoning FIRST
  ---
...
ok 930 - trap 3: the Vertex thinking budget is TRANSLATED to reasoning.max_tokens, and `google` never travels
  ---
  duration_ms: 0.0525
  type: 'test'
  ...
# Subtest: trap 3: NO DEFAULT IS INVENTED — no budget in, no reasoning out (byte-identical to before)
ok 931 - trap 3: NO DEFAULT IS INVENTED — no budget in, no reasoning out (byte-identical to before)
  ---
  duration_ms: 0.180833
  type: 'test'
  ...
# Subtest: trap 3: the reader is pure and total — any shape yields a budget or undefined
ok 932 - trap 3: the reader is pure and total — any shape yields a budget or undefined
  ---
  duration_ms: 0.05175
  type: 'test'
  ...
# Subtest: trap 3: an explicit OpenRouter reasoning block WINS — translation never overwrites it
ok 933 - trap 3: an explicit OpenRouter reasoning block WINS — translation never overwrites it
  ---
  duration_ms: 0.198167
  type: 'test'
  ...
# Subtest: trap 3: the VERTEX path is untouched — it still sends the google form and no reasoning block
ok 934 - trap 3: the VERTEX path is untouched — it still sends the google form and no reasoning block
  ---
...
ok 935 - the pin: Google-operated providers only, no fallbacks — slugs read off the endpoints listing 30 Jul 2026
  ---
  duration_ms: 0.064
  type: 'test'
  ...
# Subtest: both transports derive the slug centrally; a caller-supplied openrouter slug takes precedence
ok 936 - both transports derive the slug centrally; a caller-supplied openrouter slug takes precedence
  ---
  duration_ms: 0.0665
  type: 'test'
  ...
# Subtest: the Ollama last-leg fallback is untouched in both transports
ok 937 - the Ollama last-leg fallback is untouched in both transports
  ---
  duration_ms: 0.087916
  type: 'test'
  ...
# Subtest: T-5: the hardcoded 'gemini-2.5-pro' literal is GONE from the worker — it hid this incident for four days
ok 938 - T-5: the hardcoded 'gemini-2.5-pro' literal is GONE from the worker — it hid this incident for four days
  ---
  duration_ms: 0.049709
  type: 'test'
  ...
# Subtest: T-5: servedCallFor reads the POST-fallback model from the audit trace, null when unknown
ok 939 - T-5: servedCallFor reads the POST-fallback model from the audit trace, null when unknown
  ---
...
ok 940 - changelog: the bridge entry exists, scoring:false, and names the step change as a provider restoration
  ---
  duration_ms: 0.132417
  type: 'test'
  ...
# Subtest: GATE: a cloud row at 0.81.17 beats a qwen row at 0.81.20 — the exact live shape
ok 941 - GATE: a cloud row at 0.81.17 beats a qwen row at 0.81.20 — the exact live shape
  ---
  duration_ms: 0.803875
  type: 'test'
  ...
# Subtest: GATE: two cloud rows at different versions ⇒ the HIGHER version still wins
ok 942 - GATE: two cloud rows at different versions ⇒ the HIGHER version still wins
  ---
  duration_ms: 0.159417
  type: 'test'
  ...
# Subtest: isLocalGrader catches BOTH signals: the qwen model and the -mini suffix
ok 943 - isLocalGrader catches BOTH signals: the qwen model and the -mini suffix
  ---
  duration_ms: 0.069625
  type: 'test'
  ...
# Subtest: the grader tier is a SEPARATE question from REFERENCE_MODELS — neither list is overloaded
ok 944 - the grader tier is a SEPARATE question from REFERENCE_MODELS — neither list is overloaded
  ---
  duration_ms: 0.226334
  type: 'test'
  ...
# Subtest: CANONICAL_RANK_SQL leads with the grader tier, then version, then reference, then audited_at
ok 945 - CANONICAL_RANK_SQL leads with the grader tier, then version, then reference, then audited_at
  ---
  duration_ms: 0.064542
  type: 'test'
  ...
# Subtest: D1: prodTag and the mini_backfill_prod settings keys are DELETED repo-wide
ok 946 - D1: prodTag and the mini_backfill_prod settings keys are DELETED repo-wide
  ---
  duration_ms: 0.499416
  type: 'test'
  ...
# Subtest: the trap comment no longer claims a guard is unnecessary
ok 947 - the trap comment no longer claims a guard is unnecessary
  ---
  duration_ms: 0.145708
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: stripEvenNonProse removes base64 blobs and image refs, keeps prose
ok 948 - stripEvenNonProse removes base64 blobs and image refs, keeps prose
  ---
  duration_ms: 2.132417
  type: 'test'
  ...
# Subtest: parseEvenProtocols splits on H1/H2/H3 with group-prefixed section paths + slug anchors
ok 949 - parseEvenProtocols splits on H1/H2/H3 with group-prefixed section paths + slug anchors
  ---
  duration_ms: 0.345875
  type: 'test'
  ...
# Subtest: chunkSections drops < 120-char chunks, stamps guideline type + section anchor
ok 950 - chunkSections drops < 120-char chunks, stamps guideline type + section anchor
  ---
  duration_ms: 0.250167
  type: 'test'
  ...
# Subtest: each built row re-chunks to exactly one piece (per-row insert stays 1:1)
ok 951 - each built row re-chunks to exactly one piece (per-row insert stays 1:1)
  ---
  duration_ms: 0.335833
  type: 'test'
  ...
# Subtest: parseIcmr yields page-anchored sections with detected chapter headings
ok 952 - parseIcmr yields page-anchored sections with detected chapter headings
  ---
  duration_ms: 0.263125
  type: 'test'
  ...
# Subtest: detectIcmrHeading: title/upper headings yes, sentences no
ok 953 - detectIcmrHeading: title/upper headings yes, sentences no
  ---
  duration_ms: 0.060709
  type: 'test'
  ...
# Subtest: slugify → stable kebab anchor fragment
ok 954 - slugify → stable kebab anchor fragment
  ---
  duration_ms: 0.09875
  type: 'test'
  ...
# Subtest: CORPUS_QUARANTINE_INSERT_SQL — item_number is still column $5; F13 provenance appended ONLY
ok 955 - CORPUS_QUARANTINE_INSERT_SQL — item_number is still column $5; F13 provenance appended ONLY
  ---
  duration_ms: 0.099125
  type: 'test'
  ...
# Subtest: the artifact is the full master: version, size, key shape
ok 956 - the artifact is the full master: version, size, key shape
  ---
  duration_ms: 57.91725
  type: 'test'
  ...
# Subtest: PRD spot-checks resolve to real master labels
ok 957 - PRD spot-checks resolve to real master labels
  ---
  duration_ms: 0.329958
  type: 'test'
  ...
# Subtest: override precedence: a code in both layers renders the curated phrasing
ok 958 - override precedence: a code in both layers renders the curated phrasing
  ---
  duration_ms: 0.137208
  type: 'test'
  ...
# Subtest: category fallback is EXACT-KEY only; junk still gets the neutral fallback
ok 959 - category fallback is EXACT-KEY only; junk still gets the neutral fallback
  ---
  duration_ms: 41.838709
  type: 'test'
  ...
# Subtest: Decision-D order unchanged: source display text still wins over every bundled layer
ok 960 - Decision-D order unchanged: source display text still wins over every bundled layer
  ---
  duration_ms: 0.19775
  type: 'test'
  ...
# Subtest: slice-2 payload: META carries duration + high-risk in the ratified shape (not consumed yet)
ok 961 - slice-2 payload: META carries duration + high-risk in the ratified shape (not consumed yet)
  ---
  duration_ms: 87.562041
  type: 'test'
  ...
# Subtest: a formulary-resolved combination missing ONLY strength → NO finding
ok 962 - a formulary-resolved combination missing ONLY strength → NO finding
  ---
...
ok 963 - a formulary-resolved combination missing strength AND frequency → finding, gaps list frequency ONLY
  ---
  duration_ms: 0.639083
  type: 'test'
  ...
# Subtest: a combination with nonFormulary set → finding, strength gap STILL present (DEC-2)
ok 964 - a combination with nonFormulary set → finding, strength gap STILL present (DEC-2)
  ---
  duration_ms: 0.44125
  type: 'test'
  ...
# Subtest: a single-molecule drug missing strength → finding, unchanged
ok 965 - a single-molecule drug missing strength → finding, unchanged
  ---
  duration_ms: 0.617292
  type: 'test'
  ...
# Subtest: the combination exemption needs BOTH conditions — a "+" alone is not enough
ok 966 - the combination exemption needs BOTH conditions — a "+" alone is not enough
  ---
  duration_ms: 0.240542
  type: 'test'
  ...
# Subtest: dosageForm 'topical' missing strength AND route → NO finding
ok 967 - dosageForm 'topical' missing strength AND route → NO finding
  ---
  duration_ms: 0.55125
  type: 'test'
  ...
# Subtest: dosageForm 'topical' missing duration → finding, duration ONLY
ok 968 - dosageForm 'topical' missing duration → finding, duration ONLY
  ---
  duration_ms: 0.082541
  type: 'test'
  ...
# Subtest: dosageForm 'drops' behaves exactly like topical
ok 969 - dosageForm 'drops' behaves exactly like topical
  ---
  duration_ms: 0.068833
  type: 'test'
  ...
# Subtest: dosageForm 'inhaler' and 'injection' are UNCHANGED — strength and route still gap
ok 970 - dosageForm 'inhaler' and 'injection' are UNCHANGED — strength and route still gap
  ---
  duration_ms: 2.0035
  type: 'test'
  ...
# Subtest: the other four DosageForm members are untouched: tablet, capsule, syrup, other
ok 971 - the other four DosageForm members are untouched: tablet, capsule, syrup, other
  ---
  duration_ms: 1.667125
  type: 'test'
  ...
# Subtest: a complete line emits nothing, before and after
ok 972 - a complete line emits nothing, before and after
  ---
  duration_ms: 0.093833
  type: 'test'
  ...
# Subtest: the existing isDoseExempt cases behave exactly as before — wholesale suppression intact
ok 973 - the existing isDoseExempt cases behave exactly as before — wholesale suppression intact
  ---
  duration_ms: 0.57525
  type: 'test'
  ...
# Subtest: the rationale wording is byte-identical — only the gap list inside it changes
ok 974 - the rationale wording is byte-identical — only the gap list inside it changes
  ---
  duration_ms: 0.05475
  type: 'test'
  ...
# Subtest: the emitted rationale still parses for severity-tier-core (the tier keys on this string)
ok 975 - the emitted rationale still parses for severity-tier-core (the tier keys on this string)
  ---
  duration_ms: 0.389291
  type: 'test'
  ...
# Subtest: the four contested subjects produce NO strength gap
ok 976 - the four contested subjects produce NO strength gap
  ---
  duration_ms: 0.101542
  type: 'test'
  ...
# Subtest: …but each of the four still fires when a REAL gap is present (DEC-1)
ok 977 - …but each of the four still fires when a REAL gap is present (DEC-1)
  ---
  duration_ms: 0.120209
  type: 'test'
  ...
# Subtest: a topical combination gets BOTH exemptions and still fires on frequency
ok 978 - a topical combination gets BOTH exemptions and still fires on frequency
  ---
  duration_ms: 0.047916
  type: 'test'
  ...
# Subtest: version constants match the PRD exactly
ok 979 - version constants match the PRD exactly
  ---
  duration_ms: 0.59025
  type: 'test'
  ...
# Subtest: candidate mapping per kind (PRD §5 table) — family and skeleton per kind
ok 980 - candidate mapping per kind (PRD §5 table) — family and skeleton per kind
  ---
  duration_ms: 1.065875
  type: 'test'
  ...
# Subtest: baseline buildAskSet asks are also candidates (why baseline, unknownIds [])
ok 981 - baseline buildAskSet asks are also candidates (why baseline, unknownIds [])
  ---
  duration_ms: 0.484375
  type: 'test'
  ...
# Subtest: instability_input / unmappable unknowns produce no candidate and land in dropped
ok 982 - instability_input / unmappable unknowns produce no candidate and land in dropped
  ---
  duration_ms: 0.146875
  type: 'test'
  ...
# Subtest: same-id candidates merge (allergy unknown merges into the baseline allergy ask)
ok 983 - same-id candidates merge (allergy unknown merges into the baseline allergy ask)
  ---
...
ok 984 - validateSelection (B6 numbers, B7 phrase-all): out-of-range / non-integer n rejected; duplicate rejected; NO pick cap
  ---
  duration_ms: 0.655541
  type: 'test'
  ...
# Subtest: validateSelection: rewritten family/subject never survive — candidate fields win
ok 985 - validateSelection: rewritten family/subject never survive — candidate fields win
  ---
  duration_ms: 0.092125
  type: 'test'
  ...
# Subtest: validateSelection: over-length and empty questions are rejected
ok 986 - validateSelection: over-length and empty questions are rejected
  ---
  duration_ms: 0.120709
  type: 'test'
  ...
# Subtest: validateSelection: a generic question (no subject token) is replaced by the candidate skeleton
ok 987 - validateSelection: a generic question (no subject token) is replaced by the candidate skeleton
  ---
  duration_ms: 0.23275
  type: 'test'
  ...
# Subtest: assembly: every high-alert MED_STATUS ask is ALWAYS first (ladder rank 0), regardless of picks
ok 988 - assembly: every high-alert MED_STATUS ask is ALWAYS first (ladder rank 0), regardless of picks
  ---
  duration_ms: 0.531667
  type: 'test'
  ...
# Subtest: K2 ladder (B5 ranks): rungs serve in order 0<1<3<4<5<6<7<8 regardless of the pick order fed in
ok 989 - K2 ladder (B5 ranks): rungs serve in order 0<1<3<4<5<6<7<8 regardless of the pick order fed in
  ---
...
ok 990 - B5 new-med rung: a med absent from a NON-EMPTY snapshot ranks 2 and leads over a care-gap; empty/absent snapshot stays routine 5
  ---
  duration_ms: 15.398
  type: 'test'
  ...
# Subtest: assembly: total cap stays 5 and the overflow list is preserved
ok 991 - assembly: total cap stays 5 and the overflow list is preserved
  ---
  duration_ms: 0.266125
  type: 'test'
  ...
# Subtest: K2: zero-valid-picks (parsed) is NOT a fallback — ladder assembles with skeleton phrasing, source inquiry
ok 992 - K2: zero-valid-picks (parsed) is NOT a fallback — ladder assembles with skeleton phrasing, source inquiry
  ---
  duration_ms: 0.59075
  type: 'test'
  ...
# Subtest: K2: transport failure retries ONCE, then falls back byte-identical to buildAskSet
ok 993 - K2: transport failure retries ONCE, then falls back byte-identical to buildAskSet
  ---
  duration_ms: 24.261166
  type: 'test'
  ...
# Subtest: runInquirySelection happy path: validated picks served as ask-set/0.2 with askMeta derivation
ok 994 - runInquirySelection happy path: validated picks served as ask-set/0.2 with askMeta derivation
  ---
  duration_ms: 0.359
  type: 'test'
  ...
# Subtest: B7 phrase-all: with a phrasing for EVERY candidate, all 5 ladder-served asks carry Gemini phrasing (no skeleton)
ok 995 - B7 phrase-all: with a phrasing for EVERY candidate, all 5 ladder-served asks carry Gemini phrasing (no skeleton)
  ---
  duration_ms: 0.517458
  type: 'test'
  ...
# Subtest: parseSelection tolerates prose around the JSON and rejects malformed shapes
ok 996 - parseSelection tolerates prose around the JSON and rejects malformed shapes
  ---
  duration_ms: 0.075375
  type: 'test'
  ...
# Subtest: B6: parseSelection strips markdown code fences (the live-prod fallback root cause)
ok 997 - B6: parseSelection strips markdown code fences (the live-prod fallback root cause)
  ---
...
ok 998 - B6 end-to-end: a fenced, number-based Gemini response serves source inquiry with Gemini phrasing
  ---
  duration_ms: 0.211625
  type: 'test'
  ...
# Subtest: fallbackAskSet is buildAskSet verbatim (deep-equal asks + overflow)
ok 999 - fallbackAskSet is buildAskSet verbatim (deep-equal asks + overflow)
  ---
  duration_ms: 0.088
  type: 'test'
  ...
# Subtest: scorer is deterministic and the metric arithmetic is exact
ok 1000 - scorer is deterministic and the metric arithmetic is exact
  ---
  duration_ms: 1.473
  type: 'test'
  ...
# Subtest: A1 split: family-legality is vocabulary-only; legalSlots23 lands in slotAppropriate, not the gate
ok 1001 - A1 split: family-legality is vocabulary-only; legalSlots23 lands in slotAppropriate, not the gate
  ---
  duration_ms: 0.136958
  type: 'test'
  ...
# Subtest: baseline harness runs on the shipped RATIFIED bank (deterministic arm, no LLM)
ok 1002 - baseline harness runs on the shipped RATIFIED bank (deterministic arm, no LLM)
  ---
  duration_ms: 17.93225
  type: 'test'
  ...
# Subtest: askset route: INQUIRY_ENABLED unset ⇒ byte-identical deterministic path
ok 1003 - askset route: INQUIRY_ENABLED unset ⇒ byte-identical deterministic path
  ---
  duration_ms: 0.47725
  type: 'test'
  ...
...
ok 1004 - insert is idempotent on id (ON CONFLICT (id) DO NOTHING) and repeat saves succeed
  ---
  duration_ms: 0.784167
  type: 'test'
  ...
# Subtest: reads soft-fail to empty when the table is missing / DB is down
ok 1005 - reads soft-fail to empty when the table is missing / DB is down
  ---
  duration_ms: 0.208416
  type: 'test'
  ...
# Subtest: K1.1: recomputeOutcomes preserves each row's served ask_set_version (ask-set/0.2 survives)
ok 1006 - K1.1: recomputeOutcomes preserves each row's served ask_set_version (ask-set/0.2 survives)
  ---
  duration_ms: 12.02375
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
...
ok 1007 - 1a — retrieve: no capture, and two uninstrumented runs are identical statement for statement
  ---
  duration_ms: 101.462291
  type: 'test'
  ...
# Subtest: 1a' — and adding a capture to ONE side only leaves the returned value identical
ok 1008 - 1a' — and adding a capture to ONE side only leaves the returned value identical
  ---
  duration_ms: 0.708083
  type: 'test'
  ...
# Subtest: 2 — rerank: undefined reaches judgeFn and cohereFn as the third argument, and soft failure returns early
ok 1009 - 2 — rerank: undefined reaches judgeFn and cohereFn as the third argument, and soft failure returns early
  ---
  duration_ms: 0.439
  type: 'test'
  ...
# Subtest: 3 — rerankJudge: identical array and identical request bodies, with and without a capture
ok 1010 - 3 — rerankJudge: identical array and identical request bodies, with and without a capture
  ---
  duration_ms: 18.97875
  type: 'test'
  ...
# Subtest: 4 — rerankCohere: the CapturedBatch literal at :162-174 is never constructed
ok 1011 - 4 — rerankCohere: the CapturedBatch literal at :162-174 is never constructed
  ---
  duration_ms: 0.9135
  type: 'test'
  ...
# Subtest: 5 — expandQuery: capture.expansion is never set, and here evidenceFromCompletion is INSIDE the guard
ok 1012 - 5 — expandQuery: capture.expansion is never set, and here evidenceFromCompletion is INSIDE the guard
  ---
  duration_ms: 5.226792
  type: 'test'
  ...
# Subtest: 6 — retrieveMultiQuery: armCaptures undefined, arms called with undefined, children never set
ok 1013 - 6 — retrieveMultiQuery: armCaptures undefined, arms called with undefined, children never set
  ---
  duration_ms: 0.925167
  type: 'test'
  ...
# Subtest: 7 — the MatchInput seam: no telemetry field means no capture, no declaration and no write
ok 1014 - 7 — the MatchInput seam: no telemetry field means no capture, no declaration and no write
  ---
  duration_ms: 8.556167
  type: 'test'
  ...
# Subtest: v7 §5 — Vertex is the first target when Gemini is on and the bridge flag is off
ok 1015 - v7 §5 — Vertex is the first target when Gemini is on and the bridge flag is off
  ---
  duration_ms: 283.795125
  type: 'test'
  ...
# Subtest: v7 §5 — OpenRouter is the first target when the bridge flag produces a slug
ok 1016 - v7 §5 — OpenRouter is the first target when the bridge flag produces a slug
  ---
  duration_ms: 330.497208
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: v7 §5 — Ollama with JUDGE_MODEL when no cloud tier is available, and that pair IS sanctioned
ok 1017 - v7 §5 — Ollama with JUDGE_MODEL when no cloud tier is available, and that pair IS sanctioned
  ---
  duration_ms: 102.589625
  type: 'test'
  ...
# Subtest: v7 §5 — LLM_PIPELINE=mini forces local, whatever the Gemini flags say
ok 1018 - v7 §5 — LLM_PIPELINE=mini forces local, whatever the Gemini flags say
  ---
  duration_ms: 5.669375
  type: 'test'
  ...
# Subtest: v7 §5 — Gemini flags with NO provider configuration still resolve local, not Vertex
ok 1019 - v7 §5 — Gemini flags with NO provider configuration still resolve local, not Vertex
  ---
  duration_ms: 8.372667
  type: 'test'
  ...
# Subtest: v7 §5 — Cohere resolves to OpenRouter with the effective Cohere model
ok 1020 - v7 §5 — Cohere resolves to OpenRouter with the effective Cohere model
  ---
  duration_ms: 2.204791
  type: 'test'
  ...
# Subtest: v7 §5 — the guard rejects the exact pair that reached the manifest, and accepts all four sanctioned ones
ok 1021 - v7 §5 — the guard rejects the exact pair that reached the manifest, and accepts all four sanctioned ones
  ---
  duration_ms: 1.160875
  type: 'test'
  ...
# Subtest: v7 §5 — EVERY target the resolver can produce is a sanctioned pairing, across the matrix
ok 1022 - v7 §5 — EVERY target the resolver can produce is a sanctioned pairing, across the matrix
  ---
  duration_ms: 2.73225
  type: 'test'
  ...
# Subtest: v7 §5 — no site hardcodes the impossible pair any more, and the correct Cohere site is pinned
ok 1023 - v7 §5 — no site hardcodes the impossible pair any more, and the correct Cohere site is pinned
  ---
...
ok 1024 - v7 §6 — a generic Cohere failure records unattributed, because the proof rule governs
  ---
  duration_ms: 9.337625
  type: 'test'
  ...
# [rerank] backend failed, returning input order generic
# Subtest: v7 §6 — the resolved intended pairing survives on the soft-failure record too
ok 1025 - v7 §6 — the resolved intended pairing survives on the soft-failure record too
  ---
  duration_ms: 1.665292
  type: 'test'
  ...
# Subtest: v7 §10 — a fresh capture carries the no-rerank values, not zeros
ok 1026 - v7 §10 — a fresh capture carries the no-rerank values, not zeros
  ---
  duration_ms: 2.119625
  type: 'test'
  ...
# Subtest: v7 §10 — Cohere records neither a temperature nor a seed, because it takes neither
ok 1027 - v7 §10 — Cohere records neither a temperature nor a seed, because it takes neither
  ---
  duration_ms: 2.827541
  type: 'test'
  ...
# Subtest: v7 §10 — the judge records its real temperature and `unseeded`, and the call uses the same constant
ok 1028 - v7 §10 — the judge records its real temperature and `unseeded`, and the call uses the same constant
  ---
  duration_ms: 0.157209
  type: 'test'
  ...
# Subtest: v7 §10 — the seed status vocabulary distinguishes a stripped seed from an applied one
ok 1029 - v7 §10 — the seed status vocabulary distinguishes a stripped seed from an applied one
  ---
...
ok 1044 - 2.0 theme upgrade: material themes expanded via V-ratified extras; nitpick sits in a separate minor tier
  ---
  duration_ms: 0.539667
  type: 'test'
  ...
# Subtest: the gold is de-identified: no UHID / phone / honorific-name patterns anywhere
ok 1045 - the gold is de-identified: no UHID / phone / honorific-name patterns anywhere
  ---
  duration_ms: 0.753167
  type: 'test'
  ...
# Subtest: loadIpdAuditGold rejects drift: edited case, wrong version/status, dup id, bad verdict
ok 1046 - loadIpdAuditGold rejects drift: edited case, wrong version/status, dup id, bad verdict
  ---
  duration_ms: 4.1545
  type: 'test'
  ...
...
ok 1047 - semantics: the adjudication component never touches the scored-band palette
  ---
  duration_ms: 0.568459
  type: 'test'
  ...
# Subtest: semantics: the LVC summary strip renders without band language; the finding list exists once
ok 1048 - semantics: the LVC summary strip renders without band language; the finding list exists once
  ---
  duration_ms: 0.210125
  type: 'test'
  ...
# Subtest: CaseAuditReport: the findingActions slot is optional — absent means unchanged for other callers
ok 1049 - CaseAuditReport: the findingActions slot is optional — absent means unchanged for other callers
  ---
  duration_ms: 0.559666
  type: 'test'
  ...
# Subtest: PHI posture: the row assembler cannot place a name/UHID on the audit row
ok 1050 - PHI posture: the row assembler cannot place a name/UHID on the audit row
  ---
  duration_ms: 0.555
  type: 'test'
  ...
# Subtest: PHI posture: neither the table nor the store INSERT carries a name/UHID column
ok 1051 - PHI posture: neither the table nor the store INSERT carries a name/UHID column
  ---
  duration_ms: 0.729125
  type: 'test'
  ...
# Subtest: PHI posture: db13 PHI fields are read-time only — never passed to the row assembler
ok 1052 - PHI posture: db13 PHI fields are read-time only — never passed to the row assembler
  ---
...
ok 1149 - mini path (no evalModel): n≤2, serial (concurrency 1), mini-yield honoured
  ---
  duration_ms: 0.905042
  type: 'test'
  ...
# Subtest: eval path (evalModel set): drains exactly ONE WAVE and skips the mini-yield
ok 1150 - eval path (evalModel set): drains exactly ONE WAVE and skips the mini-yield
  ---
  duration_ms: 0.113083
  type: 'test'
  ...
# Subtest: eval sliceSize == concurrency across the whole clamped range (1..EVAL_CONCURRENCY_MAX)
ok 1151 - eval sliceSize == concurrency across the whole clamped range (1..EVAL_CONCURRENCY_MAX)
  ---
  duration_ms: 0.16775
  type: 'test'
  ...
# Subtest: THE DEFECT: the old slice was ~890s of work in one invocation; the new one is one audit
ok 1152 - THE DEFECT: the old slice was ~890s of work in one invocation; the new one is one audit
  ---
  duration_ms: 0.05275
  type: 'test'
  ...
# Subtest: EVAL_TICK_MAX remains a hard ceiling on the slice (D2)
ok 1153 - EVAL_TICK_MAX remains a hard ceiling on the slice (D2)
  ---
  duration_ms: 0.097625
  type: 'test'
  ...
# Subtest: the mini branch of drainPlan is UNTOUCHED by the one-wave change
ok 1154 - the mini branch of drainPlan is UNTOUCHED by the one-wave change
  ---
  duration_ms: 0.094959
  type: 'test'
  ...
# Subtest: clampEvalConcurrency: default 10, clamp 1..25
ok 1155 - clampEvalConcurrency: default 10, clamp 1..25
  ---
  duration_ms: 0.043167
  type: 'test'
  ...
# Subtest: boundedPool never exceeds its concurrency limit and preserves result order
ok 1156 - boundedPool never exceeds its concurrency limit and preserves result order
  ---
  duration_ms: 19.513875
  type: 'test'
  ...
# Subtest: boundedPool handles limit > items and empty input
ok 1157 - boundedPool handles limit > items and empty input
  ---
  duration_ms: 0.827166
  type: 'test'
  ...
# Subtest: openRouterGenerate retries 429 then succeeds; sleeps between attempts
ok 1158 - openRouterGenerate retries 429 then succeeds; sleeps between attempts
  ---
  duration_ms: 16.933083
  type: 'test'
  ...
# Subtest: openRouterGenerate throws after OPENROUTER_MAX_TRIES persistent 5xx — no silent fallback
ok 1159 - openRouterGenerate throws after OPENROUTER_MAX_TRIES persistent 5xx — no silent fallback
  ---
  duration_ms: 1.137666
  type: 'test'
  ...
# Subtest: non-transient status (400) throws immediately — no retry
ok 1160 - non-transient status (400) throws immediately — no retry
  ---
  duration_ms: 0.322416
  type: 'test'
  ...
# Subtest: retryable statuses are exactly 429 + 5xx; backoff is jittered-exponential and positive
ok 1161 - retryable statuses are exactly 429 + 5xx; backoff is jittered-exponential and positive
  ---
  duration_ms: 0.076291
  type: 'test'
  ...
# Subtest: the eval drain still writes lab_analyses only — never opd_note_audits
ok 1162 - the eval drain still writes lab_analyses only — never opd_note_audits
  ---
  duration_ms: 0.286542
  type: 'test'
  ...
# Subtest: the tick summary carries tick_ms / slice_planned / slice_drained (D3)
ok 1163 - the tick summary carries tick_ms / slice_planned / slice_drained (D3)
  ---
  duration_ms: 0.2675
  type: 'test'
  ...
# Subtest: the D3 fields are OBSERVATION ONLY — never branched on, never thrown from
ok 1164 - the D3 fields are OBSERVATION ONLY — never branched on, never thrown from
  ---
  duration_ms: 0.581833
  type: 'test'
  ...
# Subtest: LB_LOCK_TTL_MS / labLockHeld / ttlBreach survive this build unchanged
ok 1165 - LB_LOCK_TTL_MS / labLockHeld / ttlBreach survive this build unchanged
  ---
  duration_ms: 0.241041
  type: 'test'
  ...
# Subtest: parseBatchState reads evalConcurrency; absent ⇒ default 10
ok 1166 - parseBatchState reads evalConcurrency; absent ⇒ default 10
  ---
  duration_ms: 0.136667
  type: 'test'
  ...
# Subtest: parseNdjson tolerates blank + garbled lines
ok 1167 - parseNdjson tolerates blank + garbled lines
  ---
  duration_ms: 0.51125
  type: 'test'
  ...
# Subtest: reduceDdxEvents folds a full stream
ok 1168 - reduceDdxEvents folds a full stream
  ---
  duration_ms: 0.433042
  type: 'test'
  ...
# Subtest: reduceDdxEvents surfaces an error stream as not-ok
ok 1169 - reduceDdxEvents surfaces an error stream as not-ok
  ---
  duration_ms: 0.137875
  type: 'test'
  ...
# Subtest: extractCitationIds pulls distinct sorted numeric ids
ok 1170 - extractCitationIds pulls distinct sorted numeric ids
  ---
  duration_ms: 0.120458
  type: 'test'
  ...
# Subtest: reduceAskEvents keeps the revised answer, flags uncited
ok 1171 - reduceAskEvents keeps the revised answer, flags uncited
  ---
  duration_ms: 0.130542
  type: 'test'
  ...
# Subtest: reduceAskEvents flags a long uncited answer (cite-or-label canary)
ok 1172 - reduceAskEvents flags a long uncited answer (cite-or-label canary)
  ---
  duration_ms: 0.066333
  type: 'test'
  ...
# Subtest: reduceAppropriatenessEvents captures fired CW statements (over-flag surface)
ok 1173 - reduceAppropriatenessEvents captures fired CW statements (over-flag surface)
  ---
  duration_ms: 0.127667
  type: 'test'
  ...
# Subtest: reduceAppropriatenessEvents handles the empty (nothing-fired) case
ok 1174 - reduceAppropriatenessEvents handles the empty (nothing-fired) case
  ---
  duration_ms: 0.055875
  type: 'test'
  ...
# Subtest: reduceDocAuditEvents pulls the scorecard headline/band
ok 1175 - reduceDocAuditEvents pulls the scorecard headline/band
  ---
  duration_ms: 0.249125
  type: 'test'
  ...
# Subtest: reduceDocAuditEvents surfaces a stream error
ok 1176 - reduceDocAuditEvents surfaces a stream error
  ---
  duration_ms: 0.310458
  type: 'test'
  ...
# Subtest: labSelfBaseUrl prefers explicit, then VERCEL_URL, then localhost
ok 1177 - labSelfBaseUrl prefers explicit, then VERCEL_URL, then localhost
  ---
  duration_ms: 0.107375
  type: 'test'
  ...
# Subtest: labLabel sanitises to a safe slug
ok 1178 - labLabel sanitises to a safe slug
  ---
  duration_ms: 0.478417
  type: 'test'
  ...
# Subtest: chunkText splits on paragraphs, drops tiny fragments, respects the window
ok 1179 - chunkText splits on paragraphs, drops tiny fragments, respects the window
  ---
  duration_ms: 0.195167
  type: 'test'
  ...
# Subtest: chunkText hard-splits a single monster paragraph
ok 1180 - chunkText hard-splits a single monster paragraph
  ---
  duration_ms: 0.446958
  type: 'test'
  ...
# Subtest: chunkText returns empty for whitespace-only input
ok 1181 - chunkText returns empty for whitespace-only input
  ---
  duration_ms: 0.707417
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: quarantine INSERT carries visible in the columns and false in the values
ok 1182 - quarantine INSERT carries visible in the columns and false in the values
  ---
  duration_ms: 0.523208
  type: 'test'
  ...
# Subtest: activation UPDATE sets both source and visible = true
ok 1183 - activation UPDATE sets both source and visible = true
  ---
...
ok 1356 - 1: empty body model + no transport evidence + valid content → verdict served, ONE call, unknown
  ---
  duration_ms: 1.931584
  type: 'test'
  ...
# Subtest: 2: transport names the intended Gemini model → ONE call, verified
ok 1357 - 2: transport names the intended Gemini model → ONE call, verified
  ---
  duration_ms: 0.546625
  type: 'test'
  ...
# Subtest: 3: body names a DIFFERENT model → two calls, refusal, every rec insufficient_info, wrong_model
ok 1358 - 3: body names a DIFFERENT model → two calls, refusal, every rec insufficient_info, wrong_model
  ---
  duration_ms: 0.584625
  type: 'test'
  ...
# Subtest: 4: transport names the LOCAL model → wrong_model, the verdict is never accepted
ok 1359 - 4: transport names the LOCAL model → wrong_model, the verdict is never accepted
  ---
  duration_ms: 0.774667
  type: 'test'
  ...
# Subtest: 5: a CONFLICT between the two sources is wrong_model — in BOTH directions
ok 1360 - 5: a CONFLICT between the two sources is wrong_model — in BOTH directions
  ---
  duration_ms: 0.809875
  type: 'test'
  ...
# Subtest: 6: body-only verified — no transport evidence, body names the intended Gemini → verified, ONE call
ok 1361 - 6: body-only verified — no transport evidence, body names the intended Gemini → verified, ONE call
  ---
  duration_ms: 0.460541
  type: 'test'
  ...
# Subtest: 7: a provider THROW retries once then refuses — and stays distinct from unknown
ok 1362 - 7: a provider THROW retries once then refuses — and stays distinct from unknown
  ---
  duration_ms: 0.359208
  type: 'test'
  ...
# Subtest: 7b: a first-attempt throw followed by a verified answer is served — the retry still recovers
ok 1363 - 7b: a first-attempt throw followed by a verified answer is served — the retry still recovers
  ---
  duration_ms: 0.18225
  type: 'test'
  ...
# Subtest: 8: the REAL judge call passes noLocalFallback: true — mechanically, through llmCall
ok 1364 - 8: the REAL judge call passes noLocalFallback: true — mechanically, through llmCall
  ---
  duration_ms: 0.671875
  type: 'test'
  ...
# Subtest: 8b: candidate extraction does NOT pass noLocalFallback — its options object is byte-identical
ok 1365 - 8b: candidate extraction does NOT pass noLocalFallback — its options object is byte-identical
  ---
  duration_ms: 0.706709
  type: 'test'
  ...
# Subtest: 9: attempt + invocation payloads — absent stays null, both sources stay separately visible
ok 1366 - 9: attempt + invocation payloads — absent stays null, both sources stay separately visible
  ---
  duration_ms: 0.278542
  type: 'test'
  ...
# Subtest: 9b: the pure builders — nothing absent is invented, retry_count is 0 on a single attempt
ok 1367 - 9b: the pure builders — nothing absent is invented, retry_count is 0 on a single attempt
  ---
  duration_ms: 0.10525
  type: 'test'
  ...
# Subtest: 9c: a throwing recorder can never cost a verdict
ok 1368 - 9c: a throwing recorder can never cost a verdict
  ---
  duration_ms: 0.148125
  type: 'test'
  ...
# Subtest: 10: resolveJudgeAttribution — every row of the table, exhaustively
ok 1369 - 10: resolveJudgeAttribution — every row of the table, exhaustively
  ---
  duration_ms: 0.224541
  type: 'test'
  ...
# Subtest: transport attribution is a NON-ENUMERABLE property — no existing consumer can see it
ok 1370 - transport attribution is a NON-ENUMERABLE property — no existing consumer can see it
  ---
  duration_ms: 0.076084
  type: 'test'
  ...
# Subtest: attaching to a frozen or non-object result never throws — evidence must not cost a call
ok 1371 - attaching to a frozen or non-object result never throws — evidence must not cost a call
  ---
...
ok 1372 - D-1: the judge call body carries temperature 0, the fixed seed and top_p 1
  ---
  duration_ms: 2.185125
  type: 'test'
  ...
# Subtest: D-1: the autoflag surface is pinned identically — one judge, one configuration
ok 1373 - D-1: the autoflag surface is pinned identically — one judge, one configuration
  ---
  duration_ms: 0.5155
  type: 'test'
  ...
# Subtest: D-2: a non-Gemini served model is retried ONCE, then the whole batch refuses
ok 1374 - D-2: a non-Gemini served model is retried ONCE, then the whole batch refuses
  ---
  duration_ms: 0.703625
  type: 'test'
  ...
# Subtest: D-2 → D-6: an EMPTY served model is UNKNOWN attribution, not a failure (see lvc-judge-attribution.test.ts)
ok 1375 - D-2 → D-6: an EMPTY served model is UNKNOWN attribution, not a failure (see lvc-judge-attribution.test.ts)
  ---
  duration_ms: 0.29025
  type: 'test'
  ...
# Subtest: D-2: a throw is retried once and then refuses — no soft-fail to a local answer
ok 1376 - D-2: a throw is retried once and then refuses — no soft-fail to a local answer
  ---
  duration_ms: 0.321167
  type: 'test'
  ...
# Subtest: D-2: a FIRST-attempt failure followed by an agreeing Gemini answer is served normally
ok 1377 - D-2: a FIRST-attempt failure followed by an agreeing Gemini answer is served normally
  ---
  duration_ms: 0.522208
  type: 'test'
  ...
# Subtest: D-2: the publisher prefix is not a disagreement — google/<slug> still serves
ok 1378 - D-2: the publisher prefix is not a disagreement — google/<slug> still serves
  ---
  duration_ms: 0.186834
  type: 'test'
  ...
# Subtest: D-2: forceOllama refuses BEFORE any call — no ollama call may serve a judge verdict
ok 1379 - D-2: forceOllama refuses BEFORE any call — no ollama call may serve a judge verdict
  ---
  duration_ms: 0.164875
  type: 'test'
  ...
# Subtest: D-2: with no Gemini available there is no slug to retry against — immediate refusal
ok 1380 - D-2: with no Gemini available there is no slug to retry against — immediate refusal
  ---
  duration_ms: 0.294667
  type: 'test'
  ...
# Subtest: D-2: the refusal event kind is the one the PRD names
ok 1381 - D-2: the refusal event kind is the one the PRD names
  ---
  duration_ms: 0.276667
  type: 'test'
  ...
# Subtest: §4: valid round tags resolve to themselves
ok 1382 - §4: valid round tags resolve to themselves
  ---
  duration_ms: 0.118166
  type: 'test'
  ...
# Subtest: §4: junk falls back to the r1 default — the route can never write an unfindable tag
ok 1383 - §4: junk falls back to the r1 default — the route can never write an unfindable tag
  ---
  duration_ms: 0.062667
  type: 'test'
  ...
# Subtest: §4: surrounding whitespace is trimmed, not rejected
ok 1384 - §4: surrounding whitespace is trimmed, not rejected
  ---
...
ok 1400 - §2/§7: LVC params (reasoning present) ⇒ no injected reasoning:{enabled:false}; the caller value survives
  ---
  duration_ms: 0.696083
  type: 'test'
  ...
# Subtest: §2/§7: a caller with NO reasoning (citation critic / Qwen3) still receives reasoning:{enabled:false}
ok 1401 - §2/§7: a caller with NO reasoning (citation critic / Qwen3) still receives reasoning:{enabled:false}
  ---
  duration_ms: 0.070583
  type: 'test'
  ...
# Subtest: §3/§7: the both-failed error contains BOTH the provider and the Ollama fallback messages
ok 1402 - §3/§7: the both-failed error contains BOTH the provider and the Ollama fallback messages
  ---
  duration_ms: 0.220458
  type: 'test'
  ...
# Subtest: §3/§7: runOllamaFallback returns the fallback result unchanged on success; throws both-failed on error
ok 1403 - §3/§7: runOllamaFallback returns the fallback result unchanged on success; throws both-failed on error
  ---
  duration_ms: 0.30625
  type: 'test'
  ...
...
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 49f5f589-b65a-4d42-95a2-c3148e32cafe
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall a8bc3a93-c1a6-422a-9e4d-76ad669d058f
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 53055dc8-7183-4150-81b0-41b87eaaee93
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall d24e79d5-2190-494c-b03f-91dd820ec5b8
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 9a4549d8-eefd-46fa-b2aa-a9e2f6917148
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 6f024bb4-3595-4d3a-a901-449eb705c2da
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall f47768bb-80b4-4fd1-8f17-9d02120a6e4b
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall ae9cf9e7-c0eb-4712-8277-437e941476a6
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall e51a41a9-c585-4c53-a83a-dbf1f0ba6fc3
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 1a9aa9e9-a475-4603-9b7a-9da81ac46535
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# Subtest: 42 A1 — A/A pass 0 uses DEFAULT recall and declares with role lvc_recall on route lvc_judge_aa
ok 1404 - 42 A1 — A/A pass 0 uses DEFAULT recall and declares with role lvc_recall on route lvc_judge_aa
  ---
  duration_ms: 14.104708
  type: 'test'
  ...
# Subtest: 42 A2 — the PINNED passes declare nothing: one declaration per case, not three
ok 1405 - 42 A2 — the PINNED passes declare nothing: one declaration per case, not three
  ---
  duration_ms: 6.095
  type: 'test'
  ...
# Subtest: 42 A3 — exactly ONE lvc_recall row per pass-0 recall, not two and not zero
ok 1406 - 42 A3 — exactly ONE lvc_recall row per pass-0 recall, not two and not zero
  ---
  duration_ms: 8.393083
  type: 'test'
  ...
# Subtest: 42 A4 — ONE CONTEXT PER REQUEST: one id across a request, and a different id for the next
ok 1407 - 42 A4 — ONE CONTEXT PER REQUEST: one id across a request, and a different id for the next
  ---
  duration_ms: 14.003917
  type: 'test'
  ...
# Subtest: 42 B5 — the appropriateness route passes unknown_route (SOURCE: its POST cannot be driven here)
ok 1408 - 42 B5 — the appropriateness route passes unknown_route (SOURCE: its POST cannot be driven here)
  ---
  duration_ms: 0.263375
  type: 'test'
  ...
# Subtest: 42 B6 — both right-care probe scripts write nothing (SOURCE: neither can be driven from a test)
ok 1409 - 42 B6 — both right-care probe scripts write nothing (SOURCE: neither can be driven from a test)
  ---
  duration_ms: 0.708125
  type: 'test'
  ...
# Subtest: 42 B7 — the A/A route's existing surface is unchanged (SOURCE: read from the diff, not a request)
ok 1410 - 42 B7 — the A/A route's existing surface is unchanged (SOURCE: read from the diff, not a request)
  ---
  duration_ms: 0.189625
  type: 'test'
  ...
# Subtest: 42 B7b — the context is minted ONCE, in GET, and threaded (SOURCE, alongside A4)
ok 1411 - 42 B7b — the context is minted ONCE, in GET, and threaded (SOURCE, alongside A4)
  ---
  duration_ms: 0.083459
  type: 'test'
  ...
# Subtest: lvc-value defaultRetrieveHits does NOT set useNormativeLeg (no normative frame in the judge prompt)
ok 1412 - lvc-value defaultRetrieveHits does NOT set useNormativeLeg (no normative frame in the judge prompt)
  ---
  duration_ms: 0.422416
  type: 'test'
  ...
# Subtest: linked: continued / newly-started / gap classified, provenance on the sides that exist
ok 1413 - linked: continued / newly-started / gap classified, provenance on the sides that exist
  ---
  duration_ms: 12.620875
  type: 'test'
  ...
# Subtest: unlinked: admission-list-only, no reconciliation rows (missing baseline stays visible)
ok 1414 - unlinked: admission-list-only, no reconciliation rows (missing baseline stays visible)
  ---
  duration_ms: 0.4325
  type: 'test'
  ...
# Subtest: linked but no pre-admission OPD meds: still admission-only (no baseline to compare)
ok 1415 - linked but no pre-admission OPD meds: still admission-only (no baseline to compare)
  ---
  duration_ms: 0.559541
  type: 'test'
  ...
# Subtest: flag off: the composition never fires — no admission occurrence is reconciled
ok 1416 - flag off: the composition never fires — no admission occurrence is reconciled
  ---
  duration_ms: 0.461875
  type: 'test'
  ...
# Subtest: med-rec ONLY: the view exposes no problem / allergy continuity field (Gate D scope)
ok 1417 - med-rec ONLY: the view exposes no problem / allergy continuity field (Gate D scope)
  ---
  duration_ms: 0.987666
  type: 'test'
  ...
# Subtest: version
ok 1418 - version
  ---
  duration_ms: 0.522583
  type: 'test'
  ...
# Subtest: resolveProblemLabel: map hit, source-text preference, unknown → neutral
ok 1419 - resolveProblemLabel: map hit, source-text preference, unknown → neutral
  ---
  duration_ms: 0.536708
  type: 'test'
  ...
# Subtest: classifyProblemTier: active/background/historical (recency, recurrence, incidental)
ok 1420 - classifyProblemTier: active/background/historical (recency, recurrence, incidental)
  ---
  duration_ms: 0.308416
  type: 'test'
  ...
# Subtest: flagAbnormalLabs: banding, unit-mismatch = no flag, sex-specific, trend
ok 1421 - flagAbnormalLabs: banding, unit-mismatch = no flag, sex-specific, trend
  ---
  duration_ms: 0.565
  type: 'test'
  ...
# Subtest: computeCareGaps: abnormal-not-rechecked > 6mo; recent excluded; normal excluded
ok 1422 - computeCareGaps: abnormal-not-rechecked > 6mo; recent excluded; normal excluded
  ---
  duration_ms: 0.290416
  type: 'test'
  ...
# Subtest: computePictureConfidence: Ravali-like → THIN; in-person recent → GOOD
ok 1423 - computePictureConfidence: Ravali-like → THIN; in-person recent → GOOD
  ---
  duration_ms: 0.294709
  type: 'test'
  ...
# Subtest: buildVitalsView: numbers + EWS surfaced when present; honest absence otherwise
ok 1424 - buildVitalsView: numbers + EWS surfaced when present; honest absence otherwise
  ---
  duration_ms: 0.281292
  type: 'test'
  ...
# Subtest: buildAttentionFlags: medication conflict + critical lab surface as flags
ok 1425 - buildAttentionFlags: medication conflict + critical lab surface as flags
  ---
  duration_ms: 0.231542
  type: 'test'
  ...
# Subtest: patch/1: canonicalAnalyte tolerant match — real Vit D db name → severe band
ok 1426 - patch/1: canonicalAnalyte tolerant match — real Vit D db name → severe band
  ---
  duration_ms: 0.20575
  type: 'test'
  ...
# Subtest: patch/1: real Vit D (8.01 ng/mL) surfaces in labs, gaps AND attention
ok 1427 - patch/1: real Vit D (8.01 ng/mL) surfaces in labs, gaps AND attention
  ---
  duration_ms: 0.402167
  type: 'test'
  ...
# Subtest: patch/2: safety-net item is SURFACED (labs) but NOT PROMOTED (no gap, no attention flag)
ok 1428 - patch/2: safety-net item is SURFACED (labs) but NOT PROMOTED (no gap, no attention flag)
  ---
  duration_ms: 0.079375
  type: 'test'
  ...
# Subtest: patch/2: latest source-NORMAL + no range → NOT surfaced (nothing over-flagged)
ok 1429 - patch/2: latest source-NORMAL + no range → NOT surfaced (nothing over-flagged)
  ---
  duration_ms: 0.061
  type: 'test'
  ...
# Subtest: patch/3: trend de-clutter — stable repeats collapse, differing values surface
ok 1430 - patch/3: trend de-clutter — stable repeats collapse, differing values surface
  ---
  duration_ms: 0.069209
  type: 'test'
  ...
# Subtest: determinism: fns twice → deep-equal
ok 1431 - determinism: fns twice → deep-equal
  ---
  duration_ms: 0.150542
  type: 'test'
  ...
# Subtest: inv1 / stratum4: problem omitted at a later encounter → uncertain_current_status, never resolved
ok 1432 - inv1 / stratum4: problem omitted at a later encounter → uncertain_current_status, never resolved
  ---
  duration_ms: 3.29575
  type: 'test'
  ...
# Subtest: inv1 / stratum3: explicit later resolved → documented_resolved
ok 1433 - inv1 / stratum3: explicit later resolved → documented_resolved
  ---
  duration_ms: 0.178375
  type: 'test'
  ...
# Subtest: inv2: empty memberRef is a hard error (single-member invariant)
ok 1434 - inv2: empty memberRef is a hard error (single-member invariant)
  ---
  duration_ms: 0.116125
  type: 'test'
  ...
# Subtest: inv3: two distinct raws with no dictionary hit stay separate (no fuzzy merge)
ok 1435 - inv3: two distinct raws with no dictionary hit stay separate (no fuzzy merge)
  ---
  duration_ms: 0.240625
  type: 'test'
  ...
# Subtest: inv4 + inv5: every occurrence carries provenance and the derived status keeps its occurrences
ok 1436 - inv4 + inv5: every occurrence carries provenance and the derived status keeps its occurrences
  ---
  duration_ms: 0.160833
  type: 'test'
  ...
# Subtest: inv6 / stratum5: contradictory allergy → reported_allergy AND a safety_critical status_conflict
ok 1437 - inv6 / stratum5: contradictory allergy → reported_allergy AND a safety_critical status_conflict
  ---
  duration_ms: 0.418125
  type: 'test'
  ...
# Subtest: inv7: buildMemberState is reproducible — same evidence + versions → deep-equal
ok 1438 - inv7: buildMemberState is reproducible — same evidence + versions → deep-equal
  ---
  duration_ms: 0.630625
  type: 'test'
  ...
# Subtest: inv7 / stratum13: buildMemberState does not mutate input; corrected evidence → corrected snapshot
ok 1439 - inv7 / stratum13: buildMemberState does not mutate input; corrected evidence → corrected snapshot
  ---
  duration_ms: 0.194875
  type: 'test'
  ...
# Subtest: inv9: version + as-of metadata is mandatory and stamped
ok 1440 - inv9: version + as-of metadata is mandatory and stamped
  ---
  duration_ms: 0.215875
  type: 'test'
  ...
# Subtest: inv10: unresolved concept flows through as data (null id, relation unresolved)
ok 1441 - inv10: unresolved concept flows through as data (null id, relation unresolved)
  ---
  duration_ms: 0.312084
  type: 'test'
  ...
# Subtest: stratum1: persistent chronic (multi-touch, span>1yr, no long gap) → persistent + active
ok 1442 - stratum1: persistent chronic (multi-touch, span>1yr, no long gap) → persistent + active
  ---
  duration_ms: 0.078333
  type: 'test'
  ...
# Subtest: stratum2: recurrent (present → long gap → present) → recurrent [EPISODIC concept]
ok 1443 - stratum2: recurrent (present → long gap → present) → recurrent [EPISODIC concept]
  ---
  duration_ms: 0.065292
  type: 'test'
  ...
# Subtest: R1: a chronic concept re-documented ≥2× is persistent regardless of gap length
ok 1444 - R1: a chronic concept re-documented ≥2× is persistent regardless of gap length
  ---
  duration_ms: 0.069667
  type: 'test'
  ...
# Subtest: R1 guard: an episodic concept with dense touches within a year is NOT forced persistent
ok 1445 - R1 guard: an episodic concept with dense touches within a year is NOT forced persistent
  ---
...
ok 1446 - R2: patient-reported stop then a LATER prescription → status stopped + one medication/temporal_conflict/review (both provenances)
  ---
...
ok 1458 - assembleEvidence: prescription row → opd EncounterEvidence (meds, denied allergy, icd problem, demographics)
  ---
  duration_ms: 1.092709
  type: 'test'
  ...
# Subtest: assembleEvidence: lab rows → lab encounters grouped by booking, investigation points
ok 1459 - assembleEvidence: lab rows → lab encounters grouped by booking, investigation points
  ---
  duration_ms: 0.106083
  type: 'test'
  ...
# Subtest: assembleEvidence → buildMemberState: creatinine series spans both bookings, unit consistent
ok 1460 - assembleEvidence → buildMemberState: creatinine series spans both bookings, unit consistent
  ---
  duration_ms: 1.214417
  type: 'test'
  ...
# Subtest: assembleEvidence: identifier-free — no name/mobile/dob leaks into evidence
ok 1461 - assembleEvidence: identifier-free — no name/mobile/dob leaks into evidence
  ---
  duration_ms: 0.179834
  type: 'test'
  ...
# Subtest: assembleEvidence: diagnosis_icd_codes bare-string arrays — empty elements/arrays skipped
ok 1462 - assembleEvidence: diagnosis_icd_codes bare-string arrays — empty elements/arrays skipped
  ---
  duration_ms: 4.603875
  type: 'test'
  ...
# Subtest: assembleEvidence: malformed / missing rows degrade to empty, never throw
ok 1463 - assembleEvidence: malformed / missing rows degrade to empty, never throw
  ---
  duration_ms: 0.155625
  type: 'test'
  ...
# Subtest: careCallOutcomeToEncounter: stopped+reason → care_call encounter, identifier-free, deterministic
ok 1464 - careCallOutcomeToEncounter: stopped+reason → care_call encounter, identifier-free, deterministic
  ---
  duration_ms: 1.70925
  type: 'test'
  ...
# Subtest: careCallOutcomeToEncounter: complaint resolved → complaintStatuses; empty derived → empty arrays
ok 1465 - careCallOutcomeToEncounter: complaint resolved → complaintStatuses; empty derived → empty arrays
  ---
  duration_ms: 0.284292
  type: 'test'
  ...
# Subtest: Patch B: care-call encounter dated at called_at (fresh observation), not the episode note_date
ok 1466 - Patch B: care-call encounter dated at called_at (fresh observation), not the episode note_date
  ---
...
ok 1467 - loop closure: opd prescribes X + care_call reports X stopped → frozen buildMemberState currentness = stopped
  ---
  duration_ms: 1.174708
  type: 'test'
  ...
# Subtest: loop closure R2: a LATER re-prescription after the patient-reported stop → medication/temporal_conflict/review
ok 1468 - loop closure R2: a LATER re-prescription after the patient-reported stop → medication/temporal_conflict/review
  ---
  duration_ms: 0.1475
  type: 'test'
  ...
# Subtest: gold seed is FROZEN: 20 strata, every case ratified:true, member-bank/1.0
ok 1469 - gold seed is FROZEN: 20 strata, every case ratified:true, member-bank/1.0
  ---
  duration_ms: 0.611542
  type: 'test'
  ...
# Subtest: frozen baseline member-state-baseline/1.0: the seed clears every floor (no breaches)
ok 1470 - frozen baseline member-state-baseline/1.0: the seed clears every floor (no breaches)
  ---
  duration_ms: 3.241542
  type: 'test'
  ...
# Subtest: HARD gates hold for EVERY case: retention/provenance/trust 100%, incorrect-resolution 0
ok 1471 - HARD gates hold for EVERY case: retention/provenance/trust 100%, incorrect-resolution 0
  ---
  duration_ms: 0.543584
  type: 'test'
  ...
# Subtest: EVERY invariant-class case scores zero invariantViolations against the frozen core
ok 1472 - EVERY invariant-class case scores zero invariantViolations against the frozen core
  ---
  duration_ms: 0.399708
  type: 'test'
  ...
# Subtest: S3: explicit resolution → documented_resolved
ok 1473 - S3: explicit resolution → documented_resolved
  ---
  duration_ms: 0.088708
  type: 'test'
  ...
# Subtest: S4: omitted later → uncertain, never resolved
ok 1474 - S4: omitted later → uncertain, never resolved
  ---
  duration_ms: 0.06975
  type: 'test'
  ...
# Subtest: S5: allergy reported dominates denied + safety_critical conflict
ok 1475 - S5: allergy reported dominates denied + safety_critical conflict
  ---
  duration_ms: 0.071042
  type: 'test'
  ...
# Subtest: S6: prescribed, currentness never inferred to taking
ok 1476 - S6: prescribed, currentness never inferred to taking
  ---
  duration_ms: 0.047041
  type: 'test'
  ...
# Subtest: S8: broader/narrower not merged → 2 distinct problems
ok 1477 - S8: broader/narrower not merged → 2 distinct problems
  ---
  duration_ms: 1.232167
  type: 'test'
  ...
# Subtest: S9: mixed units → unit:null + value_conflict
ok 1478 - S9: mixed units → unit:null + value_conflict
  ---
  duration_ms: 0.755041
  type: 'test'
  ...
# Subtest: S12: two simultaneous → 2 parallel problems
ok 1479 - S12: two simultaneous → 2 parallel problems
  ---
  duration_ms: 0.197625
  type: 'test'
  ...
# Subtest: S14: "rule out PE" not merged with confirmed PE
ok 1480 - S14: "rule out PE" not merged with confirmed PE
  ---
  duration_ms: 0.139208
  type: 'test'
  ...
# Subtest: S15: patient complaint resolved → documented_resolved occurrence (explicit, not silence)
ok 1481 - S15: patient complaint resolved → documented_resolved occurrence (explicit, not silence)
  ---
  duration_ms: 0.163041
  type: 'test'
  ...
# Subtest: S16: patient-reported stopped overrides prescription
ok 1482 - S16: patient-reported stopped overrides prescription
  ---
  duration_ms: 0.161083
  type: 'test'
  ...
# Subtest: S17: allergy trust-conflict records BOTH trusts in the Discrepancy detail
ok 1483 - S17: allergy trust-conflict records BOTH trusts in the Discrepancy detail
  ---
  duration_ms: 0.275375
  type: 'test'
  ...
# Subtest: S18: followUps carried, deduped by id, no overlay
ok 1484 - S18: followUps carried, deduped by id, no overlay
  ---
  duration_ms: 0.328
  type: 'test'
  ...
# Subtest: S20: neutrality — zero patient-reported → empty followUps + 1.0 statuses
ok 1485 - S20: neutrality — zero patient-reported → empty followUps + 1.0 statuses
  ---
  duration_ms: 0.1755
  type: 'test'
  ...
# Subtest: S1: chronic re-documented across years → persistent (R1 chronicity fix)
ok 1486 - S1: chronic re-documented across years → persistent (R1 chronicity fix)
  ---
  duration_ms: 0.1435
  type: 'test'
  ...
# Subtest: S2: episodic present-gap-present → recurrent (unchanged by R1)
ok 1487 - S2: episodic present-gap-present → recurrent (unchanged by R1)
  ---
  duration_ms: 0.13525
  type: 'test'
  ...
# Subtest: S7: explicit stopped reflected in status
ok 1488 - S7: explicit stopped reflected in status
  ---
...
ok 1489 - S19: keeps stopped after a re-prescription + one medication/temporal_conflict/review (both trusts)
  ---
  duration_ms: 0.181584
  type: 'test'
  ...
# Subtest: S13: evidence is not mutated; a corrected copy recomputes to a different snapshot
ok 1490 - S13: evidence is not mutated; a corrected copy recomputes to a different snapshot
  ---
  duration_ms: 0.310209
  type: 'test'
  ...
# Subtest: aggregate over the full seed: retention 1.0, zero invariant violations, zero incorrect resolutions
ok 1491 - aggregate over the full seed: retention 1.0, zero invariant violations, zero incorrect resolutions
  ---
  duration_ms: 1.089667
  type: 'test'
  ...
# Subtest: normalizeConcept: exact hit → relation exact + canonical id
ok 1492 - normalizeConcept: exact hit → relation exact + canonical id
  ---
  duration_ms: 0.893625
  type: 'test'
  ...
# Subtest: normalizeConcept: synonym hit → relation synonym, same canonical id
ok 1493 - normalizeConcept: synonym hit → relation synonym, same canonical id
  ---
  duration_ms: 0.205375
  type: 'test'
  ...
# Subtest: normalizeConcept: no dictionary hit → unresolved (null id), never a guess
ok 1494 - normalizeConcept: no dictionary hit → unresolved (null id), never a guess
  ---
  duration_ms: 0.05675
  type: 'test'
  ...
# Subtest: normalizeConcept: broader/narrower are NEVER merged (diabetes ≠ type-2-diabetes)
ok 1495 - normalizeConcept: broader/narrower are NEVER merged (diabetes ≠ type-2-diabetes)
  ---
  duration_ms: 0.066458
  type: 'test'
  ...
# Subtest: normalizeConcept: domain-scoped dictionaries (creatinine only resolves as investigation)
ok 1496 - normalizeConcept: domain-scoped dictionaries (creatinine only resolves as investigation)
  ---
  duration_ms: 0.114916
  type: 'test'
  ...
# Subtest: normalizeConcept: deterministic — same input → identical result
ok 1497 - normalizeConcept: deterministic — same input → identical result
  ---
  duration_ms: 0.33375
  type: 'test'
  ...
# Subtest: groupingKey: resolved → canonical id; two unresolved merge only on identical normalized raw
ok 1498 - groupingKey: resolved → canonical id; two unresolved merge only on identical normalized raw
  ---
  duration_ms: 0.120041
  type: 'test'
  ...
# Subtest: normalizeRaw: lowercases, strips punctuation, collapses whitespace
ok 1499 - normalizeRaw: lowercases, strips punctuation, collapses whitespace
  ---
  duration_ms: 0.04925
  type: 'test'
  ...
# Subtest: complaint resolved → problem documented_resolved (explicit signal)
ok 1500 - complaint resolved → problem documented_resolved (explicit signal)
  ---
  duration_ms: 1.551542
  type: 'test'
  ...
# Subtest: complaint worse → active (never resolved)
ok 1501 - complaint worse → active (never resolved)
  ---
  duration_ms: 0.126
  type: 'test'
  ...
# Subtest: resolved-then-silent stays resolved (a later unrelated encounter does not re-open it)
ok 1502 - resolved-then-silent stays resolved (a later unrelated encounter does not re-open it)
  ---
  duration_ms: 0.274584
  type: 'test'
  ...
# Subtest: a complaint whose concept matches no documented problem still forms its own problem
ok 1503 - a complaint whose concept matches no documented problem still forms its own problem
  ---
  duration_ms: 0.1155
  type: 'test'
  ...
# Subtest: patient-reported stopped overrides a prescription prescribed
ok 1504 - patient-reported stopped overrides a prescription prescribed
  ---
  duration_ms: 0.449334
  type: 'test'
  ...
# Subtest: patient-reported reported_taking sets taking; currentness not synthesized otherwise
ok 1505 - patient-reported reported_taking sets taking; currentness not synthesized otherwise
  ---
  duration_ms: 0.09825
  type: 'test'
  ...
# Subtest: most-recent patient-reported wins over an older patient-reported
ok 1506 - most-recent patient-reported wins over an older patient-reported
  ---
  duration_ms: 1.077083
  type: 'test'
  ...
# Subtest: stopReason is carried on the occurrence
ok 1507 - stopReason is carried on the occurrence
  ---
...
ok 1508 - patient_reported denied + structured_db reported_allergy → reported_allergy + safety_critical conflict recording both trusts
  ---
  duration_ms: 0.883625
  type: 'test'
  ...
# Subtest: followUps carried onto the snapshot, deduped by id, date-sorted
ok 1509 - followUps carried onto the snapshot, deduped by id, date-sorted
  ---
  duration_ms: 1.329
  type: 'test'
  ...
# Subtest: neutrality: no patient-reported evidence → 1.0 behaviour + empty followUps
ok 1510 - neutrality: no patient-reported evidence → 1.0 behaviour + empty followUps
  ---
  duration_ms: 0.2505
  type: 'test'
  ...
# Subtest: version + provenance passthrough
ok 1511 - version + provenance passthrough
  ---
  duration_ms: 0.51875
  type: 'test'
  ...
# Subtest: course: chronic re-documented → Persistent (warn)
ok 1512 - course: chronic re-documented → Persistent (warn)
  ---
  duration_ms: 0.372
  type: 'test'
  ...
# Subtest: status: an omitted/silent problem renders Uncertain, NEVER Active
ok 1513 - status: an omitted/silent problem renders Uncertain, NEVER Active
  ---
  duration_ms: 0.116333
  type: 'test'
  ...
# Subtest: medication currentness: prescribed carries "not confirmed taken"; stopped → Stopped
ok 1514 - medication currentness: prescribed carries "not confirmed taken"; stopped → Stopped
  ---
  duration_ms: 0.066667
  type: 'test'
  ...
# Subtest: allergy: reported_allergy + matching allergy Discrepancy → conflicted:true, critical
ok 1515 - allergy: reported_allergy + matching allergy Discrepancy → conflicted:true, critical
  ---
  duration_ms: 0.050667
  type: 'test'
  ...
# Subtest: series: two-point HbA1c → direction down; mixed-unit creatinine → mixedUnits true
ok 1516 - series: two-point HbA1c → direction down; mixed-unit creatinine → mixedUnits true
  ---
  duration_ms: 0.049833
  type: 'test'
  ...
# Subtest: conflicts sorted safety_critical → review → informational; counts.safetyCritical
ok 1517 - conflicts sorted safety_critical → review → informational; counts.safetyCritical
  ---
  duration_ms: 0.128458
  type: 'test'
  ...
# Subtest: counts reflect the view arrays
ok 1518 - counts reflect the view arrays
  ---
  duration_ms: 0.041166
  type: 'test'
  ...
# Subtest: Patch A: view dates render as YYYY-MM-DD (dayOnly, idempotent on already-day strings)
ok 1519 - Patch A: view dates render as YYYY-MM-DD (dayOnly, idempotent on already-day strings)
  ---
  duration_ms: 0.624917
  type: 'test'
  ...
# Subtest: presentMemberState is deterministic (twice → deep-equal)
ok 1520 - presentMemberState is deterministic (twice → deep-equal)
  ---
  duration_ms: 1.178917
  type: 'test'
  ...
# Subtest: version constants are the Stage-0 pinned triple
ok 1521 - version constants are the Stage-0 pinned triple
  ---
  duration_ms: 0.360833
  type: 'test'
  ...
# Subtest: emptyMemberStateSnapshot: passed-in computedAt/asOf, empty arrays, passes zod
ok 1522 - emptyMemberStateSnapshot: passed-in computedAt/asOf, empty arrays, passes zod
  ---
  duration_ms: 1.046375
  type: 'test'
  ...
# Subtest: a built snapshot validates against the zod schema
ok 1523 - a built snapshot validates against the zod schema
  ---
  duration_ms: 1.494458
  type: 'test'
  ...
# Subtest: version constant
ok 1524 - version constant
  ---
  duration_ms: 0.458208
  type: 'test'
  ...
# Subtest: retention/provenance/trust-provenance = 1.0 on well-formed input
ok 1525 - retention/provenance/trust-provenance = 1.0 on well-formed input
  ---
  duration_ms: 1.862125
  type: 'test'
  ...
# Subtest: falseMerges=1 when two distinct expected concepts collapse (synonyms merge)
ok 1526 - falseMerges=1 when two distinct expected concepts collapse (synonyms merge)
  ---
  duration_ms: 0.120708
  type: 'test'
  ...
# Subtest: falseSplits=1 when one expected concept becomes two entities
ok 1527 - falseSplits=1 when one expected concept becomes two entities
  ---
  duration_ms: 0.180416
  type: 'test'
  ...
# Subtest: conflictRecall [1,1] on a seeded allergy conflict
ok 1528 - conflictRecall [1,1] on a seeded allergy conflict
  ---
  duration_ms: 0.644041
  type: 'test'
  ...
# Subtest: problemCourseAgree [1,1] on a correctly-scored course
ok 1529 - problemCourseAgree [1,1] on a correctly-scored course
  ---
  duration_ms: 0.2545
  type: 'test'
  ...
# Subtest: incorrectResolutions=1 for a documented_resolved occurrence with no explicit basis
ok 1530 - incorrectResolutions=1 for a documented_resolved occurrence with no explicit basis
  ---
  duration_ms: 0.549959
  type: 'test'
  ...
# Subtest: scoreCase is deterministic (twice → deep-equal)
ok 1531 - scoreCase is deterministic (twice → deep-equal)
  ---
  duration_ms: 0.196125
  type: 'test'
  ...
# Subtest: aggregate rolls up the Part-C metric set
ok 1532 - aggregate rolls up the Part-C metric set
  ---
  duration_ms: 0.33075
  type: 'test'
  ...
...
ok 1533 - prescriptionsSql is byte-identical to shadow.mjs (drift fails CI)
  ---
  duration_ms: 0.505667
  type: 'test'
  ...
# Subtest: labsSql is byte-identical to shadow.mjs (drift fails CI)
ok 1534 - labsSql is byte-identical to shadow.mjs (drift fails CI)
  ---
  duration_ms: 0.105916
  type: 'test'
  ...
# Subtest: individualForPrescSql: pinned shape + injection guard (bad uid throws)
ok 1535 - individualForPrescSql: pinned shape + injection guard (bad uid throws)
  ---
  duration_ms: 0.219666
  type: 'test'
  ...
# Subtest: individualUidForPresc: a bad uid returns null WITHOUT touching the DB
ok 1536 - individualUidForPresc: a bad uid returns null WITHOUT touching the DB
  ---
  duration_ms: 0.078375
  type: 'test'
  ...
# Subtest: vitalsEver true → GREEN with the unchanged label, whatever the modality
ok 1537 - vitalsEver true → GREEN with the unchanged label, whatever the modality
  ---
  duration_ms: 2.300875
  type: 'test'
  ...
# Subtest: vitalsEver false + majority unknown → AMBER, with the new label
ok 1538 - vitalsEver false + majority unknown → AMBER, with the new label
  ---
  duration_ms: 0.336333
  type: 'test'
  ...
# Subtest: vitalsEver false + majority remote + inPerson 0 → RED, label BYTE-IDENTICAL to today
ok 1539 - vitalsEver false + majority remote + inPerson 0 → RED, label BYTE-IDENTICAL to today
  ---
  duration_ms: 0.275833
  type: 'test'
  ...
# Subtest: vitalsEver false + inPerson > 0 → AMBER, label BYTE-IDENTICAL to today
ok 1540 - vitalsEver false + inPerson > 0 → AMBER, label BYTE-IDENTICAL to today
  ---
  duration_ms: 0.131333
  type: 'test'
  ...
# Subtest: the no-visits variant of the old label is preserved (opd 0 ⇒ no count suffix)
ok 1541 - the no-visits variant of the old label is preserved (opd 0 ⇒ no count suffix)
  ---
  duration_ms: 0.135917
  type: 'test'
  ...
# Subtest: the vitals factor stays counted: true in every case
ok 1542 - the vitals factor stays counted: true in every case
  ---
  duration_ms: 0.530917
  type: 'test'
  ...
# Subtest: the MODALITY factor is unaffected in all four cases — this build did not touch it
ok 1543 - the MODALITY factor is unaffected in all four cases — this build did not touch it
  ---
  duration_ms: 0.250083
  type: 'test'
  ...
# Subtest: the contact and labs factors are unaffected
ok 1544 - the contact and labs factors are unaffected
  ---
  duration_ms: 0.919584
  type: 'test'
  ...
# Subtest: the unknown case is reachable through the OR, and remote is not
ok 1545 - the unknown case is reachable through the OR, and remote is not
  ---
  duration_ms: 14.170667
  type: 'test'
  ...
# Subtest: D-B case 1 — ALL rows documented: the ladder is unchanged
ok 1546 - D-B case 1 — ALL rows documented: the ladder is unchanged
  ---
  duration_ms: 0.558875
  type: 'test'
  ...
# Subtest: D-B case 2 — NO rows documented with total > 0 ⇒ majority unknown
ok 1547 - D-B case 2 — NO rows documented with total > 0 ⇒ majority unknown
  ---
  duration_ms: 0.081375
  type: 'test'
  ...
# Subtest: D-B case 3 — total === 0 still returns unknown, as it always did
ok 1548 - D-B case 3 — total === 0 still returns unknown, as it always did
  ---
  duration_ms: 0.05675
  type: 'test'
  ...
# Subtest: picture confidence: unknown is AMBER, still counted, with the exact label
ok 1549 - picture confidence: unknown is AMBER, still counted, with the exact label
  ---
  duration_ms: 0.264
  type: 'test'
  ...
# Subtest: picture confidence: in_person, mixed and remote branches are byte-identical
ok 1550 - picture confidence: in_person, mixed and remote branches are byte-identical
  ---
  duration_ms: 0.156708
  type: 'test'
  ...
# Subtest: buildVitalsView: unknown gets the exact note; the other branch is unchanged
ok 1551 - buildVitalsView: unknown gets the exact note; the other branch is unchanged
  ---
  duration_ms: 0.103125
  type: 'test'
  ...
# Subtest: the call-context sentence no longer claims remote care when the modality is unknown
ok 1552 - the call-context sentence no longer claims remote care when the modality is unknown
  ---
  duration_ms: 0.37275
  type: 'test'
  ...
# Subtest: THE JOIN KEY: consult_uid is matched FIRST, prescription_uid is the fallback
ok 1553 - THE JOIN KEY: consult_uid is matched FIRST, prescription_uid is the fallback
  ---
  duration_ms: 0.077541
  type: 'test'
  ...
# Subtest: the vitals SELECT now carries consult_uid, and fetchRows exposes it
ok 1554 - the vitals SELECT now carries consult_uid, and fetchRows exposes it
  ---
  duration_ms: 0.300542
  type: 'test'
  ...
# Subtest: the resolver copies individualUidForPresc: isUid guard, LIMIT 1, soft-fail
ok 1555 - the resolver copies individualUidForPresc: isUid guard, LIMIT 1, soft-fail
  ---
  duration_ms: 0.659542
  type: 'test'
  ...
# Subtest: readEncounterVitals still never throws, and readMemberVitals is untouched
ok 1556 - readEncounterVitals still never throws, and readMemberVitals is untouched
  ---
  duration_ms: 0.111833
  type: 'test'
  ...
# Subtest: Gate D · no regression: every ratified gold case is byte-identical flag-on vs flag-off
ok 1557 - Gate D · no regression: every ratified gold case is byte-identical flag-on vs flag-off
  ---
  duration_ms: 4.494208
  type: 'test'
  ...
# Subtest: Gate D · additive-only: composing an admission preserves every baseline occurrence, all deltas admission-anchored
ok 1558 - Gate D · additive-only: composing an admission preserves every baseline occurrence, all deltas admission-anchored
  ---
  duration_ms: 1.236833
  type: 'test'
  ...
# Subtest: Gate D · flag-off: the fixture is byte-identical to the frozen spine (composition does not fire)
ok 1559 - Gate D · flag-off: the fixture is byte-identical to the frozen spine (composition does not fire)
  ---
  duration_ms: 0.158958
  type: 'test'
  ...
...
ok 1560 - every relation has a ratified status, and every ratified status has a relation
  ---
  duration_ms: 0.859
  type: 'test'
  ...
# Subtest: no relation THREW — a crash is never a legitimate relation outcome
ok 1561 - no relation THREW — a crash is never a legitimate relation outcome
  ---
  duration_ms: 0.139792
  type: 'test'
  ...
# Subtest: D-1 Dose context is read — holds
ok 1562 - D-1 Dose context is read — holds
  ---
  duration_ms: 0.049417
  type: 'test'
  ...
# Subtest: D-2 Dose context, inverse — holds
ok 1563 - D-2 Dose context, inverse — holds
  ---
  duration_ms: 0.030083
  type: 'test'
  ...
# Subtest: D-3 SOS cap is applied — holds
ok 1564 - D-3 SOS cap is applied — holds
  ---
  duration_ms: 0.035333
  type: 'test'
  ...
# Subtest: D-4 Dose completeness — holds
ok 1565 - D-4 Dose completeness — holds
  ---
  duration_ms: 0.026792
  type: 'test'
  ...
# Subtest: D-5 Formulation is read — reproduces the observed defect (pinned)
ok 1566 - D-5 Formulation is read — reproduces the observed defect (pinned)
  ---
  duration_ms: 0.025584
  type: 'test'
  ...
# Subtest: D-6 Interaction needs both members — holds
ok 1567 - D-6 Interaction needs both members — holds
  ---
  duration_ms: 0.079166
  type: 'test'
  ...
# Subtest: D-7 Interaction ignores non-analgesic dose — holds
ok 1568 - D-7 Interaction ignores non-analgesic dose — holds
  ---
  duration_ms: 0.178708
  type: 'test'
  ...
# Subtest: G-1 Order independence — holds
ok 1569 - G-1 Order independence — holds
  ---
  duration_ms: 0.2835
  type: 'test'
  ...
# Subtest: G-2 Unrelated addition — holds
ok 1570 - G-2 Unrelated addition — holds
  ---
  duration_ms: 0.034875
  type: 'test'
  ...
# Subtest: G-3 Empty-field safety — holds
ok 1571 - G-3 Empty-field safety — holds
  ---
  duration_ms: 0.026084
  type: 'test'
  ...
# Subtest: G-4 Unit invariance — holds
ok 1572 - G-4 Unit invariance — holds
  ---
  duration_ms: 0.021084
  type: 'test'
  ...
# Subtest: G-5 Duplicate line — holds
ok 1573 - G-5 Duplicate line — holds
  ---
  duration_ms: 0.0205
  type: 'test'
  ...
# Subtest: G-6 Teleconsult context — holds
ok 1574 - G-6 Teleconsult context — holds
  ---
  duration_ms: 0.020042
  type: 'test'
  ...
# Subtest: G-7 Referral handoff — holds
ok 1575 - G-7 Referral handoff — holds
  ---
...
ok 1645 - expandQuery runs exactly once, on the original question
  ---
  duration_ms: 0.800417
  type: 'test'
  ...
# Subtest: the original arm retrieves on expanded text; variant arms retrieve on variant text
ok 1646 - the original arm retrieves on expanded text; variant arms retrieve on variant text
  ---
  duration_ms: 0.620042
  type: 'test'
  ...
# Subtest: variant generation runs on the original question, never the expanded paragraph
ok 1647 - variant generation runs on the original question, never the expanded paragraph
  ---
  duration_ms: 0.25475
  type: 'test'
  ...
# Subtest: skipExpand:true from the caller turns expansion OFF — expandQuery is not called
ok 1648 - skipExpand:true from the caller turns expansion OFF — expandQuery is not called
  ---
...
ok 1649 - expansion fail-open (returns the original question) leaves the original arm on the raw question
  ---
  duration_ms: 0.28525
  type: 'test'
  ...
# Subtest: per-variant retrieve() keeps reranker/weights OFF after expansion is restored
ok 1650 - per-variant retrieve() keeps reranker/weights OFF after expansion is restored
  ---
  duration_ms: 0.431958
  type: 'test'
  ...
# Subtest: expandedQuery is returned on MultiRetrieveResult
ok 1651 - expandedQuery is returned on MultiRetrieveResult
  ---
...
ok 1652 - RRF fusion: a chunk ranked \#1 by two variants beats a chunk ranked \#1 by one variant with higher cosine
  ---
  duration_ms: 1.078834
  type: 'test'
  ...
# Subtest: rerank runs once over the fused pool against the original question — never a variant
ok 1653 - rerank runs once over the fused pool against the original question — never a variant
  ---
...
ok 1654 - source weighting: a guidelines (0.95) chunk outranks an unknown-journal (0.80) chunk at equal rerank score
  ---
  duration_ms: 0.284875
  type: 'test'
  ...
# Subtest: per-variant retrieve() runs with useReranker/useSourceWeights false; fusion reranks once
ok 1655 - per-variant retrieve() runs with useReranker/useSourceWeights false; fusion reranks once
  ---
  duration_ms: 0.173208
  type: 'test'
  ...
# Subtest: variant generation returning nothing falls back to the original query alone, no throw
ok 1656 - variant generation returning nothing falls back to the original query alone, no throw
  ---
  duration_ms: 0.273417
  type: 'test'
  ...
# Subtest: multi-query hits always carry rrf_score + variant_ranks — no includeQuarantined needed
ok 1657 - multi-query hits always carry rrf_score + variant_ranks — no includeQuarantined needed
  ---
...
ok 1658 - R-6 guard: assertEmbeddingV2Available throws a named error when v2 is on but the column is absent
  ---
...
ok 1659 - every arm now RECEIVES a capture, and the fusion lifts index_version from the first that has one
  ---
  duration_ms: 1.170083
  type: 'test'
  ...
# Subtest: the manifest that results carries a non-null index_version, and validates clean on that field
ok 1660 - the manifest that results carries a non-null index_version, and validates clean on that field
  ---
  duration_ms: 0.728667
  type: 'test'
  ...
# Subtest: an arm that stamps nothing leaves a null, and the null is recorded rather than invented
ok 1661 - an arm that stamps nothing leaves a null, and the null is recorded rather than invented
  ---
...
ok 1662 - INSTRUMENTATION OFF: no arm capture is made, and the arms are called with an undefined third argument
  ---
  duration_ms: 0.378083
  type: 'test'
  ...
# Subtest: 63 — the fail-open early exit still has its literal form, and this file does not quote it in a comment
ok 1663 - 63 — the fail-open early exit still has its literal form, and this file does not quote it in a comment
  ---
...
ok 1664 - DEFAULT_NORMATIVE_SOURCES = choosing-wisely + the two activated guideline keys, in order
  ---
  duration_ms: 0.862334
  type: 'test'
  ...
# Subtest: the added keys target ACTIVATED sources (lab:), never quarantined (labq:) — inert until activation
ok 1665 - the added keys target ACTIVATED sources (lab:), never quarantined (labq:) — inert until activation
  ---
  duration_ms: 0.346167
  type: 'test'
  ...
# Subtest: sourceLabel renders "Even Guidelines" / "ICMR Guidelines" for the activated sources
ok 1666 - sourceLabel renders "Even Guidelines" / "ICMR Guidelines" for the activated sources
  ---
  duration_ms: 0.373417
  type: 'test'
  ...
# Subtest: labels are INERT while quarantined: a labq: chunk falls back to book (unchanged today)
ok 1667 - labels are INERT while quarantined: a labq: chunk falls back to book (unchanged today)
  ---
  duration_ms: 0.137958
  type: 'test'
  ...
# Subtest: choosing-wisely and every other source are byte-identical (book-driven, no override)
ok 1668 - choosing-wisely and every other source are byte-identical (book-driven, no override)
  ---
  duration_ms: 0.240292
  type: 'test'
  ...
# Subtest: the guideline anchors resolve to NO url (category/internal authority, not deterministic)
ok 1669 - the guideline anchors resolve to NO url (category/internal authority, not deterministic)
  ---
...
ok 1670 - CW gate: accept the top candidate only when its statement category == finding.lvc_category AND cosine ≥ τ
  ---
  duration_ms: 0.587417
  type: 'test'
  ...
# Subtest: guideline gate: accept iff cosine ≥ τ (no category constraint)
ok 1671 - guideline gate: accept iff cosine ≥ τ (no category constraint)
  ---
  duration_ms: 0.131167
  type: 'test'
  ...
# Subtest: mergeNormativeCitations attaches both accepted legs and dedupes against existing
ok 1672 - mergeNormativeCitations attaches both accepted legs and dedupes against existing
  ---
  duration_ms: 0.559458
  type: 'test'
  ...
# Subtest: hitToSource: guideline anchor resolves to NO url (no fake identifier), CW keeps its source/item
ok 1673 - hitToSource: guideline anchor resolves to NO url (no fake identifier), CW keeps its source/item
  ---
  duration_ms: 0.053292
  type: 'test'
  ...
# Subtest: SCORE-INVARIANCE: attaching citations leaves verdict/score/band/lvc_category byte-identical
ok 1674 - SCORE-INVARIANCE: attaching citations leaves verdict/score/band/lvc_category byte-identical
  ---
  duration_ms: 3.264625
  type: 'test'
  ...
# Subtest: attachNormativeCitations is IDEMPOTENT — a re-run adds nothing
ok 1675 - attachNormativeCitations is IDEMPOTENT — a re-run adds nothing
  ---
  duration_ms: 0.100458
  type: 'test'
  ...
# Subtest: groundFinding attaches CW+guideline when both legs return accepted hits
ok 1676 - groundFinding attaches CW+guideline when both legs return accepted hits
  ---
  duration_ms: 0.297875
  type: 'test'
  ...
# Subtest: groundFinding grounds nothing on cross-category CW / below-τ guideline, and SOFT-FAILS on throw
ok 1677 - groundFinding grounds nothing on cross-category CW / below-τ guideline, and SOFT-FAILS on throw
  ---
  duration_ms: 0.258334
  type: 'test'
  ...
# Subtest: even gate: accept iff cosine ≥ τ AND the dynamic lookup category == finding.lvc_category
ok 1678 - even gate: accept iff cosine ≥ τ AND the dynamic lookup category == finding.lvc_category
  ---
  duration_ms: 0.463334
  type: 'test'
  ...
# Subtest: citation ordering: external legs first, even-lvc LAST; dedup by (source,item_number)
ok 1679 - citation ordering: external legs first, even-lvc LAST; dedup by (source,item_number)
  ---
  duration_ms: 0.825166
  type: 'test'
  ...
# Subtest: groundFinding runs the even leg ONLY with a lookup; attaches it last, inert without a lookup
ok 1680 - groundFinding runs the even leg ONLY with a lookup; attaches it last, inert without a lookup
  ---
  duration_ms: 0.383792
  type: 'test'
  ...
# Subtest: --legs cw runs ONLY the CW leg (guideline omitted, guideline retrieve not even called)
ok 1681 - --legs cw runs ONLY the CW leg (guideline omitted, guideline retrieve not even called)
  ---
  duration_ms: 0.644625
  type: 'test'
  ...
# Subtest: --legs guideline runs ONLY the guideline leg (CW omitted)
ok 1682 - --legs guideline runs ONLY the guideline leg (CW omitted)
  ---
  duration_ms: 0.075833
  type: 'test'
  ...
# Subtest: --categories filters eligibility: a finding whose lvc_category is not listed grounds nothing
ok 1683 - --categories filters eligibility: a finding whose lvc_category is not listed grounds nothing
  ---
  duration_ms: 0.157375
  type: 'test'
  ...
# Subtest: --tau raises/lowers acceptance (same match math, different threshold)
ok 1684 - --tau raises/lowers acceptance (same match math, different threshold)
  ---
  duration_ms: 0.094083
  type: 'test'
  ...
# Subtest: DEFAULT options reproduce today's behaviour byte-identically (regression guard)
ok 1685 - DEFAULT options reproduce today's behaviour byte-identically (regression guard)
  ---
  duration_ms: 0.120625
  type: 'test'
  ...
# Subtest: CW category map: every id maps to a known lvc_category; the strong categories are covered
ok 1686 - CW category map: every id maps to a known lvc_category; the strong categories are covered
  ---
  duration_ms: 0.068167
  type: 'test'
  ...
# Subtest: isGroundableFinding: only non-informational low-value findings
ok 1687 - isGroundableFinding: only non-informational low-value findings
  ---
  duration_ms: 0.04225
  type: 'test'
  ...
...
ok 1688 - resolveNormativeSources defaults to choosing-wisely + the two activated guideline keys; never labq:% by default
  ---
  duration_ms: 1.133917
  type: 'test'
  ...
# Subtest: normativeLegK reads env NORMATIVE_LEG_K, defaults 5
ok 1689 - normativeLegK reads env NORMATIVE_LEG_K, defaults 5
  ---
  duration_ms: 0.263
  type: 'test'
  ...
# Subtest: the normative leg is the vector SQL filtered to source = ANY, capped at N_norm
ok 1690 - the normative leg is the vector SQL filtered to source = ANY, capped at N_norm
  ---
  duration_ms: 0.332125
  type: 'test'
  ...
# Subtest: the normative leg leaves the default filter clauses byte-identical
ok 1691 - the normative leg leaves the default filter clauses byte-identical
  ---
...
ok 1692 - useNormativeLeg + normativeSources thread through retrieveMultiQuery to each per-variant retrieve
  ---
  duration_ms: 0.51175
  type: 'test'
  ...
# Subtest: engine bumped to 0.81.19 (neutraliser removal) and the read family includes 0.81.8…0.81.17
ok 1693 - engine bumped to 0.81.19 (neutraliser removal) and the read family includes 0.81.8…0.81.17
  ---
  duration_ms: 1.259792
  type: 'test'
  ...
# Subtest: bug 9: an unresolved brand is surfaced but informational (never scores)
ok 1694 - bug 9: an unresolved brand is surfaced but informational (never scores)
  ---
  duration_ms: 1.55875
  type: 'test'
  ...
# Subtest: bug 6: an unresolved line never ALSO stacks incomplete dosing (consolidated)
ok 1695 - bug 6: an unresolved line never ALSO stacks incomplete dosing (consolidated)
  ---
  duration_ms: 0.328916
  type: 'test'
  ...
# Subtest: bug 7: an off-formulary cosmetic (by name) is exempt from incomplete dosing
ok 1696 - bug 7: an off-formulary cosmetic (by name) is exempt from incomplete dosing
  ---
  duration_ms: 0.246625
  type: 'test'
  ...
# Subtest: a RESOLVED real drug missing its dose STILL scores incomplete dosing
ok 1697 - a RESOLVED real drug missing its dose STILL scores incomplete dosing
  ---
  duration_ms: 0.224792
  type: 'test'
  ...
# Subtest: bug 2: a health-check package encounter is recognised and neutralises screening critiques
ok 1698 - bug 2: a health-check package encounter is recognised and neutralises screening critiques
  ---
  duration_ms: 0.460042
  type: 'test'
  ...
# Subtest: bug 10: a biotin-before-thyroid over-flag is neutralised to informational
ok 1699 - bug 10: a biotin-before-thyroid over-flag is neutralised to informational
  ---
  duration_ms: 0.774667
  type: 'test'
  ...
# Subtest: bug 5: the Antispasmodic/anticholinergic reclass does NOT change DDI tags
ok 1700 - bug 5: the Antispasmodic/anticholinergic reclass does NOT change DDI tags
  ---
  duration_ms: 0.520708
  type: 'test'
  ...
# Subtest: Part B: the 3 base categories are unchanged
ok 1701 - Part B: the 3 base categories are unchanged
  ---
  duration_ms: 0.733875
  type: 'test'
  ...
# Subtest: Part B: residual other splits into overuse sub-tags by priority
ok 1702 - Part B: residual other splits into overuse sub-tags by priority
  ---
  duration_ms: 2.384875
  type: 'test'
  ...
# Subtest: Part B: the omission guard keeps missing-safety-net / mismatch findings in other
ok 1703 - Part B: the omission guard keeps missing-safety-net / mismatch findings in other
  ---
  duration_ms: 0.436708
  type: 'test'
  ...
# Subtest: Part B: priority order — therapeutic_duplication wins over a steroid mention
ok 1704 - Part B: priority order — therapeutic_duplication wins over a steroid mention
  ---
  duration_ms: 0.042958
  type: 'test'
  ...
# Subtest: Part B: every category has a shared human label (no raw slug can render)
ok 1705 - Part B: every category has a shared human label (no raw slug can render)
  ---
  duration_ms: 0.050875
  type: 'test'
  ...
# Subtest: Part C: frequentFlierCmp orders per Decision 12
ok 1706 - Part C: frequentFlierCmp orders per Decision 12
  ---
  duration_ms: 0.122875
  type: 'test'
  ...
# Subtest: Part C: default (index) order is untouched by the comparator module
ok 1707 - Part C: default (index) order is untouched by the comparator module
  ---
...
ok 1708 - 0.81.10 S1: the muscle-relaxant finding is emitted informational (surfaced, out of the score)
  ---
  duration_ms: 0.75975
  type: 'test'
  ...
# Subtest: bug 1: xanthine for an acute URTI fires (context-guarded)
ok 1709 - bug 1: xanthine for an acute URTI fires (context-guarded)
  ---
  duration_ms: 1.329042
  type: 'test'
  ...
# Subtest: bug 1: the SAME xanthine is NOT flagged for a chronic-airways patient (J40–J47 guard)
ok 1710 - bug 1: the SAME xanthine is NOT flagged for a chronic-airways patient (J40–J47 guard)
  ---
...
ok 1711 - 0.81.14 Ruling 12: acebrophylline + acetylcysteine in an acute URTI → NO finding (rule dormant for it)
  ---
  duration_ms: 0.172834
  type: 'test'
  ...
# Subtest: 0.81.13 Decision 11: antihistamine + montelukast emits NO finding at any duration
ok 1712 - 0.81.13 Decision 11: antihistamine + montelukast emits NO finding at any duration
  ---
...
ok 1713 - 0.81.13 Decision 11: a xanthine AND antihistamine+montelukast → exactly ONE finding (the xanthine)
  ---
  duration_ms: 0.346125
  type: 'test'
  ...
# Subtest: 0.81.13 Decision 3: xanthine subject/rationale carry no "mucolytic"; guard + confidence unchanged
ok 1714 - 0.81.13 Decision 3: xanthine subject/rationale carry no "mucolytic"; guard + confidence unchanged
  ---
  duration_ms: 0.423833
  type: 'test'
  ...
# Subtest: bug 1: no acute-URTI context → nothing fires
ok 1715 - bug 1: no acute-URTI context → nothing fires
  ---
  duration_ms: 0.114208
  type: 'test'
  ...
# Subtest: 0.81.13 Decision 4: 5 → none; 7 → none; 8 and 15 → 0.7; 16 and 1 month → 0.85; unparseable → none
ok 1716 - 0.81.13 Decision 4: 5 → none; 7 → none; 8 and 15 → 0.7; 16 and 1 month → 0.85; unparseable → none
  ---
  duration_ms: 0.613
  type: 'test'
  ...
# Subtest: 0.81.13: parseDurationDays (exported) parses days/weeks/months and returns null for chronic/unparseable
ok 1717 - 0.81.13: parseDurationDays (exported) parses days/weeks/months and returns null for chronic/unparseable
  ---
  duration_ms: 0.34325
  type: 'test'
  ...
# Subtest: bug 8: BPO wash-off + leave-on is NOT a duplicate (finding dropped)
ok 1718 - bug 8: BPO wash-off + leave-on is NOT a duplicate (finding dropped)
  ---
  duration_ms: 1.160125
  type: 'test'
  ...
# Subtest: bug 8: topical + systemic sharing a molecule is not a duplicate
ok 1719 - bug 8: topical + systemic sharing a molecule is not a duplicate
  ---
  duration_ms: 0.160917
  type: 'test'
  ...
# Subtest: bug 8: a genuine same-route duplicate is KEPT
ok 1720 - bug 8: a genuine same-route duplicate is KEPT
  ---
  duration_ms: 0.094542
  type: 'test'
  ...
# Subtest: bug 8: an LLM finding (non-deterministic) is never touched by the route filter
ok 1721 - bug 8: an LLM finding (non-deterministic) is never touched by the route filter
  ---
  duration_ms: 0.037917
  type: 'test'
  ...
# Subtest: bug 4: opdCaseText surfaces the consult date exactly once with a historical guard
ok 1722 - bug 4: opdCaseText surfaces the consult date exactly once with a historical guard
  ---
  duration_ms: 0.158417
  type: 'test'
  ...
# Subtest: 0.81.14 Ruling 1: an oral + topical NSAID pair emits NO interaction finding
ok 1723 - 0.81.14 Ruling 1: an oral + topical NSAID pair emits NO interaction finding
  ---
  duration_ms: 1.051583
  type: 'test'
  ...
# Subtest: 0.81.14 Ruling 1: two ORAL NSAIDs still produce an interaction finding (unchanged)
ok 1724 - 0.81.14 Ruling 1: two ORAL NSAIDs still produce an interaction finding (unchanged)
  ---
  duration_ms: 0.290791
  type: 'test'
  ...
# Subtest: 0.81.14 Ruling 4: muscle relaxant with MSK context → none; without → fires; ctx omitted → today
ok 1725 - 0.81.14 Ruling 4: muscle relaxant with MSK context → none; without → fires; ctx omitted → today
  ---
  duration_ms: 0.0495
  type: 'test'
  ...
# Subtest: 0.81.14 Ruling 4: mskContextDocumented — "low back pain" true, ICD M54.5 true, "fever, cough" false
ok 1726 - 0.81.14 Ruling 4: mskContextDocumented — "low back pain" true, ICD M54.5 true, "fever, cough" false
  ---
...
ok 1767 - index-encounter exclusion is an as-of property: with only the index in-window, prior counts are 0 → NEW_TO_US
  ---
...
ok 1768 - no eval config ⇒ opdRetrieveOpts byte-identical to today
  ---
  duration_ms: 0.896542
  type: 'test'
  ...
# Subtest: evalNormativeLeg:true ⇒ useNormativeLeg true regardless of mini / env
ok 1769 - evalNormativeLeg:true ⇒ useNormativeLeg true regardless of mini / env
  ---
...
ok 1770 - buildOpenRouterBody carries the eval determinism config: temp0 + top_p1 + seed + reasoning-pin + provider-pin
  ---
...
ok 1771 - production defaultGenerate: Vertex-Gemini path gets temp0 + seed + top_p + fixed thinking, GATED on onGemini; mini/Ollama unchanged
  ---
  duration_ms: 0.618875
  type: 'test'
  ...
# Subtest: LVC/Kimi adjudication params: temp0 + seed + top_p + OpenRouter provider-pin
ok 1772 - LVC/Kimi adjudication params: temp0 + seed + top_p + OpenRouter provider-pin
  ---
  duration_ms: 0.207333
  type: 'test'
  ...
# Subtest: openRouterGenerate posts to the OpenRouter endpoint at temp 0 and returns the completion
ok 1773 - openRouterGenerate posts to the OpenRouter endpoint at temp 0 and returns the completion
  ---
  duration_ms: 34.653667
  type: 'test'
  ...
# Subtest: openRouterGenerate throws (does not silently fall back) when the key is missing
ok 1774 - openRouterGenerate throws (does not silently fall back) when the key is missing
  ---
  duration_ms: 0.311792
  type: 'test'
  ...
# Subtest: the lab-batch eval path never writes opd_note_audits (structural guard)
ok 1775 - the lab-batch eval path never writes opd_note_audits (structural guard)
  ---
  duration_ms: 0.272209
  type: 'test'
  ...
# Subtest: parseBatchState reads the eval config; absent ⇒ off / null
ok 1776 - parseBatchState reads the eval config; absent ⇒ off / null
  ---
  duration_ms: 0.894625
  type: 'test'
  ...
# Subtest: verdict sets are wired by scope
ok 1777 - verdict sets are wired by scope
  ---
  duration_ms: 0.831459
  type: 'test'
  ...
# Subtest: impact scope: TP-only second tap — valid tag + finding_ref required, category always null
ok 1778 - impact scope: TP-only second tap — valid tag + finding_ref required, category always null
  ---
  duration_ms: 0.27725
  type: 'test'
  ...
# Subtest: missed scope: category is REQUIRED (F6/A10.1) and whitelisted; unknown category rejected
ok 1779 - missed scope: category is REQUIRED (F6/A10.1) and whitelisted; unknown category rejected
  ---
  duration_ms: 0.193708
  type: 'test'
  ...
# Subtest: non-missed/impact scopes carry category=null
ok 1780 - non-missed/impact scopes carry category=null
  ---
  duration_ms: 0.061208
  type: 'test'
  ...
# Subtest: bad auditId is rejected before anything else
ok 1781 - bad auditId is rejected before anything else
  ---
  duration_ms: 0.048042
  type: 'test'
  ...
# Subtest: unknown scope is rejected
ok 1782 - unknown scope is rejected
  ---
  duration_ms: 0.043
  type: 'test'
  ...
# Subtest: legacy audit scope: bare comment allowed, defaults to audit, verdict optional
ok 1783 - legacy audit scope: bare comment allowed, defaults to audit, verdict optional
  ---
  duration_ms: 0.101459
  type: 'test'
  ...
# Subtest: audit scope: valid verdict kept, invalid verdict dropped to null
ok 1784 - audit scope: valid verdict kept, invalid verdict dropped to null
  ---
  duration_ms: 0.060167
  type: 'test'
  ...
# Subtest: audit scope: empty body (no verdict, no comment) rejected
ok 1785 - audit scope: empty body (no verdict, no comment) rejected
  ---
  duration_ms: 0.199584
  type: 'test'
  ...
# Subtest: finding scope: requires a finding verdict
ok 1786 - finding scope: requires a finding verdict
  ---
  duration_ms: 0.283084
  type: 'test'
  ...
# Subtest: finding scope: requires finding_ref
ok 1787 - finding scope: requires finding_ref
  ---
  duration_ms: 0.048208
  type: 'test'
  ...
# Subtest: finding scope: all four verdicts accepted, carries ref + signal_type + optional comment
ok 1788 - finding scope: all four verdicts accepted, carries ref + signal_type + optional comment
  ---
  duration_ms: 0.058208
  type: 'test'
  ...
# Subtest: missed scope: verdict forced to missed, comment required
ok 1789 - missed scope: verdict forced to missed, comment required
  ---
  duration_ms: 0.04875
  type: 'test'
  ...
# Subtest: fields are trimmed, empties collapse to null, oversized values are capped
ok 1790 - fields are trimmed, empties collapse to null, oversized values are capped
  ---
  duration_ms: 0.054625
  type: 'test'
  ...
# Subtest: dedup expression selects latest per (audit_id, finding_ref), tie-break highest id
ok 1791 - dedup expression selects latest per (audit_id, finding_ref), tie-break highest id
  ---
  duration_ms: 1.920875
  type: 'test'
  ...
# Subtest: precision_strict excludes contested; zero denominator → null
ok 1792 - precision_strict excludes contested; zero denominator → null
  ---
  duration_ms: 1.189917
  type: 'test'
  ...
# Subtest: coverage_pct = triaged/fired as a one-decimal percentage
ok 1793 - coverage_pct = triaged/fired as a one-decimal percentage
  ---
  duration_ms: 0.277542
  type: 'test'
  ...
# Subtest: parseAdjudicateArgs: valid log accepted; bad decision/action/missing rationale rejected
ok 1794 - parseAdjudicateArgs: valid log accepted; bad decision/action/missing rationale rejected
  ---
  duration_ms: 0.248166
  type: 'test'
  ...
# Subtest: parseAdjudicateArgs: monitor and all five decisions accepted; list defaults + clamp
ok 1795 - parseAdjudicateArgs: monitor and all five decisions accepted; list defaults + clamp
  ---
...
ok 1960 - no normative hits ⇒ assembleAuditContext is byte-identical to today's assembly
  ---
  duration_ms: 1.139458
  type: 'test'
  ...
# Subtest: channel mode: the literature retrieve opts are unchanged — useNormativeLeg is NOT set
ok 1961 - channel mode: the literature retrieve opts are unchanged — useNormativeLeg is NOT set
  ---
  duration_ms: 0.201541
  type: 'test'
  ...
# Subtest: normativeChannelOpts: standalone CW-only search — restrictSources, topK 4, leg NOT set
ok 1962 - normativeChannelOpts: standalone CW-only search — restrictSources, topK 4, leg NOT set
  ---
  duration_ms: 0.149958
  type: 'test'
  ...
# Subtest: channel context: literature [1-8] then the labelled normative block [9+]
ok 1963 - channel context: literature [1-8] then the labelled normative block [9+]
  ---
  duration_ms: 0.117458
  type: 'test'
  ...
# Subtest: numbering adapts when fewer than 8 literature excerpts return
ok 1964 - numbering adapts when fewer than 8 literature excerpts return
  ---
  duration_ms: 0.115083
  type: 'test'
  ...
# Subtest: buildNormativeBlock: empty hits ⇒ empty string (audit proceeds on literature alone)
ok 1965 - buildNormativeBlock: empty hits ⇒ empty string (audit proceeds on literature alone)
  ---
  duration_ms: 0.038541
  type: 'test'
  ...
# Subtest: evalNormativeChannel is independent of evalNormativeLeg — no union, no eviction
ok 1966 - evalNormativeChannel is independent of evalNormativeLeg — no union, no eviction
  ---
  duration_ms: 0.146584
  type: 'test'
  ...
# Subtest: the eval path still writes lab_analyses only — never opd_note_audits
ok 1967 - the eval path still writes lab_analyses only — never opd_note_audits
  ---
  duration_ms: 0.22725
  type: 'test'
  ...
# Subtest: OPD_AUDIT_SYSTEM is untouched — the channel header is not injected into the system prompt
ok 1968 - OPD_AUDIT_SYSTEM is untouched — the channel header is not injected into the system prompt
  ---
  duration_ms: 0.332625
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: flag off ⇒ opts byte-identical to today (no useNormativeLeg key)
ok 1969 - flag off ⇒ opts byte-identical to today (no useNormativeLeg key)
  ---
  duration_ms: 0.841792
  type: 'test'
  ...
# Subtest: flag on + non-mini ⇒ useNormativeLeg: true
ok 1970 - flag on + non-mini ⇒ useNormativeLeg: true
  ---
  duration_ms: 0.089
  type: 'test'
  ...
# Subtest: flag on + mini ⇒ no useNormativeLeg key (mini path can never enable the leg)
ok 1971 - flag on + mini ⇒ no useNormativeLeg key (mini path can never enable the leg)
  ---
  duration_ms: 0.053333
  type: 'test'
  ...
# Subtest: only OPD_NORMATIVE_LEG_ENABLED === "1" enables the leg
ok 1972 - only OPD_NORMATIVE_LEG_ENABLED === "1" enables the leg
  ---
  duration_ms: 0.186709
  type: 'test'
  ...
# Subtest: rowToOpdCase parses stringified JSONB + separates de-identified case from keys
ok 1973 - rowToOpdCase parses stringified JSONB + separates de-identified case from keys
  ---
  duration_ms: 1.724417
  type: 'test'
  ...
# Subtest: rowToOpdCase reads the NESTED GP fields (the extraction fix)
ok 1974 - rowToOpdCase reads the NESTED GP fields (the extraction fix)
  ---
  duration_ms: 1.080084
  type: 'test'
  ...
# Subtest: rowToOpdCase prefers the dpipe pipeline content over the nested source fields
ok 1975 - rowToOpdCase prefers the dpipe pipeline content over the nested source fields
  ---
  duration_ms: 0.179459
  type: 'test'
  ...
# Subtest: 0.6: referral handoff — leaflet excluded, referral + teleconsult surfaced
ok 1976 - 0.6: referral handoff — leaflet excluded, referral + teleconsult surfaced
  ---
  duration_ms: 0.354667
  type: 'test'
  ...
# Subtest: 0.6: teleconsult completeness — examination is not scored (N/A), referral counts as the plan
ok 1977 - 0.6: teleconsult completeness — examination is not scored (N/A), referral counts as the plan
  ---
  duration_ms: 0.500209
  type: 'test'
  ...
# Subtest: opdCompleteness flags the real gaps; allergy + history items removed
ok 1978 - opdCompleteness flags the real gaps; allergy + history items removed
  ---
  duration_ms: 0.68675
  type: 'test'
  ...
# Subtest: route inference: documented → used; blank → inferred from form; no form → null (real gap)
ok 1979 - route inference: documented → used; blank → inferred from form; no form → null (real gap)
  ---
  duration_ms: 0.46475
  type: 'test'
  ...
# Subtest: dose documented from the field, the strength field, or the strength embedded in the drug name
ok 1980 - dose documented from the field, the strength field, or the strength embedded in the drug name
  ---
  duration_ms: 0.567
  type: 'test'
  ...
# Subtest: prescribingChecks: dosing gap only when route is truly ambiguous / amount is absent (0.5)
ok 1981 - prescribingChecks: dosing gap only when route is truly ambiguous / amount is absent (0.5)
  ---
  duration_ms: 0.907458
  type: 'test'
  ...
# Subtest: prescribingChecks: unverified brand, duplicate by RESOLVED generic, high-alert info (v0.4)
ok 1982 - prescribingChecks: unverified brand, duplicate by RESOLVED generic, high-alert info (v0.4)
  ---
  duration_ms: 0.589167
  type: 'test'
  ...
# Subtest: parseOpdAnalysis extracts findings + PDQI-9 + suggestions and clamps citations
ok 1983 - parseOpdAnalysis extracts findings + PDQI-9 + suggestions and clamps citations
  ---
  duration_ms: 0.312833
  type: 'test'
  ...
# Subtest: C1: parseOpdAnalysis strips a reasoning <think> block (DeepSeek-R1) before parsing
ok 1984 - C1: parseOpdAnalysis strips a reasoning <think> block (DeepSeek-R1) before parsing
  ---
  duration_ms: 0.10925
  type: 'test'
  ...
# Subtest: opdSignalType maps every deterministic subject shape to the controlled vocab
ok 1985 - opdSignalType maps every deterministic subject shape to the controlled vocab
  ---
  duration_ms: 0.487167
  type: 'test'
  ...
# Subtest: opdSignalType: LLM subjects — antibiotic rule, coarse domain×verdict buckets, general fallback
ok 1986 - opdSignalType: LLM subjects — antibiotic rule, coarse domain×verdict buckets, general fallback
  ---
  duration_ms: 1.356583
  type: 'test'
  ...
# Subtest: stampFindingIdentity: stable refs, severity-change stable, distinct details distinct
ok 1987 - stampFindingIdentity: stable refs, severity-change stable, distinct details distinct
  ---
  duration_ms: 2.296083
  type: 'test'
  ...
# Subtest: 0.81.11: form/dosageForm are inert — prescribingChecks output is byte-identical with them present vs absent
ok 1988 - 0.81.11: form/dosageForm are inert — prescribingChecks output is byte-identical with them present vs absent
  ---
  duration_ms: 1.484208
  type: 'test'
  ...
# Subtest: 0.81.12 CANARY: guideline-recommended vaccine co-administration produces NO finding (LASA deleted)
ok 1989 - 0.81.12 CANARY: guideline-recommended vaccine co-administration produces NO finding (LASA deleted)
  ---
  duration_ms: 0.144208
  type: 'test'
  ...
# Subtest: 0.81.12 (Stage 2a): the SCORING molecule-subset dedup is NOT present — a mono+FDC produces no new penalty
ok 1990 - 0.81.12 (Stage 2a): the SCORING molecule-subset dedup is NOT present — a mono+FDC produces no new penalty
  ---
  duration_ms: 0.122792
  type: 'test'
  ...
# Subtest: 0.81.10: a low-value deterministic finding RETAINS its specific signal_type (no collapse to low_value_care)
ok 1991 - 0.81.10: a low-value deterministic finding RETAINS its specific signal_type (no collapse to low_value_care)
  ---
  duration_ms: 0.345375
  type: 'test'
  ...
# Subtest: 0.81.10: the muscle-relaxant documentation subject maps to signal_type muscle_relaxant_indication
ok 1992 - 0.81.10: the muscle-relaxant documentation subject maps to signal_type muscle_relaxant_indication
  ---
  duration_ms: 0.042792
  type: 'test'
  ...
# Subtest: stampFindingIdentity: within-note collision suffixes \#2, \#3 deterministically
ok 1993 - stampFindingIdentity: within-note collision suffixes \#2, \#3 deterministically
  ---
  duration_ms: 0.147125
  type: 'test'
  ...
# Subtest: stampFindingIdentity: every finding stamped non-empty (acceptance, spec §2)
ok 1994 - stampFindingIdentity: every finding stamped non-empty (acceptance, spec §2)
  ---
  duration_ms: 0.158625
  type: 'test'
  ...
# Subtest: opdCaseText includes the treating specialty line when provided (B4)
ok 1995 - opdCaseText includes the treating specialty line when provided (B4)
  ---
  duration_ms: 0.874792
  type: 'test'
  ...
# Subtest: followUpDocumented + completeness: UNKNOWN/blank excluded, real dispositions count (B2)
ok 1996 - followUpDocumented + completeness: UNKNOWN/blank excluded, real dispositions count (B2)
  ---
  duration_ms: 2.051334
  type: 'test'
  ...
# Subtest: completeness coverage excludes continuity fields — scored once (0.8)
ok 1997 - completeness coverage excludes continuity fields — scored once (0.8)
  ---
  duration_ms: 0.225166
  type: 'test'
  ...
# Subtest: opdCaseText marks a zero-medication note explicitly (B1)
ok 1998 - opdCaseText marks a zero-medication note explicitly (B1)
  ---
  duration_ms: 0.090709
  type: 'test'
  ...
# Subtest: v0.81 BUG-0.8-04: HOSPITAL_* prescription types are IN-PERSON, not teleconsult
ok 1999 - v0.81 BUG-0.8-04: HOSPITAL_* prescription types are IN-PERSON, not teleconsult
  ---
  duration_ms: 0.0475
  type: 'test'
  ...
# Subtest: v0.81 FIX I: a documented hands-on exam downgrades a teleconsult classification
ok 2000 - v0.81 FIX I: a documented hands-on exam downgrades a teleconsult classification
  ---
  duration_ms: 0.105084
  type: 'test'
  ...
# Subtest: v0.81 BUG-0.8-04: HOSPITAL_GP in-person note IS scored on examination
ok 2001 - v0.81 BUG-0.8-04: HOSPITAL_GP in-person note IS scored on examination
  ---
...
ok 2014 - R (0.8-11): a muscle relaxant is detected + consolidateDecisions drops the LLM version when a deterministic one exists
  ---
  duration_ms: 0.08425
  type: 'test'
  ...
# Subtest: Part 1: an ICD/coding-completeness gap finding is neutralised to non-scoring
ok 2015 - Part 1: an ICD/coding-completeness gap finding is neutralised to non-scoring
  ---
...
ok 2016 - obstetric adapter (flag ON): populates canonical fields from the obstetric template + current-visit selection
  ---
  duration_ms: 0.620291
  type: 'test'
  ...
# Subtest: obstetric adapter (flag OFF): the obstetric note audits via the GP path, byte-identical (no isObstetric)
ok 2017 - obstetric adapter (flag OFF): the obstetric note audits via the GP path, byte-identical (no isObstetric)
  ---
  duration_ms: 0.093791
  type: 'test'
  ...
# Subtest: obstetric mandatory set: SFH/FHR/presentation required only in the 2nd/3rd trimester
ok 2018 - obstetric mandatory set: SFH/FHR/presentation required only in the 2nd/3rd trimester
  ---
  duration_ms: 0.267917
  type: 'test'
  ...
# Subtest: obstetric vitals: BP never mandatory (credited if present); weight is the required vital
ok 2019 - obstetric vitals: BP never mandatory (credited if present); weight is the required vital
  ---
...
ok 2020 - obstetric mandatory set: rich note near-complete; follow-up scored in Continuity not Documentation
  ---
  duration_ms: 0.068208
  type: 'test'
  ...
# Subtest: obstetricDosingComplete: a 1-0-1 schedule counts even with a blank frequency field
ok 2021 - obstetricDosingComplete: a 1-0-1 schedule counts even with a blank frequency field
  ---
  duration_ms: 0.057
  type: 'test'
  ...
# Subtest: bpDocumented reads BP from the obstetric narrative (no structured BP column in db13, §9)
ok 2022 - bpDocumented reads BP from the obstetric narrative (no structured BP column in db13, §9)
  ---
...
ok 2023 - 0.81.14 Ruling 2: high-alert name-collision artifacts excluded at molecule level; real high-alerts + injectable MgSO4 still fire
  ---
  duration_ms: 0.289833
  type: 'test'
  ...
# Subtest: A-9: a non-obstetric note with an LMP gains NO mandatory LMP field; the obstetric path keeps it mandatory
ok 2024 - A-9: a non-obstetric note with an LMP gains NO mandatory LMP field; the obstetric path keeps it mandatory
  ---
  duration_ms: 0.117125
  type: 'test'
  ...
# Subtest: a complete, high-quality note scores in band A
ok 2025 - a complete, high-quality note scores in band A
  ---
  duration_ms: 0.717708
  type: 'test'
  ...
# Subtest: CANARY: three co-prescribed major interactions score prescribing safety 26/100, never 100
ok 2026 - CANARY: three co-prescribed major interactions score prescribing safety 26/100, never 100
  ---
  duration_ms: 0.187
  type: 'test'
  ...
# Subtest: 0.81.10: an informational finding (the retired muscle-relaxant prompt) does NOT enter the score
ok 2027 - 0.81.10: an informational finding (the retired muscle-relaxant prompt) does NOT enter the score
  ---
  duration_ms: 0.096084
  type: 'test'
  ...
# Subtest: a poor note (gaps + low-value order + prescribing issue + weak PDQI) scores low
ok 2028 - a poor note (gaps + low-value order + prescribing issue + weak PDQI) scores low
  ---
  duration_ms: 0.1135
  type: 'test'
  ...
# Subtest: PDQI-9 not assessed → note_quality weight collapses to 0 (does not drag the index)
ok 2029 - PDQI-9 not assessed → note_quality weight collapses to 0 (does not drag the index)
  ---
  duration_ms: 0.125834
  type: 'test'
  ...
# Subtest: PDQI-9 partial ratings average only the provided attributes
ok 2030 - PDQI-9 partial ratings average only the provided attributes
  ---
  duration_ms: 0.065458
  type: 'test'
  ...
# Subtest: weights are sane
ok 2031 - weights are sane
  ---
...
ok 2032 - documentationAdequacyFlag fires only when fields (near-)complete AND thoroughness/synthesis low
  ---
  duration_ms: 0.063292
  type: 'test'
  ...
# Subtest: computeOpdScore surfaces the thin-documentation flag without changing scores
ok 2033 - computeOpdScore surfaces the thin-documentation flag without changing scores
  ---
  duration_ms: 0.207
  type: 'test'
  ...
# Subtest: §4.1: concept_id "underuse:…" + source llm ⇒ direction underuse
ok 2034 - §4.1: concept_id "underuse:…" + source llm ⇒ direction underuse
  ---
  duration_ms: 0.691875
  type: 'test'
  ...
# Subtest: §4.1: an underuse finding contributes ZERO penalty
ok 2035 - §4.1: an underuse finding contributes ZERO penalty
  ---
...
ok 2036 - §4.1 pinned DELIBERATELY: the same finding with NO concept_id emerges with NO direction — the fresh-path behaviour, so a future change to it is visible
  ---
  duration_ms: 0.058042
  type: 'test'
  ...
# Subtest: overuse: prefix stamps overuse (the four measured 0.81.17 exhibits — non-antibiotic subjects)
ok 2037 - overuse: prefix stamps overuse (the four measured 0.81.17 exhibits — non-antibiotic subjects)
  ---
  duration_ms: 0.116417
  type: 'test'
  ...
# Subtest: §4.2 (D-6): documentation: and process: prefixes set no direction — absent stays the honest default
ok 2038 - §4.2 (D-6): documentation: and process: prefixes set no direction — absent stays the honest default
  ---
  duration_ms: 0.052791
  type: 'test'
  ...
# Subtest: §4.3 (D-3): the value written as based_on_coded_at IS the value read — identity, not recency
ok 2039 - §4.3 (D-3): the value written as based_on_coded_at IS the value read — identity, not recency
  ---
  duration_ms: 0.338708
  type: 'test'
  ...
# Subtest: the watermark SQL binds based_on_coded_at as $3 and reserves now() for rescored_at only
ok 2040 - the watermark SQL binds based_on_coded_at as $3 and reserves now() for rescored_at only
  ---
  duration_ms: 0.1235
  type: 'test'
  ...
# Subtest: the route passes the coded_at from the CANDIDATE SELECT and never re-reads it after the update
ok 2041 - the route passes the coded_at from the CANDIDATE SELECT and never re-reads it after the update
  ---
...
ok 2042 - candidate SQL: engine versions are a BOUND array param — unknown version ⇒ zero rows, never a throw
  ---
...
ok 2043 - candidate SQL: candidacy = the coder touched the note more recently than the last re-score observed
  ---
  duration_ms: 0.881708
  type: 'test'
  ...
# Subtest: candidate SQL tolerates migration 0029 not having run (displayed_band variant)
ok 2044 - candidate SQL tolerates migration 0029 not having run (displayed_band variant)
  ---
  duration_ms: 0.172791
  type: 'test'
  ...
# Subtest: ?limit= — default 800, clamped 1..3000, junk lands on the default
ok 2045 - ?limit= — default 800, clamped 1..3000, junk lands on the default
  ---
  duration_ms: 0.174958
  type: 'test'
  ...
# Subtest: A-1 §1: resolveEngineFilter(null) returns the whole family
ok 2046 - A-1 §1: resolveEngineFilter(null) returns the whole family
  ---
  duration_ms: 0.252666
  type: 'test'
  ...
# Subtest: A-1 §2: an exact family member narrows to exactly that one version
ok 2047 - A-1 §2: an exact family member narrows to exactly that one version
  ---
  duration_ms: 0.089042
  type: 'test'
  ...
# Subtest: A-1 §3: an unknown version yields [] — the fail-safe, never a widened scope
ok 2048 - A-1 §3: an unknown version yields [] — the fail-safe, never a widened scope
  ---
  duration_ms: 0.086875
  type: 'test'
  ...
# Subtest: A-1 §4: an injection-shaped value yields [] and never reaches the query as a live term
ok 2049 - A-1 §4: an injection-shaped value yields [] and never reaches the query as a live term
  ---
  duration_ms: 0.07575
  type: 'test'
  ...
# Subtest: A-1 §5: the report's engine_versions reflects the FILTERED list, not the family
ok 2050 - A-1 §5: the report's engine_versions reflects the FILTERED list, not the family
  ---
  duration_ms: 0.097583
  type: 'test'
  ...
# Subtest: a candidate-query error degrades to an EMPTY report — never a 500
ok 2051 - a candidate-query error degrades to an EMPTY report — never a 500
  ---
  duration_ms: 0.359125
  type: 'test'
  ...
# Subtest: finalize() runs stampDirection on the reuse path — the moment that already works
ok 2052 - finalize() runs stampDirection on the reuse path — the moment that already works
  ---
  duration_ms: 0.130709
  type: 'test'
  ...
# Subtest: the route threads each row's OWN engine_version into auditOpdNote, so the UPDATE is in place
ok 2053 - the route threads each row's OWN engine_version into auditOpdNote, so the UPDATE is in place
  ---
  duration_ms: 0.081208
  type: 'test'
  ...
# Subtest: ?apply=1 is the ONLY write switch — read-only without it
ok 2054 - ?apply=1 is the ONLY write switch — read-only without it
  ---
  duration_ms: 0.061
  type: 'test'
  ...
# Subtest: §2.7: no cron, no ?auto=1, no scheduler — cadence is V's decision, later
ok 2055 - §2.7: no cron, no ?auto=1, no scheduler — cadence is V's decision, later
  ---
...
ok 2065 - A-3 §1: hysteresisCaseSql('displayed_band','$3','$2::int') emits $2::int in every comparison, $3 as every result
  ---
  duration_ms: 0.181292
  type: 'test'
  ...
# Subtest: A-3 §2: the UPDATE statement deduces $2 from the SET clause and casts it in the CASE
ok 2066 - A-3 §2: the UPDATE statement deduces $2 from the SET clause and casts it in the CASE
  ---
...
ok 2067 - A-3 §3: saveOpdAudit's conflict clause still reads EXCLUDED.note_quality_index — the two call sites must never be "unified" back into this bug
  ---
  duration_ms: 0.031541
  type: 'test'
  ...
# Subtest: A-3 §4: the pure twin hysteresisBand is untouched — same thresholds, same g
ok 2068 - A-3 §4: the pure twin hysteresisBand is untouched — same thresholds, same g
  ---
  duration_ms: 0.053625
  type: 'test'
  ...
# Subtest: A-4 §1 (defect 1): the candidate comparison truncates the DB side to the watermark's precision
ok 2069 - A-4 §1 (defect 1): the candidate comparison truncates the DB side to the watermark's precision
  ---
...
ok 2070 - A-4 §2 (defect 2): an underuse finding carrying lvc_category on input emerges WITHOUT it — every other key survives
  ---
...
ok 2071 - A-4 §3 (defect 2): a non-underuse finding is unchanged — it still receives stamped[i] with its lvc_category
  ---
  duration_ms: 0.085959
  type: 'test'
  ...
...
ok 2072 - A-4 §4 (defect 2): underuse + signal_type low_value_care — specific type restored AND lvc_category dropped, both on one finding
  ---
  duration_ms: 0.579708
  type: 'test'
  ...
# Subtest: A-4 §5 (defect 3): the pass lock — lab_batch semantics, TTL pinned, held ⇒ empty report, never a 500
ok 2073 - A-4 §5 (defect 3): the pass lock — lab_batch semantics, TTL pinned, held ⇒ empty report, never a 500
  ---
  duration_ms: 1.344917
  type: 'test'
  ...
# Subtest: migration 0030: CREATE TABLE IF NOT EXISTS opd_rescore_state, keyed (uid, engine_version)
ok 2074 - migration 0030: CREATE TABLE IF NOT EXISTS opd_rescore_state, keyed (uid, engine_version)
  ---
  duration_ms: 0.205459
  type: 'test'
  ...
# Subtest: severityOf + importanceHint: known types weighted, unknown → med
ok 2075 - severityOf + importanceHint: known types weighted, unknown → med
  ---
  duration_ms: 0.515916
  type: 'test'
  ...
# Subtest: buildQueue groups by doctor→signal_type, counts, and drops informational
ok 2076 - buildQueue groups by doctor→signal_type, counts, and drops informational
  ---
  duration_ms: 0.409333
  type: 'test'
  ...
# Subtest: buildQueue ranks types by severity×frequency; noisiest marked; doctors by attention
ok 2077 - buildQueue ranks types by severity×frequency; noisiest marked; doctors by attention
  ---
  duration_ms: 0.138125
  type: 'test'
  ...
# Subtest: buildQueue overlays the latest type decision; status filter hides triaged
ok 2078 - buildQueue overlays the latest type decision; status filter hides triaged
  ---
  duration_ms: 0.361084
  type: 'test'
  ...
# Subtest: buildQueue concentrated flag: doctor holding the whole window share of a type
ok 2079 - buildQueue concentrated flag: doctor holding the whole window share of a type
  ---
  duration_ms: 0.082333
  type: 'test'
  ...
# Subtest: validateDecision: valid batch route decision normalizes
ok 2080 - validateDecision: valid batch route decision normalizes
  ---
  duration_ms: 0.118375
  type: 'test'
  ...
# Subtest: validateDecision: audit_bug forces routed=false and requires bug_type
ok 2081 - validateDecision: audit_bug forces routed=false and requires bug_type
  ---
  duration_ms: 0.103208
  type: 'test'
  ...
# Subtest: validateDecision: valid_signal requires importance; routed requires response_required
ok 2082 - validateDecision: valid_signal requires importance; routed requires response_required
  ---
  duration_ms: 0.055542
  type: 'test'
  ...
# Subtest: validateDecision: instance scope requires audit_id + finding_ref; bad enums rejected
ok 2083 - validateDecision: instance scope requires audit_id + finding_ref; bad enums rejected
  ---
  duration_ms: 0.169
  type: 'test'
  ...
# Subtest: classifyTransition: audit_bug & not-routed → dismiss; routed → resolution
ok 2084 - classifyTransition: audit_bug & not-routed → dismiss; routed → resolution
  ---
  duration_ms: 0.590041
  type: 'test'
  ...
# Subtest: requireChip: dismiss/resolution require an in-vocabulary chip
ok 2085 - requireChip: dismiss/resolution require an in-vocabulary chip
  ---
  duration_ms: 0.105
  type: 'test'
  ...
# Subtest: buildTriageEvent: enforces chip, free text optional, telemetry columns normalized
ok 2086 - buildTriageEvent: enforces chip, free text optional, telemetry columns normalized
  ---
  duration_ms: 0.693792
  type: 'test'
  ...
# Subtest: buildQueue: representative carries complexity_band/inputs + lvc_category (passthrough)
ok 2087 - buildQueue: representative carries complexity_band/inputs + lvc_category (passthrough)
  ---
  duration_ms: 0.073583
  type: 'test'
  ...
# Subtest: buildQueue: missing complexity → representative fields null (no placeholder)
ok 2088 - buildQueue: missing complexity → representative fields null (no placeholder)
  ---
  duration_ms: 0.041
  type: 'test'
  ...
# Subtest: maxTries: 1 makes exactly ONE attempt — no retry at all
ok 2089 - maxTries: 1 makes exactly ONE attempt — no retry at all
  ---
  duration_ms: 0.906791
  type: 'test'
  ...
# Subtest: maxTries: 2 makes exactly TWO attempts
ok 2090 - maxTries: 2 makes exactly TWO attempts
  ---
  duration_ms: 0.2165
  type: 'test'
  ...
# Subtest: a shortened ladder still SUCCEEDS on a later attempt within it
ok 2091 - a shortened ladder still SUCCEEDS on a later attempt within it
  ---
  duration_ms: 0.2885
  type: 'test'
  ...
# Subtest: the empty-200 class respects the shortened budget too
ok 2092 - the empty-200 class respects the shortened budget too
  ---
  duration_ms: 0.144417
  type: 'test'
  ...
# Subtest: the terminal timeout message reports the ladder ACTUALLY used, not the constant
ok 2093 - the terminal timeout message reports the ladder ACTUALLY used, not the constant
  ---
  duration_ms: 5.169375
  type: 'test'
  ...
# Subtest: maxTries absent ⇒ 3 attempts, unchanged from today
ok 2094 - maxTries absent ⇒ 3 attempts, unchanged from today
  ---
  duration_ms: 0.360125
  type: 'test'
  ...
# Subtest: maxTries zero ⇒ 3 attempts, unchanged from today
ok 2095 - maxTries zero ⇒ 3 attempts, unchanged from today
  ---
  duration_ms: 0.163083
  type: 'test'
  ...
# Subtest: maxTries negative ⇒ 3 attempts, unchanged from today
ok 2096 - maxTries negative ⇒ 3 attempts, unchanged from today
  ---
  duration_ms: 0.1345
  type: 'test'
  ...
# Subtest: maxTries NaN ⇒ 3 attempts, unchanged from today
ok 2097 - maxTries NaN ⇒ 3 attempts, unchanged from today
  ---
  duration_ms: 0.24675
  type: 'test'
  ...
# Subtest: maxTries Infinity ⇒ 3 attempts, unchanged from today
ok 2098 - maxTries Infinity ⇒ 3 attempts, unchanged from today
  ---
  duration_ms: 0.328416
  type: 'test'
  ...
# Subtest: maxTries a fraction below one ⇒ 3 attempts, unchanged from today
ok 2099 - maxTries a fraction below one ⇒ 3 attempts, unchanged from today
  ---
  duration_ms: 0.117459
  type: 'test'
  ...
# Subtest: maxTries a string ⇒ 3 attempts, unchanged from today
ok 2100 - maxTries a string ⇒ 3 attempts, unchanged from today
  ---
  duration_ms: 0.071459
  type: 'test'
  ...
# Subtest: a fractional maxTries above one TRUNCATES rather than rounding up
ok 2101 - a fractional maxTries above one TRUNCATES rather than rounding up
  ---
  duration_ms: 0.094792
  type: 'test'
  ...
# Subtest: OPENROUTER_MAX_TRIES still exports 3 and stays the default
ok 2102 - OPENROUTER_MAX_TRIES still exports 3 and stays the default
  ---
  duration_ms: 0.147083
  type: 'test'
  ...
# Subtest: the loop body reads the LOCAL maxTries, never the constant
ok 2103 - the loop body reads the LOCAL maxTries, never the constant
  ---
  duration_ms: 0.107875
  type: 'test'
  ...
# Subtest: chatWithFallback takes maxTries fifth and uses it only where a retry loop exists
ok 2104 - chatWithFallback takes maxTries fifth and uses it only where a retry loop exists
  ---
  duration_ms: 0.2325
  type: 'test'
  ...
# Subtest: governedChat threads maxTries down BOTH arms
ok 2105 - governedChat threads maxTries down BOTH arms
  ---
...
ok 2106 - the lab path and the production wrapper share ONE policy — identical bindings, not copies
  ---
  duration_ms: 0.5685
  type: 'test'
  ...
# Subtest: policy values are unchanged by the move: 3 tries, 429/5xx retryable, 110s deadline, jittered backoff
ok 2107 - policy values are unchanged by the move: 3 tries, 429/5xx retryable, 110s deadline, jittered backoff
  ---
  duration_ms: 0.121791
  type: 'test'
  ...
# Subtest: a clean completion returns on attempt 1 — one call, no sleep, no failure report
ok 2108 - a clean completion returns on attempt 1 — one call, no sleep, no failure report
  ---
  duration_ms: 0.492958
  type: 'test'
  ...
# Subtest: every attempt carries OUR deadline and the SDK retries OFF — the budget must not multiply
ok 2109 - every attempt carries OUR deadline and the SDK retries OFF — the budget must not multiply
  ---
  duration_ms: 0.108792
  type: 'test'
  ...
# Subtest: a transport error (no HTTP status) is retryable; success on attempt 2 returns normally
ok 2110 - a transport error (no HTTP status) is retryable; success on attempt 2 returns normally
  ---
  duration_ms: 0.146542
  type: 'test'
  ...
# Subtest: 429/5xx retry on the bounded budget; the FINAL attempt rethrows the provider error
ok 2111 - 429/5xx retry on the bounded budget; the FINAL attempt rethrows the provider error
  ---
  duration_ms: 0.41975
  type: 'test'
  ...
# Subtest: a non-transient status (4xx) throws IMMEDIATELY — the call site fallback path is unchanged for it
ok 2112 - a non-transient status (4xx) throws IMMEDIATELY — the call site fallback path is unchanged for it
  ---
  duration_ms: 0.20725
  type: 'test'
  ...
# Subtest: an EMPTY 200 is a retryable failure, not a terminal one — d6efe39 made it visible, this makes it survivable
ok 2113 - an EMPTY 200 is a retryable failure, not a terminal one — d6efe39 made it visible, this makes it survivable
  ---
  duration_ms: 0.856625
  type: 'test'
  ...
# Subtest: an empty 200 on the FINAL attempt throws the MARKED error so call sites refuse the Ollama fallback (§2.3)
ok 2114 - an empty 200 on the FINAL attempt throws the MARKED error so call sites refuse the Ollama fallback (§2.3)
  ---
...
ok 2115 - an abort error is retryable — an abort that was not retryable would make the deadline strictly worse than no deadline
  ---
  duration_ms: 0.554042
  type: 'test'
  ...
# Subtest: the onAttemptFailure hook can never be the thing that fails the call
ok 2116 - the onAttemptFailure hook can never be the thing that fails the call
  ---
  duration_ms: 0.601958
  type: 'test'
  ...
...
ok 2117 - streams are the CALLER's exclusion, not the wrapper's — the governed call sites keep the bare create() for stream:true
  ---
  duration_ms: 1.063375
  type: 'test'
  ...
# Subtest: NO timeoutMs → OPENROUTER_TIMEOUT_MS, byte-identical to before
ok 2118 - NO timeoutMs → OPENROUTER_TIMEOUT_MS, byte-identical to before
  ---
  duration_ms: 0.750709
  type: 'test'
  ...
# Subtest: timeoutMs: 600_000 → the doAttempt timeout is 600 000, not 110 000
ok 2119 - timeoutMs: 600_000 → the doAttempt timeout is 600 000, not 110 000
  ---
  duration_ms: 0.112125
  type: 'test'
  ...
# Subtest: timeoutMs: 600_000 → the ABORTCONTROLLER deadline is 600 000 too, not just the SDK belt
ok 2120 - timeoutMs: 600_000 → the ABORTCONTROLLER deadline is 600 000 too, not just the SDK belt
  ---
  duration_ms: 0.128042
  type: 'test'
  ...
# Subtest: a junk timeoutMs degrades to the default — a deadline may never be switched off
ok 2121 - a junk timeoutMs degrades to the default — a deadline may never be switched off
  ---
  duration_ms: 0.185458
  type: 'test'
  ...
# Subtest: the terminal error message reports the APPLIED timeout, not the constant
ok 2122 - the terminal error message reports the APPLIED timeout, not the constant
  ---
  duration_ms: 27.084042
  type: 'test'
  ...
# Subtest: OPENROUTER_MAX_TRIES is still 3 and OPENROUTER_TIMEOUT_MS still defaults to 110 000
ok 2123 - OPENROUTER_MAX_TRIES is still 3 and OPENROUTER_TIMEOUT_MS still defaults to 110 000
  ---
  duration_ms: 0.27
  type: 'test'
  ...
# Subtest: retry CLASSIFICATION is unchanged: 429 and 5xx retry, 4xx does not
ok 2124 - retry CLASSIFICATION is unchanged: 429 and 5xx retry, 4xx does not
  ---
  duration_ms: 0.125875
  type: 'test'
  ...
# Subtest: a 4xx throws immediately — one attempt, no retry
ok 2125 - a 4xx throws immediately — one attempt, no retry
  ---
  duration_ms: 0.231458
  type: 'test'
  ...
# Subtest: a 429 retries the full budget
ok 2126 - a 429 retries the full budget
  ---
  duration_ms: 0.269417
  type: 'test'
  ...
# Subtest: an ABORT retries — a deadline that ended the call must not end the budget
ok 2127 - an ABORT retries — a deadline that ended the call must not end the budget
  ---
  duration_ms: 16.786833
  type: 'test'
  ...
# Subtest: the backoff curve is untouched
ok 2128 - the backoff curve is untouched
  ---
  duration_ms: 0.11875
  type: 'test'
  ...
# Subtest: EVERY openrouterCreateWithRetry call site forwards the caller timeout AND maxTries
ok 2129 - EVERY openrouterCreateWithRetry call site forwards the caller timeout AND maxTries
  ---
  duration_ms: 0.37275
  type: 'test'
  ...
# Subtest: there are exactly FOUR provider call sites — a fifth must be enumerated
ok 2130 - there are exactly FOUR provider call sites — a fifth must be enumerated
  ---
  duration_ms: 2.590958
  type: 'test'
  ...
# Subtest: chatWithFallback's OpenRouter branch passes the caller's timeout through
ok 2131 - chatWithFallback's OpenRouter branch passes the caller's timeout through
  ---
  duration_ms: 0.311584
  type: 'test'
  ...
# Subtest: tracedChat's OpenRouter branch — THE PRODUCTION PATH — passes both through
ok 2132 - tracedChat's OpenRouter branch — THE PRODUCTION PATH — passes both through
  ---
  duration_ms: 0.148125
  type: 'test'
  ...
# Subtest: the IPD worker box is 800 s, matching the OPD worker
ok 2133 - the IPD worker box is 800 s, matching the OPD worker
  ---
  duration_ms: 0.156959
  type: 'test'
  ...
# Subtest: this build did not disturb the OPD cron window
ok 2134 - this build did not disturb the OPD cron window
  ---
  duration_ms: 0.083959
  type: 'test'
  ...
# Subtest: §2.1 — the DDL re-applies BOTH failure-table constraints, in one keyed statement
ok 2135 - §2.1 — the DDL re-applies BOTH failure-table constraints, in one keyed statement
  ---
  duration_ms: 1.602625
  type: 'test'
  ...
# Subtest: §2.1 — the re-applied CHECKs carry the widened phase list
ok 2136 - §2.1 — the re-applied CHECKs carry the widened phase list
  ---
  duration_ms: 0.355334
  type: 'test'
  ...
# Subtest: §2.1 — the statement is idempotent: DROP tolerates absence, ADD names a now-free constraint
ok 2137 - §2.1 — the statement is idempotent: DROP tolerates absence, ADD names a now-free constraint
  ---
  duration_ms: 0.306084
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: §2.1 — a FRESH table run issues create, then the one ALTER, in order
ok 2138 - §2.1 — a FRESH table run issues create, then the one ALTER, in order
  ---
  duration_ms: 48.09075
  type: 'test'
  ...
# Subtest: §2.1 — a PRE-EXISTING table carrying the OLD constraints still ends with the widened form
ok 2139 - §2.1 — a PRE-EXISTING table carrying the OLD constraints still ends with the widened form
  ---
  duration_ms: 3.294542
  type: 'test'
  ...
# Subtest: §2.1 — migrations/0035 is in parity with the re-applied constraints
ok 2140 - §2.1 — migrations/0035 is in parity with the re-applied constraints
  ---
  duration_ms: 0.189125
  type: 'test'
  ...
# Subtest: §2.2 — the version is 3, and a payload built today claims 3
ok 2141 - §2.2 — the version is 3, and a payload built today claims 3
  ---
  duration_ms: 0.449125
  type: 'test'
  ...
# Subtest: §2.2 — a VERSION-2 manifest is unrecognized, which is the point of the bump
ok 2142 - §2.2 — a VERSION-2 manifest is unrecognized, which is the point of the bump
  ---
  duration_ms: 0.523583
  type: 'test'
  ...
# Subtest: §2.2 — ABSENT and explicit NULL stay distinguishable, which is why `has` is used
ok 2143 - §2.2 — ABSENT and explicit NULL stay distinguishable, which is why `has` is used
  ---
  duration_ms: 0.366042
  type: 'test'
  ...
# Subtest: §2.2 — both fields are TYPE-checked, not merely present
ok 2144 - §2.2 — both fields are TYPE-checked, not merely present
  ---
  duration_ms: 1.530709
  type: 'test'
  ...
# Subtest: §2.2 — the seed vocabulary is ONE object, not two that agree
ok 2145 - §2.2 — the seed vocabulary is ONE object, not two that agree
  ---
  duration_ms: 1.425166
  type: 'test'
  ...
# [rerank] backend failed, returning input order generic, untyped
# Subtest: §2.3 — the JUDGE arm no longer claims an unproven not_served
ok 2146 - §2.3 — the JUDGE arm no longer claims an unproven not_served
  ---
  duration_ms: 148.420791
  type: 'test'
  ...
# [rerank] backend failed, returning input order generic, untyped
# Subtest: §2.3 — the COHERE arm is unchanged from pass 0
ok 2147 - §2.3 — the COHERE arm is unchanged from pass 0
  ---
  duration_ms: 9.3
  type: 'test'
  ...
...
ok 2148 - §2.3 — THE REGRESSION GUARD: no synthesised boundary on EITHER arm may assert proof
  ---
  duration_ms: 17.598375
  type: 'test'
  ...
# Subtest: §2.3 — where transport proof EXISTS, the served class still stands
ok 2149 - §2.3 — where transport proof EXISTS, the served class still stands
  ---
  duration_ms: 0.109584
  type: 'test'
  ...
# Subtest: parseSkeleton forces DDx hand-off on a low-certainty / anchored diagnosis
ok 2150 - parseSkeleton forces DDx hand-off on a low-certainty / anchored diagnosis
  ---
  duration_ms: 0.799041
  type: 'test'
  ...
# Subtest: normStageKind maps synonyms + defaults to assessment
ok 2151 - normStageKind maps synonyms + defaults to assessment
  ---
  duration_ms: 0.117375
  type: 'test'
  ...
# Subtest: normStageFlag maps synonyms + defaults to routine
ok 2152 - normStageFlag maps synonyms + defaults to routine
  ---
  duration_ms: 0.054375
  type: 'test'
  ...
# Subtest: orderAndIdStages enforces canonical order, stable within kind, sequential ids
ok 2153 - orderAndIdStages enforces canonical order, stable within kind, sequential ids
  ---
  duration_ms: 0.3675
  type: 'test'
  ...
# Subtest: orderAndIdStages caps the spine
ok 2154 - orderAndIdStages caps the spine
  ---
  duration_ms: 0.076334
  type: 'test'
  ...
# Subtest: STAGE_ORDER is strictly increasing along the canonical path
ok 2155 - STAGE_ORDER is strictly increasing along the canonical path
  ---
  duration_ms: 0.049292
  type: 'test'
  ...
# Subtest: parseSkeleton parses a fenced JSON skeleton
ok 2156 - parseSkeleton parses a fenced JSON skeleton
  ---
  duration_ms: 0.161
  type: 'test'
  ...
# Subtest: parseSkeleton forces needsDdx for undifferentiated low-certainty presentation
ok 2157 - parseSkeleton forces needsDdx for undifferentiated low-certainty presentation
  ---
  duration_ms: 0.05875
  type: 'test'
  ...
# Subtest: parseSkeleton returns null on garbage / empty stages
ok 2158 - parseSkeleton returns null on garbage / empty stages
  ---
  duration_ms: 0.179333
  type: 'test'
  ...
# Subtest: parseEnrichment parses, filters unknown ids, dedups, separates evidence/estimates
ok 2159 - parseEnrichment parses, filters unknown ids, dedups, separates evidence/estimates
  ---
  duration_ms: 0.456708
  type: 'test'
  ...
# Subtest: parseEnrichment returns null on garbage
ok 2160 - parseEnrichment returns null on garbage
  ---
  duration_ms: 0.052666
  type: 'test'
  ...
# Subtest: mergeStages overlays enrichment by id and marks enriched
ok 2161 - mergeStages overlays enrichment by id and marks enriched
  ---
...
ok 2162 - §2.7.1 fact vs inference survives: provenance/extractionMethod/confidence pass through verbatim
  ---
  duration_ms: 1.993375
  type: 'test'
  ...
# Subtest: §2.7.2 negatives ≠ unknowns, and assessedInputs ≠ missingInputs — never merged, never dropped when empty
ok 2163 - §2.7.2 negatives ≠ unknowns, and assessedInputs ≠ missingInputs — never merged, never dropped when empty
  ---
  duration_ms: 1.619
  type: 'test'
  ...
# Subtest: §2.7.3 conflicts surface and are NEVER resolved, filtered or collapsed — including safety_critical
ok 2164 - §2.7.3 conflicts surface and are NEVER resolved, filtered or collapsed — including safety_critical
  ---
  duration_ms: 0.339125
  type: 'test'
  ...
# Subtest: §2.7.4 as_of comes from the snapshot's own field — never recomputed, only re-FORMATTED
ok 2165 - §2.7.4 as_of comes from the snapshot's own field — never recomputed, only re-FORMATTED
  ---
  duration_ms: 0.210083
  type: 'test'
  ...
# Subtest: §2.4 the LAST served observation wins, and a clean frontier run is not degraded
ok 2166 - §2.4 the LAST served observation wins, and a clean frontier run is not degraded
  ---
  duration_ms: 0.246792
  type: 'test'
  ...
# Subtest: §2.4 THE T-5 SCENARIO: a fallback leg ⇒ degraded, whatever the intent said
ok 2167 - §2.4 THE T-5 SCENARIO: a fallback leg ⇒ degraded, whatever the intent said
  ---
  duration_ms: 0.185917
  type: 'test'
  ...
# Subtest: §2.4 "we do not know" is DEGRADED — never the happy path
ok 2168 - §2.4 "we do not know" is DEGRADED — never the happy path
  ---
  duration_ms: 0.222708
  type: 'test'
  ...
# Subtest: §2.4 a partial assembly (a state leg failed) is degraded even when the model was clean
ok 2169 - §2.4 a partial assembly (a state leg failed) is degraded even when the model was clean
  ---
  duration_ms: 0.112625
  type: 'test'
  ...
# Subtest: §2.4 the wired reader takes provider/model from llm_response — NEVER llm_request (that is intent)
ok 2170 - §2.4 the wired reader takes provider/model from llm_response — NEVER llm_request (that is intent)
  ---
...
ok 2171 - §2.5 commercial is a SIBLING of clinical, never nested inside it, and carries its own definition
  ---
  duration_ms: 0.688041
  type: 'test'
  ...
# Subtest: §2.5 the commercial layer SHIPS — it is not omitted
ok 2172 - §2.5 the commercial layer SHIPS — it is not omitted
  ---
  duration_ms: 0.143958
  type: 'test'
  ...
# Subtest: §2.6 the disclaimer is rewritten for a physician pre-encounter and EMITTED in the JSON
ok 2173 - §2.6 the disclaimer is rewritten for a physician pre-encounter and EMITTED in the JSON
  ---
  duration_ms: 0.152041
  type: 'test'
  ...
# Subtest: §2.3 every namespace is present and the envelope carries its required fields
ok 2174 - §2.3 every namespace is present and the envelope carries its required fields
  ---
  duration_ms: 0.290375
  type: 'test'
  ...
# Subtest: §2.3 actions.follow_ups carries the snapshot's own followUps — never re-derived
ok 2175 - §2.3 actions.follow_ups carries the snapshot's own followUps — never re-derived
  ---
  duration_ms: 0.151625
  type: 'test'
  ...
# Subtest: §2.1 POST answers 202 with a job id and a poll url; the poll route returns 202/200/404
ok 2176 - §2.1 POST answers 202 with a job id and a poll url; the poll route returns 202/200/404
  ---
  duration_ms: 0.096916
  type: 'test'
  ...
# Subtest: §2.1 the 202 shape is documented as load-bearing — V2 precompute must not change the contract
ok 2177 - §2.1 the 202 shape is documented as load-bearing — V2 precompute must not change the contract
  ---
  duration_ms: 0.179917
  type: 'test'
  ...
# Subtest: §2.2 auth reuses CRON_SECRET and RECORDS that it is pilot-scoped and must be split
ok 2178 - §2.2 auth reuses CRON_SECRET and RECORDS that it is pilot-scoped and must be split
  ---
  duration_ms: 0.169083
  type: 'test'
  ...
# Subtest: the poll route tells Pulse it is REQUIRED to render a degraded package differently
ok 2179 - the poll route tells Pulse it is REQUIRED to render a degraded package differently
  ---
  duration_ms: 0.076875
  type: 'test'
  ...
# Subtest: job ids are well-formed and validated
ok 2180 - job ids are well-formed and validated
  ---
  duration_ms: 0.271709
  type: 'test'
  ...
# Subtest: §1.1 the CCB card and its live PHI count query are gone; OPD Audit Triage is untouched
ok 2181 - §1.1 the CCB card and its live PHI count query are gone; OPD Audit Triage is untouched
  ---
  duration_ms: 0.121
  type: 'test'
  ...
# Subtest: §1.1 /care/briefs stays REACHABLE — no gate was added (V overruled 404-ing it)
ok 2182 - §1.1 /care/briefs stays REACHABLE — no gate was added (V overruled 404-ing it)
  ---
  duration_ms: 0.321916
  type: 'test'
  ...
# Subtest: §1.3 every preserved-mechanics file carries the RETIRED header WITH the CCB_ENABLED hazard
ok 2183 - §1.3 every preserved-mechanics file carries the RETIRED header WITH the CCB_ENABLED hazard
  ---
  duration_ms: 1.128583
  type: 'test'
  ...
# Subtest: §2.3 the ExtractedReport[] sink is ADDITIVE — the envelope and every existing caller are untouched
ok 2184 - §2.3 the ExtractedReport[] sink is ADDITIVE — the envelope and every existing caller are untouched
  ---
  duration_ms: 0.210125
  type: 'test'
  ...
...
ok 2185 - ungrounded: citation_coverage_pct === 0 flips envelope.ungrounded — a zero-grounded package must not pass as well-grounded
  ---
  duration_ms: 0.142875
  type: 'test'
  ...
# Subtest: state_llm: rejected[] is surfaced in the envelope — the hallucination meter is not discarded
ok 2186 - state_llm: rejected[] is surfaced in the envelope — the hallucination meter is not discarded
  ---
...
ok 2187 - state_conflicts: a concept in BOTH positives and negatives is surfaced, normalised, never resolved
  ---
  duration_ms: 0.283541
  type: 'test'
  ...
# Subtest: a flag-on stage-2 failure is DEGRADED — the state shipped thinner than the default contract
ok 2188 - a flag-on stage-2 failure is DEGRADED — the state shipped thinner than the default contract
  ---
  duration_ms: 0.067458
  type: 'test'
  ...
# Subtest: the wired stage 2 is flag-gated DEFAULT ON, governed, and rides the brief trace
ok 2189 - the wired stage 2 is flag-gated DEFAULT ON, governed, and rides the brief trace
  ---
  duration_ms: 0.052
  type: 'test'
  ...
# Subtest: stage 2 caps its thinking — the Vertex form, gated on a resolved Gemini model, never zero
ok 2190 - stage 2 caps its thinking — the Vertex form, gated on a resolved Gemini model, never zero
  ---
  duration_ms: 0.075958
  type: 'test'
  ...
# Subtest: the audit budget is NOT changed by the stage-2 cap — separate constants, separate files
ok 2191 - the audit budget is NOT changed by the stage-2 cap — separate constants, separate files
  ---
  duration_ms: 0.265209
  type: 'test'
  ...
# Subtest: T-13 §5.1: the captured failure now reports degraded, naming the dead leg
ok 2192 - T-13 §5.1: the captured failure now reports degraded, naming the dead leg
  ---
  duration_ms: 0.065625
  type: 'test'
  ...
# Subtest: T-13 §5.2: a HEALTHY package still reports degraded:false — the check must not mark everything
ok 2193 - T-13 §5.2: a HEALTHY package still reports degraded:false — the check must not mark everything
  ---
  duration_ms: 0.040292
  type: 'test'
  ...
# Subtest: T-13 §5.4: each check fires ALONE, with the other disabled
ok 2194 - T-13 §5.4: each check fires ALONE, with the other disabled
  ---
...
ok 2195 - T-13: the content check needs BOTH conditions — a grounded package with 0 findings, or an ungrounded one with findings, is not "empty"
  ---
  duration_ms: 0.051125
  type: 'test'
  ...
# Subtest: T-13: failed legs are deduped and named in sorted order, and junk never crashes the reason
ok 2196 - T-13: failed legs are deduped and named in sorted order, and junk never crashes the reason
  ---
  duration_ms: 0.048791
  type: 'test'
  ...
# Subtest: T-13: the reasons compose — every independent cause is stated, none replaces another
ok 2197 - T-13: the reasons compose — every independent cause is stated, none replaces another
  ---
  duration_ms: 0.05175
  type: 'test'
  ...
# Subtest: T-13 §4: as_of is normalised to full ISO 8601 — a bare date keeps its calendar day
ok 2198 - T-13 §4: as_of is normalised to full ISO 8601 — a bare date keeps its calendar day
  ---
  duration_ms: 1.296208
  type: 'test'
  ...
# Subtest: T-13 §4: the snapshot itself is NOT recomputed — only the envelope is formatted
ok 2199 - T-13 §4: the snapshot itself is NOT recomputed — only the envelope is formatted
  ---
  duration_ms: 0.11925
  type: 'test'
  ...
# Subtest: T-13 §2: the envelope reads the failure signal that already existed on the trace
ok 2200 - T-13 §2: the envelope reads the failure signal that already existed on the trace
  ---
...
ok 2201 - THE DEFECT: a 200 with empty content THROWS instead of returning an empty string
  ---
  duration_ms: 32.681875
  type: 'test'
  ...
# Subtest: empty content is RETRYABLE on the EXISTING budget — only the final attempt throws
ok 2202 - empty content is RETRYABLE on the EXISTING budget — only the final attempt throws
  ---
  duration_ms: 1.035375
  type: 'test'
  ...
# Subtest: the try budget is NOT raised — still exactly 3
ok 2203 - the try budget is NOT raised — still exactly 3
  ---
  duration_ms: 0.076125
  type: 'test'
  ...
# Subtest: persistent empty content exhausts the budget and throws — no silent fallback
ok 2204 - persistent empty content exhausts the budget and throws — no silent fallback
  ---
  duration_ms: 0.479458
  type: 'test'
  ...
# Subtest: a NON-empty response still returns normally — the happy path is untouched
ok 2205 - a NON-empty response still returns normally — the happy path is untouched
  ---
  duration_ms: 0.531291
  type: 'test'
  ...
# Subtest: THE ERROR MESSAGE is verbatim per PRD §4 and carries the whole envelope
ok 2206 - THE ERROR MESSAGE is verbatim per PRD §4 and carries the whole envelope
  ---
  duration_ms: 0.080667
  type: 'test'
  ...
# Subtest: the message renders missing envelope fields as null rather than undefined or blank
ok 2207 - the message renders missing envelope fields as null rather than undefined or blank
  ---
  duration_ms: 0.20825
  type: 'test'
  ...
# Subtest: the thrown message actually reaches the caller with the envelope in it
ok 2208 - the thrown message actually reaches the caller with the envelope in it
  ---
  duration_ms: 2.228791
  type: 'test'
  ...
# Subtest: onEnvelope fires on EVERY attempt — success, empty content and HTTP failure alike
ok 2209 - onEnvelope fires on EVERY attempt — success, empty content and HTTP failure alike
  ---
  duration_ms: 0.966458
  type: 'test'
  ...
# Subtest: ENVELOPE CAPTURE NEVER THROWS — a broken callback cannot cost a real result
ok 2210 - ENVELOPE CAPTURE NEVER THROWS — a broken callback cannot cost a real result
  ---
  duration_ms: 1.498166
  type: 'test'
  ...
# Subtest: readLlmEnvelope is total — any shape yields a defined envelope, never a throw
ok 2211 - readLlmEnvelope is total — any shape yields a defined envelope, never a throw
  ---
  duration_ms: 0.779666
  type: 'test'
  ...
# Subtest: OPENROUTER_TIMEOUT_MS defaults to 110s and is env-overridable
ok 2212 - OPENROUTER_TIMEOUT_MS defaults to 110s and is env-overridable
  ---
  duration_ms: 0.389042
  type: 'test'
  ...
# Subtest: an AbortSignal is passed to the fetch, and the timer is always cleared
ok 2213 - an AbortSignal is passed to the fetch, and the timer is always cleared
  ---
  duration_ms: 0.304208
  type: 'test'
  ...
# Subtest: a timeout is a NORMAL RETRYABLE failure on the same bounded budget
ok 2214 - a timeout is a NORMAL RETRYABLE failure on the same bounded budget
  ---
  duration_ms: 0.537167
  type: 'test'
  ...
# Subtest: a persistent transport failure exhausts the budget and throws a named error
ok 2215 - a persistent transport failure exhausts the budget and throws a named error
  ---
  duration_ms: 0.539666
  type: 'test'
  ...
# Subtest: the transport catch still emits an envelope, so a hung attempt is visible
ok 2216 - the transport catch still emits an envelope, so a hung attempt is visible
  ---
  duration_ms: 0.341791
  type: 'test'
  ...
# Subtest: THE CATCH BLOCK rethrows ONLY for eval; the non-eval return is byte-identical
ok 2217 - THE CATCH BLOCK rethrows ONLY for eval; the non-eval return is byte-identical
  ---
  duration_ms: 0.093708
  type: 'test'
  ...
# Subtest: the production defaultGenerate params are byte-identical — no eval change leaked in
ok 2218 - the production defaultGenerate params are byte-identical — no eval change leaked in
  ---
  duration_ms: 0.194542
  type: 'test'
  ...
# Subtest: buildOpenRouterBody is BYTE-IDENTICAL — no max_tokens, no response_format (D3)
ok 2219 - buildOpenRouterBody is BYTE-IDENTICAL — no max_tokens, no response_format (D3)
  ---
  duration_ms: 0.52775
  type: 'test'
  ...
# Subtest: no engine version bump, and the retry predicate is unchanged
ok 2220 - no engine version bump, and the retry predicate is unchanged
  ---
  duration_ms: 11.032666
  type: 'test'
  ...
# Subtest: runMiniOpdToLab writes the envelope on success and cannot write one on failure
ok 2221 - runMiniOpdToLab writes the envelope on success and cannot write one on failure
  ---
  duration_ms: 0.282667
  type: 'test'
  ...
# Subtest: the lab-batch core is untouched — drainPlan and the locks still stand
ok 2222 - the lab-batch core is untouched — drainPlan and the locks still stand
  ---
  duration_ms: 0.116667
  type: 'test'
  ...
# Subtest: parses a full draft: caps, ranking, counts, version, disclaimer
ok 2223 - parses a full draft: caps, ranking, counts, version, disclaimer
  ---
  duration_ms: 3.018542
  type: 'test'
  ...
# Subtest: citation ids bounded by the SHARED source count
ok 2224 - citation ids bounded by the SHARED source count
  ---
  duration_ms: 6.375
  type: 'test'
  ...
# Subtest: unknown enums fall to safe defaults
ok 2225 - unknown enums fall to safe defaults
  ---
  duration_ms: 0.216833
  type: 'test'
  ...
# Subtest: caps: complications 8, safety-net 10
ok 2226 - caps: complications 8, safety-net 10
  ---
  duration_ms: 0.390958
  type: 'test'
  ...
# Subtest: malformed / empty inputs return null
ok 2227 - malformed / empty inputs return null
  ---
  duration_ms: 0.276542
  type: 'test'
  ...
# Subtest: summary fallback built when model omits it
ok 2228 - summary fallback built when model omits it
  ---
  duration_ms: 0.252542
  type: 'test'
  ...
# Subtest: buildPxUser: lens per doc type + documented plan block + empty-plan fallback
ok 2229 - buildPxUser: lens per doc type + documented plan block + empty-plan fallback
  ---
  duration_ms: 0.3865
  type: 'test'
  ...
# Subtest: parsePxCritique reads PX-specific keys; needs_revision inferred from non-empty arrays
ok 2230 - parsePxCritique reads PX-specific keys; needs_revision inferred from non-empty arrays
  ---
  duration_ms: 0.154
  type: 'test'
  ...
# Subtest: R5: offsetPrognosisCitations shifts every citation id by the analyze-source count
ok 2231 - R5: offsetPrognosisCitations shifts every citation id by the analyze-source count
  ---
  duration_ms: 0.279542
  type: 'test'
  ...
# Subtest: modifiers parsed with direction defaulting to raises; capped at 6
ok 2232 - modifiers parsed with direction defaulting to raises; capped at 6
  ---
  duration_ms: 0.462125
  type: 'test'
  ...
# Subtest: §7.1 hash stability: spacing and casing variants produce the SAME hash
ok 2233 - §7.1 hash stability: spacing and casing variants produce the SAME hash
  ---
  duration_ms: 0.712958
  type: 'test'
  ...
# Subtest: the hash is EXACTLY sha256(normalized) hex first 16 — the stored contract, pinned
ok 2234 - the hash is EXACTLY sha256(normalized) hex first 16 — the stored contract, pinned
  ---
  duration_ms: 0.202083
  type: 'test'
  ...
# Subtest: ADDENDUM A §1.2 — the ten cross-engine vectors, pinned with their literal hashes
ok 2235 - ADDENDUM A §1.2 — the ten cross-engine vectors, pinned with their literal hashes
  ---
  duration_ms: 0.247917
  type: 'test'
  ...
# Subtest: normalization is trim + lower-case + collapse internal whitespace, nothing more
ok 2236 - normalization is trim + lower-case + collapse internal whitespace, nothing more
  ---
  duration_ms: 0.114292
  type: 'test'
  ...
# Subtest: §7.2 re-audit resilience: the array reorders, the hash still finds the right complication
ok 2237 - §7.2 re-audit resilience: the array reorders, the hash still finds the right complication
  ---
  duration_ms: 0.479375
  type: 'test'
  ...
# Subtest: §7.3 engine bump: an outcome linked at engine A resolves against engine B when the name survived
ok 2238 - §7.3 engine bump: an outcome linked at engine A resolves against engine B when the name survived
  ---
  duration_ms: 0.058958
  type: 'test'
  ...
# Subtest: §7.3 engine bump: a renamed complication renders UNRESOLVED — never re-pointed by index
ok 2239 - §7.3 engine bump: a renamed complication renders UNRESOLVED — never re-pointed by index
  ---
  duration_ms: 0.173834
  type: 'test'
  ...
# Subtest: a NULL hash reads as unpredicted, and junk shapes never throw
ok 2240 - a NULL hash reads as unpredicted, and junk shapes never throw
  ---
  duration_ms: 0.1035
  type: 'test'
  ...
# Subtest: §7.5 each classification is produced by the correct form state
ok 2241 - §7.5 each classification is produced by the correct form state
  ---
  duration_ms: 0.294
  type: 'test'
  ...
# Subtest: §7.5 no_adverse_outcome FORCES a null complication hash, whatever the form held
ok 2242 - §7.5 no_adverse_outcome FORCES a null complication hash, whatever the form held
  ---
  duration_ms: 0.290708
  type: 'test'
  ...
# Subtest: the vocabularies are exactly the PRD’s
ok 2243 - the vocabularies are exactly the PRD’s
  ---
  duration_ms: 0.689959
  type: 'test'
  ...
# Subtest: §7.4 currentRows: the default view shows only non-superseded rows; history shows all
ok 2244 - §7.4 currentRows: the default view shows only non-superseded rows; history shows all
  ---
  duration_ms: 0.089708
  type: 'test'
  ...
# Subtest: §7.6 a document with no rows is not_followed_up and OUTSIDE the over-warning denominator
ok 2245 - §7.6 a document with no rows is not_followed_up and OUTSIDE the over-warning denominator
  ---
...
ok 2246 - §7.6 an event row alone follows the document up but does NOT admit it to the over-warning denominator
  ---
  duration_ms: 0.038708
  type: 'test'
  ...
# Subtest: §7.6 a no_adverse_outcome row admits the document; a superseded one does not
ok 2247 - §7.6 a no_adverse_outcome row admits the document; a superseded one does not
  ---
  duration_ms: 0.034792
  type: 'test'
  ...
# Subtest: §7.6 no_adverse alongside an event row: followed up, in the denominator, both persist
ok 2248 - §7.6 no_adverse alongside an event row: followed up, in the denominator, both persist
  ---
  duration_ms: 0.0345
  type: 'test'
  ...
# Subtest: §7.7 idempotent migration: every statement is IF NOT EXISTS — running it twice is a no-op
ok 2249 - §7.7 idempotent migration: every statement is IF NOT EXISTS — running it twice is a no-op
  ---
  duration_ms: 0.222334
  type: 'test'
  ...
# Subtest: P-7 in the store: supersede is ONE atomic statement — flag-flip CTE + insert, no content UPDATE, no DELETE
ok 2250 - P-7 in the store: supersede is ONE atomic statement — flag-flip CTE + insert, no content UPDATE, no DELETE
  ---
...
ok 2251 - §7.6 the view emits the not_followed_up bucket, and the over-warning columns go NULL outside it
  ---
  duration_ms: 0.061708
  type: 'test'
  ...
# Subtest: the view reads only non-superseded rows and resolves by the SAME hash as the core
ok 2252 - the view reads only non-superseded rows and resolves by the SAME hash as the core
  ---
  duration_ms: 0.06175
  type: 'test'
  ...
# Subtest: the migrate route creates the table BEFORE the view, mirroring migrations/0033 exactly
ok 2253 - the migrate route creates the table BEFORE the view, mirroring migrations/0033 exactly
  ---
  duration_ms: 0.052375
  type: 'test'
  ...
# Subtest: P-8: the table and the view pass the SQL guard, and lib/sql-guard-core.ts is untouched
ok 2254 - P-8: the table and the view pass the SQL guard, and lib/sql-guard-core.ts is untouched
  ---
...
ok 2255 - A-2: horizon_days is DERIVED in SQL against the canonical discharged_at — never typed, never audited_at
  ---
  duration_ms: 0.058958
  type: 'test'
  ...
...
ok 2256 - SQL honesty: reads degrade to unavailable and writes to a refusal — never a throw (no DB in this sandbox)
  ---
  duration_ms: 27.614916
  type: 'test'
  ...
# Subtest: promotion threshold is 5
ok 2257 - promotion threshold is 5
  ---
  duration_ms: 0.487125
  type: 'test'
  ...
# Subtest: a selection recurring ≥ threshold → promotion candidate
ok 2258 - a selection recurring ≥ threshold → promotion candidate
  ---
  duration_ms: 0.60375
  type: 'test'
  ...
# Subtest: below threshold → collecting
ok 2259 - below threshold → collecting
  ---
  duration_ms: 0.090958
  type: 'test'
  ...
# Subtest: threshold override is honoured
ok 2260 - threshold override is honoured
  ---
  duration_ms: 0.0675
  type: 'test'
  ...
# Subtest: dominant selection wins over a minority variant; edited count tracks divergence from generated
ok 2261 - dominant selection wins over a minority variant; edited count tracks divergence from generated
  ---
  duration_ms: 0.226458
  type: 'test'
  ...
# Subtest: selection order and duplicates do not split a recurring set
ok 2262 - selection order and duplicates do not split a recurring set
  ---
  duration_ms: 0.073792
  type: 'test'
  ...
# Subtest: records with no procedure context are skipped
ok 2263 - records with no procedure context are skipped
  ---
  duration_ms: 0.105667
  type: 'test'
  ...
# Subtest: procedure grouping is case/whitespace-insensitive
ok 2264 - procedure grouping is case/whitespace-insensitive
  ---
  duration_ms: 0.058125
  type: 'test'
  ...
# Subtest: candidates sort before collecting; ties by recurrence desc
ok 2265 - candidates sort before collecting; ties by recurrence desc
  ---
  duration_ms: 0.242917
  type: 'test'
  ...
# Subtest: grouping is deterministic (twice → deep-equal)
ok 2266 - grouping is deterministic (twice → deep-equal)
  ---
  duration_ms: 0.416083
  type: 'test'
  ...
# Subtest: empty input → empty queue
ok 2267 - empty input → empty queue
  ---
  duration_ms: 0.090083
  type: 'test'
  ...
# Subtest: suggestSetName maps a procedure to hs-<word>
ok 2268 - suggestSetName maps a procedure to hs-<word>
  ---
  duration_ms: 0.104
  type: 'test'
  ...
# Subtest: a dominant selection of real bank ids survives validateAdhocSelection
ok 2269 - a dominant selection of real bank ids survives validateAdhocSelection
  ---
  duration_ms: 0.275292
  type: 'test'
  ...
# Subtest: promResponsesToEncounter: scored instruments → one dated care_call encounter with investigation points
ok 2270 - promResponsesToEncounter: scored instruments → one dated care_call encounter with investigation points
  ---
  duration_ms: 0.494917
  type: 'test'
  ...
# Subtest: promResponsesToEncounter: unscored (null) instruments are dropped from the fold
ok 2271 - promResponsesToEncounter: unscored (null) instruments are dropped from the fold
  ---
  duration_ms: 0.077875
  type: 'test'
  ...
# Subtest: promResponsesToEncounter: empty / all-null → empty investigations, no date (fold filters it)
ok 2272 - promResponsesToEncounter: empty / all-null → empty investigations, no date (fold filters it)
  ---
  duration_ms: 0.059958
  type: 'test'
  ...
# Subtest: promResponsesToEncounter: deterministic (twice → deep-equal)
ok 2273 - promResponsesToEncounter: deterministic (twice → deep-equal)
  ---
...
ok 2364 - S1 (0.81.10): muscle_relaxant_indication → deterministic_completeness (documentation prompt, same class as incomplete_dosing)
  ---
  duration_ms: 0.07475
  type: 'test'
  ...
# Subtest: 0.81.14: vitamin_d_repletion_duration + pregnancy_risk_verify → deterministic_completeness (documentation prompts)
ok 2365 - 0.81.14: vitamin_d_repletion_duration + pregnancy_risk_verify → deterministic_completeness (documentation prompts)
  ---
  duration_ms: 0.0315
  type: 'test'
  ...
# Subtest: V1/V2: incomplete_dosing → deterministic_completeness; duplicate_* → deterministic_logical
ok 2366 - V1/V2: incomplete_dosing → deterministic_completeness; duplicate_* → deterministic_logical
  ---
...
ok 2476 - a value drawn AFTER discharge cannot be an omission — the discharge decision could not have known it
  ---
  duration_ms: 0.054291
  type: 'test'
  ...
# Subtest: the derived audit runs ONLY on an explicitly stated tier 1, never on an inferred one
ok 2477 - the derived audit runs ONLY on an explicitly stated tier 1, never on an inferred one
  ---
  duration_ms: 0.075708
  type: 'test'
  ...
# Subtest: stability claims are the discharge-condition kind, not any use of the word
ok 2478 - stability claims are the discharge-condition kind, not any use of the word
  ---
  duration_ms: 0.16725
  type: 'test'
  ...
# Subtest: tier 2 caps an omission at moderate — a summary-vs-summary contradiction is never high-confidence
ok 2479 - tier 2 caps an omission at moderate — a summary-vs-summary contradiction is never high-confidence
  ---
  duration_ms: 0.352542
  type: 'test'
  ...
# Subtest: tier 3 emits no omissions at all and records the refusal
ok 2480 - tier 3 emits no omissions at all and records the refusal
  ---
  duration_ms: 0.155667
  type: 'test'
  ...
# Subtest: only a STRUCTURED value can corroborate a stability claim
ok 2481 - only a STRUCTURED value can corroborate a stability claim
  ---
  duration_ms: 0.143541
  type: 'test'
  ...
# Subtest: the range is a JSON OBJECT: bounds come from .l/.h numerically
ok 2482 - the range is a JSON OBJECT: bounds come from .l/.h numerically
  ---
  duration_ms: 0.310542
  type: 'test'
  ...
# Subtest: an UNPARSEABLE range yields no numeric flag — never a guessed one
ok 2483 - an UNPARSEABLE range yields no numeric flag — never a guessed one
  ---
  duration_ms: 0.1985
  type: 'test'
  ...
# Subtest: an abnormal value against the live object range flags; an in-range one does not
ok 2484 - an abnormal value against the live object range flags; an in-range one does not
  ---
...
ok 2485 - a value whose range will not parse produces NO derived omission, even under an explicit tier 1
  ---
  duration_ms: 0.086917
  type: 'test'
  ...
# Subtest: refRangeDisplay prefers the lab's own wording over our reconstruction
ok 2486 - refRangeDisplay prefers the lab's own wording over our reconstruction
  ---
  duration_ms: 0.068041
  type: 'test'
  ...
# Subtest: the analyte-name matcher handles the real db13 names (LOINC is absent, so this is the primary path)
ok 2487 - the analyte-name matcher handles the real db13 names (LOINC is absent, so this is the primary path)
  ---
  duration_ms: 0.516
  type: 'test'
  ...
# Subtest: with loinc_id absent the NAME decides; the code is only the fallback
ok 2488 - with loinc_id absent the NAME decides; the code is only the fallback
  ---
  duration_ms: 0.056
  type: 'test'
  ...
# Subtest: the LOINC table still resolves where a code exists — kept as the fallback, not the primary path
ok 2489 - the LOINC table still resolves where a code exists — kept as the fallback, not the primary path
  ---
...
ok 2490 - a renamed diagnosis cannot move the organ bundle: same failing organ both sides → SAME condition
  ---
...
ok 2491 - a derived omission and the model's version of the same one collapse to one row, derived winning
  ---
  duration_ms: 0.16275
  type: 'test'
  ...
# Subtest: the tier and its provenance ride the finding for the reviewer
ok 2492 - the tier and its provenance ride the finding for the reviewer
  ---
  duration_ms: 0.14675
  type: 'test'
  ...
# Subtest: lanes render clearest-first, and an UNKNOWN lane never hides in the collapsed block
ok 2493 - lanes render clearest-first, and an UNKNOWN lane never hides in the collapsed block
  ---
  duration_ms: 2.292625
  type: 'test'
  ...
# Subtest: within a lane, needs_human_review comes first and then the most recent readmission
ok 2494 - within a lane, needs_human_review comes first and then the most recent readmission
  ---
  duration_ms: 0.270208
  type: 'test'
  ...
# Subtest: the review count is audited AND (avoidable | needs_adjudication) — nothing else
ok 2495 - the review count is audited AND (avoidable | needs_adjudication) — nothing else
  ---
  duration_ms: 0.101584
  type: 'test'
  ...
# Subtest: out-of-network never shows an avoidable verdict, and not_auditable says why
ok 2496 - out-of-network never shows an avoidable verdict, and not_auditable says why
  ---
  duration_ms: 0.079291
  type: 'test'
  ...
# Subtest: an excluded row says "Held out", never lane-D’s "No verdict"
ok 2497 - an excluded row says "Held out", never lane-D’s "No verdict"
  ---
  duration_ms: 0.139
  type: 'test'
  ...
# Subtest: the held-out sample groups last and collapsed, with the audited lanes untouched
ok 2498 - the held-out sample groups last and collapsed, with the audited lanes untouched
  ---
  duration_ms: 0.152625
  type: 'test'
  ...
# Subtest: tiles are blind to excluded rows — the route filters, and the filter is the contract
ok 2499 - tiles are blind to excluded rows — the route filters, and the filter is the contract
  ---
  duration_ms: 0.145875
  type: 'test'
  ...
# Subtest: a badge is omitted rather than guessed — unknown planned, ambiguous department, absent tier
ok 2500 - a badge is omitted rather than guessed — unknown planned, ambiguous department, absent tier
  ---
  duration_ms: 0.868292
  type: 'test'
  ...
# Subtest: the verdict chip never borrows another verdict’s confidence
ok 2501 - the verdict chip never borrows another verdict’s confidence
  ---
  duration_ms: 2.435667
  type: 'test'
  ...
# Subtest: the 30-day rate is null without a real denominator — never a rate over a guess
ok 2502 - the 30-day rate is null without a real denominator — never a rate over a guess
  ---
  duration_ms: 0.758917
  type: 'test'
  ...
# Subtest: a failed name join degrades to the UHID, never a blank card
ok 2503 - a failed name join degrades to the UHID, never a blank card
  ---
  duration_ms: 1.20525
  type: 'test'
  ...
# Subtest: every promptRef tag across all tagged files resolves to a real registry id
ok 2504 - every promptRef tag across all tagged files resolves to a real registry id
  ---
  duration_ms: 1.995375
  type: 'test'
  ...
# Subtest: governedChat is exact delegation (transport-equivalence pin)
ok 2505 - governedChat is exact delegation (transport-equivalence pin)
  ---
  duration_ms: 0.67875
  type: 'test'
  ...
# Subtest: governance config sanity: four call patterns, three governed files, fold declared
ok 2506 - governance config sanity: four call patterns, three governed files, fold declared
  ---
  duration_ms: 7.628334
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: promptFingerprint resolves from the committed registry; unknown id → null, never a throw
ok 2507 - promptFingerprint resolves from the committed registry; unknown id → null, never a throw
  ---
  duration_ms: 2.601583
  type: 'test'
  ...
# Subtest: buildEnvelope: promptRef set → fingerprint columns; unset → call facts only
ok 2508 - buildEnvelope: promptRef set → fingerprint columns; unset → call facts only
  ---
  duration_ms: 0.784083
  type: 'test'
  ...
# Subtest: ENVELOPE_UPDATE_SQL writes exactly the ten normative columns
ok 2509 - ENVELOPE_UPDATE_SQL writes exactly the ten normative columns
  ---
  duration_ms: 0.089042
  type: 'test'
  ...
# Subtest: withTrace finalizes exactly once — on success AND on throw
ok 2510 - withTrace finalizes exactly once — on success AND on throw
  ---
  duration_ms: 0.394292
  type: 'test'
  ...
# Subtest: migration 0012 is additive + idempotent and covers every normative column
ok 2511 - migration 0012 is additive + idempotent and covers every normative column
  ---
  duration_ms: 0.436667
  type: 'test'
  ...
# Subtest: governance gate (Stage 4): the repo scan is CLEAN; synthetic direct calls are flagged
ok 2512 - governance gate (Stage 4): the repo scan is CLEAN; synthetic direct calls are flagged
  ---
  duration_ms: 226.226375
  type: 'test'
  ...
# Subtest: every Right Care promptRef tag resolves to a REAL registry id
ok 2513 - every Right Care promptRef tag resolves to a REAL registry id
  ---
  duration_ms: 0.940291
  type: 'test'
  ...
# Subtest: countNonEnumVerdicts (A04) counts exactly the out-of-enum verdicts
ok 2514 - countNonEnumVerdicts (A04) counts exactly the out-of-enum verdicts
  ---
  duration_ms: 0.508833
  type: 'test'
  ...
# Subtest: outcomeForPrompt maps the committed scorecard to the right prompt version/hash
ok 2515 - outcomeForPrompt maps the committed scorecard to the right prompt version/hash
  ---
  duration_ms: 0.566541
  type: 'test'
  ...
# Subtest: RECOMPUTE: arm stats re-derive from the raw runs via the harness scorer — no drift
ok 2516 - RECOMPUTE: arm stats re-derive from the raw runs via the harness scorer — no drift
  ---
  duration_ms: 0.631459
  type: 'test'
  ...
# Subtest: maturity gate: mature requires a cleared gold; the LIVE manifests pass (CI assertion)
ok 2517 - maturity gate: mature requires a cleared gold; the LIVE manifests pass (CI assertion)
  ---
  duration_ms: 0.543667
  type: 'test'
  ...
# Subtest: provenance rider: cwus-ahaacchrs-001 labels as guideline-derived
ok 2518 - provenance rider: cwus-ahaacchrs-001 labels as guideline-derived
  ---
  duration_ms: 0.22625
  type: 'test'
  ...
# Subtest: determinism + evidence currency
ok 2519 - determinism + evidence currency
  ---
  duration_ms: 0.287542
  type: 'test'
  ...
# Subtest: registry generation is deterministic and the committed artifact is current
ok 2520 - registry generation is deterministic and the committed artifact is current
  ---
  duration_ms: 231.408958
  type: 'test'
  ...
# Subtest: every extracted prompt has non-empty text and a valid sha256 of exactly that text
ok 2521 - every extracted prompt has non-empty text and a valid sha256 of exactly that text
  ---
  duration_ms: 1.154166
  type: 'test'
  ...
# Subtest: the research export contains prompt/rubric/metadata keys ONLY — no clinical/patient/trace content
ok 2522 - the research export contains prompt/rubric/metadata keys ONLY — no clinical/patient/trace content
  ---
  duration_ms: 2.528375
  type: 'test'
  ...
# Subtest: manifest merge: registered id gets its metadata; unknown id → unregistered, never a throw
ok 2523 - manifest merge: registered id gets its metadata; unknown id → unregistered, never a throw
  ---
  duration_ms: 1.793125
  type: 'test'
  ...
# Subtest: rubric inclusion: nabh/6e external-json + the five embedded-in-prompt rubrics
ok 2524 - rubric inclusion: nabh/6e external-json + the five embedded-in-prompt rubrics
  ---
...
ok 2525 - count invariant: counts match the committed artifact contents (30 prompts / 7 rubrics / 32 builders)
  ---
  duration_ms: 0.115458
  type: 'test'
  ...
# Subtest: registryTabRows maps generated + manifest correctly
ok 2526 - registryTabRows maps generated + manifest correctly
  ---
  duration_ms: 0.9065
  type: 'test'
  ...
# Subtest: groupPromptVersionCost sums the 4th breakdown correctly
ok 2527 - groupPromptVersionCost sums the 4th breakdown correctly
  ---
  duration_ms: 0.157708
  type: 'test'
  ...
# Subtest: fingerprint + rollup tolerate NULL columns (pre-Stage-1 rows) without throwing
ok 2528 - fingerprint + rollup tolerate NULL columns (pre-Stage-1 rows) without throwing
  ---
  duration_ms: 18.015833
  type: 'test'
  ...
# Subtest: PHI-safety: new views surface registry/envelope fields only
ok 2529 - PHI-safety: new views surface registry/envelope fields only
  ---
  duration_ms: 2.339583
  type: 'test'
  ...
# Subtest: shortVersion / shortPromptRef formatters
ok 2530 - shortVersion / shortPromptRef formatters
  ---
  duration_ms: 0.097709
  type: 'test'
  ...
# Subtest: promptVersionChanges detects a rollout inside the watch window
ok 2531 - promptVersionChanges detects a rollout inside the watch window
  ---
  duration_ms: 0.198375
  type: 'test'
  ...
# Subtest: GOVERNANCE_SNAPSHOT matches the live scan — the coverage panel cannot rot
ok 2532 - GOVERNANCE_SNAPSHOT matches the live scan — the coverage panel cannot rot
  ---
  duration_ms: 237.776125
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: 55 — every failure-phase to reconciler-state mapping, all four rows of D13's table
ok 2533 - 55 — every failure-phase to reconciler-state mapping, all four rows of D13's table
  ---
  duration_ms: 0.658959
  type: 'test'
  ...
# Subtest: 55 — the selection is bounded, non-terminal only, and oldest first
ok 2534 - 55 — the selection is bounded, non-terminal only, and oldest first
  ---
  duration_ms: 3.194292
  type: 'test'
  ...
# Subtest: 55 — the update is a compare-and-set on the expected revision, and cannot move a terminal row
ok 2535 - 55 — the update is a compare-and-set on the expected revision, and cannot move a terminal row
  ---
  duration_ms: 0.156125
  type: 'test'
  ...
# Subtest: 55 — a revision mismatch causes ONE reread and reclassification, never a blind retry
ok 2536 - 55 — a revision mismatch causes ONE reread and reclassification, never a blind retry
  ---
  duration_ms: 0.104333
  type: 'test'
  ...
# Subtest: 55 — every state the reconciler can assign is a legal transition from where it assigns it
ok 2537 - 55 — every state the reconciler can assign is a legal transition from where it assigns it
  ---
  duration_ms: 0.100792
  type: 'test'
  ...
# Subtest: 55 — the reconciler owns an invocation of its own kind, and closes it
ok 2538 - 55 — the reconciler owns an invocation of its own kind, and closes it
  ---
  duration_ms: 0.210708
  type: 'test'
  ...
# Subtest: 55 runtime — an ordinary one-row pass: one pinned write, no reread
ok 2539 - 55 runtime — an ordinary one-row pass: one pinned write, no reread
  ---
  duration_ms: 10.473291
  type: 'test'
  ...
# Subtest: 55 runtime — the statement the route ACTUALLY SENT is the pinned one, complete
ok 2540 - 55 runtime — the statement the route ACTUALLY SENT is the pinned one, complete
  ---
  duration_ms: 2.577875
  type: 'test'
  ...
# Subtest: 55 runtime — each row's write binds ITS OWN revision
ok 2541 - 55 runtime — each row's write binds ITS OWN revision
  ---
  duration_ms: 1.249583
  type: 'test'
  ...
# Subtest: 55 runtime — THE STALE DECISION DOES NOT LAND: reread, reclassify, write the FRESH state
ok 2542 - 55 runtime — THE STALE DECISION DOES NOT LAND: reread, reclassify, write the FRESH state
  ---
  duration_ms: 0.934208
  type: 'test'
  ...
# Subtest: 55 runtime — a TERMINAL row wins, and no second write is issued
ok 2543 - 55 runtime — a TERMINAL row wins, and no second write is issued
  ---
  duration_ms: 0.821167
  type: 'test'
  ...
# Subtest: 55 runtime — the CUTOFF is the request time minus the preregistered grace
ok 2544 - 55 runtime — the CUTOFF is the request time minus the preregistered grace
  ---
  duration_ms: 0.601875
  type: 'test'
  ...
# Subtest: 55 runtime — a FORBIDDEN transition is refused, and nothing is written
ok 2545 - 55 runtime — a FORBIDDEN transition is refused, and nothing is written
  ---
  duration_ms: 0.369792
  type: 'test'
  ...
# Subtest: the stub fails CLOSED on every body it does not model
ok 2546 - the stub fails CLOSED on every body it does not model
  ---
  duration_ms: 1.399458
  type: 'test'
  ...
# Subtest: 55 behaviour — the SELECT sent at run time is the complete pinned selection
ok 2547 - 55 behaviour — the SELECT sent at run time is the complete pinned selection
  ---
  duration_ms: 1.4225
  type: 'test'
  ...
# Subtest: 55 behaviour — the first write binds the revision the SELECTION returned
ok 2548 - 55 behaviour — the first write binds the revision the SELECTION returned
  ---
  duration_ms: 3.827083
  type: 'test'
  ...
# Subtest: 55 behaviour — a transport error on the write is a 500, never a fabricated verdict
ok 2549 - 55 behaviour — a transport error on the write is a 500, never a fabricated verdict
  ---
  duration_ms: 2.997709
  type: 'test'
  ...
# Subtest: 55 behaviour — a SECOND conflict on the reread path stops after two writes and one reread
ok 2550 - 55 behaviour — a SECOND conflict on the reread path stops after two writes and one reread
  ---
  duration_ms: 0.621875
  type: 'test'
  ...
# Subtest: 55 summary — a slice of TERMINAL rows tallies no reconciliations at all
ok 2551 - 55 summary — a slice of TERMINAL rows tallies no reconciliations at all
  ---
  duration_ms: 0.805042
  type: 'test'
  ...
# Subtest: 55 summary — more_may_remain is TRUE on a full slice and FALSE on a short one
ok 2552 - 55 summary — more_may_remain is TRUE on a full slice and FALSE on a short one
  ---
  duration_ms: 0.776167
  type: 'test'
  ...
# Subtest: 55 behaviour — an unauthenticated request is 401 and touches the database not at all
ok 2553 - 55 behaviour — an unauthenticated request is 401 and touches the database not at all
  ---
  duration_ms: 0.504666
  type: 'test'
  ...
# Subtest: 58 — WORKER_MAX_DURATION_SECONDS equals the worker route's own maxDuration literal
ok 2554 - 58 — WORKER_MAX_DURATION_SECONDS equals the worker route's own maxDuration literal
  ---
  duration_ms: 0.3505
  type: 'test'
  ...
# Subtest: 59 — the reconciler fires at 10:01 UTC, outside every hour the OPD worker runs
ok 2555 - 59 — the reconciler fires at 10:01 UTC, outside every hour the OPD worker runs
  ---
  duration_ms: 0.3235
  type: 'test'
  ...
# Subtest: 64 — both files assert 17, and neither still asserts 16
ok 2556 - 64 — both files assert 17, and neither still asserts 16
  ---
  duration_ms: 0.340792
  type: 'test'
  ...
# Subtest: 64 — undo the one authorised line and each file hashes to exactly what it did at 177adc9
ok 2557 - 64 — undo the one authorised line and each file hashes to exactly what it did at 177adc9
  ---
  duration_ms: 0.532417
  type: 'test'
  ...
# Subtest: 64 — provider-switch-unit-d's sql-guard assertion is untouched, and is nowhere near line 270
ok 2558 - 64 — provider-switch-unit-d's sql-guard assertion is untouched, and is nowhere near line 270
  ---
  duration_ms: 0.137625
  type: 'test'
  ...
# Subtest: artifact — THE ROUTE HAS NOT RUN IN THIS PROCESS
ok 2559 - artifact — THE ROUTE HAS NOT RUN IN THIS PROCESS
  ---
  duration_ms: 0.697125
  type: 'test'
  ...
# Subtest: artifact — the reconciler route is byte-for-byte the reviewed file
ok 2560 - artifact — the reconciler route is byte-for-byte the reviewed file
  ---
  duration_ms: 0.234292
  type: 'test'
  ...
# Subtest: artifact — what this pin does NOT cover
ok 2561 - artifact — what this pin does NOT cover
  ---
...
ok 2562 - v7 §8 — a rejected terminal write now leaves DURABLE evidence, not just a console.warn
  ---
  duration_ms: 4.593833
  type: 'test'
  ...
# Subtest: v7 §8 — the reread distinguishes an already-terminal row from a moved revision
ok 2563 - v7 §8 — the reread distinguishes an already-terminal row from a moved revision
  ---
  duration_ms: 0.897958
  type: 'test'
  ...
# Subtest: v7 §8 — an existing terminal row is NEVER downgraded, and that is structural
ok 2564 - v7 §8 — an existing terminal row is NEVER downgraded, and that is structural
  ---
  duration_ms: 0.303709
  type: 'test'
  ...
# Subtest: v7 §8 — the handle is returned unadvanced, and nothing is retried
ok 2565 - v7 §8 — the handle is returned unadvanced, and nothing is retried
  ---
  duration_ms: 2.163166
  type: 'test'
  ...
# Subtest: v7 §8 — a failed reread still records the evidence, because the reread is diagnostic
ok 2566 - v7 §8 — a failed reread still records the evidence, because the reread is diagnostic
  ---
...
ok 2567 - v7 §8 — the new phase is run-scoped and in the vocabulary, and the reconciler deliberately ignores it
  ---
  duration_ms: 0.218875
  type: 'test'
  ...
# Subtest: v7 §8 — the generated CHECK and the mirrored .sql agree on the new phase
ok 2568 - v7 §8 — the generated CHECK and the mirrored .sql agree on the new phase
  ---
  duration_ms: 0.560375
  type: 'test'
  ...
# Subtest: v7 §7 — the field is present-and-null when there is no active run, and that validates clean
ok 2569 - v7 §7 — the field is present-and-null when there is no active run, and that validates clean
  ---
  duration_ms: 0.766084
  type: 'test'
  ...
# Subtest: v7 §7 — an ABSENT field is a defect, which is what makes the null a claim rather than a gap
ok 2570 - v7 §7 — an ABSENT field is a defect, which is what makes the null a claim rather than a gap
  ---
  duration_ms: 0.535375
  type: 'test'
  ...
# Subtest: v7 §7 — the definition is recorded, and BackfillRun still has no `target` field
ok 2571 - v7 §7 — the definition is recorded, and BackfillRun still has no `target` field
  ---
  duration_ms: 3.042708
  type: 'test'
  ...
# Subtest: v7 §7 — the writer maps no-active-run to null on all three fields
ok 2572 - v7 §7 — the writer maps no-active-run to null on all three fields
  ---
  duration_ms: 0.434
  type: 'test'
  ...
...
ok 2573 - resolveRerankBackend: explicit override wins, else env default; only judge|cohere
  ---
  duration_ms: 0.562459
  type: 'test'
  ...
# Subtest: no backend arg routes to the env default (judge in the test env), not cohere
ok 2574 - no backend arg routes to the env default (judge in the test env), not cohere
  ---
  duration_ms: 0.200333
  type: 'test'
  ...
# Subtest: assertRerankBackendHealthy passes when rel=0.8, irr=0.02
ok 2575 - assertRerankBackendHealthy passes when rel=0.8, irr=0.02
  ---
  duration_ms: 1.512
  type: 'test'
  ...
# Subtest: probe fails RerankBackendUnhealthy when the margin < MIN_MARGIN (rel=0.5, irr=0.45)
ok 2576 - probe fails RerankBackendUnhealthy when the margin < MIN_MARGIN (rel=0.5, irr=0.45)
  ---
  duration_ms: 0.37775
  type: 'test'
  ...
# Subtest: probe fails Unhealthy when the backend returns CONSTANT scores (no discrimination)
ok 2577 - probe fails Unhealthy when the backend returns CONSTANT scores (no discrimination)
  ---
  duration_ms: 0.253375
  type: 'test'
  ...
# Subtest: probe fails RerankBackendUnreachable on fetch throw, non-200, or missing key
ok 2578 - probe fails RerankBackendUnreachable on fetch throw, non-200, or missing key
  ---
  duration_ms: 0.253584
  type: 'test'
  ...
# Subtest: probe is memoized within the TTL — two calls ⇒ one fetch
ok 2579 - probe is memoized within the TTL — two calls ⇒ one fetch
  ---
  duration_ms: 0.278084
  type: 'test'
  ...
# Subtest: rerankCohere maps index→candidate, uses relevance_score directly (no sigmoid), tags cohere
ok 2580 - rerankCohere maps index→candidate, uses relevance_score directly (no sigmoid), tags cohere
  ---
...
ok 2581 - explicit cohere runs the health probe BEFORE scoring, and a probe failure propagates (not swallowed)
  ---
  duration_ms: 0.571458
  type: 'test'
  ...
# Subtest: a TRANSIENT (generic, non-typed) failure still soft-falls to input order
ok 2582 - a TRANSIENT (generic, non-typed) failure still soft-falls to input order
  ---
  duration_ms: 0.454667
  type: 'test'
  ...
# Subtest: D2.1 env-default cohere: a typed cohere failure ⇒ falls back to JUDGE (tier 1), never throws
ok 2583 - D2.1 env-default cohere: a typed cohere failure ⇒ falls back to JUDGE (tier 1), never throws
  ---
  duration_ms: 0.163125
  type: 'test'
  ...
# Subtest: D2.2 env-default cohere: cohere AND judge both throw ⇒ INPUT ORDER (none), never throws
ok 2584 - D2.2 env-default cohere: cohere AND judge both throw ⇒ INPUT ORDER (none), never throws
  ---
  duration_ms: 0.123625
  type: 'test'
  ...
# Subtest: D2.3 EXPLICIT cohere: a typed cohere failure PROPAGATES (strict — NO fallback to judge)
ok 2585 - D2.3 EXPLICIT cohere: a typed cohere failure PROPAGATES (strict — NO fallback to judge)
  ---
  duration_ms: 0.075334
  type: 'test'
  ...
# Subtest: D2.4 env-default cohere HEALTHY ⇒ cohere scores; probe invoked once (memoization proven in §5.5)
ok 2586 - D2.4 env-default cohere HEALTHY ⇒ cohere scores; probe invoked once (memoization proven in §5.5)
  ---
  duration_ms: 0.060375
  type: 'test'
  ...
# Subtest: D3 a successful cohere rerank records ONE cost entry carrying the response usage.cost
ok 2587 - D3 a successful cohere rerank records ONE cost entry carrying the response usage.cost
  ---
  duration_ms: 0.455208
  type: 'test'
  ...
# Subtest: the rerank module no longer contains any bge symbol
ok 2588 - the rerank module no longer contains any bge symbol
  ---
  duration_ms: 0.296291
  type: 'test'
  ...
# Subtest: discrimination thresholds default to 0.40 / 0.15
ok 2589 - discrimination thresholds default to 0.40 / 0.15
  ---
  duration_ms: 0.040959
  type: 'test'
  ...
# Subtest: §5.1 the LIVE production case: RERANK_BACKEND=Cohere resolves to judge AND warns
ok 2590 - §5.1 the LIVE production case: RERANK_BACKEND=Cohere resolves to judge AND warns
  ---
  duration_ms: 0.045
  type: 'test'
  ...
# Subtest: §5.1b …and the warning actually FIRES at real module load (cold-start proof, subprocess)
ok 2591 - §5.1b …and the warning actually FIRES at real module load (cold-start proof, subprocess)
  ---
  duration_ms: 246.108541
  type: 'test'
  ...
# Subtest: §5.2 exact lowercase cohere (whitespace-trimmed) selects cohere silently; COHERE warns to judge
ok 2592 - §5.2 exact lowercase cohere (whitespace-trimmed) selects cohere silently; COHERE warns to judge
  ---
  duration_ms: 0.321
  type: 'test'
  ...
# Subtest: §5.3 judge, trimmed judge and unset are silent; any other value warns to judge
ok 2593 - §5.3 judge, trimmed judge and unset are silent; any other value warns to judge
  ---
  duration_ms: 0.484291
  type: 'test'
  ...
# Subtest: §5.4 miniPipeline normalizes: Mini and " mini " both select the mini pipeline
ok 2594 - §5.4 miniPipeline normalizes: Mini and " mini " both select the mini pipeline
  ---
  duration_ms: 0.111625
  type: 'test'
  ...
# Subtest: §5.5 INVARIANCE: no rerankBackend ⇒ retrieve options deep-equal to today, no extra key
ok 2595 - §5.5 INVARIANCE: no rerankBackend ⇒ retrieve options deep-equal to today, no extra key
  ---
  duration_ms: 0.172416
  type: 'test'
  ...
# Subtest: §5.6 rerankBackend:cohere reaches retrieve() — carried in the opts and threaded at the call sites
ok 2596 - §5.6 rerankBackend:cohere reaches retrieve() — carried in the opts and threaded at the call sites
  ---
  duration_ms: 0.565125
  type: 'test'
  ...
# Subtest: §5.7 explicit cohere via the threaded path stays STRICT — typed errors propagate, no fallback
ok 2597 - §5.7 explicit cohere via the threaded path stays STRICT — typed errors propagate, no fallback
  ---
  duration_ms: 0.169
  type: 'test'
  ...
# Subtest: pickScoreFields drops text/section, keeps ids + scores
ok 2598 - pickScoreFields drops text/section, keeps ids + scores
  ---
  duration_ms: 0.09
  type: 'test'
  ...
...
ok 2599 - 2.1 — rerankCohere with an injected fetchImpl serves, stamps the capture, and opens no real socket
  ---
  duration_ms: 157.551917
  type: 'test'
  ...
# Subtest: 2.2 — the DEFAULT cohereFn adapter inside rerank passes the capture in the CAPTURE position, not the fetch position
ok 2600 - 2.2 — the DEFAULT cohereFn adapter inside rerank passes the capture in the CAPTURE position, not the fetch position
  ---
  duration_ms: 10.567833
  type: 'test'
  ...
...
ok 2601 - 16.1 — Cohere selected, an UNTYPED throw: inputOrder returned, one synthesised terminal_failure batch, expected == recorded, soft_failed true, row not partial — by the REAL validation chain, both arms
  ---
  duration_ms: 1.002125
  type: 'test'
  ...
# Subtest: 17.1 — a per-batch throw inside rerankJudge warns, continues, and leaves rerank_soft_failed FALSE
ok 2602 - 17.1 — a per-batch throw inside rerankJudge warns, continues, and leaves rerank_soft_failed FALSE
  ---
...
ok 2603 - 18.1 — JUDGE served: expected_batch_count == ceil(pool / JUDGE_BATCH), JUDGE_BATCH read from source text
  ---
  duration_ms: 16.541875
  type: 'test'
  ...
# Subtest: 18.2 — COHERE served: expected_batch_count == 1, whatever the pool — stamped by the REAL rerankCohere
ok 2604 - 18.2 — COHERE served: expected_batch_count == 1, whatever the pool — stamped by the REAL rerankCohere
  ---
  duration_ms: 2.039166
  type: 'test'
  ...
...
ok 2605 - 18.3 — DERIVED FROM served_backend, NEVER intended_backend: intended Cohere, judge serves, expected is the JUDGE count
  ---
  duration_ms: 5.310291
  type: 'test'
  ...
# [rerank] env-default cohere unavailable → falling back to judge: rerank backend 'cohere' unreachable (rerank-v3.5): probe refused by design
...
ok 2606 - 70.1 — RUNTIME ORDER, observed AT INVOCATION: checkHealthy throws RerankBackendError FIRST, the judge is invoked SECOND
  ---
  duration_ms: 3.673625
  type: 'test'
  ...
# [rerank] env-default cohere unavailable → falling back to judge: rerank backend 'cohere' unreachable (rerank-v3.5): probe refused by design
...
ok 2607 - 70.2 — MANIFEST FACTS: intended_backend cohere, served_backend judge, rerank_backend_downgraded true, expected_batch_count matches the judge
  ---
  duration_ms: 2.089042
  type: 'test'
  ...
# Subtest: 70.3 — the row is persisted_complete by the REAL chain, and a broken payload is persisted_partial
ok 2608 - 70.3 — the row is persisted_complete by the REAL chain, and a broken payload is persisted_partial
  ---
  duration_ms: 1.568209
  type: 'test'
  ...
# Subtest: 70.4 — SOURCE PARITY: provider selection and fallback order in lib/rerank.ts are byte-identical to 72960baa
ok 2609 - 70.4 — SOURCE PARITY: provider selection and fallback order in lib/rerank.ts are byte-identical to 72960baa
  ---
  duration_ms: 63.3255
  type: 'test'
  ...
...
ok 2610 - J2.1 — under a JUDGE default: explicit judge, on success and on failure, calls neither checkHealthy nor cohereFn
  ---
  duration_ms: 6.284042
  type: 'test'
  ...
# [rerank judge] batch failed 0 - 5 Unexpected token 'o', "not json" is not valid JSON
# [rerank judge] batch failed 5 - 6 Unexpected token 'o', "not json" is not valid JSON
...
ok 2611 - J2.2 — under a HOSTILE COHERE default: explicit judge, on success and on failure, still calls neither — and each failure arm PROVES its failure happened
  ---
  duration_ms: 2.200667
  type: 'test'
  ...
# Subtest: THE EXHIBIT: "Atarax Cream…" resolves to NOTHING — not Hydroxyzine, not approximate
ok 2612 - THE EXHIBIT: "Atarax Cream…" resolves to NOTHING — not Hydroxyzine, not approximate
  ---
  duration_ms: 0.895333
  type: 'test'
  ...
# Subtest: the gate fires on the TEXT alone too — no route needed
ok 2613 - the gate fires on the TEXT alone too — no route needed
  ---
  duration_ms: 0.15725
  type: 'test'
  ...
# Subtest: the ORAL Atarax lines still resolve, confident — the gate is surgical
ok 2614 - the ORAL Atarax lines still resolve, confident — the gate is surgical
  ---
  duration_ms: 0.811959
  type: 'test'
  ...
# Subtest: a topical line matching a family that HAS a topical row still resolves
ok 2615 - a topical line matching a family that HAS a topical row still resolves
  ---
  duration_ms: 0.08425
  type: 'test'
  ...
# Subtest: route vocabulary: Topical, "topical " (trailing space) and local ALL count as topical
ok 2616 - route vocabulary: Topical, "topical " (trailing space) and local ALL count as topical
  ---
  duration_ms: 0.132042
  type: 'test'
  ...
# Subtest: phase 1.1 route PHRASES: application/apply-locally/intranasal variants all count as topical
ok 2617 - phase 1.1 route PHRASES: application/apply-locally/intranasal variants all count as topical
  ---
  duration_ms: 0.046291
  type: 'test'
  ...
# Subtest: the topical-form regex is the normative one, and word boundaries hold
ok 2618 - the topical-form regex is the normative one, and word boundaries hold
  ---
  duration_ms: 0.10525
  type: 'test'
  ...
# Subtest: tier 5 (brand-prefix, APPROX): a topical line never takes an oral approximate match
ok 2619 - tier 5 (brand-prefix, APPROX): a topical line never takes an oral approximate match
  ---
  duration_ms: 0.048791
  type: 'test'
  ...
# Subtest: TIERS 1–3 UNCHANGED: source generic, exact brand and embedded molecule ignore the gate
ok 2620 - TIERS 1–3 UNCHANGED: source generic, exact brand and embedded molecule ignore the gate
  ---
  duration_ms: 0.328458
  type: 'test'
  ...
# Subtest: CONFIDENT_MATCH and classifyUnmatched/NUTRA are untouched
ok 2621 - CONFIDENT_MATCH and classifyUnmatched/NUTRA are untouched
  ---
  duration_ms: 0.522042
  type: 'test'
  ...
# Subtest: the category gate: the three enums, case-sensitive, trimmed — matcher skipped entirely
ok 2622 - the category gate: the three enums, case-sensitive, trimmed — matcher skipped entirely
  ---
  duration_ms: 57.099083
  type: 'test'
  ...
# Subtest: the category gate is CASE-SENSITIVE and enum-exact — near-misses fall through to the matcher
ok 2623 - the category gate is CASE-SENSITIVE and enum-exact — near-misses fall through to the matcher
  ---
  duration_ms: 4.764542
  type: 'test'
  ...
# Subtest: rowToOpdCase carries default_opd_service_category verbatim, fail-safe on absence
ok 2624 - rowToOpdCase carries default_opd_service_category verbatim, fail-safe on absence
  ---
  duration_ms: 0.193583
  type: 'test'
  ...
# Subtest: §5.1: the gate still fires for a category-gated TOPICAL line — the Atarax exhibit
ok 2625 - §5.1: the gate still fires for a category-gated TOPICAL line — the Atarax exhibit
  ---
  duration_ms: 3.392333
  type: 'test'
  ...
# Subtest: §5.2: a category-gated ORAL line runs the matcher — Crocin resolves to Paracetamol
ok 2626 - §5.2: a category-gated ORAL line runs the matcher — Crocin resolves to Paracetamol
  ---
  duration_ms: 5.268625
  type: 'test'
  ...
# Subtest: §5.3 THE PHASE-3 UNBLOCKER: Depura 60000 IU Vitamin D3 Oral Solution resolves to Vitamin D3
ok 2627 - §5.3 THE PHASE-3 UNBLOCKER: Depura 60000 IU Vitamin D3 Oral Solution resolves to Vitamin D3
  ---
  duration_ms: 0.987209
  type: 'test'
  ...
# Subtest: §5.4: category + BLANK route + form word in the brand ⇒ still gated (text is evidence)
ok 2628 - §5.4: category + BLANK route + form word in the brand ⇒ still gated (text is evidence)
  ---
  duration_ms: 1.437084
  type: 'test'
  ...
# Subtest: §5.5: category + blank route + NO form word ⇒ matcher runs — Zincovit Tablet
ok 2629 - §5.5: category + blank route + NO form word ⇒ matcher runs — Zincovit Tablet
  ---
  duration_ms: 1.417959
  type: 'test'
  ...
# Subtest: §5.7: the tier-4/5 form gate from phase 1 is unchanged for non-category topical lines
ok 2630 - §5.7: the tier-4/5 form gate from phase 1 is unchanged for non-category topical lines
  ---
  duration_ms: 0.094167
  type: 'test'
  ...
# Subtest: phase 1.1 direction check: the widened regex can only WITHHOLD matches, never create one
ok 2631 - phase 1.1 direction check: the widened regex can only WITHHOLD matches, never create one
  ---
  duration_ms: 0.238958
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: 28 — ONE multi-row insert, app_source bound explicitly, no stamper involved
ok 2632 - 28 — ONE multi-row insert, app_source bound explicitly, no stamper involved
  ---
  duration_ms: 1.717333
  type: 'test'
  ...
# Subtest: 62 — app_source binds APP_SOURCE when set, and 'standalone' when absent, never null
ok 2633 - 62 — app_source binds APP_SOURCE when set, and 'standalone' when absent, never null
  ---
  duration_ms: 0.271583
  type: 'test'
  ...
# Subtest: the three A/A experiment columns are BOUND, so opd_art_experiment_idx is populable
ok 2634 - the three A/A experiment columns are BOUND, so opd_art_experiment_idx is populable
  ---
  duration_ms: 0.685917
  type: 'test'
  ...
# Subtest: declared_retrievals counts the rows that LANDED, not the rows that were asked for
ok 2635 - declared_retrievals counts the rows that LANDED, not the rows that were asked for
  ---
  duration_ms: 0.324959
  type: 'test'
  ...
# Subtest: a declaration that lands nothing bumps nothing at all
ok 2636 - a declaration that lands nothing bumps nothing at all
  ---
  duration_ms: 0.093541
  type: 'test'
  ...
# Subtest: an empty run list writes nothing and returns an empty handle
ok 2637 - an empty run list writes nothing and returns an empty handle
  ---
  duration_ms: 0.054292
  type: 'test'
  ...
# Subtest: 29 — a failed batch declaration writes ONE work_declaration failure row per generated run
ok 2638 - 29 — a failed batch declaration writes ONE work_declaration failure row per generated run
  ---
...
ok 2639 - 29 — a run-scoped failure phase with no run id is refused before it reaches the CHECK
  ---
  duration_ms: 8.112959
  type: 'test'
  ...
# Subtest: 29 — when the failure store ITSELF fails, the invocation counter is the last evidence
ok 2640 - 29 — when the failure store ITSELF fails, the invocation counter is the last evidence
  ---
  duration_ms: 0.472084
  type: 'test'
  ...
# Subtest: 30 — an invocation insert failure is fail-open, and leaves evidence
ok 2641 - 30 — an invocation insert failure is fail-open, and leaves evidence
  ---
  duration_ms: 0.409292
  type: 'test'
  ...
# Subtest: 30 — the invocation row is inserted once, with its kind and route class
ok 2642 - 30 — the invocation row is inserted once, with its kind and route class
  ---
  duration_ms: 0.089584
  type: 'test'
  ...
# Subtest: closeInvocation is fail-open too, and records a closure failure
ok 2643 - closeInvocation is fail-open too, and records a closure failure
  ---
  duration_ms: 0.206375
  type: 'test'
  ...
# Subtest: the write-failure counter never throws, even when its own UPDATE fails
ok 2644 - the write-failure counter never throws, even when its own UPDATE fails
  ---
  duration_ms: 0.0905
  type: 'test'
  ...
...
ok 2645 - RETRIEVAL_LLM_SEED is the fixed shared seed (42), defined once in expand.ts
  ---
  duration_ms: 0.424166
  type: 'test'
  ...
# Subtest: expand.ts: temperature 0 + seed, num_ctx preserved, prompt + fail-open untouched
ok 2646 - expand.ts: temperature 0 + seed, num_ctx preserved, prompt + fail-open untouched
  ---
  duration_ms: 0.122375
  type: 'test'
  ...
# Subtest: multi-query generateQueryVariants: temperature 0 + seed, num_ctx preserved, prompt + fail-open untouched
ok 2647 - multi-query generateQueryVariants: temperature 0 + seed, num_ctx preserved, prompt + fail-open untouched
  ---
...
ok 2648 - 60 A — useReranker false: instrumentation on and off return byte-identical results
  ---
  duration_ms: 613.333708
  type: 'test'
  ...
# Subtest: 60 B — useReranker true: identical results, batch boundaries and prompts across on and off
ok 2649 - 60 B — useReranker true: identical results, batch boundaries and prompts across on and off
  ---
  duration_ms: 69.843042
  type: 'test'
  ...
# Subtest: 60 C — the production opts: identical results with source weighting, expansion and embedding all live
ok 2650 - 60 C — the production opts: identical results with source weighting, expansion and embedding all live
  ---
  duration_ms: 26.671167
  type: 'test'
  ...
# Subtest: 60 — THE CALL-FORM PIN: one side omits the capture argument, per case
ok 2651 - 60 — THE CALL-FORM PIN: one side omits the capture argument, per case
  ---
  duration_ms: 0.819084
  type: 'test'
  ...
# Subtest: 60 — the seven routing fragments are pairwise non-overlapping on the statements that ran
ok 2652 - 60 — the seven routing fragments are pairwise non-overlapping on the statements that ran
  ---
  duration_ms: 0.561875
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
...
ok 2653 - 51 — every settlement outcome has a state, and the mapping is D9's table exactly
  ---
  duration_ms: 0.848125
  type: 'test'
  ...
# Subtest: 51 — saveOpdAudit's four return values each map to their D9 outcome, including skipped
ok 2654 - 51 — saveOpdAudit's four return values each map to their D9 outcome, including skipped
  ---
  duration_ms: 0.076
  type: 'test'
  ...
# Subtest: 51 — settlement writes ONCE per run, and the write carries the mapped state and the audit id
ok 2655 - 51 — settlement writes ONCE per run, and the write carries the mapped state and the audit id
  ---
  duration_ms: 1.253917
  type: 'test'
  ...
# Subtest: 15 — a retrieval_failure with a persisted audit settles persisted_complete, not partial
ok 2656 - 15 — a retrieval_failure with a persisted audit settles persisted_complete, not partial
  ---
  duration_ms: 0.15075
  type: 'test'
  ...
# Subtest: 33 — primary settles, normative fails: one settled, one failed, one failure row, nothing thrown
ok 2657 - 33 — primary settles, normative fails: one settled, one failed, one failure row, nothing thrown
  ---
  duration_ms: 0.940083
  type: 'test'
  ...
# Subtest: 33 — a role still at revision 0 is NOT linked: audit id null, and the state is not the outcome's
ok 2658 - 33 — a role still at revision 0 is NOT linked: audit id null, and the state is not the outcome's
  ---
  duration_ms: 2.362708
  type: 'test'
  ...
# Subtest: revision 0 with retrieval_terminal evidence settles telemetry_persistence_failed
ok 2659 - revision 0 with retrieval_terminal evidence settles telemetry_persistence_failed
  ---
  duration_ms: 0.625708
  type: 'test'
  ...
# Subtest: revision 0 with NO evidence settles aborted
ok 2660 - revision 0 with NO evidence settles aborted
  ---
  duration_ms: 0.359792
  type: 'test'
  ...
# Subtest: revision 0 KEEPS an outcome a never-retrieved run can honestly carry
ok 2661 - revision 0 KEEPS an outcome a never-retrieved run can honestly carry
  ---
  duration_ms: 1.753666
  type: 'test'
  ...
# Subtest: a REJECTED write is reported as rejected, never as settled, and leaves durable evidence
ok 2662 - a REJECTED write is reported as rejected, never as settled, and leaves durable evidence
  ---
  duration_ms: 9.021791
  type: 'test'
  ...
# Subtest: an identical-content retry stays SETTLED and burns no revision
ok 2663 - an identical-content retry stays SETTLED and burns no revision
  ---
  duration_ms: 0.584792
  type: 'test'
  ...
# Subtest: v9 §4.2 — settlement REFUSES a duplicate role and reports it, rather than settling it twice
ok 2664 - v9 §4.2 — settlement REFUSES a duplicate role and reports it, rather than settling it twice
  ---
  duration_ms: 0.38725
  type: 'test'
  ...
# Subtest: v9 §4.2 — `status` stays at D12's three values; the new class rides in `rejection`
ok 2665 - v9 §4.2 — `status` stays at D12's three values; the new class rides in `rejection`
  ---
  duration_ms: 0.266875
  type: 'test'
  ...
# Subtest: v9 §4.2 — the vocabulary has SIX classes, and the sixth needs no migration
ok 2666 - v9 §4.2 — the vocabulary has SIX classes, and the sixth needs no migration
  ---
  duration_ms: 0.883542
  type: 'test'
  ...
# Subtest: v9 §4.2 — a duplicate role at the TERMINAL WRITE throws, as an undeclared role already does
ok 2667 - v9 §4.2 — a duplicate role at the TERMINAL WRITE throws, as an undeclared role already does
  ---
  duration_ms: 0.657875
  type: 'test'
  ...
# Subtest: v9 §4.2 — a handle with one run per role is untouched by the guard
ok 2668 - v9 §4.2 — a handle with one run per role is untouched by the guard
  ---
  duration_ms: 0.254333
  type: 'test'
  ...
# Subtest: v9 §4.1 — the base outcome type excludes persisted_dirty, and the mappers cannot produce it
ok 2669 - v9 §4.1 — the base outcome type excludes persisted_dirty, and the mappers cannot produce it
  ---
  duration_ms: 0.114
  type: 'test'
  ...
# Subtest: the runtime states and the state CHECK are the same list, in the same order
ok 2670 - the runtime states and the state CHECK are the same list, in the same order
  ---
  duration_ms: 2.280208
  type: 'test'
  ...
# Subtest: the outcome CHECK pins its two state lists the same way, and neither slice is empty
ok 2671 - the outcome CHECK pins its two state lists the same way, and neither slice is empty
  ---
  duration_ms: 0.348459
  type: 'test'
  ...
# Subtest: the three outcome-CHECK sets PARTITION all fourteen states — no overlap, none omitted
ok 2672 - the three outcome-CHECK sets PARTITION all fourteen states — no overlap, none omitted
  ---
  duration_ms: 0.09975
  type: 'test'
  ...
# Subtest: two states are non-terminal, and a window cannot close on either
ok 2673 - two states are non-terminal, and a window cannot close on either
  ---
  duration_ms: 0.081208
  type: 'test'
  ...
# Subtest: the migration still declares its retention, access and deletion controls (§4.2)
ok 2674 - the migration still declares its retention, access and deletion controls (§4.2)
  ---
  duration_ms: 0.215625
  type: 'test'
  ...
# Subtest: the HMAC is keyed, versioned, and unreproducible with an unkeyed hash
ok 2675 - the HMAC is keyed, versioned, and unreproducible with an unkeyed hash
  ---
  duration_ms: 3.769834
  type: 'test'
  ...
# Subtest: key version travels with the value, so a rotation is visible rather than inferred
ok 2676 - key version travels with the value, so a rotation is visible rather than inferred
  ---
  duration_ms: 0.447958
  type: 'test'
  ...
# Subtest: a missing secret THROWS, and a whitespace-only key counts as missing (D8, test 71)
ok 2677 - a missing secret THROWS, and a whitespace-only key counts as missing (D8, test 71)
  ---
  duration_ms: 0.477167
  type: 'test'
  ...
# Subtest: every served class increments its OWN counter, and a null increments none
ok 2678 - every served class increments its OWN counter, and a null increments none
  ---
  duration_ms: 0.836791
  type: 'test'
  ...
# Subtest: counters derive from the manifest, so row and payload cannot disagree
ok 2679 - counters derive from the manifest, so row and payload cannot disagree
  ---
  duration_ms: 0.794541
  type: 'test'
  ...
# Subtest: rerank_429_attempts is the count of http_429 attempts, wherever they happened (test 13)
ok 2680 - rerank_429_attempts is the count of http_429 attempts, wherever they happened (test 13)
  ---
  duration_ms: 0.172584
  type: 'test'
  ...
# Subtest: batch order is a property of candidate boundaries, never of completion order (constraint 7)
ok 2681 - batch order is a property of candidate boundaries, never of completion order (constraint 7)
  ---
  duration_ms: 0.815167
  type: 'test'
  ...
# Subtest: neither field-bearing manifest declaration has a field that could carry clinical text
ok 2682 - neither field-bearing manifest declaration has a field that could carry clinical text
  ---
  duration_ms: 1.169542
  type: 'test'
  ...
# Subtest: the ban loop really bans — it fails when a banned field is added
ok 2683 - the ban loop really bans — it fails when a banned field is added
  ---
  duration_ms: 0.286791
  type: 'test'
  ...
# Subtest: TelemetryCapture is not declared in the core — the raw bytes live elsewhere (D5)
ok 2684 - TelemetryCapture is not declared in the core — the raw bytes live elsewhere (D5)
  ---
  duration_ms: 0.650833
  type: 'test'
  ...
# Subtest: StampedRetrievalManifest is EXACTLY the intersection, so nothing can be smuggled through it
ok 2685 - StampedRetrievalManifest is EXACTLY the intersection, so nothing can be smuggled through it
  ---
  duration_ms: 2.573958
  type: 'test'
  ...
# Subtest: every route maps to a class, and an unknown caller is never assigned to the nearest match
ok 2686 - every route maps to a class, and an unknown caller is never assigned to the nearest match
  ---
  duration_ms: 0.183167
  type: 'test'
  ...
# Subtest: the reconciler is an INVOCATION route and never a retrieval route (D17)
ok 2687 - the reconciler is an INVOCATION route and never a retrieval route (D17)
  ---
  duration_ms: 0.130708
  type: 'test'
  ...
# Subtest: the five roles are closed, and the appropriateness exclusion is by ROUTE not by role
ok 2688 - the five roles are closed, and the appropriateness exclusion is by ROUTE not by role
  ---
  duration_ms: 0.061875
  type: 'test'
  ...
# Subtest: usage aggregates by served provider/model, not by intended
ok 2689 - usage aggregates by served provider/model, not by intended
  ---
  duration_ms: 12.759167
  type: 'test'
  ...
# Subtest: a bucket with no usage reports null tokens and counts the unknowns — never zero (§4.6)
ok 2690 - a bucket with no usage reports null tokens and counts the unknowns — never zero (§4.6)
  ---
  duration_ms: 0.218833
  type: 'test'
  ...
# Subtest: partial usage is summed without inventing the missing half
ok 2691 - partial usage is summed without inventing the missing half
  ---
  duration_ms: 0.155375
  type: 'test'
  ...
# Subtest: local, not-served and skipped stages are UNPRICED; unattributed and parse failures are not
ok 2692 - local, not-served and skipped stages are UNPRICED; unattributed and parse failures are not
  ---
  duration_ms: 0.41375
  type: 'test'
  ...
# Subtest: this module prices nothing — money has ONE source of truth
ok 2693 - this module prices nothing — money has ONE source of truth
  ---
  duration_ms: 0.727292
  type: 'test'
  ...
# Subtest: the row contract and the manifest contract version independently (§4.3)
ok 2694 - the row contract and the manifest contract version independently (§4.3)
  ---
  duration_ms: 0.29725
  type: 'test'
  ...
...
ok 2695 - 19 — the predeclared run id is the one every later write targets, and no second row is inserted
  ---
  duration_ms: 2.221625
  type: 'test'
  ...
# Subtest: 20 — declare returns 0, the terminal write returns 1, and a stale handle is REJECTED
ok 2696 - 20 — declare returns 0, the terminal write returns 1, and a stale handle is REJECTED
  ---
  duration_ms: 0.629292
  type: 'test'
  ...
# Subtest: 20 — revisions advance PER ROLE: a normative write cannot invalidate the primary's handle
ok 2697 - 20 — revisions advance PER ROLE: a normative write cannot invalidate the primary's handle
  ---
  duration_ms: 0.875333
  type: 'test'
  ...
# Subtest: 20 — a terminal write that matches nothing is NOT retried, and does not advance the handle
ok 2698 - 20 — a terminal write that matches nothing is NOT retried, and does not advance the handle
  ---
  duration_ms: 0.287875
  type: 'test'
  ...
# Subtest: 31 — the reuse guard returns BEFORE any telemetry statement exists
ok 2699 - 31 — the reuse guard returns BEFORE any telemetry statement exists
  ---
  duration_ms: 0.343667
  type: 'test'
  ...
# Subtest: 32 — the attached handle is absent from JSON.stringify, from the keys, and from a spread
ok 2700 - 32 — the attached handle is absent from JSON.stringify, from the keys, and from a spread
  ---
  duration_ms: 0.118167
  type: 'test'
  ...
# Subtest: 34 — terminal write fails, failure row fails: the invocation counter is the only evidence left
ok 2701 - 34 — terminal write fails, failure row fails: the invocation counter is the only evidence left
  ---
  duration_ms: 0.455875
  type: 'test'
  ...
# Subtest: 34 — and when the counter ALSO fails: a log line, nothing else, still no propagation
ok 2702 - 34 — and when the counter ALSO fails: a log line, nothing else, still no propagation
  ---
  duration_ms: 0.3445
  type: 'test'
  ...
# Subtest: 34 — NO MODULE-LEVEL COUNTER EXISTS ANYWHERE, asserted by source search
ok 2703 - 34 — NO MODULE-LEVEL COUNTER EXISTS ANYWHERE, asserted by source search
  ---
  duration_ms: 0.631959
  type: 'test'
  ...
# Subtest: 52 — every owner in the D9 matrix settles, including both scripts and both MCP paths
ok 2704 - 52 — every owner in the D9 matrix settles, including both scripts and both MCP paths
  ---
  duration_ms: 0.946542
  type: 'test'
  ...
# Subtest: 53 — the callback carries the audit id, and its failure never changes the save result
ok 2705 - 53 — the callback carries the audit id, and its failure never changes the save result
  ---
...
ok 2706 - CANARY-GATE HAZARD — primary rejected, normative lands: the audit persisted and the Stage 0b primary-link gate fails
  ---
  duration_ms: 10.863208
  type: 'test'
  ...
# Subtest: CANARY-GATE HAZARD — the mirror: primary lands, normative rejected
ok 2707 - CANARY-GATE HAZARD — the mirror: primary lands, normative rejected
  ---
  duration_ms: 4.158458
  type: 'test'
  ...
# Subtest: 54 — fourteen states, two of them non-terminal, and the two sets partition the whole
ok 2708 - 54 — fourteen states, two of them non-terminal, and the two sets partition the whole
  ---
  duration_ms: 0.518959
  type: 'test'
  ...
# Subtest: 54 — the implemented table IS D12's table, in both directions
ok 2709 - 54 — the implemented table IS D12's table, in both directions
  ---
  duration_ms: 0.3855
  type: 'test'
  ...
# Subtest: 54 — every one of the 196 ordered pairs answers the way D12 says
ok 2710 - 54 — every one of the 196 ordered pairs answers the way D12 says
  ---
  duration_ms: 0.196084
  type: 'test'
  ...
# Subtest: 54 — TERMINAL STATES NEVER TRANSITION, to anything, including themselves
ok 2711 - 54 — TERMINAL STATES NEVER TRANSITION, to anything, including themselves
  ---
  duration_ms: 0.0925
  type: 'test'
  ...
# Subtest: 54 — the two deliberate asymmetries are both present, and are not accidents
ok 2712 - 54 — the two deliberate asymmetries are both present, and are not accidents
  ---
  duration_ms: 0.092625
  type: 'test'
  ...
...
ok 2713 - 54 — every settlement outcome lands on a state, and the settlement table names none of the three reconciler-mapped states
  ---
  duration_ms: 0.086625
  type: 'test'
  ...
# Subtest: 54 — every reconciler-assigned state is a LEGAL transition from the state it is assigned from
ok 2714 - 54 — every reconciler-assigned state is a LEGAL transition from the state it is assigned from
  ---
...
ok 2715 - default path: filter clause array is byte-identical to production, no params
  ---
  duration_ms: 0.766916
  type: 'test'
  ...
# Subtest: default path with structural filters keeps the base guards and remaps $FP offsets per leg
ok 2716 - default path with structural filters keeps the base guards and remaps $FP offsets per leg
  ---
  duration_ms: 0.094125
  type: 'test'
  ...
# Subtest: relaxed path: both quarantine guards gain a bound OR arm on both legs
ok 2717 - relaxed path: both quarantine guards gain a bound OR arm on both legs
  ---
  duration_ms: 0.193375
  type: 'test'
  ...
# Subtest: relaxed path ordering: the quarantine label takes $FP_0, structural filters follow
ok 2718 - relaxed path ordering: the quarantine label takes $FP_0, structural filters follow
  ---
  duration_ms: 0.082666
  type: 'test'
  ...
# Subtest: hostile labels are slugged by labLabel and cannot widen the filter
ok 2719 - hostile labels are slugged by labLabel and cannot widen the filter
  ---
  duration_ms: 0.221
  type: 'test'
  ...
# Subtest: empty/whitespace includeQuarantined is treated as omitted (byte-identical default path)
ok 2720 - empty/whitespace includeQuarantined is treated as omitted (byte-identical default path)
  ---
  duration_ms: 0.108667
  type: 'test'
  ...
# Subtest: clampLabRetrieveTopK clamps to [1,20], default 8
ok 2721 - clampLabRetrieveTopK clamps to [1,20], default 8
  ---
...
ok 2722 - restrictSources omitted ⇒ default clauses + no params (byte-identical)
  ---
  duration_ms: 0.880042
  type: 'test'
  ...
# Subtest: restrictSources: [choosing-wisely] ⇒ both legs source = ANY, bound array param
ok 2723 - restrictSources: [choosing-wisely] ⇒ both legs source = ANY, bound array param
  ---
  duration_ms: 0.14125
  type: 'test'
  ...
# Subtest: a named labq: source is admitted through the quarantine guard; un-named stays excluded
ok 2724 - a named labq: source is admitted through the quarantine guard; un-named stays excluded
  ---
  duration_ms: 0.115708
  type: 'test'
  ...
# Subtest: empty or all-blank restrictSources falls back to the default filter (no restriction)
ok 2725 - empty or all-blank restrictSources falls back to the default filter (no restriction)
  ---
  duration_ms: 0.139209
  type: 'test'
  ...
# Subtest: restrictSources stacks with book/chunk filters and supersedes the single-source filter
ok 2726 - restrictSources stacks with book/chunk filters and supersedes the single-source filter
  ---
  duration_ms: 0.057458
  type: 'test'
  ...
# Subtest: group key exactness: same (subject, signal_type) groups; different signal splits
ok 2727 - group key exactness: same (subject, signal_type) groups; different signal splits
  ---
  duration_ms: 0.663417
  type: 'test'
  ...
# Subtest: '' signal folds like stewardship (empty signal groups together)
ok 2728 - '' signal folds like stewardship (empty signal groups together)
  ---
  duration_ms: 0.091542
  type: 'test'
  ...
# Subtest: ≥2 threshold: a pair groups, a singleton does not
ok 2729 - ≥2 threshold: a pair groups, a singleton does not
  ---
  duration_ms: 0.352875
  type: 'test'
  ...
# Subtest: section order + group sort (size desc, newest tie-break) + singles in queue order
ok 2730 - section order + group sort (size desc, newest tie-break) + singles in queue order
  ---
  duration_ms: 0.289083
  type: 'test'
  ...
# Subtest: disagreement pinning: disagreements section leads the order, groups/singles below
ok 2731 - disagreement pinning: disagreements section leads the order, groups/singles below
  ---
  duration_ms: 0.141416
  type: 'test'
  ...
# Subtest: traversal: within-group → next group → singles; wrap; exhausted → null
ok 2732 - traversal: within-group → next group → singles; wrap; exhausted → null
  ---
  duration_ms: 0.1115
  type: 'test'
  ...
# Subtest: skip sinks within its section and traversal passes it over
ok 2733 - skip sinks within its section and traversal passes it over
  ---
  duration_ms: 0.14425
  type: 'test'
  ...
# Subtest: labeled stays in place (not sunk) with its status; k/n progress counts labeled+skipped
ok 2734 - labeled stays in place (not sunk) with its status; k/n progress counts labeled+skipped
  ---
  duration_ms: 0.063417
  type: 'test'
  ...
# Subtest: determinism: same input → identical output
ok 2735 - determinism: same input → identical output
  ---
  duration_ms: 0.203959
  type: 'test'
  ...
# Subtest: hashBucket is deterministic and in 0..99
ok 2736 - hashBucket is deterministic and in 0..99
  ---
  duration_ms: 0.949417
  type: 'test'
  ...
# Subtest: overlap is ~20% and buckets < 20
ok 2737 - overlap is ~20% and buckets < 20
  ---
  duration_ms: 0.516334
  type: 'test'
  ...
# Subtest: overlap findings are served to EVERY reviewer; partitioned to exactly one
ok 2738 - overlap findings are served to EVERY reviewer; partitioned to exactly one
  ---
  duration_ms: 0.590542
  type: 'test'
  ...
# Subtest: a reviewer not on the roster still gets the overlap set (only)
ok 2739 - a reviewer not on the roster still gets the overlap set (only)
  ---
  duration_ms: 0.130292
  type: 'test'
  ...
# Subtest: partition is roughly even across the roster
ok 2740 - partition is roughly even across the roster
  ---
  duration_ms: 0.162208
  type: 'test'
  ...
# Subtest: balanceBySignalType interleaves types and is newest-first within a type
ok 2741 - balanceBySignalType interleaves types and is newest-first within a type
  ---
  duration_ms: 0.435375
  type: 'test'
  ...
# Subtest: disagreement items come first, then fresh; limit respected
ok 2742 - disagreement items come first, then fresh; limit respected
  ---
  duration_ms: 0.297542
  type: 'test'
  ...
# Subtest: passthrough: optional uid + prescription_url survive buildReviewQueue onto emitted items
ok 2743 - passthrough: optional uid + prescription_url survive buildReviewQueue onto emitted items
  ---
  duration_ms: 0.080084
  type: 'test'
  ...
# Subtest: excludes labeled-by-this-reviewer, informational, unassigned, and filtered-out findings
ok 2744 - excludes labeled-by-this-reviewer, informational, unassigned, and filtered-out findings
  ---
  duration_ms: 2.3175
  type: 'test'
  ...
# Subtest: parseGoal: valid / missing / garbage → exact defaults; personal ceil
ok 2745 - parseGoal: valid / missing / garbage → exact defaults; personal ceil
  ---
  duration_ms: 0.73425
  type: 'test'
  ...
# Subtest: prevDay + istWeekStart (Monday-start)
ok 2746 - prevDay + istWeekStart (Monday-start)
  ---
  duration_ms: 1.093916
  type: 'test'
  ...
# Subtest: countedLabels: impact excluded, missed included, roster filter, finding current-state (later wins)
ok 2747 - countedLabels: impact excluded, missed included, roster filter, finding current-state (later wins)
  ---
  duration_ms: 0.204167
  type: 'test'
  ...
# Subtest: streak: threshold exactly 15, consecutive, yesterday-grace, today-only, gap → 0
ok 2748 - streak: threshold exactly 15, consecutive, yesterday-grace, today-only, gap → 0
  ---
  duration_ms: 0.252625
  type: 'test'
  ...
# Subtest: agreement: pair construction (2 & 3 reviewers), tier match/mismatch, overlap-only
ok 2749 - agreement: pair construction (2 & 3 reviewers), tier match/mismatch, overlap-only
  ---
  duration_ms: 0.216541
  type: 'test'
  ...
# Subtest: agreement: current-state dedup feeds pairs (later verdict wins), then match recomputed
ok 2750 - agreement: current-state dedup feeds pairs (later verdict wins), then match recomputed
  ---
  duration_ms: 0.125625
  type: 'test'
  ...
# Subtest: computeReviewStats: ≥20-pair display boundary, week total, badges shape
ok 2751 - computeReviewStats: ≥20-pair display boundary, week total, badges shape
  ---
  duration_ms: 0.440584
  type: 'test'
  ...
# Subtest: the committed gold artifact is frozen, ratified, and catalog-consistent
ok 2752 - the committed gold artifact is frozen, ratified, and catalog-consistent
  ---
  duration_ms: 2.3655
  type: 'test'
  ...
# Subtest: loadCheckGold rejects drift: wrong version, unratified, polarity/target mismatch
ok 2753 - loadCheckGold rejects drift: wrong version, unratified, polarity/target mismatch
  ---
  duration_ms: 5.682875
  type: 'test'
  ...
# Subtest: the committed 2.0 artifact is frozen, ratified, family-split, and catalog-consistent
ok 2754 - the committed 2.0 artifact is frozen, ratified, family-split, and catalog-consistent
  ---
  duration_ms: 1.871667
  type: 'test'
  ...
# Subtest: loadCheckGold2: accepts the delivered shape — empty targets legal, L carries annex/memberHistory
ok 2755 - loadCheckGold2: accepts the delivered shape — empty targets legal, L carries annex/memberHistory
  ---
  duration_ms: 0.446625
  type: 'test'
  ...
# Subtest: loadCheckGold2 rejects drift: version, status, dup ids, missing verdict fields, annex misuse
ok 2756 - loadCheckGold2 rejects drift: version, status, dup ids, missing verdict fields, annex misuse
  ---
  duration_ms: 1.257834
  type: 'test'
  ...
# Subtest: splitCheckGold2: P/N/C form the scored floor, L is the annex — never folded together
ok 2757 - splitCheckGold2: P/N/C form the scored floor, L is the annex — never folded together
  ---
  duration_ms: 0.926625
  type: 'test'
  ...
# Subtest: checkGold2CatalogGaps: unbound polarity-side targets are flagged, bound ones are not
ok 2758 - checkGold2CatalogGaps: unbound polarity-side targets are flagged, bound ones are not
  ---
  duration_ms: 0.45025
  type: 'test'
  ...
# Subtest: scoreCheckAgainstGold: per-target-rec, deterministic, ignores non-target firings
ok 2759 - scoreCheckAgainstGold: per-target-rec, deterministic, ignores non-target firings
  ---
  duration_ms: 0.363334
  type: 'test'
  ...
# Subtest: aggregateCheckGold: hand-computed recall / specificity / precision / F1
ok 2760 - aggregateCheckGold: hand-computed recall / specificity / precision / F1
  ---
  duration_ms: 0.779042
  type: 'test'
  ...
# Subtest: Fix A: ANALYZE_SYSTEM carries the verdict discipline (uncertain = equipoise only)
ok 2761 - Fix A: ANALYZE_SYSTEM carries the verdict discipline (uncertain = equipoise only)
  ---
  duration_ms: 0.456709
  type: 'test'
  ...
# Subtest: Fix A: normNetValue contract unchanged, but the parse fallback is now visible
ok 2762 - Fix A: normNetValue contract unchanged, but the parse fallback is now visible
  ---
  duration_ms: 0.165375
  type: 'test'
  ...
# Subtest: Fix B: the two syncope recs are in the seed, verified, unique, well-formed
ok 2763 - Fix B: the two syncope recs are in the seed, verified, unique, well-formed
  ---
  duration_ms: 0.1165
  type: 'test'
  ...
# Subtest: Fix B gold: deterministic recall hits C04 with BOTH new recs, and no other check case
ok 2764 - Fix B gold: deterministic recall hits C04 with BOTH new recs, and no other check case
  ---
  duration_ms: 0.57725
  type: 'test'
  ...
# Subtest: flag-off byte-identical: every grounded builder without the param equals Slice 1 exactly
ok 2765 - flag-off byte-identical: every grounded builder without the param equals Slice 1 exactly
  ---
  duration_ms: 0.794792
  type: 'test'
  ...
# Subtest: grounded: the picture lands between the input and the downstream sections, verbatim
ok 2766 - grounded: the picture lands between the input and the downstream sections, verbatim
  ---
  duration_ms: 0.13875
  type: 'test'
  ...
# Subtest: patientPictureBlock: formatClinicalState content + the two prompt rules
ok 2767 - patientPictureBlock: formatClinicalState content + the two prompt rules
  ---
  duration_ms: 1.316875
  type: 'test'
  ...
# Subtest: grounding flag is double-gated on the master flag
ok 2768 - grounding flag is double-gated on the master flag
  ---
  duration_ms: 0.101
  type: 'test'
  ...
# Subtest: frozen bank right-care-eval/1.0: pinned, unique ids, per-mode shape
ok 2769 - frozen bank right-care-eval/1.0: pinned, unique ids, per-mode shape
  ---
  duration_ms: 0.762959
  type: 'test'
  ...
# Subtest: pair-judge parser: defensive on directions, safety classes, fences
ok 2770 - pair-judge parser: defensive on directions, safety classes, fences
  ---
  duration_ms: 0.270125
  type: 'test'
  ...
# Subtest: deterministic check diff: added / removed / kept by rec id
ok 2771 - deterministic check diff: added / removed / kept by rec id
  ---
  duration_ms: 0.230375
  type: 'test'
  ...
# Subtest: scorecard gates: FAIL_SAFETY dominates; PASS needs net improvement clearing noise
ok 2772 - scorecard gates: FAIL_SAFETY dominates; PASS needs net improvement clearing noise
  ---
...
ok 2773 - Order check constructs a ClinicalState from the provided input with counts > 0
  ---
  duration_ms: 3.995458
  type: 'test'
  ...
# Subtest: Care pathway constructs from the presentation field with counts > 0
ok 2774 - Care pathway constructs from the presentation field with counts > 0
  ---
  duration_ms: 0.42175
  type: 'test'
  ...
# Subtest: Record audit adapts the existing ExtractedCase and round-trips on the shared fields
ok 2775 - Record audit adapts the existing ExtractedCase and round-trips on the shared fields
  ---
  duration_ms: 0.75375
  type: 'test'
  ...
# Subtest: fail-open: a throwing LLM stage keeps the deterministic state; junk input never throws
ok 2776 - fail-open: a throwing LLM stage keeps the deterministic state; junk input never throws
  ---
  duration_ms: 0.3405
  type: 'test'
  ...
# Subtest: flag-off neutrality: no gate flag → feature inert; UI field off → {}
ok 2777 - flag-off neutrality: no gate flag → feature inert; UI field off → {}
  ---
  duration_ms: 0.503958
  type: 'test'
  ...
# Subtest: save-run reconstruction: same pure builders, schema-valid, per mode
ok 2778 - save-run reconstruction: same pure builders, schema-valid, per mode
  ---
  duration_ms: 0.6575
  type: 'test'
  ...
# Subtest: member link: strict validation, and identity stays OUT of the state
ok 2779 - member link: strict validation, and identity stays OUT of the state
  ---
...
ok 2780 - 1 — a PRIMARY defect leaves the NORMATIVE run clean
  ---
  duration_ms: 2.266333
  type: 'test'
  ...
# Subtest: 2 — a NORMATIVE defect leaves the PRIMARY run clean
ok 2781 - 2 — a NORMATIVE defect leaves the PRIMARY run clean
  ---
  duration_ms: 0.564792
  type: 'test'
  ...
# Subtest: 3 — both roles dirty: both settle dirty
ok 2782 - 3 — both roles dirty: both settle dirty
  ---
  duration_ms: 1.174083
  type: 'test'
  ...
# Subtest: 4 — neither dirty: both settle clean
ok 2783 - 4 — neither dirty: both settle clean
  ---
  duration_ms: 0.294625
  type: 'test'
  ...
# Subtest: 4b — an EMPTY array is clean; an ABSENT key on a linkable run is NOT
ok 2784 - 4b — an EMPTY array is clean; an ABSENT key on a linkable run is NOT
  ---
  duration_ms: 1.204125
  type: 'test'
  ...
# Subtest: 5 — no map at all: a single-role save behaves exactly as before
ok 2785 - 5 — no map at all: a single-role save behaves exactly as before
  ---
  duration_ms: 5.084042
  type: 'test'
  ...
# Subtest: 5b — only the CLEAN branch is upgraded; a losing race or a skip is never made partial
ok 2786 - 5b — only the CLEAN branch is upgraded; a losing race or a skip is never made partial
  ---
  duration_ms: 0.102958
  type: 'test'
  ...
# Subtest: 5c — a revision-0 role is still not linked, and the per-run upgrade did not disturb that
ok 2787 - 5c — a revision-0 role is still not linked, and the per-run upgrade did not disturb that
  ---
...
ok 2788 - 6 — every persistence owner passes the role map, and none passes an empty one where it holds defects
  ---
  duration_ms: 1.458625
  type: 'test'
  ...
# Subtest: 6b — the upgrade is applied in settlement, not by the owners
ok 2789 - 6b — the upgrade is applied in settlement, not by the owners
  ---
  duration_ms: 1.016292
  type: 'test'
  ...
# Subtest: 6c — settleOwned still takes ONE base outcome and makes ONE settlement call
ok 2790 - 6c — settleOwned still takes ONE base outcome and makes ONE settlement call
  ---
...
ok 2791 - 6d — settlement is fail-safe: a role map on a handle with no runs settles nothing and throws nothing
  ---
  duration_ms: 0.058791
  type: 'test'
  ...
# Subtest: 7 — the three cases of `verdictForRun`, stated directly
ok 2792 - 7 — the three cases of `verdictForRun`, stated directly
  ---
  duration_ms: 0.3865
  type: 'test'
  ...
# Subtest: 7b — requirement 10: an INHERITED key is not a verdict
ok 2793 - 7b — requirement 10: an INHERITED key is not a verdict
  ---
  duration_ms: 0.167333
  type: 'test'
  ...
# Subtest: 7c — an inherited key does not rescue a run from partial, through settlement
ok 2794 - 7c — an inherited key does not rescue a run from partial, through settlement
  ---
  duration_ms: 0.153541
  type: 'test'
  ...
# Subtest: 7d — THE PLACEMENT TEST: the rule reaches a linkable run and stops at a revision-zero one
ok 2795 - 7d — THE PLACEMENT TEST: the rule reaches a linkable run and stops at a revision-zero one
  ---
  duration_ms: 0.234375
  type: 'test'
  ...
# Subtest: 7e — requirement 3 in the only place it is observable: settlement never mutates the map
ok 2796 - 7e — requirement 3 in the only place it is observable: settlement never mutates the map
  ---
  duration_ms: 0.126708
  type: 'test'
  ...
# Subtest: 7f — the base outcome still decides: only a CLEAN run is made partial by a missing key
ok 2797 - 7f — the base outcome still decides: only a CLEAN run is made partial by a missing key
  ---
  duration_ms: 0.042917
  type: 'test'
  ...
# Subtest: 7g — no new settlement outcome value was added, and nothing writes the synthetic code
ok 2798 - 7g — no new settlement outcome value was added, and nothing writes the synthetic code
  ---
  duration_ms: 0.2945
  type: 'test'
  ...
# Subtest: matchRoomCategory prefers the longest alias and falls back
ok 2799 - matchRoomCategory prefers the longest alias and falls back
  ---
  duration_ms: 0.574958
  type: 'test'
  ...
# Subtest: excessBedDays = LOS − benchmark, floored at 0
ok 2800 - excessBedDays = LOS − benchmark, floored at 0
  ---
  duration_ms: 0.085125
  type: 'test'
  ...
# Subtest: computeBedDayCost: 8-day single room over-stay = 7 × 6500 = 45,500 (est.)
ok 2801 - computeBedDayCost: 8-day single room over-stay = 7 × 6500 = 45,500 (est.)
  ---
  duration_ms: 25.613125
  type: 'test'
  ...
# Subtest: computeBedDayCost returns 0 when not flagged, day-care, or single-day
ok 2802 - computeBedDayCost returns 0 when not flagged, day-care, or single-day
  ---
  duration_ms: 0.09975
  type: 'test'
  ...
# Subtest: tariff-status table drops the (est.) label
ok 2803 - tariff-status table drops the (est.) label
  ---
  duration_ms: 1.1495
  type: 'test'
  ...
...
ok 2804 - the routes still declare the numbers this guard reads
  ---
  duration_ms: 1.314375
  type: 'test'
  ...
# Subtest: the body extractor is not vacuous — it finds real code, not an empty default param
ok 2805 - the body extractor is not vacuous — it finds real code, not an empty default param
  ---
  duration_ms: 1.316916
  type: 'test'
  ...
# Subtest: IPD_ANALYZE_LEGS equals the analyze call sites in lib/doc-audit.ts — a FOURTH leg fails here
ok 2806 - IPD_ANALYZE_LEGS equals the analyze call sites in lib/doc-audit.ts — a FOURTH leg fails here
  ---
  duration_ms: 2.613917
  type: 'test'
  ...
# Subtest: OPD_AUDIT_LEGS equals the EXECUTABLE legs in auditOpdNote — a third fails here
ok 2807 - OPD_AUDIT_LEGS equals the EXECUTABLE legs in auditOpdNote — a third fails here
  ---
  duration_ms: 4.083791
  type: 'test'
  ...
# Subtest: THE IPD WORKER FITS ITS BOX, for every provider that can serve it
ok 2808 - THE IPD WORKER FITS ITS BOX, for every provider that can serve it
  ---
  duration_ms: 0.447083
  type: 'test'
  ...
# Subtest: ipd-audit-now fits the same box on the same basis (DEC-B5)
ok 2809 - ipd-audit-now fits the same box on the same basis (DEC-B5)
  ---
  duration_ms: 0.108667
  type: 'test'
  ...
# Subtest: THE OPD WORKER FITS ITS BOX, for every provider that can serve it
ok 2810 - THE OPD WORKER FITS ITS BOX, for every provider that can serve it
  ---
  duration_ms: 0.383875
  type: 'test'
  ...
# Subtest: the OPD call site sends a per-attempt ceiling that matches its budget (DEC-B9)
ok 2811 - the OPD call site sends a per-attempt ceiling that matches its budget (DEC-B9)
  ---
  duration_ms: 0.637875
  type: 'test'
  ...
# Subtest: raising max, lowering the box, or adding a retry FAILS the guard
ok 2812 - raising max, lowering the box, or adding a retry FAILS the guard
  ---
  duration_ms: 0.345167
  type: 'test'
  ...
# Subtest: a null budget means REFUSE, never substitute a default
ok 2813 - a null budget means REFUSE, never substitute a default
  ---
  duration_ms: 0.626375
  type: 'test'
  ...
# Subtest: the IPD cron interval clears the IPD box — and this is NOT extended to OPD
ok 2814 - the IPD cron interval clears the IPD box — and this is NOT extended to OPD
  ---
  duration_ms: 0.207625
  type: 'test'
  ...
# Subtest: OPENROUTER_TIMEOUT_MS and OPENROUTER_MAX_TRIES still read 110,000 and 3
ok 2815 - OPENROUTER_TIMEOUT_MS and OPENROUTER_MAX_TRIES still read 110,000 and 3
  ---
  duration_ms: 0.045416
  type: 'test'
  ...
# Subtest: check mode → Runs + Interventions + CW_Flags + Citations, normalized by run_id
ok 2816 - check mode → Runs + Interventions + CW_Flags + Citations, normalized by run_id
  ---
  duration_ms: 2.63825
  type: 'test'
  ...
# Subtest: pathway mode → PathwayStages merges skeleton+enrichment by id, ordered
ok 2817 - pathway mode → PathwayStages merges skeleton+enrichment by id, ordered
  ---
  duration_ms: 0.392209
  type: 'test'
  ...
# Subtest: audit mode → findings/completeness/diff/suggestions/idealised/extracted/citations
ok 2818 - audit mode → findings/completeness/diff/suggestions/idealised/extracted/citations
  ---
  duration_ms: 0.521583
  type: 'test'
  ...
# Subtest: mergeRunSheets stacks rows across runs by sheet name
ok 2819 - mergeRunSheets stacks rows across runs by sheet name
  ---
  duration_ms: 0.354667
  type: 'test'
  ...
# Subtest: THE INVARIANT: all-Standard weighting reproduces legacy completeness EXACTLY
ok 2820 - THE INVARIANT: all-Standard weighting reproduces legacy completeness EXACTLY
  ---
  duration_ms: 1.296917
  type: 'test'
  ...
# Subtest: THE INVARIANT holds for the null vector too (PRD §8.1 fallback = legacy behaviour)
ok 2821 - THE INVARIANT holds for the null vector too (PRD §8.1 fallback = legacy behaviour)
  ---
  duration_ms: 0.157584
  type: 'test'
  ...
# Subtest: THE INVARIANT holds at all-Minor as well (PRD §8.4: mathematically identical to all-Standard)
ok 2822 - THE INVARIANT holds at all-Minor as well (PRD §8.4: mathematically identical to all-Standard)
  ---
  duration_ms: 0.202917
  type: 'test'
  ...
# Subtest: the fixture set actually exercises the hard cases (guards against a vacuous invariant)
ok 2823 - the fixture set actually exercises the hard cases (guards against a vacuous invariant)
  ---
  duration_ms: 0.135917
  type: 'test'
  ...
# Subtest: the na-policy divergence is REAL and this build takes the legacy branch (flagged deviation)
ok 2824 - the na-policy divergence is REAL and this build takes the legacy branch (flagged deviation)
  ---
  duration_ms: 0.120667
  type: 'test'
  ...
# Subtest: a CONDITIONAL na leaves both sides under BOTH policies (mandatoryTotal 20 vs 21, PRD §2.9)
ok 2825 - a CONDITIONAL na leaves both sides under BOTH policies (mandatoryTotal 20 vs 21, PRD §2.9)
  ---
  duration_ms: 0.083833
  type: 'test'
  ...
# Subtest: tier points are exactly Critical 8 · Important 4 · Standard 2 · Minor 1, and none is zero
ok 2826 - tier points are exactly Critical 8 · Important 4 · Standard 2 · Minor 1, and none is zero
  ---
  duration_ms: 0.0965
  type: 'test'
  ...
# Subtest: normalised weights sum to 100.0 ± 0.05 for every combination (PRD §10)
ok 2827 - normalised weights sum to 100.0 ± 0.05 for every combination (PRD §10)
  ---
  duration_ms: 1.165666
  type: 'test'
  ...
# Subtest: weighting actually MOVES the score when tiers differ (the change is not a no-op)
ok 2828 - weighting actually MOVES the score when tiers differ (the change is not a no-op)
  ---
  duration_ms: 0.21925
  type: 'test'
  ...
# Subtest: partial is exactly 0.5, and na is not partial
ok 2829 - partial is exactly 0.5, and na is not partial
  ---
  duration_ms: 0.29375
  type: 'test'
  ...
# Subtest: all-na document returns 100 without dividing by zero (PRD §8.5)
ok 2830 - all-na document returns 100 without dividing by zero (PRD §8.5)
  ---
  duration_ms: 0.054834
  type: 'test'
  ...
# Subtest: unknown key defaults to Standard; empty/garbage vector falls back to equal weights (PRD §8.2)
ok 2831 - unknown key defaults to Standard; empty/garbage vector falls back to equal weights (PRD §8.2)
  ---
  duration_ms: 0.05575
  type: 'test'
  ...
# Subtest: malformed input never throws and never produces a wrong-looking score
ok 2832 - malformed input never throws and never produces a wrong-looking score
  ---
  duration_ms: 0.107042
  type: 'test'
  ...
# Subtest: rounding is half-up, applied via legacy's DOUBLE round
ok 2833 - rounding is half-up, applied via legacy's DOUBLE round
  ---
  duration_ms: 0.060209
  type: 'test'
  ...
# Subtest: missingMandatory lists applicable missing fields by label (the unweighted gap count)
ok 2834 - missingMandatory lists applicable missing fields by label (the unweighted gap count)
  ---
  duration_ms: 0.037166
  type: 'test'
  ...
# Subtest: legacyCompleteness (the independent path) agrees with the null-vector weighted path
ok 2835 - legacyCompleteness (the independent path) agrees with the null-vector weighted path
  ---
  duration_ms: 0.232625
  type: 'test'
  ...
# Subtest: the re-stated domain weights match the closed cores VERBATIM (drift guard)
ok 2836 - the re-stated domain weights match the closed cores VERBATIM (drift guard)
  ---
  duration_ms: 0.391833
  type: 'test'
  ...
# Subtest: OPD index reproduces the core formula on a worked case
ok 2837 - OPD index reproduces the core formula on a worked case
  ---
  duration_ms: 0.193083
  type: 'test'
  ...
# Subtest: PDQI-9 absent ⇒ note_quality drops and the divisor is 0.75 (PRD §2.6)
ok 2838 - PDQI-9 absent ⇒ note_quality drops and the divisor is 0.75 (PRD §2.6)
  ---
  duration_ms: 0.094667
  type: 'test'
  ...
# Subtest: Care-Value Index reproduces the six-domain formula
ok 2839 - Care-Value Index reproduces the six-domain formula
  ---
  duration_ms: 0.226208
  type: 'test'
  ...
# Subtest: substituting a new documentation score moves the index and can re-band
ok 2840 - substituting a new documentation score moves the index and can re-band
  ---
  duration_ms: 0.160416
  type: 'test'
  ...
# Subtest: band boundaries at 39/40, 54/55, 69/70, 84/85
ok 2841 - band boundaries at 39/40, 54/55, 69/70, 84/85
  ---
  duration_ms: 0.120042
  type: 'test'
  ...
# Subtest: no domain scores at all ⇒ index 0, not NaN
ok 2842 - no domain scores at all ⇒ index 0, not NaN
  ---
  duration_ms: 0.092792
  type: 'test'
  ...
# Subtest: the weights-version label is exact (PRD §2.8, §8.3)
ok 2843 - the weights-version label is exact (PRD §2.8, §8.3)
  ---
  duration_ms: 0.112541
  type: 'test'
  ...
# Subtest: preview: an unchanged candidate moves nothing
ok 2844 - preview: an unchanged candidate moves nothing
  ---
  duration_ms: 0.874792
  type: 'test'
  ...
# Subtest: preview: making a widely-missing field Critical moves the mean and reports movers
ok 2845 - preview: making a widely-missing field Critical moves the mean and reports movers
  ---
  duration_ms: 26.035542
  type: 'test'
  ...
# Subtest: preview: empty cohort yields zeroed stats, no throw (the OPD empty state)
ok 2846 - preview: empty cohort yields zeroed stats, no throw (the OPD empty state)
  ---
  duration_ms: 0.107208
  type: 'test'
  ...
# Subtest: preview: SD is population SD and a single row has SD 0
ok 2847 - preview: SD is population SD and a single row has SD 0
  ---
  duration_ms: 0.075542
  type: 'test'
  ...
# Subtest: missingPrevalence excludes `na` from the base, and reports a percentage
ok 2848 - missingPrevalence excludes `na` from the base, and reports a percentage
  ---
  duration_ms: 0.20225
  type: 'test'
  ...
# Subtest: systemic-defect warning fires only above 50% missing AND only at Critical; it never blocks
ok 2849 - systemic-defect warning fires only above 50% missing AND only at Critical; it never blocks
  ---
  duration_ms: 0.11725
  type: 'test'
  ...
# Subtest: the systemic-defect copy is verbatim per PRD §5.3
ok 2850 - the systemic-defect copy is verbatim per PRD §5.3
  ---
  duration_ms: 0.045333
  type: 'test'
  ...
# Subtest: scoreRow routes IPD and OPD to different index formulas
ok 2851 - scoreRow routes IPD and OPD to different index formulas
  ---
  duration_ms: 0.078875
  type: 'test'
  ...
# Subtest: the 21 discharge_summary fields match data/nabh-rubric.json EXACTLY (key, label, section)
ok 2852 - the 21 discharge_summary fields match data/nabh-rubric.json EXACTLY (key, label, section)
  ---
  duration_ms: 0.1945
  type: 'test'
  ...
# Subtest: cause_of_death is the ONE conditional key, read from the rubric
ok 2853 - cause_of_death is the ONE conditional key, read from the rubric
  ---
  duration_ms: 0.109167
  type: 'test'
  ...
# Subtest: the OPD label→key mapping covers every live-observed label (companion spec §4.7)
ok 2854 - the OPD label→key mapping covers every live-observed label (companion spec §4.7)
  ---
  duration_ms: 0.153125
  type: 'test'
  ...
# Subtest: the OPD engine's ACTUAL emitted keys are all in the catalogue (no orphan can appear)
ok 2855 - the OPD engine's ACTUAL emitted keys are all in the catalogue (no orphan can appear)
  ---
  duration_ms: 0.278583
  type: 'test'
  ...
# Subtest: the OPD structured emission is ADDITIVE: status/section added, present/mandatory preserved
ok 2856 - the OPD structured emission is ADDITIVE: status/section added, present/mandatory preserved
  ---
  duration_ms: 43.82
  type: 'test'
  ...
# Subtest: the OPD engine emits the structured shape from BOTH completeness paths (GP and obstetric)
ok 2857 - the OPD engine emits the structured shape from BOTH completeness paths (GP and obstetric)
  ---
  duration_ms: 0.632958
  type: 'test'
  ...
# The `fetchConnectionCache` option is deprecated (now always `true`)
# Subtest: A.1 THE FALLBACK RULE: a NULL completeness_items row keeps its stored scores, untouched
ok 2858 - A.1 THE FALLBACK RULE: a NULL completeness_items row keeps its stored scores, untouched
  ---
  duration_ms: 710.747333
  type: 'test'
  ...
# Subtest: A.1 a missing array is never read as 100 NOR as 0 — both directions
ok 2859 - A.1 a missing array is never read as 100 NOR as 0 — both directions
  ---
  duration_ms: 26.445917
  type: 'test'
  ...
# Subtest: A.1 a row WITH items is weighted, and weights_not_applicable flips to false
ok 2860 - A.1 a row WITH items is weighted, and weights_not_applicable flips to false
  ---
  duration_ms: 3.747375
  type: 'test'
  ...
# Subtest: A.1 continuity items are EXCLUDED from the OPD denominator (reproduces the engine's coverage)
ok 2861 - A.1 continuity items are EXCLUDED from the OPD denominator (reproduces the engine's coverage)
  ---
  duration_ms: 1.433625
  type: 'test'
  ...
# Subtest: A.1 applyOpdScoringPolicy never throws and handles an empty batch
ok 2862 - A.1 applyOpdScoringPolicy never throws and handles an empty batch
  ---
  duration_ms: 17.746541
  type: 'test'
  ...
# Subtest: A.1 parseOpdCompletenessItems drops malformed entries rather than throwing
ok 2863 - A.1 parseOpdCompletenessItems drops malformed entries rather than throwing
  ---
  duration_ms: 1.2405
  type: 'test'
  ...
# Subtest: A.1 the OPD write path persists the array, guarded by a column probe
ok 2864 - A.1 the OPD write path persists the array, guarded by a column probe
  ---
  duration_ms: 0.48575
  type: 'test'
  ...
# Subtest: A.1 the migration runner exists, is admin-guarded, and every statement is idempotent
ok 2865 - A.1 the migration runner exists, is admin-guarded, and every statement is idempotent
  ---
  duration_ms: 0.24975
  type: 'test'
  ...
# Subtest: A.1 the runner's inlined DDL matches the two .sql files it stands in for
ok 2866 - A.1 the runner's inlined DDL matches the two .sql files it stands in for
  ---
  duration_ms: 0.553709
  type: 'test'
  ...
# Subtest: A.1 no backfill: nothing in the build writes completeness_items to historical rows
ok 2867 - A.1 no backfill: nothing in the build writes completeness_items to historical rows
  ---
  duration_ms: 0.168917
  type: 'test'
  ...
# Subtest: the three continuity fields are EXCLUDED from the OPD weight vector (kickoff normative list)
ok 2868 - the three continuity fields are EXCLUDED from the OPD weight vector (kickoff normative list)
  ---
  duration_ms: 0.061375
  type: 'test'
  ...
# Subtest: the near-duplicate pairs are kept SEPARATE and flagged, not merged
ok 2869 - the near-duplicate pairs are kept SEPARATE and flagged, not merged
  ---
  duration_ms: 0.068917
  type: 'test'
  ...
# Subtest: labelToOpdKey: dynamic obstetric labels match by prefix; unknown returns null (never guesses)
ok 2870 - labelToOpdKey: dynamic obstetric labels match by prefix; unknown returns null (never guesses)
  ---
  duration_ms: 0.215042
  type: 'test'
  ...
# Subtest: every catalogued OPD label round-trips through the mapping
ok 2871 - every catalogued OPD label round-trips through the mapping
  ---
  duration_ms: 0.051458
  type: 'test'
  ...
# Subtest: fieldsFor / weightedKeysFor route by note type and never return an empty key space
ok 2872 - fieldsFor / weightedKeysFor route by note type and never return an empty key space
  ---
  duration_ms: 0.032792
  type: 'test'
  ...
# Subtest: diffVectors reports only real changes, with old → new tiers
ok 2873 - diffVectors reports only real changes, with old → new tiers
  ---
  duration_ms: 0.17925
  type: 'test'
  ...
# Subtest: vectorsEqual treats absent as Standard (so a seeded v1 equals an empty draft)
ok 2874 - vectorsEqual treats absent as Standard (so a seeded v1 equals an empty draft)
  ---
  duration_ms: 0.047834
  type: 'test'
  ...
# Subtest: validateVector rejects non-objects but coerces unknown tiers rather than failing
ok 2875 - validateVector rejects non-objects but coerces unknown tiers rather than failing
  ---
  duration_ms: 0.106
  type: 'test'
  ...
# Subtest: canonicalVectorJson is stable regardless of key insertion order
ok 2876 - canonicalVectorJson is stable regardless of key insertion order
  ---
  duration_ms: 0.050084
  type: 'test'
  ...
# Subtest: bySection groups and preserves first-seen order
ok 2877 - bySection groups and preserves first-seen order
  ---
  duration_ms: 0.080208
  type: 'test'
  ...
# Subtest: computeSignalHealth: FP rate, latest-per-doctor, top reasons, healable
ok 2878 - computeSignalHealth: FP rate, latest-per-doctor, top reasons, healable
  ---
  duration_ms: 1.138542
  type: 'test'
  ...
# Subtest: computeSignalHealth: ranks noisiest (audit_bug × rate) first
ok 2879 - computeSignalHealth: ranks noisiest (audit_bug × rate) first
  ---
  duration_ms: 0.17625
  type: 'test'
  ...
# Subtest: findingMatchesSuppression: type/scope/discriminator/active gates
ok 2880 - findingMatchesSuppression: type/scope/discriminator/active gates
  ---
  duration_ms: 0.122959
  type: 'test'
  ...
# Subtest: applySuppressions: drop removes, downgrade sets informational, no active = no-op
ok 2881 - applySuppressions: drop removes, downgrade sets informational, no active = no-op
  ---
  duration_ms: 0.188333
  type: 'test'
  ...
# Subtest: previewCollateral: dual-label invariant — refuses to remove a validated signal
ok 2882 - previewCollateral: dual-label invariant — refuses to remove a validated signal
  ---
  duration_ms: 0.103083
  type: 'test'
  ...
# Subtest: every ratified tier-2 kind maps to tier 2, not unlisted
ok 2883 - every ratified tier-2 kind maps to tier 2, not unlisted
  ---
  duration_ms: 1.113917
  type: 'test'
  ...
# Subtest: every ratified tier-3 kind maps to tier 3 — log only
ok 2884 - every ratified tier-3 kind maps to tier 3 — log only
  ---
  duration_ms: 0.13675
  type: 'test'
  ...
# Subtest: banned_fdc is ratified TIER 2 (not higher) and pregnancy_risk_verify is ratified TIER 3
ok 2885 - banned_fdc is ratified TIER 2 (not higher) and pregnancy_risk_verify is ratified TIER 3
  ---
  duration_ms: 0.052417
  type: 'test'
  ...
# Subtest: O1c: a model-invented kind lands in tier 2 and is flagged unlisted
ok 2886 - O1c: a model-invented kind lands in tier 2 and is flagged unlisted
  ---
...
ok 2887 - praise: *_high_value kinds and any high-value verdict (antibiotic_stewardship praise) are excluded and counted
  ---
  duration_ms: 0.048875
  type: 'test'
  ...
# Subtest: antibiotic_stewardship VIOLATION (low-value — antibiotic for a viral URTI) is tier 2
ok 2888 - antibiotic_stewardship VIOLATION (low-value — antibiotic for a viral URTI) is tier 2
  ---
...
ok 2889 - incomplete_dosing: missing strength / duration alone → tier 3 (ratified rows: findings 20, 24, 37, 41 + chronic continuation)
  ---
  duration_ms: 0.390959
  type: 'test'
  ...
# Subtest: incomplete_dosing: a missing frequency or route changes what the patient does → tier 2; unparseable → tier 2
ok 2890 - incomplete_dosing: a missing frequency or route changes what the patient does → tier 2; unparseable → tier 2
  ---
...
ok 2891 - E-1 (finding 36): a time-critical cardiac pattern in the finding text promotes to tier 1 — from any kind
  ---
  duration_ms: 0.19825
  type: 'test'
  ...
# Subtest: E-2 (finding 49): persistent swelling ≥ 4 weeks with no follow-through promotes; with follow-through it does not
ok 2892 - E-2 (finding 49): persistent swelling ≥ 4 weeks with no follow-through promotes; with follow-through it does not
  ---
  duration_ms: 1.402
  type: 'test'
  ...
# Subtest: praise never escalates: a high-value finding praising an appropriate ACS referral stays praise
ok 2893 - praise never escalates: a high-value finding praising an appropriate ACS referral stays praise
  ---
  duration_ms: 0.052375
  type: 'test'
  ...
# Subtest: bucketByTier: buckets are disjoint, complete, and count unlisted kinds
ok 2894 - bucketByTier: buckets are disjoint, complete, and count unlisted kinds
  ---
  duration_ms: 0.078
  type: 'test'
  ...
...
ok 2895 - dedupeTwins: same (finding_ref, doctor_uid, day) collapses with an occurrence count; different notes same day still collapse; different days do not
  ---
  duration_ms: 0.3635
  type: 'test'
  ...
# Subtest: dedupeTwins: unkeyable rows (no ref / no doctor / no date) never merge
ok 2896 - dedupeTwins: unkeyable rows (no ref / no doctor / no date) never merge
  ---
  duration_ms: 0.03875
  type: 'test'
  ...
# Subtest: allows SELECT / WITH and auto-adds LIMIT
ok 2897 - allows SELECT / WITH and auto-adds LIMIT
  ---
  duration_ms: 0.996666
  type: 'test'
  ...
# Subtest: rejects writes, DDL, multiple statements, non-SELECT, over-cap LIMIT, system fns
ok 2898 - rejects writes, DDL, multiple statements, non-SELECT, over-cap LIMIT, system fns
  ---
  duration_ms: 0.093875
  type: 'test'
  ...
# Subtest: blocks PHI-bearing relations anywhere in the query
ok 2899 - blocks PHI-bearing relations anywhere in the query
  ---
  duration_ms: 0.105375
  type: 'test'
  ...
# Subtest: honors a smaller caller cap
ok 2900 - honors a smaller caller cap
  ---
...
ok 2901 - coverage floor (PRD §4): ≥5 dose-ceiling, ≥3 SOS, ≥3 banned-FDC, ≥4 interaction, ≥4 incomplete-dosing positives; exactly 6 negatives
  ---
  duration_ms: 1.92075
  type: 'test'
  ...
# Subtest: fixtures carry no PHI, no db13 uid, and banned-FDC fixtures use placeholder molecules only
ok 2902 - fixtures carry no PHI, no db13 uid, and banned-FDC fixtures use placeholder molecules only
  ---
  duration_ms: 0.474166
  type: 'test'
  ...
...
ok 2903 - positive POS-DOSE-1 fires dose_ceiling_exceeded — paracetamol stacked across two products: 650 QID + 500 TDS = 4100 mg/day > 4000
  ---
...
ok 2904 - positive POS-DOSE-2 fires dose_ceiling_exceeded — ibuprofen 800 QID = 3200 mg/day > 2400 (single product)
  ---
  duration_ms: 5.567625
  type: 'test'
  ...
# Subtest: positive POS-DOSE-3 fires dose_ceiling_exceeded — diclofenac 75 TDS = 225 mg/day > 150
ok 2905 - positive POS-DOSE-3 fires dose_ceiling_exceeded — diclofenac 75 TDS = 225 mg/day > 150
  ---
  duration_ms: 0.111375
  type: 'test'
  ...
...
ok 2906 - positive POS-DOSE-4 fires dose_ceiling_exceeded — etoricoxib 120 OD with NO documented gout > the 90 mg/day default ceiling (Decision 6)
  ---
  duration_ms: 0.074667
  type: 'test'
  ...
# Subtest: positive POS-DOSE-5 fires dose_ceiling_exceeded — mefenamic acid 500 QID = 2000 mg/day > 1500
ok 2907 - positive POS-DOSE-5 fires dose_ceiling_exceeded — mefenamic acid 500 QID = 2000 mg/day > 1500
  ---
  duration_ms: 0.220208
  type: 'test'
  ...
...
ok 2908 - positive POS-SOS-1 fires dose_ceiling_sos — paracetamol 1000 TDS scheduled + 650 SOS uncapped (default cap 3) → 4950 potential > 4000
  ---
  duration_ms: 0.074417
  type: 'test'
  ...
# Subtest: positive POS-SOS-2 fires dose_ceiling_sos — etoricoxib 90 SOS with an EXPLICIT max 2/day → 180 potential > 90
ok 2909 - positive POS-SOS-2 fires dose_ceiling_sos — etoricoxib 90 SOS with an EXPLICIT max 2/day → 180 potential > 90
  ---
...
ok 2910 - positive POS-SOS-3 fires dose_ceiling_sos — ibuprofen 600 grid 1-0-1 + 600 SOS uncapped → 3000 potential > 2400
  ---
  duration_ms: 3.224291
  type: 'test'
  ...
# Subtest: positive POS-FDC-1 fires banned_fdc — exact two-molecule banned set (placeholders mol-a + mol-b)
ok 2911 - positive POS-FDC-1 fires banned_fdc — exact two-molecule banned set (placeholders mol-a + mol-b)
  ---
...
ok 2912 - positive POS-FDC-2 fires banned_fdc — exact three-molecule banned set (placeholders mol-c/mol-d/mol-e)
  ---
  duration_ms: 0.03575
  type: 'test'
  ...
# Subtest: positive POS-FDC-3 fires banned_fdc — order-swapped banned pair (mol-b + mol-a) still matches the stored set
ok 2913 - positive POS-FDC-3 fires banned_fdc — order-swapped banned pair (mol-b + mol-a) still matches the stored set
  ---
  duration_ms: 0.022709
  type: 'test'
  ...
# Subtest: positive POS-DDI-1 fires drug_interaction — warfarin + ibuprofen — anticoagulant + NSAID (major)
ok 2914 - positive POS-DDI-1 fires drug_interaction — warfarin + ibuprofen — anticoagulant + NSAID (major)
  ---
  duration_ms: 0.021
  type: 'test'
  ...
# Subtest: positive POS-DDI-2 fires drug_interaction — atorvastatin + clarithromycin — statin + macrolide (major)
ok 2915 - positive POS-DDI-2 fires drug_interaction — atorvastatin + clarithromycin — statin + macrolide (major)
  ---
  duration_ms: 0.019958
  type: 'test'
  ...
# Subtest: positive POS-DDI-3 fires drug_interaction — sertraline + tramadol — two serotonergic drugs (major)
ok 2916 - positive POS-DDI-3 fires drug_interaction — sertraline + tramadol — two serotonergic drugs (major)
  ---
  duration_ms: 0.020917
  type: 'test'
  ...
# Subtest: positive POS-DDI-4 fires drug_interaction — telmisartan + spironolactone — ACE-I/ARB + potassium-sparing diuretic (major)
ok 2917 - positive POS-DDI-4 fires drug_interaction — telmisartan + spironolactone — ACE-I/ARB + potassium-sparing diuretic (major)
  ---
  duration_ms: 0.021834
  type: 'test'
  ...
# Subtest: positive POS-DOSING-1 fires incomplete_dosing — dose/strength blanked (no dose field, no strength, none in the name)
ok 2918 - positive POS-DOSING-1 fires incomplete_dosing — dose/strength blanked (no dose field, no strength, none in the name)
  ---
  duration_ms: 0.023875
  type: 'test'
  ...
# Subtest: positive POS-DOSING-2 fires incomplete_dosing — frequency blanked
ok 2919 - positive POS-DOSING-2 fires incomplete_dosing — frequency blanked
  ---
  duration_ms: 0.019833
  type: 'test'
  ...
# Subtest: positive POS-DOSING-3 fires incomplete_dosing — duration blanked
ok 2920 - positive POS-DOSING-3 fires incomplete_dosing — duration blanked
  ---
  duration_ms: 0.019625
  type: 'test'
  ...
...
ok 2921 - positive POS-DOSING-4 fires incomplete_dosing — route blanked and not inferable (no dosage-form word anywhere on the line)
  ---
  duration_ms: 0.019834
  type: 'test'
  ...
# Subtest: negative NEG-1 stays silent — ibuprofen 800 TDS = exactly 2400 mg/day — AT the ceiling, not over it
ok 2922 - negative NEG-1 stays silent — ibuprofen 800 TDS = exactly 2400 mg/day — AT the ceiling, not over it
  ---
...
ok 2923 - negative NEG-2 stays silent — etoricoxib 120 OD WITH a documented gout diagnosis — the conditional 120 ceiling applies
  ---
...
ok 2924 - negative NEG-3 stays silent — amoxicillin + paracetamol — just OUTSIDE every interaction pair (no shared mechanism tag)
  ---
  duration_ms: 0.021
  type: 'test'
  ...
# Subtest: negative NEG-4 stays silent — a COMPLETE prescription — dose, frequency, duration and route all present
ok 2925 - negative NEG-4 stays silent — a COMPLETE prescription — dose, frequency, duration and route all present
  ---
  duration_ms: 0.019542
  type: 'test'
  ...
# Subtest: negative NEG-5 stays silent — banned core + one extra molecule (mol-a + mol-b + mol-z) — the C5 superset boundary
ok 2926 - negative NEG-5 stays silent — banned core + one extra molecule (mol-a + mol-b + mol-z) — the C5 superset boundary
  ---
  duration_ms: 0.023
  type: 'test'
  ...
# Subtest: negative NEG-6 stays silent — paracetamol 500 SOS max 3/day = 1500 mg potential — well inside the 4000 ceiling
ok 2927 - negative NEG-6 stays silent — paracetamol 500 SOS max 3/day = 1500 mg potential — well inside the 4000 ceiling
  ---
  duration_ms: 0.02
  type: 'test'
  ...
# Subtest: recall_det = fired / planted, over the deterministic leg only (no LLM recall claim — PRD §6)
ok 2928 - recall_det = fired / planted, over the deterministic leg only (no LLM recall claim — PRD §6)
  ---
  duration_ms: 0.036166
  type: 'test'
  ...
# Subtest: 57 case 1 — production Vercel build with no key at all: MISSING
ok 2929 - 57 case 1 — production Vercel build with no key at all: MISSING
  ---
  duration_ms: 1.317208
  type: 'test'
  ...
# Subtest: 57 case 2 — production Vercel build with an unusable key (empty or whitespace): MISSING
ok 2930 - 57 case 2 — production Vercel build with an unusable key (empty or whitespace): MISSING
  ---
  duration_ms: 0.175667
  type: 'test'
  ...
# Subtest: 57 case 3 — production Vercel build with a usable key: NOT missing
ok 2931 - 57 case 3 — production Vercel build with a usable key: NOT missing
  ---
  duration_ms: 0.140417
  type: 'test'
  ...
# Subtest: 57 case 4 — a Vercel build that is not production: NOT missing, at any key value
ok 2932 - 57 case 4 — a Vercel build that is not production: NOT missing, at any key value
  ---
  duration_ms: 0.292667
  type: 'test'
  ...
# Subtest: 57 case 5 — not a Vercel build, even when the environment says production: NOT missing
ok 2933 - 57 case 5 — not a Vercel build, even when the environment says production: NOT missing
  ---
  duration_ms: 0.153208
  type: 'test'
  ...
# Subtest: 57 EXECUTED — importing the config in production with no key throws, and names the variable
ok 2934 - 57 EXECUTED — importing the config in production with no key throws, and names the variable
  ---
  duration_ms: 49.911958
  type: 'test'
  ...
# Subtest: 57 EXECUTED — a production import WITH a key succeeds
ok 2935 - 57 EXECUTED — a production import WITH a key succeeds
  ---
  duration_ms: 57.509625
  type: 'test'
  ...
# Subtest: 57 EXECUTED — a non-production import with no key succeeds
ok 2936 - 57 EXECUTED — a non-production import with no key succeeds
  ---
  duration_ms: 43.475291
  type: 'test'
  ...
# Subtest: 57 whole file — it parses, and holds EXACTLY three top-level statements in order
ok 2937 - 57 whole file — it parses, and holds EXACTLY three top-level statements in order
  ---
  duration_ms: 21.083458
  type: 'test'
  ...
# Subtest: 57 whole file — the declaration and the export are exactly what they must be
ok 2938 - 57 whole file — the declaration and the export are exactly what they must be
  ---
  duration_ms: 0.839875
  type: 'test'
  ...
# Subtest: 57 whole file — nothing executable outside the guard
ok 2939 - 57 whole file — nothing executable outside the guard
  ---
  duration_ms: 0.288709
  type: 'test'
  ...
# Subtest: 57 pin — each copy is exactly D8's three clauses, and there is no fourth of any kind
ok 2940 - 57 pin — each copy is exactly D8's three clauses, and there is no fourth of any kind
  ---
  duration_ms: 0.76625
  type: 'test'
  ...
# Subtest: 57 pin — next.config.mjs and telemetry-key-guard.ts express the SAME condition
ok 2941 - 57 pin — next.config.mjs and telemetry-key-guard.ts express the SAME condition
  ---
  duration_ms: 0.212084
  type: 'test'
  ...
# Subtest: 57 pin — the guard THROWS one Error, and the message a reader sees names the variable
ok 2942 - 57 pin — the guard THROWS one Error, and the message a reader sees names the variable
  ---
  duration_ms: 0.153625
  type: 'test'
  ...
# Subtest: no surface outside the allow-list SELECTs from the telemetry tables
ok 2943 - no surface outside the allow-list SELECTs from the telemetry tables
  ---
  duration_ms: 155.467917
  type: 'test'
  ...
# Subtest: the scan can actually fail — it is not passing because the matcher never matches
ok 2944 - the scan can actually fail — it is not passing because the matcher never matches
  ---
  duration_ms: 0.120708
  type: 'test'
  ...
# Subtest: the allow-list is by EXACT path, and every entry is one this build owns
ok 2945 - the allow-list is by EXACT path, and every entry is one this build owns
  ---
  duration_ms: 0.169
  type: 'test'
  ...
# Subtest: no clinician-facing or patient-facing route names a telemetry table at all
ok 2946 - no clinician-facing or patient-facing route names a telemetry table at all
  ---
  duration_ms: 21.120209
  type: 'test'
  ...
# Subtest: GUARD 1 — admin: a set ADMIN_TOKEN with nothing presented is refused, and isAdminUnlocked is NOT consulted
ok 2947 - GUARD 1 — admin: a set ADMIN_TOKEN with nothing presented is refused, and isAdminUnlocked is NOT consulted
  ---
  duration_ms: 270.0085
  type: 'test'
  ...
# Subtest: GUARD 3 — preview: VERCEL_ENV=production is refused even with everything else correct
ok 2948 - GUARD 3 — preview: VERCEL_ENV=production is refused even with everything else correct
  ---
  duration_ms: 257.517625
  type: 'test'
  ...
# Subtest: GUARD 3 — preview: VERCEL_ENV=preview on a DIFFERENT branch is refused
ok 2949 - GUARD 3 — preview: VERCEL_ENV=preview on a DIFFERENT branch is refused
  ---
  duration_ms: 285.47775
  type: 'test'
  ...
# Subtest: GUARD 4 — arming: an unset CDMSS_OVERHEAD_MEASURE is refused
ok 2950 - GUARD 4 — arming: an unset CDMSS_OVERHEAD_MEASURE is refused
  ---
  duration_ms: 224.93475
  type: 'test'
  ...
# Subtest: GUARD 5 — THE ONE THAT MATTERS: a production endpoint id is refused
ok 2951 - GUARD 5 — THE ONE THAT MATTERS: a production endpoint id is refused
  ---
  duration_ms: 159.724625
  type: 'test'
  ...
# Subtest: GUARD 5 — an UNSET expectation refuses, it does not pass
ok 2952 - GUARD 5 — an UNSET expectation refuses, it does not pass
  ---
  duration_ms: 138.244166
  type: 'test'
  ...
# Subtest: GUARD 5 — an unparseable DATABASE_URL refuses
ok 2953 - GUARD 5 — an unparseable DATABASE_URL refuses
  ---
  duration_ms: 150.212
  type: 'test'
  ...
# Subtest: GUARD 5 — a password containing @ cannot shift the parsed host
ok 2954 - GUARD 5 — a password containing @ cannot shift the parsed host
  ---
  duration_ms: 154.034166
  type: 'test'
  ...
# Subtest: GUARD 2 — expiry: past the hard UTC date every request is 410
ok 2955 - GUARD 2 — expiry: past the hard UTC date every request is 410
  ---
  duration_ms: 150.026542
  type: 'test'
  ...
# Subtest: GUARD 2 — before the expiry the route still runs
ok 2956 - GUARD 2 — before the expiry the route still runs
  ---
  duration_ms: 213.219375
  type: 'test'
  ...
# Subtest: ALL FIVE PASS — the route runs, writes route=script, and reports the first sample separately
ok 2957 - ALL FIVE PASS — the route runs, writes route=script, and reports the first sample separately
  ---
  duration_ms: 148.091542
  type: 'test'
  ...
# Subtest: FIX 2 + FIX 1 — a REAL 500, driven by an unparseable URL that still satisfies guard 5
ok 2958 - FIX 2 + FIX 1 — a REAL 500, driven by an unparseable URL that still satisfies guard 5
  ---
  duration_ms: 140.986834
  type: 'test'
  ...
# Subtest: v5 FIX 1 — two of the three leaking shapes are refused BEFORE the driver ever sees them
ok 2959 - v5 FIX 1 — two of the three leaking shapes are refused BEFORE the driver ever sees them
  ---
  duration_ms: 305.13
  type: 'test'
  ...
# Subtest: FIX 3 — a query parameter cannot move the parsed host away from the one the driver uses
ok 2960 - FIX 3 — a query parameter cannot move the parsed host away from the one the driver uses
  ---
  duration_ms: 152.95125
  type: 'test'
  ...
# Subtest: FIX 4 — the denylist refuses production even when the expected value also names it
ok 2961 - FIX 4 — the denylist refuses production even when the expected value also names it
  ---
  duration_ms: 149.378459
  type: 'test'
  ...
# Subtest: FIX 4 — an ABSENT denylist refuses: a denylist that is not there is not a denylist
ok 2962 - FIX 4 — an ABSENT denylist refuses: a denylist that is not there is not a denylist
  ---
  duration_ms: 144.525083
  type: 'test'
  ...
# Subtest: FIX 5 — p95 and p99 are withheld below their floors, never a maximum in disguise
ok 2963 - FIX 5 — p95 and p99 are withheld below their floors, never a maximum in disguise
  ---
  duration_ms: 293.446833
  type: 'test'
  ...
# Subtest: FIX 5 — and p95 IS emitted once its floor is met, so the floor is not a blanket refusal
ok 2964 - FIX 5 — and p95 IS emitted once its floor is met, so the floor is not a blanket refusal
  ---
  duration_ms: 155.556458
  type: 'test'
  ...
# Subtest: FIX 6 — the invocation insert has its own cell
ok 2965 - FIX 6 — the invocation insert has its own cell
  ---
  duration_ms: 311.636
  type: 'test'
  ...
# Subtest: FIX 7 — the shape is true per cell, and conc is gone
ok 2966 - FIX 7 — the shape is true per cell, and conc is gone
  ---
  duration_ms: 301.55375
  type: 'test'
  ...
# Subtest: FIX 8 — the real-audit arm refuses rather than silently becoming the null arm
ok 2967 - FIX 8 — the real-audit arm refuses rather than silently becoming the null arm
  ---
  duration_ms: 443.255375
  type: 'test'
  ...
# Subtest: v6 FIX 1 — the branch POOLED host passes against the bare expected id
ok 2968 - v6 FIX 1 — the branch POOLED host passes against the bare expected id
  ---
  duration_ms: 157.662584
  type: 'test'
  ...
# Subtest: v6 FIX 1 — the branch DIRECT host passes against the bare expected id
ok 2969 - v6 FIX 1 — the branch DIRECT host passes against the bare expected id
  ---
  duration_ms: 153.813041
  type: 'test'
  ...
# Subtest: v6 FIX 1 — the branch POOLED host passes against a POOLED expected id
ok 2970 - v6 FIX 1 — the branch POOLED host passes against a POOLED expected id
  ---
  duration_ms: 147.417542
  type: 'test'
  ...
# Subtest: v6 FIX 2 — production on its POOLED host refuses forbidden_endpoint, NOT endpoint_mismatch
ok 2971 - v6 FIX 2 — production on its POOLED host refuses forbidden_endpoint, NOT endpoint_mismatch
  ---
  duration_ms: 144.530541
  type: 'test'
  ...
# Subtest: v6 FIX 2 — production on its DIRECT host also refuses forbidden_endpoint
ok 2972 - v6 FIX 2 — production on its DIRECT host also refuses forbidden_endpoint
  ---
  duration_ms: 139.518291
  type: 'test'
  ...
# Subtest: v6 — `pooler` in the MIDDLE of a label is part of the id and is never truncated
ok 2973 - v6 — `pooler` in the MIDDLE of a label is part of the id and is never truncated
  ---
  duration_ms: 315.700667
  type: 'test'
  ...
# Subtest: v6 — a doubled `-pooler-pooler` strips exactly ONE, and that is the stated rule
ok 2974 - v6 — a doubled `-pooler-pooler` strips exactly ONE, and that is the stated rule
  ---
  duration_ms: 453.688958
  type: 'test'
  ...
# Subtest: v6 — normalisation cannot make two DIFFERENT endpoints compare equal
ok 2975 - v6 — normalisation cannot make two DIFFERENT endpoints compare equal
  ---
  duration_ms: 577.418167
  type: 'test'
  ...
# Subtest: NO OUTPUT ANYWHERE CARRIES A DATABASE_URL SUBSTRING — every response shape, stdout and stderr
ok 2976 - NO OUTPUT ANYWHERE CARRIES A DATABASE_URL SUBSTRING — every response shape, stdout and stderr
  ---
  duration_ms: 1622.696292
  type: 'test'
  ...
# Subtest: the route is POST-only, and carries its own expiry and deletion notice in source
ok 2977 - the route is POST-only, and carries its own expiry and deletion notice in source
  ---
...
ok 3008 - a total transport failure records no served provider, and says so explicitly
  ---
  duration_ms: 0.816084
  type: 'test'
  ...
# Subtest: a null attempt list means NOT COLLECTED, and is distinguishable from an empty one
ok 3009 - a null attempt list means NOT COLLECTED, and is distinguishable from an empty one
  ---
  duration_ms: 0.437416
  type: 'test'
  ...
# Subtest: every terminal phase is a stable NAME — never a message, never an interpolated value
ok 3010 - every terminal phase is a stable NAME — never a message, never an interpolated value
  ---
  duration_ms: 1.344542
  type: 'test'
  ...
# Subtest: the intended-local path records ONE attempt rather than an empty list
ok 3011 - the intended-local path records ONE attempt rather than an empty list
  ---
  duration_ms: 0.522417
  type: 'test'
  ...
# Subtest: both local arms record their attempt — the by-design one and the substitution
ok 3012 - both local arms record their attempt — the by-design one and the substitution
  ---
  duration_ms: 0.508333
  type: 'test'
  ...
# Subtest: the local success attempt is well-formed, and is at most one per invocation
ok 3013 - the local success attempt is well-formed, and is at most one per invocation
  ---
  duration_ms: 0.638125
  type: 'test'
  ...
# Subtest: classifyLocalAttempt reads what the SDK declared, and guesses nothing
ok 3014 - classifyLocalAttempt reads what the SDK declared, and guesses nothing
  ---
  duration_ms: 0.338625
  type: 'test'
  ...
# Subtest: the 429 rule has exactly one home — classifyLocalAttempt delegates, never duplicates
ok 3015 - the 429 rule has exactly one home — classifyLocalAttempt delegates, never duplicates
  ---
  duration_ms: 0.330792
  type: 'test'
  ...
# Subtest: the two terminal throws are BYTE-IDENTICAL to before this build
ok 3016 - the two terminal throws are BYTE-IDENTICAL to before this build
  ---
  duration_ms: 0.394834
  type: 'test'
  ...
# Subtest: the phase selector cannot drift from the throws it describes
ok 3017 - the phase selector cannot drift from the throws it describes
  ---
  duration_ms: 0.545209
  type: 'test'
  ...
# Subtest: the failure attach is a statement, not a control-flow change
ok 3018 - the failure attach is a statement, not a control-flow change
  ---
  duration_ms: 0.269291
  type: 'test'
  ...
# Subtest: nothing in the failure path touches the outbound request object
ok 3019 - nothing in the failure path touches the outbound request object
  ---
  duration_ms: 0.234459
  type: 'test'
  ...
# Subtest: the local calls are wrapped, and the SDK call expression itself is unchanged
ok 3020 - the local calls are wrapped, and the SDK call expression itself is unchanged
  ---
  duration_ms: 0.197167
  type: 'test'
  ...
# Subtest: failure evidence is a SEPARATE property, invisible to every success-attribution reader
ok 3021 - failure evidence is a SEPARATE property, invisible to every success-attribution reader
  ---
  duration_ms: 0.088083
  type: 'test'
  ...
# Subtest: the low-value-care judge reader is untouched — it reads two fields of the success shape
ok 3022 - the low-value-care judge reader is untouched — it reads two fields of the success shape
  ---
  duration_ms: 0.195
  type: 'test'
  ...
# Subtest: tracedChat is not touched — D14 is scoped to the traceless arm
ok 3023 - tracedChat is not touched — D14 is scoped to the traceless arm
  ---
  duration_ms: 0.2885
  type: 'test'
  ...
# Subtest: every new name also resolves through lib/trace.ts, so no importer has to know the core path
ok 3024 - every new name also resolves through lib/trace.ts, so no importer has to know the core path
  ---
  duration_ms: 0.049208
  type: 'test'
  ...
# Subtest: failure evidence is IMMUTABLE — a later frame cannot rewrite what failed
ok 3025 - failure evidence is IMMUTABLE — a later frame cannot rewrite what failed
  ---
  duration_ms: 0.091917
  type: 'test'
  ...
# Subtest: a hostile or exotic error cannot break the transport
ok 3026 - a hostile or exotic error cannot break the transport
  ---
  duration_ms: 0.121792
  type: 'test'
  ...
# Subtest: failure evidence carries enums and counts only — no message, no body, no identifier
ok 3027 - failure evidence carries enums and counts only — no message, no body, no identifier
  ---
  duration_ms: 0.090958
  type: 'test'
  ...
# Subtest: the attempt shape admits the local provider, and nothing else new
ok 3028 - the attempt shape admits the local provider, and nothing else new
  ---
  duration_ms: 1.159167
  type: 'test'
  ...
# Subtest: a finish_reason defect is NOT retryable; an empty 200 still is
ok 3029 - a finish_reason defect is NOT retryable; an empty 200 still is
  ---
  duration_ms: 0.597292
  type: 'test'
  ...
# Subtest: THE 54 SECONDS: a truncating call is attempted ONCE, not three times
ok 3030 - THE 54 SECONDS: a truncating call is attempted ONCE, not three times
  ---
  duration_ms: 1.250458
  type: 'test'
  ...
# Subtest: …and the empty-200 retry budget is spent in full, exactly as before
ok 3031 - …and the empty-200 retry budget is spent in full, exactly as before
  ---
  duration_ms: 2.376875
  type: 'test'
  ...
# Subtest: the terminal error still names the truncation, so the sizing bug is readable
ok 3032 - the terminal error still names the truncation, so the sizing bug is readable
  ---
  duration_ms: 0.314
  type: 'test'
  ...
# Subtest: transport failures are untouched by this rule — only BODY verdicts changed
ok 3033 - transport failures are untouched by this rule — only BODY verdicts changed
  ---
  duration_ms: 0.886208
  type: 'test'
  ...
# Subtest: the mini-sized cap is raised to a FLOOR on the bedrock path
ok 3034 - the mini-sized cap is raised to a FLOOR on the bedrock path
  ---
  duration_ms: 0.348792
  type: 'test'
  ...
# Subtest: ⚠️ BYTE-IDENTITY: the floor is the BEDROCK transport’s, and reaches no other provider
ok 3035 - ⚠️ BYTE-IDENTITY: the floor is the BEDROCK transport’s, and reaches no other provider
  ---
  duration_ms: 1.607667
  type: 'test'
  ...
# Subtest: a critique that never completed is recorded as UNAUDITED, not as clean
ok 3036 - a critique that never completed is recorded as UNAUDITED, not as clean
  ---
  duration_ms: 0.589083
  type: 'test'
  ...
# Subtest: the probe reducers carry critic_ran, so a lab row can tell the two apart
ok 3037 - the probe reducers carry critic_ran, so a lab row can tell the two apart
  ---
  duration_ms: 1.112834
  type: 'test'
  ...
# Subtest: unknown_finding + missing_critical + instability_input derive from a ClinicalState
ok 3038 - unknown_finding + missing_critical + instability_input derive from a ClinicalState
  ---
  duration_ms: 11.680791
  type: 'test'
  ...
# Subtest: med_contradiction derives from an open medication conflict (member stateRef)
ok 3039 - med_contradiction derives from an open medication conflict (member stateRef)
  ---
  duration_ms: 0.318416
  type: 'test'
  ...
# Subtest: med_contradiction: conflict on an episode HIGH-ALERT med is safety-critical
ok 3040 - med_contradiction: conflict on an episode HIGH-ALERT med is safety-critical
  ---
...
ok 3041 - med_contradiction also derives from reconciled status stopped/not_taking/unknown without a conflict row
  ---
  duration_ms: 0.554875
  type: 'test'
  ...
# Subtest: new_medication (B5): derives ONLY for meds absent from a NON-EMPTY snapshot med list
ok 3042 - new_medication (B5): derives ONLY for meds absent from a NON-EMPTY snapshot med list
  ---
  duration_ms: 0.396458
  type: 'test'
  ...
# Subtest: new_medication (B5): a high-alert episode med is skipped (it wins rank 0 anyway)
ok 3043 - new_medication (B5): a high-alert episode med is skipped (it wins rank 0 anyway)
  ---
  duration_ms: 0.089458
  type: 'test'
  ...
# Subtest: care_gap derives from a stale mapped-range abnormal (detail verbatim, severity mapped)
ok 3044 - care_gap derives from a stale mapped-range abnormal (detail verbatim, severity mapped)
  ---
  duration_ms: 0.380792
  type: 'test'
  ...
# Subtest: followup_open derives from advice keywords; suppressed when a committed follow-up matches
ok 3045 - followup_open derives from advice keywords; suppressed when a committed follow-up matches
  ---
  duration_ms: 0.301958
  type: 'test'
  ...
# Subtest: allergy_unconfirmed only when the note allergy field is blank
ok 3046 - allergy_unconfirmed only when the note allergy field is blank
  ---
  duration_ms: 0.216667
  type: 'test'
  ...
# Subtest: snapshot absent ⇒ member-derived kinds simply absent (episode-only degradation, D14)
ok 3047 - snapshot absent ⇒ member-derived kinds simply absent (episode-only degradation, D14)
  ---
  duration_ms: 0.313083
  type: 'test'
  ...
# Subtest: determinism: identical inputs ⇒ deep-equal output (double run)
ok 3048 - determinism: identical inputs ⇒ deep-equal output (double run)
  ---
  duration_ms: 0.165625
  type: 'test'
  ...
# Subtest: every UnknownItem carries ≥1 sourceRef and a stateRef
ok 3049 - every UnknownItem carries ≥1 sourceRef and a stateRef
  ---
  duration_ms: 0.110042
  type: 'test'
  ...
# Subtest: stable ordering: safety before review before info, then kind, then subject
ok 3050 - stable ordering: safety before review before info, then kind, then subject
  ---
  duration_ms: 0.175333
  type: 'test'
  ...
# Subtest: bandFor thresholds
ok 3051 - bandFor thresholds
  ---
  duration_ms: 0.417708
  type: 'test'
  ...
# Subtest: findingPenalty scales with verdict severity and confidence
ok 3052 - findingPenalty scales with verdict severity and confidence
  ---
  duration_ms: 0.093542
  type: 'test'
  ...
# Subtest: a clean, complete episode scores high (band A)
ok 3053 - a clean, complete episode scores high (band A)
  ---
  duration_ms: 0.277834
  type: 'test'
  ...
# Subtest: domains route by tag; cost driven by low-value tariff spend; untagged → appropriateness
ok 3054 - domains route by tag; cost driven by low-value tariff spend; untagged → appropriateness
  ---
  duration_ms: 11.649666
  type: 'test'
  ...
# Subtest: estimated bed-day cost dents the cost domain even with no tariffed spend
ok 3055 - estimated bed-day cost dents the cost domain even with no tariffed spend
  ---
  duration_ms: 0.229834
  type: 'test'
  ...
# Subtest: weights are configurable and normalised
ok 3056 - weights are configurable and normalised
  ---
...
ok 3057 - cloudLadder: Vertex first, OpenRouter second — and GEMINI_VIA_OPENROUTER=1 inverts it
  ---
  duration_ms: 100.870333
  type: 'test'
  ...
# Subtest: cloudLadder: a second tier exists ONLY with a leg budget, and only when it can serve
ok 3058 - cloudLadder: a second tier exists ONLY with a leg budget, and only when it can serve
  ---
  duration_ms: 0.194833
  type: 'test'
  ...
# Subtest: the tier-2 slug derivation is the same google/ prefixing, flag or no flag
ok 3059 - the tier-2 slug derivation is the same google/ prefixing, flag or no flag
  ---
  duration_ms: 0.074583
  type: 'test'
  ...
# Subtest: the flag itself is NOT touched by this unit — one code read, no default, no write
ok 3060 - the flag itself is NOT touched by this unit — one code read, no default, no write
  ---
  duration_ms: 0.256833
  type: 'test'
  ...
# Subtest: tierCeilingMs: tier 1 gets the full budget, tier 2 the remainder, a spent leg gets 0
ok 3061 - tierCeilingMs: tier 1 gets the full budget, tier 2 the remainder, a spent leg gets 0
  ---
  duration_ms: 0.092208
  type: 'test'
  ...
# Subtest: a leg never exceeds its budget across both tiers — the naive sum would blow the box
ok 3062 - a leg never exceeds its budget across both tiers — the naive sum would blow the box
  ---
  duration_ms: 0.047333
  type: 'test'
  ...
# Subtest: ladderSkipError names the skipped tier and carries the earlier failure, capped
ok 3063 - ladderSkipError names the skipped tier and carries the earlier failure, capped
  ---
  duration_ms: 0.145791
  type: 'test'
  ...
# Subtest: both transports run the SAME ladder mechanics — no second budget idiom
ok 3064 - both transports run the SAME ladder mechanics — no second budget idiom
  ---
  duration_ms: 0.177375
  type: 'test'
  ...
...
ok 3065 - F1: Vertex tier fails → the OpenRouter tier serves the SAME leg (the hop is real)
  ---
  duration_ms: 34.591375
  type: 'test'
  ...
# [provider-fallback] gemini gemini-2.5-pro failed → openrouter: {"provider":"gemini","label":"chatWithFallback","feature":null,"fellBackTo":"openrouter","intended_model":"gemini-2.5-pro","fallback_model":null,"region":"asia-south1","sa_identity":null,"inFlightAtError":1,"in_flight_by_provider":{"gemini":1},"http_status":null,"error_status":null,"error_code":null,"message":"GCP_SA_KEY is not valid JSON (or base64 JSON)","details":null}
# Subtest: F2: with NO leg budget there is NO second tier — the utility path is byte-identical
ok 3066 - F2: with NO leg budget there is NO second tier — the utility path is byte-identical
  ---
  duration_ms: 8.215292
  type: 'test'
  ...
...
ok 3067 - F3: noLocalFallback=true → both tiers failing THROWS; Ollama is not called
  ---
  duration_ms: 5.361209
  type: 'test'
  ...
# [provider-retry] openrouter google/gemini-2.5-pro attempt 1/1 http 500 — giving up: 500 "boom"
...
ok 3068 - F4: noLocalFallback absent → both tiers failing still falls back to Ollama (today's behaviour)
  ---
  duration_ms: 7.478291
  type: 'test'
  ...
...
ok 3069 - F5: GEMINI_VIA_OPENROUTER=1 makes OpenRouter tier 1 — the inversion is live, not just typed
  ---
  duration_ms: 1183.006708
  type: 'test'
  ...
...
ok 3117 - a note with an ID absent from the vitals table is still no-vitals; one present is still covered
  ---
  duration_ms: 0.044875
  type: 'test'
  ...
# Subtest: the MEASURED window reproduces: 160 of 561 = 28.5%
ok 3118 - the MEASURED window reproduces: 160 of 561 = 28.5%
  ---
  duration_ms: 0.133833
  type: 'test'
  ...
# Subtest: rows outside the window are DROPPED — a boundary sliver is not a day
ok 3119 - rows outside the window are DROPPED — a boundary sliver is not a day
  ---
  duration_ms: 0.3595
  type: 'test'
  ...
# Subtest: Metabase type wobble is absorbed: string counts and ISO timestamps
ok 3120 - Metabase type wobble is absorbed: string counts and ISO timestamps
  ---
  duration_ms: 0.043666
  type: 'test'
  ...
# Subtest: junk never produces a number that looks real
ok 3121 - junk never produces a number that looks real
  ---
  duration_ms: 0.070833
  type: 'test'
  ...
# Subtest: the core is PURE and dependency-free — it must not reach the engine or any score
ok 3122 - the core is PURE and dependency-free — it must not reach the engine or any score
  ---
  duration_ms: 6.329583
  type: 'test'
  ...
# Subtest: GATE 2 — flag OFF: all three fetch SQL strings are byte-identical to today's
ok 3123 - GATE 2 — flag OFF: all three fetch SQL strings are byte-identical to today's
  ---
  duration_ms: 2.059792
  type: 'test'
  ...
# Subtest: flag ON: vitals LEFT JOIN present, DISTINCT ON newest _update_time, scan bounded, quoted table
ok 3124 - flag ON: vitals LEFT JOIN present, DISTINCT ON newest _update_time, scan bounded, quoted table
  ---
  duration_ms: 0.6575
  type: 'test'
  ...
# Subtest: SWEEP-1 (D2) — the day fetch deduplicates by uid; the single/bulk uid fetches do NOT change
ok 3125 - SWEEP-1 (D2) — the day fetch deduplicates by uid; the single/bulk uid fetches do NOT change
  ---
  duration_ms: 0.135666
  type: 'test'
  ...
# Subtest: GATE 5 — no selected column ends in _tag, flag on or off (R-11: numbers, not judgments)
ok 3126 - GATE 5 — no selected column ends in _tag, flag on or off (R-11: numbers, not judgments)
  ---
  duration_ms: 0.130042
  type: 'test'
  ...
# Subtest: GATE 3 — synthetic control: a vitals row parses to the exact case shape
ok 3127 - GATE 3 — synthetic control: a vitals row parses to the exact case shape
  ---
  duration_ms: 1.005958
  type: 'test'
  ...
# Subtest: GATE 3 — no vitals row → vitalsRecorded false + vitals null (weight/height still mapped)
ok 3128 - GATE 3 — no vitals row → vitalsRecorded false + vitals null (weight/height still mapped)
  ---
...
ok 3129 - a record with every measurement blank is STILL vitalsRecorded true — a different finding from "no record"
  ---
  duration_ms: 0.098875
  type: 'test'
  ...
# Subtest: bp parse: null unless the string matches ^\\d+\\/\\d+$ (the raw string is kept as recorded)
ok 3130 - bp parse: null unless the string matches ^\\d+\\/\\d+$ (the raw string is kept as recorded)
  ---
  duration_ms: 0.105583
  type: 'test'
  ...
# Subtest: recordedAt: null when the note timestamp is missing (no wall clock ever leaks)
ok 3131 - recordedAt: null when the note timestamp is missing (no wall clock ever leaks)
  ---
...
ok 3132 - fail-safe: an error in the vitals leg resets to the safe state and leaves the rest of the case intact
  ---
  duration_ms: 0.358542
  type: 'test'
  ...
# Subtest: flag OFF: the A1 fields stay absent — every existing case literal and behaviour unchanged
ok 3133 - flag OFF: the A1 fields stay absent — every existing case literal and behaviour unchanged
  ---
  duration_ms: 0.081834
  type: 'test'
  ...
# Subtest: GATE 1 — opdCaseText is byte-identical with and without the vitals block (A1 is score-invariant)
ok 3134 - GATE 1 — opdCaseText is byte-identical with and without the vitals block (A1 is score-invariant)
  ---
  duration_ms: 0.54275
  type: 'test'
  ...
# Subtest: GATE 4 — OpdKeys carries no vitals field, no weight, no height
ok 3135 - GATE 4 — OpdKeys carries no vitals field, no weight, no height
  ---
...
# skipped 0
# todo 0
# duration_ms 19633.669084
--- end output ---
EXIT: 0
ENDED: 2026-08-17T05:15:34Z
```
...
```text
COMMAND: cp lib/architecture/map.generated.ts /Users/vinaybhardwaj/cdmss-pass2-gate-recorder-16-aug-2026/gen1.ts
STARTED: 2026-08-17T05:16:14Z
--- output (stdout+stderr merged) ---
--- end output ---
EXIT: 0
ENDED: 2026-08-17T05:16:14Z
...
```text
COMMAND: cmp lib/architecture/map.generated.ts /Users/vinaybhardwaj/cdmss-pass2-gate-recorder-16-aug-2026/gen1.ts
STARTED: 2026-08-17T05:16:14Z
--- output (stdout+stderr merged) ---
--- end output ---
EXIT: 0
...
⚠️ How the quotes were preserved (the method that worked last round): the
...
```text
GATE START 2026-08-17T05:15:14Z at commit 614da5425439264ffd8a56dea6f704a6b14463c7
...
GATE END 2026-08-17T05:16:51Z
```

## Part 3 — evidence integrity

The five earlier evidence files, hashed at this pass (the first three must
match the kickoff's pinned values; the fourth and fifth are computed and
recorded):
...
All three pinned digests match. No earlier evidence file, report, addendum or
review was edited in this round; addendum v18 stands uncorrected in place, with
the v19 §2 erratum as its correction.
