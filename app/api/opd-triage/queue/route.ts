...
export async function GET(req: NextRequest) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(day)) {
    const latest = await run(
      `SELECT to_char(max((note_date AT TIME ZONE 'Asia/Kolkata')::date),'YYYY-MM-DD') d
       FROM opd_note_audits WHERE app_source = $1 AND engine_version = $2`,
  ...
  // that predate finding identity still get signal_type + finding_ref.
  const params: unknown[] = [APP, OPD_ENGINE_VERSION, from, to];
  let where = `app_source = $1 AND engine_version = $2 AND (note_date AT TIME ZONE 'Asia/Kolkata')::date BETWEEN $3 AND $4`;
  if (doctorFilter) { params.push(doctorFilter); where += ` AND doctor_uid = $${params.length}`; }

  const rows = await run(
    `SELECT id::text AS id, doctor_uid, to_char(note_date AT TIME ZONE 'Asia/Kolkata','YYYY-MM-DD') AS note_date, findings
