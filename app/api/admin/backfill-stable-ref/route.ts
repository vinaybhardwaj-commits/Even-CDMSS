...
async function runBatch(apply: boolean): Promise<Record<string, unknown>> {
  ...
  const rows = await run(
    `SELECT id, uid, note_date, engine_version, findings FROM opd_note_audits
     WHERE app_source = $1 AND engine_version LIKE $2 AND jsonb_typeof(findings) = 'array'
       AND note_date IS NOT NULL AND note_date > $3::timestamptz
     ORDER BY note_date ASC, id ASC LIMIT ${BATCH}`,
    [APP, ENGINE_LIKE, cursor]).catch(() => []) as unknown as Row[];

  let rowsScanned = 0, findingsSeen = 0, refsComputed = 0, refsNull = 0, alreadyStamped = 0;
  let collisions = 0, rowsWithCollision = 0, rowsChanged = 0, lastDate = cursor;
  ...
  // The cursor only advances on a REAL apply — a dry run must be repeatable and must never consume
  // the sweep it is previewing.
  if (apply && rowsScanned > 0) await setCursor(lastDate);
  ...
  return {
    ...
    cursor_after: apply ? lastDate : cursor,
    done,
    note: apply
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, ...(await runBatch(apply)) });
  } catch (e) { return NextResponse.json({ ok: false, error: String((e as Error).message) }, { status: 500 }); }
}
