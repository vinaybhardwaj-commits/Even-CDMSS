...
export default function CareLogin() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true); setErr('');
    try {
      const r = await fetch('/api/care/login', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token }),
      });
      if (r.ok) { window.location.href = '/care'; return; }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#f4f7fb', fontFamily: 'system-ui, sans-serif' }}>
      <form onSubmit={submit} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: 28, width: 360 }}>
        <div style={{ fontWeight: 800, fontSize: 18 }}>Care Conversation Brief</div>
        <div style={{ color: '#0f766e', fontWeight: 700, fontSize: 12, margin: '4px 0 18px' }}>CARE MANAGER · EHRC</div>
