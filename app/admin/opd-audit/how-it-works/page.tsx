    ...
    <div className="max-w-3xl">
      ...
      <Section id="deterministic" kicker="Mechanics" title="Deterministic prescribing checks">
        <ul className="ml-5 list-disc space-y-1.5">
          <li><strong>Dosing completeness</strong> — a med line needs a dose, frequency, route and duration. Reads what the note <em>actually documents</em>: strength embedded in the drug name counts (&quot;Cefix 200mg Tab&quot;), and a route inferable from the dosage form counts (tab→oral, inj→parenteral). Only truly ambiguous gaps flag. Nutraceuticals / supplements are exempt — a proprietary supplement has no clinically meaningful strength (0.81.2).</li>
          ...
          <li><strong>Drug–drug interactions</strong> — over confidently-resolved molecules, including a history-drug × current-drug check. NSAID detection is <em>ingredient-level</em> and formulary-independent, so a combination or topical whose parsed primary is a non-NSAID (e.g. Methyl Salicylate over Diclofenac) still counts (0.81.2). It is <em>route-aware</em> — a topically-applied NSAID has low systemic absorption, so an oral+topical overlap is context-dependent, not a full systemic interaction (0.81.2). A deterministic interaction and the LLM&apos;s own therapeutic-duplication finding for the SAME pair are consolidated into one card — <em>one decision, one finding</em>, now enforced in code, not just the prompt (0.81.2).</li>
          <li><strong>Daily-dose ceilings</strong> — total daily dose per molecule aggregated ACROSS combination products and checked against per-molecule ceilings (<span className="font-mono text-[12px]">data/dose-limits.json {DOSE_LIMITS_VERSION}</span>). Volumetric/liquid formulations (syrups dosed by ml/tsp) are detected robustly and kept out of the tablet model, so a &quot;10 ml (2 tsp)&quot; dose is never mis-read as 10 tablets (0.81.2).</li>
          ...
          <li><strong>Muscle relaxant (FDC)</strong> — a fixed-tier (context-dependent) advisory when a muscle-relaxant combination (e.g. chlorzoxazone / thiocolchicoside) is prescribed, so the objection is consistent run-to-run instead of the model raising it unpredictably (0.81.2).</li>
          <li><strong>Non-scoring metadata</strong> — a finding about an inaccurate drug-class label, or a pure ICD/coding-completeness gap, stays VISIBLE but is scored zero on clinical domains: the class tags are the engine&apos;s own formulary metadata and a code gap is chart metadata, not a care decision — the clinician is never penalised for either (0.81.2).</li>
        </ul>
      </Section>
      ...
      <Section id="right-care" kicker="Right Care · v0.81.6" title="Low-value care as a case-mix-fair rate">
        <p>
        ...
        <p className="mt-2">
          <strong>Rule attribution (v0.81.6):</strong> at stamp time each low-value finding is keyword-matched against the
          active <code>lvc_recommendations</code>. Each keyword is a <em>trigger phrase</em>; a rule matches when <strong>any</strong>
          of its keyword phrases is present, and a phrase is present only when <strong>every</strong> word of it appears as a
          <strong> whole word</strong> (so &ldquo;incomplete&rdquo; never matches &ldquo;complete&rdquo;). The rule with the
          <strong> longest matched phrase</strong> (most specific) wins — attaching <code>rule_ref</code> + the rule&apos;s category.
          No hit → <code>rule_ref: null</code>.
