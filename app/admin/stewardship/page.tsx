...
type DeptRow = {
  consult_type: string; n_notes: number; avg_nqi: number; pct_ab: number;
...
// Latest audit per note (DISTINCT ON uid) over the window, aggregated by consult_type (department
// proxy). Nested subquery because Neon's HTTP driver can't GROUP BY over a DISTINCT ON in one pass.
const DEPT_SQL = `
  SELECT consult_type,
  ...
  FROM (
    SELECT DISTINCT ON (uid) uid, consult_type, note_quality_index, band,
...
function deptLabel(s: string): string {
...
export default async function StewardshipPage() {
  ...
  const depts: DeptRow[] = deptRaw.map((r) => ({
    consult_type: String(r.consult_type || ''), n_notes: n(r.n_notes), avg_nqi: n(r.avg_nqi), pct_ab: n(r.pct_ab),
        ...
        <>
          ...
          <div className="mt-4 overflow-x-auto rounded-xl border border-slate-200 bg-white">
            <table className="w-full text-[12.5px]">
              ...
              <tbody>
                {depts.map((d) => (
                  <tr key={d.consult_type || 'none'} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                    <td className="px-3 py-2.5 font-medium text-slate-800">{deptLabel(d.consult_type)}</td>
