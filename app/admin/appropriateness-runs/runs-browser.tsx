...
const MODE_LABEL: Record<string, string> = { check: 'Appropriateness check', pathway: 'Pathway', audit: 'Case audit' };
const MODE_BADGE: Record<string, string> = { check: 'bg-blue-50 text-blue-700', pathway: 'bg-teal-50 text-teal-700', audit: 'bg-amber-50 text-amber-700' };
        ...
        <div className="flex gap-1.5">
          {(['all', 'check', 'pathway', 'audit'] as const).map((f) => (
            <button key={f} type="button" onClick={() => setFilter(f)}
              className={`rounded-full px-3 py-1 text-xs ${filter === f ? 'bg-slate-900 text-white' : 'border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
              {f === 'all' ? 'All' : MODE_LABEL[f]}
            </button>
          ))}
        </div>
        <button type="button" disabled={!shown.length || !!busy}
          onClick={() => dl(shown, `appropriateness-runs-${filter}-${shown.length}.xlsx`, 'bulk')}
              ...
              <tr key={r.id} className="border-t border-slate-100 align-top">
                ...
                <td className="px-3 py-2 text-right">
                  <button type="button" disabled={!!busy}
                    onClick={() => dl([r], `appropriateness-${r.mode}-${r.id.slice(0, 8)}.xlsx`, r.id)}
