...
/**
 * Full admin monitor for the Even-LVC grounding worker (CDMSS-EVEN-LVC-GROUNDING-WORKER §7) — mirrors
 * the mini-backfill monitor's shell: state header, throughput (citations/tick), state timeline, drain %
 * + ETA-at-current-rate, the full tick feed, Pause/Resume, and a manual one-tick probe. Server-rendered
 * initial data (props); polls the status endpoint every 15s for live updates. Read-only aggregates.
 */
...
const STATE_STYLE: Record<string, { dot: string; text: string; label: string }> = {
  ...
  disabled: { dot: 'bg-slate-400', text: 'text-slate-500', label: 'Disabled' },
};
...
export default function LvcGroundMonitor({ initialStatus, initialTicks, batch, cadenceMin }: {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (r.ok && j.ok) { setSt(j); setTicks(j.recent_ticks ?? []); }
    } catch { /* soft */ }
  }, []);
  ...
  const perTick = st?.last_tick?.processed || batch;   // notes per tick (fallback: the batch cap)
  ...
  return (
    <div className="mt-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-[12px] text-slate-500">epoch {st?.epoch ?? '—'} · {num(st?.active_assertions)} active assertions</span>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Stat label="Citations added (total)" value={num(st?.citations_added_total)} />
