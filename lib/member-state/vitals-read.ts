...
const memberVitalsSql = (individualUid: string): string => {
  ...
  return `SELECT created_at, source, prescription_uid,
...
function summariseModality(rows: Record<string, unknown>[]): ModalityMix {
  ...
  let inPerson = 0, remoteOrUndoc = 0, total = 0;
  let lastAssessMode: string | null = null, lastAssessAt: string | null = null;
  for (const r of rows) {
    total++;
    const mode = (str(r.assess_mode) || 'UNDOCUMENTED').toUpperCase();
  ...
  const majority: ModalityMix['majority'] = total === 0 ? 'unknown'
    ...
    : inPerson > 0 ? 'mixed'
    : 'remote';
  return { total, counts, inPerson, remoteOrUndocumented: remoteOrUndoc, majority, lastAssessMode, lastAssessAt };
}

async function fetchRows(individualUid: string): Promise<{ vitals: VitalsRead[]; prescOf: (string | null)[]; modality: ModalityMix }> {
...
export async function readEncounterVitals(individualUid: string, prescUid: string): Promise<MemberVitals> {
  if (!isUid(individualUid)) return EMPTY_MEMBER_VITALS;
  const { vitals, prescOf, modality } = await fetchRows(individualUid);
  const trend = vitals.filter(hasAnyVital);
