...
// ── version bump ─────────────────────────────────────────────────────────────
test('engine bumped to 0.81.10 (Signal-Type Collapse) and the read family includes 0.81.8 + 0.81.9 + 0.81.10', () => {
  assert.equal(OPD_ENGINE_VERSION, 'opd-note-audit/0.81.10');
  ...
  assert.ok(OPD_ENGINE_VERSIONS_CURRENT.includes('opd-note-audit/0.81.10'));  // the new version must be added or the index empties (decision 21)
});
