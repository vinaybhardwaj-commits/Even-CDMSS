...
// fails. S1 (0.81.15) RE-PINNED 26 → 17: quantizeConfidence(0.8) = 1.0 (was raw 0.8), so the
// penalty per finding rose 36 → 45 and 100 × 0.64³ = 26 became 100 × 0.55³ = 17. MORE penalised,
// same intent — a major interaction is a real safety signal and must never wash out.
test('CANARY: three co-prescribed major interactions score prescribing safety 17/100, never 100', () => {
  ...
  assert.equal(presc, 17, 'the QT-prolongation cluster must remain penalised — a major interaction is a real safety signal');
