# CDMSS gate evidence — pass 3 (the two D16 payload corrections and proofs 35, 45, 46, 47, 49, 56), 18 August 2026
Authority: Saul review 34 (closed pass 2, released pass 3). Addendum v23, AUTHORIZED by the orchestrator on V's explicit
delegation, 18 August 2026 (v23 §0: the digest ritual of v20-v22 is withdrawn; there was no signature to verify, and stop
condition 3 — the status line reads AUTHORIZED — was checked instead).
The implementation commit (commit 13): 84040291ca9fcede333d0373c749d97ae58723e2 — four paths: lib/retrieval-capture.ts (the two
corrections), lib/__tests__/retrieval-telemetry-core.test.ts (proof 35), lib/__tests__/retrieval-telemetry-validation.test.ts
(proofs 45, 46, 47, 49; NEW), lib/__tests__/retrieval-telemetry-canonicalization.test.ts (proof 56; NEW).
Gate run: 2026-08-18T14:05:26Z to 2026-08-18T14:07:04Z UTC, from commit 13.
Mutation table (SIXTEEN rows) run BEFORE the gate, completed before commit 13 was created; the gate then ran once, from
commit 13.

⚠️ DISCLOSED — ONE AMEND, ONE SUPERSEDED FIRST RUN. Commit 13 was first created as 947d7de and the sixteen-row table and
the gate were run against it (2026-08-18T13:57:02Z–13:57:06Z and 13:58:12Z–13:59:50Z). While writing the report the builder
found that the validation file carried a DUPLICATE test title — two tests numbered 45.99 (a hard-numbered licence test
colliding with the generated per-field rows). The file was corrected (the licence test is 45.102), commit 13 was AMENDED
to 8404029 (commit 13 is this pass's own, unpushed commit; commits 1-12 are untouched), and BOTH the sixteen-row table and
the full gate were re-run from a fresh sandbox against the amended bytes. This file's Parts 1 and 2 are that second run.
The first run's captures are preserved verbatim under `…/superseded-first-run/` and are not the evidence. Upstream at the start of the pass: 01c2375 (pass 2's eight commits were pushed under review 34's release); nothing
was pushed in this pass.

Raw capture directory: $HOME/cdmss-pass3-gate-18-aug-2026
...
## Part 1 — the mutation table: sixteen rows, BEFORE the gate (addendum v23 §9; kickoff §5)

The kickoff's eight required rows are rows 1 to 8, in its order. Rows 9 to 16 add one row per further load-bearing claim
(kickoff §5: "Add rows wherever a proof has more than one load-bearing claim"). Sandbox per the established shape
...
`lib/__tests__` present with all three test files, `.git` absent, `.next` absent, `node_modules` a symlink; the four changed
files `cmp`-identical to the worktree, and `lib/retrieval-telemetry-core.ts` too); deleted after the table. cwd = worktree,
test path = sandbox copy; no git command inside the sandbox. Each row applies ONE exact string replacement to the sandbox
copy of the named production file, records `diff -u` of the worktree original against the mutated sandbox copy, runs ONLY
its named test file, and restores the file with `cp`, verified with `cmp`.

⚠️ RECORDER GUARDRAIL (v23 §8, kickoff §5). NO ROW alters judge request lifecycle or recorder counting. Rows mutate only
`lib/retrieval-capture.ts` (rows 1, 2, 3, 7, 9, 10, 11, 14, 15) and `lib/retrieval-telemetry-core.ts` (rows 4, 5, 6, 8, 12,
13, 16). No row touches `lib/rerank.ts`, `lib/multi-query.ts` or `lib/__tests__/judge-server-stub.ts`, and each row runs
only its named test file — `retrieval-telemetry-core.test.ts`, `retrieval-telemetry-validation.test.ts` or
`retrieval-telemetry-canonicalization.test.ts` — none of which imports the judge stub. (Proof 35.8 imports `lib/rerank.ts`
dynamically with injected throwers and a no-op health check; that module is never mutated and no socket is opened.)

### The byte-equality proof (kickoff §5) — four files, four hashes each

Hash 1 (worktree, before the table) · Hash 2 (sandbox baseline) · Hash 3 (worktree, after the table) · Hash 4
(`git show HEAD:<path> | shasum -a 256` after commit 13):
...
496d445557b433e2a61a0e136d8a9349b1369a400ffc7232c0174207f59f5fe9  lib/__tests__/retrieval-telemetry-validation.test.ts
08633d47ed01a59dd9e56249e1f762aceb94029cbfeddce6872ebd8e1ca74884  lib/__tests__/retrieval-telemetry-canonicalization.test.ts
# hash 4 — git show HEAD:<path> after commit 13
b39b15f45b7f4b0657e91353052049ce60b638d2513b2ebffce3e67503308bf6  lib/retrieval-capture.ts
13de25781785b3f27df25a505727f0e4c0f22171e45638b0150d597419d8ef41  lib/__tests__/retrieval-telemetry-core.test.ts
496d445557b433e2a61a0e136d8a9349b1369a400ffc7232c0174207f59f5fe9  lib/__tests__/retrieval-telemetry-validation.test.ts
08633d47ed01a59dd9e56249e1f762aceb94029cbfeddce6872ebd8e1ca74884  lib/__tests__/retrieval-telemetry-canonicalization.test.ts
...
All four agree for every file. The tested bytes are the committed bytes. The sixteen rows were run ONCE, against these bytes.

### Row 1 — a SKIPPED expansion stage records attempts: null again instead of [] (the 2.1 correction reverted at the stage site)

mutated file: lib/retrieval-capture.ts. test file: lib/__tests__/retrieval-telemetry-core.test.ts. must fail by name: 35.6.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-capture.ts	2026-08-18 19:16:52
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-capture.ts	2026-08-18 19:34:45
@@ -336,7 +336,7 @@
...
+      attempts: expansionSkipped ? null : manifestAttempts(expansion?.evidence ?? null),
     },
...
```text
cd /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry && node --test --import tsx /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/retrieval-telemetry-core.test.ts
exit=1
STARTED: 2026-08-18T14:04:45Z
ENDED: 2026-08-18T14:04:46Z
```

Failing tests, by name:

```text
not ok 30 - 35.6 — ROW 5 and the v11 §6.1 CORRECTION: a SKIPPED expansion stage records served_route_class null (NOT not_served), no route counter, no not_served counter — and attempts: [] , never null
```
...
# fail 1
# cancelled 0
# duration_ms 246.363125
```

### Row 2 — a variant-generation stage that RAN and failed records null instead of unattributed (the 2.2 correction reverted)

mutated file: lib/retrieval-capture.ts. test file: lib/__tests__/retrieval-telemetry-core.test.ts. must fail by name: 35.12.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-capture.ts	2026-08-18 19:16:52
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-capture.ts	2026-08-18 19:34:46
@@ -397,7 +397,7 @@
...
```text
not ok 36 - 35.12 — ROW 10 and the v11 §6.2 CORRECTION: a variant-generation stage that RAN and failed_open records not_served ONLY with proof, otherwise UNATTRIBUTED — and only a stage that did NOT run records the stage-level null
```
...
# fail 1
# cancelled 0
# duration_ms 192.387958
```

### Row 3 — one D16 row's mapping flipped: ollama success → 'openrouter' instead of 'local'

mutated file: lib/retrieval-capture.ts. test file: lib/__tests__/retrieval-telemetry-core.test.ts. must fail by name: 35.2, 35.9.
...
```text
not ok 26 - 35.2 — ROW 1, provider success: vertex → vertex, openrouter → openrouter, ollama → local; that route counter += 1, failed unchanged, served model and attempts PRESERVED
not ok 33 - 35.9 — ROW 7, intended local request: exactly one ollama attempt; local on success, not_served on PROVEN failure
```
...
# fail 2
# cancelled 0
# duration_ms 190.191584
```

### Row 4 — one required D17 field's absent-check removed: operational.trace_id

mutated file: lib/retrieval-telemetry-core.ts. test file: lib/__tests__/retrieval-telemetry-validation.test.ts. must fail by name: 45.1, trace_id.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-16 06:53:06
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-18 19:34:46
@@ -678,7 +678,6 @@
...
```text
cd /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry && node --test --import tsx /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/retrieval-telemetry-validation.test.ts
exit=1
STARTED: 2026-08-18T14:04:46Z
ENDED: 2026-08-18T14:04:46Z
```
...
```text
not ok 14 - 45.14 — operational.trace_id: missing (null is permitted) → trace_id_field_absent
not ok 102 - 45.1 — OWN-PROPERTY CHECKS: missing, explicit null, empty array, empty string and invalid number are FIVE different answers, and the validator tells them apart
```
...
# fail 2
# cancelled 0
# duration_ms 205.143792
```

### Row 5 — expansion.served_route_class null + skipped marked partial: the `&& !skipped` condition dropped

mutated file: lib/retrieval-telemetry-core.ts. test file: lib/__tests__/retrieval-telemetry-validation.test.ts. must fail by name: 46.1, 46.2.
...
+    else if (ex.served_route_class === null) v.push('expansion_served_route_class_absent');
...
```text
not ok 104 - 46.1 — expansion.served_route_class null with status skipped is valid: no expansion code, so a normative_channel row is not partial by construction
not ok 105 - 46.2 — through the REAL builder: a normative_channel capture (expansion never set — the leg sets skipExpand unconditionally) produces a payload that validates clean
not ok 107 - 47.2 — the EMPTY-STRING case: zero candidates render an empty citedContext, and the HMAC of the empty string is a DEFINED value — never null because reranking was skipped
not ok 108 - 47.3 — on the other FOUR roles the HMAC is null, and those nulls are NOT partial
not ok 109 - 49.1 — EMPTY FUSION: retrieve() returns before the rerank block exists — fused 0, hydrated 0, expected 0, recorded 0, batches [], backend and model none
not ok 110 - 49.2 — HYDRATE EMPTIED: fused > 0, hydrated 0 — the same shape, and the TWO COUNTS DIFFER, which is the point
not ok 112 - 49.4 — ONE HYDRATED CANDIDATE: rerank() is never entered — hydrated 1, expected 0, recorded 0, batches [], backend and model none
not ok 113 - 49.5 — RERANKER DISABLED (normative_channel always; lab_direct when the caller sets it): several hydrated candidates, the same shape as the one-candidate case
```
...
# fail 8
# cancelled 0
# duration_ms 139.786542
```

### Row 6 — the scorer-context HMAC required on a non-primary role (lvc_recall)

mutated file: lib/retrieval-telemetry-core.ts. test file: lib/__tests__/retrieval-telemetry-validation.test.ts. must fail by name: 47.3.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-16 06:53:06
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-18 19:34:47
@@ -830,7 +830,7 @@
...
+  else if ((role === 'primary' || role === 'lvc_recall') && m.scorer_context_hmac === null && !keyAbsent) v.push('scorer_context_hmac_absent');
...
cd /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry && node --test --import tsx /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/retrieval-telemetry-validation.test.ts
exit=1
STARTED: 2026-08-18T14:04:47Z
ENDED: 2026-08-18T14:04:47Z
```

Failing tests, by name:

```text
not ok 1 - 45.0 — the fixture is CLEAN: the valid manifest returns no code, so every code below is caused by the one mutation named beside it
not ok 108 - 47.3 — on the other FOUR roles the HMAC is null, and those nulls are NOT partial
```
...
# fail 2
# cancelled 0
# duration_ms 138.962542
```

### Row 7 — the two zero-candidate shapes made to produce the same counts: fused_candidate_count taken from the hydrated list

mutated file: lib/retrieval-capture.ts. test file: lib/__tests__/retrieval-telemetry-validation.test.ts. must fail by name: 49.2, 49.3.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-capture.ts	2026-08-18 19:16:52
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-capture.ts	2026-08-18 19:34:47
@@ -343,7 +343,7 @@
...
+    fused_candidate_count: capture.hydratedCandidateIds.length,
...
```text
not ok 110 - 49.2 — HYDRATE EMPTIED: fused > 0, hydrated 0 — the same shape, and the TWO COUNTS DIFFER, which is the point
not ok 111 - 49.3 — the two zero-candidate shapes are DISTINGUISHABLE: empty fusion (0/0) and hydrate emptied (3/0) differ in fused_candidate_count and agree in hydrated_candidate_count
```
...
# fail 2
# cancelled 0
# duration_ms 139.257583
```

### Row 8 — canonicalization accepts an undefined array element (nulled instead of thrown)

mutated file: lib/retrieval-telemetry-core.ts. test file: lib/__tests__/retrieval-telemetry-canonicalization.test.ts. must fail by name: 56.5.
...
```text
cd /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry && node --test --import tsx /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/__tests__/retrieval-telemetry-canonicalization.test.ts
exit=1
STARTED: 2026-08-18T14:04:47Z
ENDED: 2026-08-18T14:04:47Z
```
...
```text
not ok 5 - 56.5 — an UNDEFINED ARRAY ELEMENT is REJECTED (thrown, not dropped and not nulled), while undefined in an object is omitted
```
...
# fail 1
# cancelled 0
# duration_ms 117.236375
```

### Row 9 — the 2.1 correction reverted at manifestAttempts: ABSENT evidence records null again instead of []

mutated file: lib/retrieval-capture.ts. test file: lib/__tests__/retrieval-telemetry-core.test.ts. must fail by name: 35.7.
...
```text
not ok 31 - 35.7 — the v11 §6.1 CORRECTION at the other two sites: ABSENT evidence records attempts [] on a batch and on variant generation, while a transport that did NOT COLLECT (attempts: null on real evidence) keeps null
```
...
# fail 1
# cancelled 0
# duration_ms 260.2485
```

### Row 10 — the proof rule flipped: PROVEN non-delivery maps to 'unattributed' instead of 'not_served'

mutated file: lib/retrieval-capture.ts. test file: lib/__tests__/retrieval-telemetry-core.test.ts. must fail by name: 35.3, 35.5, 35.9.
...
+  if (ev.provenNotServed) return 'unattributed';
   switch (ev.servedProvider) {
     case 'vertex': return 'vertex';
     case 'openrouter': return 'openrouter';
```
...
exit=1
STARTED: 2026-08-18T14:04:47Z
ENDED: 2026-08-18T14:04:48Z
```

Failing tests, by name:

```text
not ok 27 - 35.3 — ROW 2, terminal failure PROVEN to have returned no completion: not_served += 1, failed += 1, served_route_class not_served, served_model null
not ok 29 - 35.5 — ROW 4, timeout: the attempt records outcome timeout, the batch outcome is timeout, and the served class follows the PROOF rule — not_served with proof, unattributed without
not ok 32 - 35.8 — ROW 6 AS AMENDED (v7 §6, v8 §2): a Cohere soft failure without transport proof synthesises one terminal_failure record per planned boundary with served_route_class UNATTRIBUTED — not the not_served D16 wrote — rerank_soft_failed true, and not_served stands only where proof exists
not ok 33 - 35.9 — ROW 7, intended local request: exactly one ollama attempt; local on success, not_served on PROVEN failure
not ok 36 - 35.12 — ROW 10 and the v11 §6.2 CORRECTION: a variant-generation stage that RAN and failed_open records not_served ONLY with proof, otherwise UNATTRIBUTED — and only a stage that did NOT run records the stage-level null
```
...
### Row 11 — the amended row 6 undone: absent evidence maps to an INFERRED 'not_served' instead of 'unattributed'

mutated file: lib/retrieval-capture.ts. test file: lib/__tests__/retrieval-telemetry-core.test.ts. must fail by name: 35.4, 35.12.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-capture.ts	2026-08-18 19:16:52
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-capture.ts	2026-08-18 19:34:48
@@ -122,7 +122,7 @@
  * Given evidence, this function's own honest floor is 'unattributed', never null.
  */
 export function servedClassOf(ev: TransportEvidence | null): ServedRouteClass {
-  if (!ev) return 'unattributed';
+  if (!ev) return 'not_served';
   if (ev.provenNotServed) return 'not_served';
   switch (ev.servedProvider) {
     case 'vertex': return 'vertex';
```
...
```text
not ok 28 - 35.4 — ROW 3, a completion may have arrived and attribution is unavailable: unattributed += 1 (rerank_unattributed_batches), failed FOLLOWS the batch outcome, served_route_class unattributed
not ok 36 - 35.12 — ROW 10 and the v11 §6.2 CORRECTION: a variant-generation stage that RAN and failed_open records not_served ONLY with proof, otherwise UNATTRIBUTED — and only a stage that did NOT run records the stage-level null
```
...
# fail 2
# cancelled 0
# duration_ms 191.893417
```

### Row 12 — one D16 COUNTER row flipped: not_served increments unattributed

mutated file: lib/retrieval-telemetry-core.ts. test file: lib/__tests__/retrieval-telemetry-core.test.ts. must fail by name: 35.1.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-16 06:53:06
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-18 19:34:48
@@ -928,7 +928,7 @@
...
+    else if (b.served_route_class === 'not_served') c.unattributed += 1;
...
```text
not ok 9 - 35.1 — every served class increments its OWN counter, and a null increments none (D16 rows: provider success → that route counter; proven non-delivery → not_served; attribution gap → unattributed; a stage-level null → nothing)
not ok 27 - 35.3 — ROW 2, terminal failure PROVEN to have returned no completion: not_served += 1, failed += 1, served_route_class not_served, served_model null
not ok 33 - 35.9 — ROW 7, intended local request: exactly one ollama attempt; local on success, not_served on PROVEN failure
```
...
# fail 3
# cancelled 0
# duration_ms 191.757041
```

### Row 13 — canonicalization stops recursing: nested values are copied, not canonicalized

mutated file: lib/retrieval-telemetry-core.ts. test file: lib/__tests__/retrieval-telemetry-canonicalization.test.ts. must fail by name: 56.1, 56.2, 56.3.
...
```text
not ok 1 - 56.1 — RECURSIVE canonicalization: keys are sorted at EVERY depth, inside objects and inside objects nested in arrays, and array order is preserved
not ok 2 - 56.2 — NESTED-KEY PERMUTATION: two documents whose keys are inserted in different orders at several depths canonicalize to the SAME string
not ok 3 - 56.3 — JSONB ROUND TRIP: canonical(parse(stored)) equals canonical(original) even when the store hands the keys back in jsonb order at every depth
not ok 5 - 56.5 — an UNDEFINED ARRAY ELEMENT is REJECTED (thrown, not dropped and not nulled), while undefined in an object is omitted
not ok 6 - 56.6 — NON-FINITE numbers are rejected at any depth, and finite ones pass
```
...
# fail 5
# cancelled 0
# duration_ms 114.413083
```

### Row 14 — the scorer-context HMAC computed over a TRIMMED context instead of the exact citedContext

mutated file: lib/retrieval-capture.ts. test file: lib/__tests__/retrieval-telemetry-validation.test.ts. must fail by name: 47.1.
...
```text
not ok 106 - 47.1 — on role primary the scorer-context HMAC is REQUIRED: computed over the EXACT citedContext bytes, and it changes when one byte does
```
...
# fail 1
# cancelled 0
# duration_ms 144.31775
```

### Row 15 — the empty-string citedContext yields null instead of the HMAC of the empty string

mutated file: lib/retrieval-capture.ts. test file: lib/__tests__/retrieval-telemetry-validation.test.ts. must fail by name: 47.2.
...
@@ -361,7 +361,7 @@
     ...
     // renders a context, and the HMAC of the empty string is a defined value.
-    scorer_context_hmac: capture.role === 'primary' && opts.scorerContext !== null && keyed
+    scorer_context_hmac: capture.role === 'primary' && opts.scorerContext && keyed
       ? hmac(opts.scorerContext)
       : null,
...
# fail 3
# cancelled 0
# duration_ms 144.51825
```

### Row 16 — an invalid number accepted: the negative check on fused_candidate_count removed

mutated file: lib/retrieval-telemetry-core.ts. test file: lib/__tests__/retrieval-telemetry-validation.test.ts. must fail by name: 45.1, fused_candidate_count: invalid number: negative.

Mutation diff (worktree original vs mutated sandbox copy):

```diff
--- /Users/vinaybhardwaj/dev/Even-CDMSS-rerank-telemetry/lib/retrieval-telemetry-core.ts	2026-08-16 06:53:06
+++ /Users/vinaybhardwaj/cdmss-pass2-sandbox/repo/lib/retrieval-telemetry-core.ts	2026-08-18 19:34:49
@@ -730,7 +730,7 @@
   // ── Candidates ───────────────────────────────────────────────────────────────────────────────
...
+  if (!isFiniteNum(m.fused_candidate_count)) v.push('fused_candidate_count_absent');
...
```text
not ok 41 - 45.41 — fused_candidate_count: invalid number: negative → fused_candidate_count_absent
not ok 102 - 45.1 — OWN-PROPERTY CHECKS: missing, explicit null, empty array, empty string and invalid number are FIVE different answers, and the validator tells them apart
```
...
# fail 2
# cancelled 0
# duration_ms 144.08775
```

Every row failed its named test by name; every run completed in under two seconds; no row was accepted on a file-level
timeout. Row 11's description file records a corrected expectation (35.8 was over-listed by the runner; its named failures
are 35.4 and 35.12) — stated there, not hidden.
...
form (addendum v18 §4.1, adopted by v19 §6, v22 §7 and v23 §11): the five-line block proves determinism (`cmp` of generation
two against generation one) and currency (`git diff --exit-code` of the committed map) with no git write. No `git add` was
used anywhere in the gate. ⚠️ `lib/retrieval-capture.ts` is in the import graph and changed in this pass; the map did NOT
move — both generations wrote 90492 "bytes" (UTF-16 code units; `wc -c` 90494) and `git diff --exit-code` exited 0.
...
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 3b96c77a-bc52-43ab-bfa8-00130c186c32
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 8ac59729-8fb0-4277-884b-900e978160d5
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 75859166-9b53-4f99-a615-f605db13443a
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall e2603e4e-545d-4c13-97f5-aa5bfbcb7698
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall b50acb94-76ca-4cf0-bb34-4ac52bb9aa94
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 739b0c0c-1f84-4088-9e87-941baebf3282
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 1f15591d-b538-467c-9022-9d287815501c
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall eea852f5-e881-49ee-beba-94135fec64c9
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall bc6d4616-8eb8-45e3-ab76-20c3ee2400b5
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] semantic recall failed Invalid URL
# [retrieval-telemetry] terminal write rejected (revision or state moved) lvc_recall — row state unknown
# [retrieval-telemetry] settlement rejected: no_row lvc_recall 50bf5137-b647-4a84-98c7-1b1472bcc56d
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
# [lvc] judge REFUSED (no_gemini_model_resolved): served 'none' != intended 'none' — every rec insufficient_info, no flag fires
...
ok 2672 - 56.1 — RECURSIVE canonicalization: keys are sorted at EVERY depth, inside objects and inside objects nested in arrays, and array order is preserved
  ---
  duration_ms: 0.534583
  type: 'test'
  ...
...
ok 2673 - 56.2 — NESTED-KEY PERMUTATION: two documents whose keys are inserted in different orders at several depths canonicalize to the SAME string
  ---
  duration_ms: 0.116791
  type: 'test'
  ...
# Subtest: 56.3 — JSONB ROUND TRIP: canonical(parse(stored)) equals canonical(original) even when the store hands the keys back in jsonb order at every depth
ok 2674 - 56.3 — JSONB ROUND TRIP: canonical(parse(stored)) equals canonical(original) even when the store hands the keys back in jsonb order at every depth
  ---
  duration_ms: 0.21725
  type: 'test'
  ...
# Subtest: 56.4 — ARRAY REORDER IS NOT EQUAL: array order is content, at the top level and nested
ok 2675 - 56.4 — ARRAY REORDER IS NOT EQUAL: array order is content, at the top level and nested
  ---
...
ok 2676 - 56.5 — an UNDEFINED ARRAY ELEMENT is REJECTED (thrown, not dropped and not nulled), while undefined in an object is omitted
  ---
  duration_ms: 0.229959
  type: 'test'
  ...
# Subtest: 56.6 — NON-FINITE numbers are rejected at any depth, and finite ones pass
ok 2677 - 56.6 — NON-FINITE numbers are rejected at any depth, and finite ones pass
  ---
  duration_ms: 0.085333
  type: 'test'
  ...
# Subtest: the runtime states and the state CHECK are the same list, in the same order
ok 2678 - the runtime states and the state CHECK are the same list, in the same order
  ---
  duration_ms: 1.264667
  type: 'test'
  ...
# Subtest: the outcome CHECK pins its two state lists the same way, and neither slice is empty
ok 2679 - the outcome CHECK pins its two state lists the same way, and neither slice is empty
  ---
  duration_ms: 0.367459
  type: 'test'
  ...
# Subtest: the three outcome-CHECK sets PARTITION all fourteen states — no overlap, none omitted
ok 2680 - the three outcome-CHECK sets PARTITION all fourteen states — no overlap, none omitted
  ---
  duration_ms: 0.102875
  type: 'test'
  ...
# Subtest: two states are non-terminal, and a window cannot close on either
ok 2681 - two states are non-terminal, and a window cannot close on either
  ---
  duration_ms: 0.080959
  type: 'test'
  ...
# Subtest: the migration still declares its retention, access and deletion controls (§4.2)
ok 2682 - the migration still declares its retention, access and deletion controls (§4.2)
  ---
  duration_ms: 0.195625
  type: 'test'
  ...
# Subtest: the HMAC is keyed, versioned, and unreproducible with an unkeyed hash
ok 2683 - the HMAC is keyed, versioned, and unreproducible with an unkeyed hash
  ---
  duration_ms: 35.068583
  type: 'test'
  ...
# Subtest: key version travels with the value, so a rotation is visible rather than inferred
ok 2684 - key version travels with the value, so a rotation is visible rather than inferred
  ---
  duration_ms: 0.7275
  type: 'test'
  ...
# Subtest: a missing secret THROWS, and a whitespace-only key counts as missing (D8, test 71)
ok 2685 - a missing secret THROWS, and a whitespace-only key counts as missing (D8, test 71)
  ---
...
ok 2686 - 35.1 — every served class increments its OWN counter, and a null increments none (D16 rows: provider success → that route counter; proven non-delivery → not_served; attribution gap → unattributed; a stage-level null → nothing)
  ---
  duration_ms: 0.404417
  type: 'test'
  ...
# Subtest: counters derive from the manifest, so row and payload cannot disagree
ok 2687 - counters derive from the manifest, so row and payload cannot disagree
  ---
  duration_ms: 0.379791
  type: 'test'
  ...
# Subtest: rerank_429_attempts is the count of http_429 attempts, wherever they happened (test 13)
ok 2688 - rerank_429_attempts is the count of http_429 attempts, wherever they happened (test 13)
  ---
  duration_ms: 0.077584
  type: 'test'
  ...
# Subtest: batch order is a property of candidate boundaries, never of completion order (constraint 7)
ok 2689 - batch order is a property of candidate boundaries, never of completion order (constraint 7)
  ---
  duration_ms: 0.3595
  type: 'test'
  ...
# Subtest: neither field-bearing manifest declaration has a field that could carry clinical text
ok 2690 - neither field-bearing manifest declaration has a field that could carry clinical text
  ---
  duration_ms: 0.52875
  type: 'test'
  ...
# Subtest: the ban loop really bans — it fails when a banned field is added
ok 2691 - the ban loop really bans — it fails when a banned field is added
  ---
  duration_ms: 0.12225
  type: 'test'
  ...
# Subtest: TelemetryCapture is not declared in the core — the raw bytes live elsewhere (D5)
ok 2692 - TelemetryCapture is not declared in the core — the raw bytes live elsewhere (D5)
  ---
  duration_ms: 0.359458
  type: 'test'
  ...
# Subtest: StampedRetrievalManifest is EXACTLY the intersection, so nothing can be smuggled through it
ok 2693 - StampedRetrievalManifest is EXACTLY the intersection, so nothing can be smuggled through it
  ---
  duration_ms: 1.223833
  type: 'test'
  ...
# Subtest: every route maps to a class, and an unknown caller is never assigned to the nearest match
ok 2694 - every route maps to a class, and an unknown caller is never assigned to the nearest match
  ---
  duration_ms: 0.077292
  type: 'test'
  ...
# Subtest: the reconciler is an INVOCATION route and never a retrieval route (D17)
ok 2695 - the reconciler is an INVOCATION route and never a retrieval route (D17)
  ---
  duration_ms: 0.043458
  type: 'test'
  ...
# Subtest: the five roles are closed, and the appropriateness exclusion is by ROUTE not by role
ok 2696 - the five roles are closed, and the appropriateness exclusion is by ROUTE not by role
  ---
  duration_ms: 0.043083
  type: 'test'
  ...
# Subtest: usage aggregates by served provider/model, not by intended
ok 2697 - usage aggregates by served provider/model, not by intended
  ---
  duration_ms: 6.516167
  type: 'test'
  ...
# Subtest: a bucket with no usage reports null tokens and counts the unknowns — never zero (§4.6)
ok 2698 - a bucket with no usage reports null tokens and counts the unknowns — never zero (§4.6)
  ---
  duration_ms: 0.067209
  type: 'test'
  ...
# Subtest: partial usage is summed without inventing the missing half
ok 2699 - partial usage is summed without inventing the missing half
  ---
  duration_ms: 0.053042
  type: 'test'
  ...
# Subtest: local, not-served and skipped stages are UNPRICED; unattributed and parse failures are not
ok 2700 - local, not-served and skipped stages are UNPRICED; unattributed and parse failures are not
  ---
  duration_ms: 0.142833
  type: 'test'
  ...
# Subtest: this module prices nothing — money has ONE source of truth
ok 2701 - this module prices nothing — money has ONE source of truth
  ---
  duration_ms: 0.377292
  type: 'test'
  ...
# Subtest: the row contract and the manifest contract version independently (§4.3)
ok 2702 - the row contract and the manifest contract version independently (§4.3)
  ---
  duration_ms: 0.114708
  type: 'test'
  ...
...
ok 2703 - 35.2 — ROW 1, provider success: vertex → vertex, openrouter → openrouter, ollama → local; that route counter += 1, failed unchanged, served model and attempts PRESERVED
  ---
  duration_ms: 0.538833
  type: 'test'
  ...
...
ok 2704 - 35.3 — ROW 2, terminal failure PROVEN to have returned no completion: not_served += 1, failed += 1, served_route_class not_served, served_model null
  ---
  duration_ms: 0.084333
  type: 'test'
  ...
...
ok 2705 - 35.4 — ROW 3, a completion may have arrived and attribution is unavailable: unattributed += 1 (rerank_unattributed_batches), failed FOLLOWS the batch outcome, served_route_class unattributed
  ---
...
ok 2706 - 35.5 — ROW 4, timeout: the attempt records outcome timeout, the batch outcome is timeout, and the served class follows the PROOF rule — not_served with proof, unattributed without
  ---
  duration_ms: 0.10275
  type: 'test'
  ...
...
ok 2707 - 35.6 — ROW 5 and the v11 §6.1 CORRECTION: a SKIPPED expansion stage records served_route_class null (NOT not_served), no route counter, no not_served counter — and attempts: [] , never null
  ---
...
ok 2708 - 35.7 — the v11 §6.1 CORRECTION at the other two sites: ABSENT evidence records attempts [] on a batch and on variant generation, while a transport that did NOT COLLECT (attempts: null on real evidence) keeps null
  ---
...
# [rerank] backend failed, returning input order generic, untyped
# [rerank] backend failed, returning input order generic, untyped
# Subtest: 35.8 — ROW 6 AS AMENDED (v7 §6, v8 §2): a Cohere soft failure without transport proof synthesises one terminal_failure record per planned boundary with served_route_class UNATTRIBUTED — not the not_served D16 wrote — rerank_soft_failed true, and not_served stands only where proof exists
ok 2709 - 35.8 — ROW 6 AS AMENDED (v7 §6, v8 §2): a Cohere soft failure without transport proof synthesises one terminal_failure record per planned boundary with served_route_class UNATTRIBUTED — not the not_served D16 wrote — rerank_soft_failed true, and not_served stands only where proof exists
  ---
  duration_ms: 84.198542
  type: 'test'
  ...
# Subtest: 35.9 — ROW 7, intended local request: exactly one ollama attempt; local on success, not_served on PROVEN failure
ok 2710 - 35.9 — ROW 7, intended local request: exactly one ollama attempt; local on success, not_served on PROVEN failure
  ---
...
ok 2711 - 35.10 — ROW 8, variant parse_failure: served provider, model, attempts and BOTH token counts are PRESERVED, status parse_failure, never not_served
  ---
...
ok 2712 - 35.11 — ROW 9, variant parsed_empty / all_invalid / not_an_array: served provider, model and usage preserved
  ---
  duration_ms: 0.078458
  type: 'test'
  ...
...
ok 2713 - 35.12 — ROW 10 and the v11 §6.2 CORRECTION: a variant-generation stage that RAN and failed_open records not_served ONLY with proof, otherwise UNATTRIBUTED — and only a stage that did NOT run records the stage-level null
  ---
  duration_ms: 0.507208
  type: 'test'
  ...
...
ok 2714 - 35.13 — D16, Bedrock defensively: a Bedrock completion on this path is unattributed, never quietly mapped to a plausible class
  ---
  duration_ms: 0.218
  type: 'test'
  ...
...
ok 2715 - 19 — the predeclared run id is the one every later write targets, and no second row is inserted
  ---
  duration_ms: 3.382791
  type: 'test'
  ...
# Subtest: 20 — declare returns 0, the terminal write returns 1, and a stale handle is REJECTED
ok 2716 - 20 — declare returns 0, the terminal write returns 1, and a stale handle is REJECTED
  ---
  duration_ms: 0.666208
  type: 'test'
  ...
# Subtest: 20 — revisions advance PER ROLE: a normative write cannot invalidate the primary's handle
ok 2717 - 20 — revisions advance PER ROLE: a normative write cannot invalidate the primary's handle
  ---
  duration_ms: 0.888416
  type: 'test'
  ...
# Subtest: 20 — a terminal write that matches nothing is NOT retried, and does not advance the handle
ok 2718 - 20 — a terminal write that matches nothing is NOT retried, and does not advance the handle
  ---
  duration_ms: 0.254708
  type: 'test'
  ...
# Subtest: 31 — the reuse guard returns BEFORE any telemetry statement exists
ok 2719 - 31 — the reuse guard returns BEFORE any telemetry statement exists
  ---
  duration_ms: 0.295333
  type: 'test'
  ...
# Subtest: 32 — the attached handle is absent from JSON.stringify, from the keys, and from a spread
ok 2720 - 32 — the attached handle is absent from JSON.stringify, from the keys, and from a spread
  ---
  duration_ms: 0.096375
  type: 'test'
  ...
# Subtest: 34 — terminal write fails, failure row fails: the invocation counter is the only evidence left
ok 2721 - 34 — terminal write fails, failure row fails: the invocation counter is the only evidence left
  ---
  duration_ms: 0.316333
  type: 'test'
  ...
# Subtest: 34 — and when the counter ALSO fails: a log line, nothing else, still no propagation
ok 2722 - 34 — and when the counter ALSO fails: a log line, nothing else, still no propagation
  ---
  duration_ms: 0.292292
  type: 'test'
  ...
# Subtest: 34 — NO MODULE-LEVEL COUNTER EXISTS ANYWHERE, asserted by source search
ok 2723 - 34 — NO MODULE-LEVEL COUNTER EXISTS ANYWHERE, asserted by source search
  ---
  duration_ms: 0.567875
  type: 'test'
  ...
# Subtest: 52 — every owner in the D9 matrix settles, including both scripts and both MCP paths
ok 2724 - 52 — every owner in the D9 matrix settles, including both scripts and both MCP paths
  ---
  duration_ms: 0.937125
  type: 'test'
  ...
# Subtest: 53 — the callback carries the audit id, and its failure never changes the save result
ok 2725 - 53 — the callback carries the audit id, and its failure never changes the save result
  ---
...
ok 2726 - CANARY-GATE HAZARD — primary rejected, normative lands: the audit persisted and the Stage 0b primary-link gate fails
  ---
  duration_ms: 12.813541
  type: 'test'
  ...
# Subtest: CANARY-GATE HAZARD — the mirror: primary lands, normative rejected
ok 2727 - CANARY-GATE HAZARD — the mirror: primary lands, normative rejected
  ---
  duration_ms: 3.328417
  type: 'test'
  ...
# Subtest: 54 — fourteen states, two of them non-terminal, and the two sets partition the whole
ok 2728 - 54 — fourteen states, two of them non-terminal, and the two sets partition the whole
  ---
  duration_ms: 1.11425
  type: 'test'
  ...
# Subtest: 54 — the implemented table IS D12's table, in both directions
ok 2729 - 54 — the implemented table IS D12's table, in both directions
  ---
  duration_ms: 0.44225
  type: 'test'
  ...
# Subtest: 54 — every one of the 196 ordered pairs answers the way D12 says
ok 2730 - 54 — every one of the 196 ordered pairs answers the way D12 says
  ---
  duration_ms: 0.224375
  type: 'test'
  ...
# Subtest: 54 — TERMINAL STATES NEVER TRANSITION, to anything, including themselves
ok 2731 - 54 — TERMINAL STATES NEVER TRANSITION, to anything, including themselves
  ---
  duration_ms: 0.114958
  type: 'test'
  ...
# Subtest: 54 — the two deliberate asymmetries are both present, and are not accidents
ok 2732 - 54 — the two deliberate asymmetries are both present, and are not accidents
  ---
...
ok 2733 - 54 — every settlement outcome lands on a state, and the settlement table names none of the three reconciler-mapped states
  ---
  duration_ms: 0.09525
  type: 'test'
  ...
# Subtest: 54 — every reconciler-assigned state is a LEGAL transition from the state it is assigned from
ok 2734 - 54 — every reconciler-assigned state is a LEGAL transition from the state it is assigned from
  ---
  duration_ms: 0.113
  type: 'test'
  ...
...
ok 2735 - 45.0 — the fixture is CLEAN: the valid manifest returns no code, so every code below is caused by the one mutation named beside it
  ---
  duration_ms: 3.434167
  type: 'test'
  ...
# Subtest: 45.2 — manifest_schema_version: null → manifest_version_unrecognized
ok 2736 - 45.2 — manifest_schema_version: null → manifest_version_unrecognized
  ---
  duration_ms: 0.23125
  type: 'test'
  ...
# Subtest: 45.3 — manifest_schema_version: wrong version 2 → manifest_version_unrecognized
ok 2737 - 45.3 — manifest_schema_version: wrong version 2 → manifest_version_unrecognized
  ---
  duration_ms: 0.090125
  type: 'test'
  ...
# Subtest: 45.4 — hmac_key_version: null WITHOUT hmac_key_absent → hmac_key_version_absent
ok 2738 - 45.4 — hmac_key_version: null WITHOUT hmac_key_absent → hmac_key_version_absent
  ---
  duration_ms: 0.08125
  type: 'test'
  ...
# Subtest: 45.5 — hmac_key_version: empty string → hmac_key_version_absent
ok 2739 - 45.5 — hmac_key_version: empty string → hmac_key_version_absent
  ---
  duration_ms: 0.076959
  type: 'test'
  ...
# Subtest: 45.6 — operational: missing → operational_absent
ok 2740 - 45.6 — operational: missing → operational_absent
  ---
  duration_ms: 0.270583
  type: 'test'
  ...
# Subtest: 45.7 — operational.route: missing → route_absent_or_invalid
ok 2741 - 45.7 — operational.route: missing → route_absent_or_invalid
  ---
  duration_ms: 0.087334
  type: 'test'
  ...
# Subtest: 45.8 — operational.route: invalid (not in RETRIEVAL_ROUTES) → route_absent_or_invalid
ok 2742 - 45.8 — operational.route: invalid (not in RETRIEVAL_ROUTES) → route_absent_or_invalid
  ---
  duration_ms: 0.111333
  type: 'test'
  ...
# Subtest: 45.9 — operational.route_class: null → route_class_absent
ok 2743 - 45.9 — operational.route_class: null → route_class_absent
  ---
  duration_ms: 0.226125
  type: 'test'
  ...
# Subtest: 45.10 — operational.retrieval_role: invalid → retrieval_role_absent_or_invalid
ok 2744 - 45.10 — operational.retrieval_role: invalid → retrieval_role_absent_or_invalid
  ---
  duration_ms: 0.296916
  type: 'test'
  ...
# Subtest: 45.11 — operational.started_at: null → started_at_absent
ok 2745 - 45.11 — operational.started_at: null → started_at_absent
  ---
  duration_ms: 0.064667
  type: 'test'
  ...
# Subtest: 45.12 — operational.completed_at: empty string → completed_at_absent
ok 2746 - 45.12 — operational.completed_at: empty string → completed_at_absent
  ---
  duration_ms: 0.049375
  type: 'test'
  ...
# Subtest: 45.13 — operational.invocation_id: empty string → invocation_id_absent
ok 2747 - 45.13 — operational.invocation_id: empty string → invocation_id_absent
  ---
  duration_ms: 0.048209
  type: 'test'
  ...
# Subtest: 45.14 — operational.trace_id: missing (null is permitted) → trace_id_field_absent
ok 2748 - 45.14 — operational.trace_id: missing (null is permitted) → trace_id_field_absent
  ---
  duration_ms: 0.046875
  type: 'test'
  ...
# Subtest: 45.15 — operational.deployment_sha: missing (null is permitted) → deployment_sha_field_absent
ok 2749 - 45.15 — operational.deployment_sha: missing (null is permitted) → deployment_sha_field_absent
  ---
  duration_ms: 0.053375
  type: 'test'
  ...
# Subtest: 45.16 — operational.routing_flags: null ({} is permitted) → routing_flags_absent
ok 2750 - 45.16 — operational.routing_flags: null ({} is permitted) → routing_flags_absent
  ---
  duration_ms: 0.045167
  type: 'test'
  ...
# Subtest: 45.17 — operational.routing_flags: an array is not a flags object → routing_flags_absent
ok 2751 - 45.17 — operational.routing_flags: an array is not a flags object → routing_flags_absent
  ---
...
ok 2752 - 45.18 — operational.active_backfill_run_id: missing (null is permitted) → active_backfill_run_id_field_absent
  ---
...
ok 2753 - 45.19 — operational.active_backfill_target: missing (null is permitted) → active_backfill_target_field_absent
  ---
...
ok 2754 - 45.20 — operational.active_backfill_state: missing (null is permitted) → active_backfill_state_field_absent
  ---
...
ok 2755 - 45.21 — operational.active_backfill_state: invalid (not active/idle) → active_backfill_state_invalid
  ---
  duration_ms: 0.044583
  type: 'test'
  ...
# Subtest: 45.22 — operational.active_lab_experiment_id: missing (null is permitted) → active_lab_experiment_id_field_absent
ok 2756 - 45.22 — operational.active_lab_experiment_id: missing (null is permitted) → active_lab_experiment_id_field_absent
  ---
  duration_ms: 0.048959
  type: 'test'
  ...
# Subtest: 45.23 — retrieval_outcome: null → retrieval_outcome_absent_or_invalid
ok 2757 - 45.23 — retrieval_outcome: null → retrieval_outcome_absent_or_invalid
  ---
  duration_ms: 0.046667
  type: 'test'
  ...
# Subtest: 45.24 — retrieval_outcome: invalid → retrieval_outcome_absent_or_invalid
ok 2758 - 45.24 — retrieval_outcome: invalid → retrieval_outcome_absent_or_invalid
  ---
...
ok 2759 - 45.25 — retrieval_error_class: missing (null is permitted on success) → retrieval_error_class_field_absent
  ---
...
ok 2783 - 45.49 — pre_rerank_passage_hmacs: cardinality ≠ hydrated_candidate_ids (one per HYDRATED row) → passage_hmac_cardinality_mismatch
  ---
  duration_ms: 0.044709
  type: 'test'
  ...
# Subtest: 45.50 — intended_backend: null (the string none is permitted) → intended_backend_absent
ok 2784 - 45.50 — intended_backend: null (the string none is permitted) → intended_backend_absent
  ---
  duration_ms: 0.044958
  type: 'test'
  ...
# Subtest: 45.51 — intended_backend: empty string → intended_backend_absent
ok 2785 - 45.51 — intended_backend: empty string → intended_backend_absent
  ---
  duration_ms: 0.043042
  type: 'test'
  ...
# Subtest: 45.52 — intended_model: empty string → intended_model_absent
ok 2786 - 45.52 — intended_model: empty string → intended_model_absent
  ---
  duration_ms: 0.043042
  type: 'test'
  ...
# Subtest: 45.53 — served_backend: missing (null is permitted with no batches) → served_backend_field_absent
ok 2787 - 45.53 — served_backend: missing (null is permitted with no batches) → served_backend_field_absent
  ---
  duration_ms: 0.045333
  type: 'test'
  ...
# Subtest: 45.54 — served_backend: null once a batch record exists → served_backend_absent_with_batches
ok 2788 - 45.54 — served_backend: null once a batch record exists → served_backend_absent_with_batches
  ---
  duration_ms: 0.046542
  type: 'test'
  ...
# Subtest: 45.55 — rerank_backend_downgraded: null → rerank_backend_downgraded_absent
ok 2789 - 45.55 — rerank_backend_downgraded: null → rerank_backend_downgraded_absent
  ---
  duration_ms: 0.042708
  type: 'test'
  ...
# Subtest: 45.56 — rerank_soft_failed: a string, not a boolean → rerank_soft_failed_absent
ok 2790 - 45.56 — rerank_soft_failed: a string, not a boolean → rerank_soft_failed_absent
  ---
  duration_ms: 0.041708
  type: 'test'
  ...
# Subtest: 45.57 — expected_batch_count: null → expected_batch_count_absent
ok 2791 - 45.57 — expected_batch_count: null → expected_batch_count_absent
  ---
  duration_ms: 0.054292
  type: 'test'
  ...
# Subtest: 45.58 — recorded_rerank_batches: invalid number: NaN → recorded_rerank_batches_absent
ok 2792 - 45.58 — recorded_rerank_batches: invalid number: NaN → recorded_rerank_batches_absent
  ---
  duration_ms: 0.055125
  type: 'test'
  ...
# Subtest: 45.59 — recorded_rerank_batches: disagrees with batches.length → recorded_batch_count_mismatch
ok 2793 - 45.59 — recorded_rerank_batches: disagrees with batches.length → recorded_batch_count_mismatch
  ---
  duration_ms: 0.04475
  type: 'test'
  ...
# Subtest: 45.60 — expected_batch_count: disagrees with batches.length (§7, never waived) → batch_count_mismatch
ok 2794 - 45.60 — expected_batch_count: disagrees with batches.length (§7, never waived) → batch_count_mismatch
  ---
  duration_ms: 0.040708
  type: 'test'
  ...
# Subtest: 45.61 — batches: null ([] is permitted) → batches_absent
ok 2795 - 45.61 — batches: null ([] is permitted) → batches_absent
  ---
  duration_ms: 0.042125
  type: 'test'
  ...
# Subtest: 45.62 — batch.batch_index: missing → batch_index_absent
ok 2796 - 45.62 — batch.batch_index: missing → batch_index_absent
  ---
  duration_ms: 0.058125
  type: 'test'
  ...
# Subtest: 45.63 — batch.batch_index: duplicated → duplicate_batch_index
ok 2797 - 45.63 — batch.batch_index: duplicated → duplicate_batch_index
  ---
  duration_ms: 0.064958
  type: 'test'
  ...
# Subtest: 45.64 — batch.candidate_start/end: null → batch_boundaries_absent
ok 2798 - 45.64 — batch.candidate_start/end: null → batch_boundaries_absent
  ---
  duration_ms: 0.043042
  type: 'test'
  ...
# Subtest: 45.65 — batch.candidate_start/end: end <= start → bad_candidate_boundaries
ok 2799 - 45.65 — batch.candidate_start/end: end <= start → bad_candidate_boundaries
  ---
  duration_ms: 0.043209
  type: 'test'
  ...
# Subtest: 45.66 — batch.intended_provider: empty string → batch_intended_provider_absent
ok 2800 - 45.66 — batch.intended_provider: empty string → batch_intended_provider_absent
  ---
  duration_ms: 0.044459
  type: 'test'
  ...
# Subtest: 45.67 — batch.intended_model: null → batch_intended_model_absent
ok 2801 - 45.67 — batch.intended_model: null → batch_intended_model_absent
  ---
  duration_ms: 0.045959
  type: 'test'
  ...
# Subtest: 45.68 — batch.served_route_class: missing → batch_served_route_class_absent
ok 2802 - 45.68 — batch.served_route_class: missing → batch_served_route_class_absent
  ---
  duration_ms: 0.047791
  type: 'test'
  ...
# Subtest: 45.69 — batch.served_route_class: null — the type permits it, the contract does not (A6) → batch_served_route_class_absent
ok 2803 - 45.69 — batch.served_route_class: null — the type permits it, the contract does not (A6) → batch_served_route_class_absent
  ---
  duration_ms: 0.044583
  type: 'test'
  ...
# Subtest: 45.70 — batch.served_route_class: invalid → batch_served_route_class_invalid
ok 2804 - 45.70 — batch.served_route_class: invalid → batch_served_route_class_invalid
  ---
  duration_ms: 0.09025
  type: 'test'
  ...
# Subtest: 45.71 — batch.served_model: missing (null is permitted) → batch_served_model_field_absent
ok 2805 - 45.71 — batch.served_model: missing (null is permitted) → batch_served_model_field_absent
  ---
  duration_ms: 0.060875
  type: 'test'
  ...
# Subtest: 45.72 — batch.served_model: non-null on an unattributed batch (§10) → unattributed_with_model
ok 2806 - 45.72 — batch.served_model: non-null on an unattributed batch (§10) → unattributed_with_model
  ---
  duration_ms: 0.047417
  type: 'test'
  ...
# Subtest: 45.73 — batch.served_model: non-null on a not_served batch (§10) → not_served_with_model
ok 2807 - 45.73 — batch.served_model: non-null on a not_served batch (§10) → not_served_with_model
  ---
  duration_ms: 0.045167
  type: 'test'
  ...
# Subtest: 45.74 — batch.attempts: missing (null is permitted) → batch_attempts_field_absent
ok 2808 - 45.74 — batch.attempts: missing (null is permitted) → batch_attempts_field_absent
  ---
  duration_ms: 0.045416
  type: 'test'
  ...
# Subtest: 45.75 — batch.attempts: a member with an outcome outside the six → attempt_outcome_absent_or_invalid
ok 2809 - 45.75 — batch.attempts: a member with an outcome outside the six → attempt_outcome_absent_or_invalid
  ---
  duration_ms: 0.049167
  type: 'test'
  ...
# Subtest: 45.76 — batch.outcome: invalid → batch_outcome_absent_or_invalid
ok 2810 - 45.76 — batch.outcome: invalid → batch_outcome_absent_or_invalid
  ---
  duration_ms: 0.043458
  type: 'test'
  ...
# Subtest: 45.77 — batch.expected_score_keys: null → expected_score_keys_absent
ok 2811 - 45.77 — batch.expected_score_keys: null → expected_score_keys_absent
  ---
  duration_ms: 0.047917
  type: 'test'
  ...
# Subtest: 45.78 — batch.finite_score_keys: invalid number: negative → finite_score_keys_absent
ok 2812 - 45.78 — batch.finite_score_keys: invalid number: negative → finite_score_keys_absent
  ---
  duration_ms: 0.044375
  type: 'test'
  ...
# Subtest: 45.79 — batch.finite_score_keys: more finite keys than expected → score_keys_exceed_expected
ok 2813 - 45.79 — batch.finite_score_keys: more finite keys than expected → score_keys_exceed_expected
  ---
  duration_ms: 0.042917
  type: 'test'
  ...
# Subtest: 45.80 — ordered_final_candidate_ids: null ([] is permitted) → ordered_final_candidate_ids_absent
ok 2814 - 45.80 — ordered_final_candidate_ids: null ([] is permitted) → ordered_final_candidate_ids_absent
  ---
  duration_ms: 0.046667
  type: 'test'
  ...
# Subtest: 45.81 — retrieval_config: null → retrieval_config_absent
ok 2815 - 45.81 — retrieval_config: null → retrieval_config_absent
  ---
  duration_ms: 0.04675
  type: 'test'
  ...
# Subtest: 45.82 — retrieval_config: an array is not a config object → retrieval_config_absent
ok 2816 - 45.82 — retrieval_config: an array is not a config object → retrieval_config_absent
  ---
  duration_ms: 0.044167
  type: 'test'
  ...
# Subtest: 45.83 — retrieval_config.rerank_temperature: missing (v7 §10: required as of manifest v3; null is permitted) → rerank_temperature_field_absent
ok 2817 - 45.83 — retrieval_config.rerank_temperature: missing (v7 §10: required as of manifest v3; null is permitted) → rerank_temperature_field_absent
  ---
  duration_ms: 0.045041
  type: 'test'
  ...
# Subtest: 45.84 — retrieval_config.rerank_temperature: invalid number: NaN → rerank_temperature_invalid
ok 2818 - 45.84 — retrieval_config.rerank_temperature: invalid number: NaN → rerank_temperature_invalid
  ---
  duration_ms: 0.044792
  type: 'test'
  ...
# Subtest: 45.85 — retrieval_config.rerank_seed_status: missing (v7 §10) → rerank_seed_status_field_absent
ok 2819 - 45.85 — retrieval_config.rerank_seed_status: missing (v7 §10) → rerank_seed_status_field_absent
  ---
  duration_ms: 0.048208
  type: 'test'
  ...
# Subtest: 45.86 — retrieval_config.rerank_seed_status: null — never null, not_applicable is the value for no decode → rerank_seed_status_invalid
ok 2820 - 45.86 — retrieval_config.rerank_seed_status: null — never null, not_applicable is the value for no decode → rerank_seed_status_invalid
  ---
  duration_ms: 0.050333
  type: 'test'
  ...
# Subtest: 45.87 — corpus_version: missing (null is permitted) → corpus_version_field_absent
ok 2821 - 45.87 — corpus_version: missing (null is permitted) → corpus_version_field_absent
  ---
  duration_ms: 0.052167
  type: 'test'
  ...
# Subtest: 45.88 — index_version: null → index_version_absent
ok 2822 - 45.88 — index_version: null → index_version_absent
  ---
  duration_ms: 0.043958
  type: 'test'
  ...
# Subtest: 45.89 — index_version: empty string → index_version_absent
ok 2823 - 45.89 — index_version: empty string → index_version_absent
  ---
  duration_ms: 0.044083
  type: 'test'
  ...
# Subtest: 45.90 — scorer_context_hmac: missing → scorer_context_hmac_field_absent
ok 2824 - 45.90 — scorer_context_hmac: missing → scorer_context_hmac_field_absent
  ---
  duration_ms: 0.046667
  type: 'test'
  ...
# Subtest: 45.91 — scorer_context_hmac: null on role primary WITHOUT hmac_key_absent → scorer_context_hmac_absent
ok 2825 - 45.91 — scorer_context_hmac: null on role primary WITHOUT hmac_key_absent → scorer_context_hmac_absent
  ---
  duration_ms: 0.044333
  type: 'test'
  ...
# Subtest: 45.92 — multi_query: present on a role that is not lab_multi_query → multi_query_on_non_multi_query_role
ok 2826 - 45.92 — multi_query: present on a role that is not lab_multi_query → multi_query_on_non_multi_query_role
  ---
  duration_ms: 0.05875
  type: 'test'
  ...
# Subtest: 45.93 — the lab_multi_query fixture is CLEAN before its rows run
ok 2827 - 45.93 — the lab_multi_query fixture is CLEAN before its rows run
  ---
  duration_ms: 0.078625
  type: 'test'
  ...
# Subtest: 45.94 — multi_query: missing on lab_multi_query → multi_query_absent
ok 2828 - 45.94 — multi_query: missing on lab_multi_query → multi_query_absent
  ---
  duration_ms: 0.064167
  type: 'test'
  ...
# Subtest: 45.95 — multi_query.variant_generation: null → variant_generation_absent
ok 2829 - 45.95 — multi_query.variant_generation: null → variant_generation_absent
  ---
  duration_ms: 0.04775
  type: 'test'
  ...
# Subtest: 45.96 — multi_query.variant_generation.status: invalid → variant_generation_status_absent_or_invalid
ok 2830 - 45.96 — multi_query.variant_generation.status: invalid → variant_generation_status_absent_or_invalid
  ---
  duration_ms: 0.048958
  type: 'test'
  ...
# Subtest: 45.97 — multi_query.variant_generation.served_route_class: missing (null is permitted for a stage that did not run) → variant_generation_served_route_class_field_absent
ok 2831 - 45.97 — multi_query.variant_generation.served_route_class: missing (null is permitted for a stage that did not run) → variant_generation_served_route_class_field_absent
  ---
  duration_ms: 0.0495
  type: 'test'
  ...
# Subtest: 45.98 — multi_query.variant_generation.generated_variant_count: null → generated_variant_count_absent
ok 2832 - 45.98 — multi_query.variant_generation.generated_variant_count: null → generated_variant_count_absent
  ---
  duration_ms: 0.047667
  type: 'test'
  ...
# Subtest: 45.99 — multi_query.variant_generation.attempts: a member with an outcome outside the six → attempt_outcome_absent_or_invalid
ok 2833 - 45.99 — multi_query.variant_generation.attempts: a member with an outcome outside the six → attempt_outcome_absent_or_invalid
  ---
  duration_ms: 0.082208
  type: 'test'
  ...
# Subtest: 45.100 — multi_query.variants: null → variants_absent
ok 2834 - 45.100 — multi_query.variants: null → variants_absent
  ---
  duration_ms: 0.347625
  type: 'test'
  ...
# Subtest: 45.101 — multi_query.variants: length ≠ generated_variant_count + 1 → variant_arity_mismatch
ok 2835 - 45.101 — multi_query.variants: length ≠ generated_variant_count + 1 → variant_arity_mismatch
  ---
  duration_ms: 0.319042
  type: 'test'
  ...
...
ok 2836 - 45.1 — OWN-PROPERTY CHECKS: missing, explicit null, empty array, empty string and invalid number are FIVE different answers, and the validator tells them apart
  ---
  duration_ms: 2.507083
  type: 'test'
  ...
# Subtest: 45.102 — the HMAC-absent licence covers EXACTLY the four D8 fields, and only when telemetry_error declares it
ok 2837 - 45.102 — the HMAC-absent licence covers EXACTLY the four D8 fields, and only when telemetry_error declares it
  ---
...
ok 2838 - 46.1 — expansion.served_route_class null with status skipped is valid: no expansion code, so a normative_channel row is not partial by construction
  ---
...
ok 2839 - 46.2 — through the REAL builder: a normative_channel capture (expansion never set — the leg sets skipExpand unconditionally) produces a payload that validates clean
  ---
  duration_ms: 0.462
  type: 'test'
  ...
...
ok 2840 - 47.1 — on role primary the scorer-context HMAC is REQUIRED: computed over the EXACT citedContext bytes, and it changes when one byte does
  ---
  duration_ms: 0.833791
  type: 'test'
  ...
...
ok 2841 - 47.2 — the EMPTY-STRING case: zero candidates render an empty citedContext, and the HMAC of the empty string is a DEFINED value — never null because reranking was skipped
  ---
  duration_ms: 0.421667
  type: 'test'
  ...
# Subtest: 47.3 — on the other FOUR roles the HMAC is null, and those nulls are NOT partial
ok 2842 - 47.3 — on the other FOUR roles the HMAC is null, and those nulls are NOT partial
  ---
...
ok 2843 - 49.1 — EMPTY FUSION: retrieve() returns before the rerank block exists — fused 0, hydrated 0, expected 0, recorded 0, batches [], backend and model none
  ---
...
ok 2844 - 49.2 — HYDRATE EMPTIED: fused > 0, hydrated 0 — the same shape, and the TWO COUNTS DIFFER, which is the point
  ---
  duration_ms: 0.26075
  type: 'test'
  ...
...
ok 2845 - 49.3 — the two zero-candidate shapes are DISTINGUISHABLE: empty fusion (0/0) and hydrate emptied (3/0) differ in fused_candidate_count and agree in hydrated_candidate_count
  ---
  duration_ms: 0.146084
  type: 'test'
  ...
...
ok 2846 - 49.4 — ONE HYDRATED CANDIDATE: rerank() is never entered — hydrated 1, expected 0, recorded 0, batches [], backend and model none
  ---
...
ok 2847 - 49.5 — RERANKER DISABLED (normative_channel always; lab_direct when the caller sets it): several hydrated candidates, the same shape as the one-candidate case
  ---
...
ok 2848 - default path: filter clause array is byte-identical to production, no params
  ---
  duration_ms: 0.94175
  type: 'test'
  ...
# Subtest: default path with structural filters keeps the base guards and remaps $FP offsets per leg
ok 2849 - default path with structural filters keeps the base guards and remaps $FP offsets per leg
  ---
  duration_ms: 0.116916
  type: 'test'
  ...
# Subtest: relaxed path: both quarantine guards gain a bound OR arm on both legs
ok 2850 - relaxed path: both quarantine guards gain a bound OR arm on both legs
  ---
  duration_ms: 0.218333
  type: 'test'
  ...
# Subtest: relaxed path ordering: the quarantine label takes $FP_0, structural filters follow
ok 2851 - relaxed path ordering: the quarantine label takes $FP_0, structural filters follow
  ---
  duration_ms: 0.092
  type: 'test'
  ...
# Subtest: hostile labels are slugged by labLabel and cannot widen the filter
ok 2852 - hostile labels are slugged by labLabel and cannot widen the filter
  ---
  duration_ms: 0.241708
  type: 'test'
  ...
# Subtest: empty/whitespace includeQuarantined is treated as omitted (byte-identical default path)
ok 2853 - empty/whitespace includeQuarantined is treated as omitted (byte-identical default path)
  ---
  duration_ms: 0.103542
  type: 'test'
  ...
# Subtest: clampLabRetrieveTopK clamps to [1,20], default 8
ok 2854 - clampLabRetrieveTopK clamps to [1,20], default 8
  ---
...
ok 2855 - restrictSources omitted ⇒ default clauses + no params (byte-identical)
  ---
  duration_ms: 0.753541
  type: 'test'
  ...
# Subtest: restrictSources: [choosing-wisely] ⇒ both legs source = ANY, bound array param
ok 2856 - restrictSources: [choosing-wisely] ⇒ both legs source = ANY, bound array param
  ---
  duration_ms: 0.128208
  type: 'test'
  ...
# Subtest: a named labq: source is admitted through the quarantine guard; un-named stays excluded
ok 2857 - a named labq: source is admitted through the quarantine guard; un-named stays excluded
  ---
  duration_ms: 0.160625
  type: 'test'
  ...
# Subtest: empty or all-blank restrictSources falls back to the default filter (no restriction)
ok 2858 - empty or all-blank restrictSources falls back to the default filter (no restriction)
  ---
  duration_ms: 0.135125
  type: 'test'
  ...
# Subtest: restrictSources stacks with book/chunk filters and supersedes the single-source filter
ok 2859 - restrictSources stacks with book/chunk filters and supersedes the single-source filter
  ---
  duration_ms: 0.05475
  type: 'test'
  ...
# Subtest: group key exactness: same (subject, signal_type) groups; different signal splits
ok 2860 - group key exactness: same (subject, signal_type) groups; different signal splits
  ---
  duration_ms: 1.319792
  type: 'test'
  ...
# Subtest: '' signal folds like stewardship (empty signal groups together)
ok 2861 - '' signal folds like stewardship (empty signal groups together)
  ---
  duration_ms: 0.104334
  type: 'test'
  ...
# Subtest: ≥2 threshold: a pair groups, a singleton does not
ok 2862 - ≥2 threshold: a pair groups, a singleton does not
  ---
  duration_ms: 0.39325
  type: 'test'
  ...
# Subtest: section order + group sort (size desc, newest tie-break) + singles in queue order
ok 2863 - section order + group sort (size desc, newest tie-break) + singles in queue order
  ---
  duration_ms: 0.315917
  type: 'test'
  ...
# Subtest: disagreement pinning: disagreements section leads the order, groups/singles below
ok 2864 - disagreement pinning: disagreements section leads the order, groups/singles below
  ---
  duration_ms: 0.160542
  type: 'test'
  ...
# Subtest: traversal: within-group → next group → singles; wrap; exhausted → null
ok 2865 - traversal: within-group → next group → singles; wrap; exhausted → null
  ---
  duration_ms: 0.111834
  type: 'test'
  ...
# Subtest: skip sinks within its section and traversal passes it over
ok 2866 - skip sinks within its section and traversal passes it over
  ---
  duration_ms: 0.143417
  type: 'test'
  ...
# Subtest: labeled stays in place (not sunk) with its status; k/n progress counts labeled+skipped
ok 2867 - labeled stays in place (not sunk) with its status; k/n progress counts labeled+skipped
  ---
  duration_ms: 0.09275
  type: 'test'
  ...
# Subtest: determinism: same input → identical output
ok 2868 - determinism: same input → identical output
  ---
  duration_ms: 0.225667
  type: 'test'
  ...
# Subtest: hashBucket is deterministic and in 0..99
ok 2869 - hashBucket is deterministic and in 0..99
  ---
  duration_ms: 3.621375
  type: 'test'
  ...
# Subtest: overlap is ~20% and buckets < 20
ok 2870 - overlap is ~20% and buckets < 20
  ---
  duration_ms: 1.138542
  type: 'test'
  ...
# Subtest: overlap findings are served to EVERY reviewer; partitioned to exactly one
ok 2871 - overlap findings are served to EVERY reviewer; partitioned to exactly one
  ---
  duration_ms: 0.692875
  type: 'test'
  ...
# Subtest: a reviewer not on the roster still gets the overlap set (only)
ok 2872 - a reviewer not on the roster still gets the overlap set (only)
  ---
  duration_ms: 0.151584
  type: 'test'
  ...
# Subtest: partition is roughly even across the roster
ok 2873 - partition is roughly even across the roster
  ---
  duration_ms: 0.178625
  type: 'test'
  ...
# Subtest: balanceBySignalType interleaves types and is newest-first within a type
ok 2874 - balanceBySignalType interleaves types and is newest-first within a type
  ---
  duration_ms: 0.496459
  type: 'test'
  ...
# Subtest: disagreement items come first, then fresh; limit respected
ok 2875 - disagreement items come first, then fresh; limit respected
  ---
  duration_ms: 0.305041
  type: 'test'
  ...
# Subtest: passthrough: optional uid + prescription_url survive buildReviewQueue onto emitted items
ok 2876 - passthrough: optional uid + prescription_url survive buildReviewQueue onto emitted items
  ---
  duration_ms: 0.084542
  type: 'test'
  ...
# Subtest: excludes labeled-by-this-reviewer, informational, unassigned, and filtered-out findings
ok 2877 - excludes labeled-by-this-reviewer, informational, unassigned, and filtered-out findings
  ---
  duration_ms: 0.43325
  type: 'test'
  ...
# Subtest: parseGoal: valid / missing / garbage → exact defaults; personal ceil
ok 2878 - parseGoal: valid / missing / garbage → exact defaults; personal ceil
  ---
  duration_ms: 1.173542
  type: 'test'
  ...
# Subtest: prevDay + istWeekStart (Monday-start)
ok 2879 - prevDay + istWeekStart (Monday-start)
  ---
  duration_ms: 1.975583
  type: 'test'
  ...
# Subtest: countedLabels: impact excluded, missed included, roster filter, finding current-state (later wins)
ok 2880 - countedLabels: impact excluded, missed included, roster filter, finding current-state (later wins)
  ---
  duration_ms: 0.420625
  type: 'test'
  ...
# Subtest: streak: threshold exactly 15, consecutive, yesterday-grace, today-only, gap → 0
ok 2881 - streak: threshold exactly 15, consecutive, yesterday-grace, today-only, gap → 0
  ---
  duration_ms: 0.512833
  type: 'test'
  ...
# Subtest: agreement: pair construction (2 & 3 reviewers), tier match/mismatch, overlap-only
ok 2882 - agreement: pair construction (2 & 3 reviewers), tier match/mismatch, overlap-only
  ---
  duration_ms: 0.568292
  type: 'test'
  ...
# Subtest: agreement: current-state dedup feeds pairs (later verdict wins), then match recomputed
ok 2883 - agreement: current-state dedup feeds pairs (later verdict wins), then match recomputed
  ---
  duration_ms: 0.29475
  type: 'test'
  ...
# Subtest: computeReviewStats: ≥20-pair display boundary, week total, badges shape
ok 2884 - computeReviewStats: ≥20-pair display boundary, week total, badges shape
  ---
  duration_ms: 1.010833
  type: 'test'
  ...
# Subtest: the committed gold artifact is frozen, ratified, and catalog-consistent
ok 2885 - the committed gold artifact is frozen, ratified, and catalog-consistent
  ---
  duration_ms: 3.961708
  type: 'test'
  ...
# Subtest: loadCheckGold rejects drift: wrong version, unratified, polarity/target mismatch
ok 2886 - loadCheckGold rejects drift: wrong version, unratified, polarity/target mismatch
  ---
  duration_ms: 2.298875
  type: 'test'
  ...
# Subtest: the committed 2.0 artifact is frozen, ratified, family-split, and catalog-consistent
ok 2887 - the committed 2.0 artifact is frozen, ratified, family-split, and catalog-consistent
  ---
  duration_ms: 0.717042
  type: 'test'
  ...
# Subtest: loadCheckGold2: accepts the delivered shape — empty targets legal, L carries annex/memberHistory
ok 2888 - loadCheckGold2: accepts the delivered shape — empty targets legal, L carries annex/memberHistory
  ---
  duration_ms: 0.174542
  type: 'test'
  ...
# Subtest: loadCheckGold2 rejects drift: version, status, dup ids, missing verdict fields, annex misuse
ok 2889 - loadCheckGold2 rejects drift: version, status, dup ids, missing verdict fields, annex misuse
  ---
  duration_ms: 0.544959
  type: 'test'
  ...
# Subtest: splitCheckGold2: P/N/C form the scored floor, L is the annex — never folded together
ok 2890 - splitCheckGold2: P/N/C form the scored floor, L is the annex — never folded together
  ---
  duration_ms: 0.405666
  type: 'test'
  ...
# Subtest: checkGold2CatalogGaps: unbound polarity-side targets are flagged, bound ones are not
ok 2891 - checkGold2CatalogGaps: unbound polarity-side targets are flagged, bound ones are not
  ---
  duration_ms: 0.158292
  type: 'test'
  ...
# Subtest: scoreCheckAgainstGold: per-target-rec, deterministic, ignores non-target firings
ok 2892 - scoreCheckAgainstGold: per-target-rec, deterministic, ignores non-target firings
  ---
  duration_ms: 0.214792
  type: 'test'
  ...
# Subtest: aggregateCheckGold: hand-computed recall / specificity / precision / F1
ok 2893 - aggregateCheckGold: hand-computed recall / specificity / precision / F1
  ---
  duration_ms: 0.305334
  type: 'test'
  ...
# Subtest: Fix A: ANALYZE_SYSTEM carries the verdict discipline (uncertain = equipoise only)
ok 2894 - Fix A: ANALYZE_SYSTEM carries the verdict discipline (uncertain = equipoise only)
  ---
  duration_ms: 0.452042
  type: 'test'
  ...
# Subtest: Fix A: normNetValue contract unchanged, but the parse fallback is now visible
ok 2895 - Fix A: normNetValue contract unchanged, but the parse fallback is now visible
  ---
  duration_ms: 0.163583
  type: 'test'
  ...
# Subtest: Fix B: the two syncope recs are in the seed, verified, unique, well-formed
ok 2896 - Fix B: the two syncope recs are in the seed, verified, unique, well-formed
  ---
  duration_ms: 0.118625
  type: 'test'
  ...
# Subtest: Fix B gold: deterministic recall hits C04 with BOTH new recs, and no other check case
ok 2897 - Fix B gold: deterministic recall hits C04 with BOTH new recs, and no other check case
  ---
  duration_ms: 1.515541
  type: 'test'
  ...
# Subtest: flag-off byte-identical: every grounded builder without the param equals Slice 1 exactly
ok 2898 - flag-off byte-identical: every grounded builder without the param equals Slice 1 exactly
  ---
  duration_ms: 0.823166
  type: 'test'
  ...
# Subtest: grounded: the picture lands between the input and the downstream sections, verbatim
ok 2899 - grounded: the picture lands between the input and the downstream sections, verbatim
  ---
  duration_ms: 0.145334
  type: 'test'
  ...
# Subtest: patientPictureBlock: formatClinicalState content + the two prompt rules
ok 2900 - patientPictureBlock: formatClinicalState content + the two prompt rules
  ---
  duration_ms: 1.387334
  type: 'test'
  ...
# Subtest: grounding flag is double-gated on the master flag
ok 2901 - grounding flag is double-gated on the master flag
  ---
  duration_ms: 0.104208
  type: 'test'
  ...
# Subtest: frozen bank right-care-eval/1.0: pinned, unique ids, per-mode shape
ok 2902 - frozen bank right-care-eval/1.0: pinned, unique ids, per-mode shape
  ---
  duration_ms: 0.765375
  type: 'test'
  ...
# Subtest: pair-judge parser: defensive on directions, safety classes, fences
ok 2903 - pair-judge parser: defensive on directions, safety classes, fences
  ---
  duration_ms: 0.267625
  type: 'test'
  ...
# Subtest: deterministic check diff: added / removed / kept by rec id
ok 2904 - deterministic check diff: added / removed / kept by rec id
  ---
  duration_ms: 0.237375
  type: 'test'
  ...
# Subtest: scorecard gates: FAIL_SAFETY dominates; PASS needs net improvement clearing noise
ok 2905 - scorecard gates: FAIL_SAFETY dominates; PASS needs net improvement clearing noise
  ---
...
ok 2906 - Order check constructs a ClinicalState from the provided input with counts > 0
  ---
  duration_ms: 16.808125
  type: 'test'
  ...
# Subtest: Care pathway constructs from the presentation field with counts > 0
ok 2907 - Care pathway constructs from the presentation field with counts > 0
  ---
  duration_ms: 0.409708
  type: 'test'
  ...
# Subtest: Record audit adapts the existing ExtractedCase and round-trips on the shared fields
ok 2908 - Record audit adapts the existing ExtractedCase and round-trips on the shared fields
  ---
  duration_ms: 17.380584
  type: 'test'
  ...
# Subtest: fail-open: a throwing LLM stage keeps the deterministic state; junk input never throws
ok 2909 - fail-open: a throwing LLM stage keeps the deterministic state; junk input never throws
  ---
  duration_ms: 0.415375
  type: 'test'
  ...
# Subtest: flag-off neutrality: no gate flag → feature inert; UI field off → {}
ok 2910 - flag-off neutrality: no gate flag → feature inert; UI field off → {}
  ---
  duration_ms: 0.5545
  type: 'test'
  ...
# Subtest: save-run reconstruction: same pure builders, schema-valid, per mode
ok 2911 - save-run reconstruction: same pure builders, schema-valid, per mode
  ---
  duration_ms: 0.702625
  type: 'test'
  ...
# Subtest: member link: strict validation, and identity stays OUT of the state
ok 2912 - member link: strict validation, and identity stays OUT of the state
  ---
...
ok 2913 - 1 — a PRIMARY defect leaves the NORMATIVE run clean
  ---
  duration_ms: 5.471709
  type: 'test'
  ...
# Subtest: 2 — a NORMATIVE defect leaves the PRIMARY run clean
ok 2914 - 2 — a NORMATIVE defect leaves the PRIMARY run clean
  ---
  duration_ms: 0.324291
  type: 'test'
  ...
# Subtest: 3 — both roles dirty: both settle dirty
ok 2915 - 3 — both roles dirty: both settle dirty
  ---
  duration_ms: 0.424958
  type: 'test'
  ...
# Subtest: 4 — neither dirty: both settle clean
ok 2916 - 4 — neither dirty: both settle clean
  ---
  duration_ms: 0.204792
  type: 'test'
  ...
# Subtest: 4b — an EMPTY array is clean; an ABSENT key on a linkable run is NOT
ok 2917 - 4b — an EMPTY array is clean; an ABSENT key on a linkable run is NOT
  ---
  duration_ms: 0.217583
  type: 'test'
  ...
# Subtest: 5 — no map at all: a single-role save behaves exactly as before
ok 2918 - 5 — no map at all: a single-role save behaves exactly as before
  ---
  duration_ms: 4.709583
  type: 'test'
  ...
# Subtest: 5b — only the CLEAN branch is upgraded; a losing race or a skip is never made partial
ok 2919 - 5b — only the CLEAN branch is upgraded; a losing race or a skip is never made partial
  ---
  duration_ms: 0.239833
  type: 'test'
  ...
# Subtest: 5c — a revision-0 role is still not linked, and the per-run upgrade did not disturb that
ok 2920 - 5c — a revision-0 role is still not linked, and the per-run upgrade did not disturb that
  ---
  duration_ms: 0.70575
  type: 'test'
  ...
# Subtest: 6 — every persistence owner passes the role map, and none passes an empty one where it holds defects
ok 2921 - 6 — every persistence owner passes the role map, and none passes an empty one where it holds defects
  ---
  duration_ms: 2.603459
  type: 'test'
  ...
# Subtest: 6b — the upgrade is applied in settlement, not by the owners
ok 2922 - 6b — the upgrade is applied in settlement, not by the owners
  ---
  duration_ms: 1.415291
  type: 'test'
  ...
# Subtest: 6c — settleOwned still takes ONE base outcome and makes ONE settlement call
ok 2923 - 6c — settleOwned still takes ONE base outcome and makes ONE settlement call
  ---
  duration_ms: 0.244958
  type: 'test'
  ...
# Subtest: 6d — settlement is fail-safe: a role map on a handle with no runs settles nothing and throws nothing
ok 2924 - 6d — settlement is fail-safe: a role map on a handle with no runs settles nothing and throws nothing
  ---
  duration_ms: 0.065875
  type: 'test'
  ...
# Subtest: 7 — the three cases of `verdictForRun`, stated directly
ok 2925 - 7 — the three cases of `verdictForRun`, stated directly
  ---
  duration_ms: 0.393791
  type: 'test'
  ...
# Subtest: 7b — requirement 10: an INHERITED key is not a verdict
ok 2926 - 7b — requirement 10: an INHERITED key is not a verdict
  ---
  duration_ms: 0.33
  type: 'test'
  ...
# Subtest: 7c — an inherited key does not rescue a run from partial, through settlement
ok 2927 - 7c — an inherited key does not rescue a run from partial, through settlement
  ---
  duration_ms: 0.205583
  type: 'test'
  ...
# Subtest: 7d — THE PLACEMENT TEST: the rule reaches a linkable run and stops at a revision-zero one
ok 2928 - 7d — THE PLACEMENT TEST: the rule reaches a linkable run and stops at a revision-zero one
  ---
  duration_ms: 0.269541
  type: 'test'
  ...
# Subtest: 7e — requirement 3 in the only place it is observable: settlement never mutates the map
ok 2929 - 7e — requirement 3 in the only place it is observable: settlement never mutates the map
  ---
  duration_ms: 0.157375
  type: 'test'
  ...
# Subtest: 7f — the base outcome still decides: only a CLEAN run is made partial by a missing key
ok 2930 - 7f — the base outcome still decides: only a CLEAN run is made partial by a missing key
  ---
  duration_ms: 0.052541
  type: 'test'
  ...
# Subtest: 7g — no new settlement outcome value was added, and nothing writes the synthetic code
ok 2931 - 7g — no new settlement outcome value was added, and nothing writes the synthetic code
  ---
  duration_ms: 0.336167
  type: 'test'
  ...
# Subtest: matchRoomCategory prefers the longest alias and falls back
ok 2932 - matchRoomCategory prefers the longest alias and falls back
  ---
  duration_ms: 0.586667
  type: 'test'
  ...
# Subtest: excessBedDays = LOS − benchmark, floored at 0
ok 2933 - excessBedDays = LOS − benchmark, floored at 0
  ---
  duration_ms: 0.09175
  type: 'test'
  ...
# Subtest: computeBedDayCost: 8-day single room over-stay = 7 × 6500 = 45,500 (est.)
ok 2934 - computeBedDayCost: 8-day single room over-stay = 7 × 6500 = 45,500 (est.)
  ---
  duration_ms: 10.097958
  type: 'test'
  ...
# Subtest: computeBedDayCost returns 0 when not flagged, day-care, or single-day
ok 2935 - computeBedDayCost returns 0 when not flagged, day-care, or single-day
  ---
  duration_ms: 0.077667
  type: 'test'
  ...
# Subtest: tariff-status table drops the (est.) label
ok 2936 - tariff-status table drops the (est.) label
  ---
  duration_ms: 0.146875
  type: 'test'
  ...
...
ok 2937 - the routes still declare the numbers this guard reads
  ---
  duration_ms: 2.665916
  type: 'test'
  ...
# Subtest: the body extractor is not vacuous — it finds real code, not an empty default param
ok 2938 - the body extractor is not vacuous — it finds real code, not an empty default param
  ---
  duration_ms: 2.362875
  type: 'test'
  ...
# Subtest: IPD_ANALYZE_LEGS equals the analyze call sites in lib/doc-audit.ts — a FOURTH leg fails here
ok 2939 - IPD_ANALYZE_LEGS equals the analyze call sites in lib/doc-audit.ts — a FOURTH leg fails here
  ---
  duration_ms: 0.669792
  type: 'test'
  ...
# Subtest: OPD_AUDIT_LEGS equals the EXECUTABLE legs in auditOpdNote — a third fails here
ok 2940 - OPD_AUDIT_LEGS equals the EXECUTABLE legs in auditOpdNote — a third fails here
  ---
  duration_ms: 0.932916
  type: 'test'
  ...
# Subtest: THE IPD WORKER FITS ITS BOX, for every provider that can serve it
ok 2941 - THE IPD WORKER FITS ITS BOX, for every provider that can serve it
  ---
  duration_ms: 0.453792
  type: 'test'
  ...
# Subtest: ipd-audit-now fits the same box on the same basis (DEC-B5)
ok 2942 - ipd-audit-now fits the same box on the same basis (DEC-B5)
  ---
  duration_ms: 0.124541
  type: 'test'
  ...
# Subtest: THE OPD WORKER FITS ITS BOX, for every provider that can serve it
ok 2943 - THE OPD WORKER FITS ITS BOX, for every provider that can serve it
  ---
  duration_ms: 13.9925
  type: 'test'
  ...
# Subtest: the OPD call site sends a per-attempt ceiling that matches its budget (DEC-B9)
ok 2944 - the OPD call site sends a per-attempt ceiling that matches its budget (DEC-B9)
  ---
  duration_ms: 1.514667
  type: 'test'
  ...
# Subtest: raising max, lowering the box, or adding a retry FAILS the guard
ok 2945 - raising max, lowering the box, or adding a retry FAILS the guard
  ---
  duration_ms: 0.7325
  type: 'test'
  ...
# Subtest: a null budget means REFUSE, never substitute a default
ok 2946 - a null budget means REFUSE, never substitute a default
  ---
  duration_ms: 1.293583
  type: 'test'
  ...
# Subtest: the IPD cron interval clears the IPD box — and this is NOT extended to OPD
ok 2947 - the IPD cron interval clears the IPD box — and this is NOT extended to OPD
  ---
  duration_ms: 0.484833
  type: 'test'
  ...
# Subtest: OPENROUTER_TIMEOUT_MS and OPENROUTER_MAX_TRIES still read 110,000 and 3
ok 2948 - OPENROUTER_TIMEOUT_MS and OPENROUTER_MAX_TRIES still read 110,000 and 3
  ---
  duration_ms: 0.110417
  type: 'test'
  ...
# Subtest: check mode → Runs + Interventions + CW_Flags + Citations, normalized by run_id
ok 2949 - check mode → Runs + Interventions + CW_Flags + Citations, normalized by run_id
  ---
  duration_ms: 1.243666
  type: 'test'
  ...
# Subtest: pathway mode → PathwayStages merges skeleton+enrichment by id, ordered
ok 2950 - pathway mode → PathwayStages merges skeleton+enrichment by id, ordered
  ---
  duration_ms: 0.165833
  type: 'test'
  ...
# Subtest: audit mode → findings/completeness/diff/suggestions/idealised/extracted/citations
ok 2951 - audit mode → findings/completeness/diff/suggestions/idealised/extracted/citations
  ---
  duration_ms: 1.587292
  type: 'test'
  ...
# Subtest: mergeRunSheets stacks rows across runs by sheet name
ok 2952 - mergeRunSheets stacks rows across runs by sheet name
  ---
  duration_ms: 0.190792
  type: 'test'
  ...
# Subtest: THE INVARIANT: all-Standard weighting reproduces legacy completeness EXACTLY
ok 2953 - THE INVARIANT: all-Standard weighting reproduces legacy completeness EXACTLY
  ---
  duration_ms: 2.776
  type: 'test'
  ...
# Subtest: THE INVARIANT holds for the null vector too (PRD §8.1 fallback = legacy behaviour)
ok 2954 - THE INVARIANT holds for the null vector too (PRD §8.1 fallback = legacy behaviour)
  ---
  duration_ms: 0.43875
  type: 'test'
  ...
# Subtest: THE INVARIANT holds at all-Minor as well (PRD §8.4: mathematically identical to all-Standard)
ok 2955 - THE INVARIANT holds at all-Minor as well (PRD §8.4: mathematically identical to all-Standard)
  ---
  duration_ms: 0.48975
  type: 'test'
  ...
# Subtest: the fixture set actually exercises the hard cases (guards against a vacuous invariant)
ok 2956 - the fixture set actually exercises the hard cases (guards against a vacuous invariant)
  ---
  duration_ms: 0.332375
  type: 'test'
  ...
# Subtest: the na-policy divergence is REAL and this build takes the legacy branch (flagged deviation)
ok 2957 - the na-policy divergence is REAL and this build takes the legacy branch (flagged deviation)
  ---
  duration_ms: 0.707417
  type: 'test'
  ...
# Subtest: a CONDITIONAL na leaves both sides under BOTH policies (mandatoryTotal 20 vs 21, PRD §2.9)
ok 2958 - a CONDITIONAL na leaves both sides under BOTH policies (mandatoryTotal 20 vs 21, PRD §2.9)
  ---
  duration_ms: 0.412375
  type: 'test'
  ...
# Subtest: tier points are exactly Critical 8 · Important 4 · Standard 2 · Minor 1, and none is zero
ok 2959 - tier points are exactly Critical 8 · Important 4 · Standard 2 · Minor 1, and none is zero
  ---
  duration_ms: 0.427166
  type: 'test'
  ...
# Subtest: normalised weights sum to 100.0 ± 0.05 for every combination (PRD §10)
ok 2960 - normalised weights sum to 100.0 ± 0.05 for every combination (PRD §10)
  ---
  duration_ms: 0.60825
  type: 'test'
  ...
# Subtest: weighting actually MOVES the score when tiers differ (the change is not a no-op)
ok 2961 - weighting actually MOVES the score when tiers differ (the change is not a no-op)
  ---
  duration_ms: 0.241375
  type: 'test'
  ...
# Subtest: partial is exactly 0.5, and na is not partial
ok 2962 - partial is exactly 0.5, and na is not partial
  ---
  duration_ms: 0.29275
  type: 'test'
  ...
# Subtest: all-na document returns 100 without dividing by zero (PRD §8.5)
ok 2963 - all-na document returns 100 without dividing by zero (PRD §8.5)
  ---
  duration_ms: 0.059459
  type: 'test'
  ...
# Subtest: unknown key defaults to Standard; empty/garbage vector falls back to equal weights (PRD §8.2)
ok 2964 - unknown key defaults to Standard; empty/garbage vector falls back to equal weights (PRD §8.2)
  ---
  duration_ms: 0.055792
  type: 'test'
  ...
# Subtest: malformed input never throws and never produces a wrong-looking score
ok 2965 - malformed input never throws and never produces a wrong-looking score
  ---
  duration_ms: 0.121875
  type: 'test'
  ...
# Subtest: rounding is half-up, applied via legacy's DOUBLE round
ok 2966 - rounding is half-up, applied via legacy's DOUBLE round
  ---
  duration_ms: 0.066292
  type: 'test'
  ...
# Subtest: missingMandatory lists applicable missing fields by label (the unweighted gap count)
ok 2967 - missingMandatory lists applicable missing fields by label (the unweighted gap count)
  ---
  duration_ms: 0.03925
  type: 'test'
  ...
# Subtest: legacyCompleteness (the independent path) agrees with the null-vector weighted path
ok 2968 - legacyCompleteness (the independent path) agrees with the null-vector weighted path
  ---
  duration_ms: 0.252625
  type: 'test'
  ...
# Subtest: the re-stated domain weights match the closed cores VERBATIM (drift guard)
ok 2969 - the re-stated domain weights match the closed cores VERBATIM (drift guard)
  ---
  duration_ms: 0.210916
  type: 'test'
  ...
# Subtest: OPD index reproduces the core formula on a worked case
ok 2970 - OPD index reproduces the core formula on a worked case
  ---
  duration_ms: 0.107416
  type: 'test'
  ...
# Subtest: PDQI-9 absent ⇒ note_quality drops and the divisor is 0.75 (PRD §2.6)
ok 2971 - PDQI-9 absent ⇒ note_quality drops and the divisor is 0.75 (PRD §2.6)
  ---
  duration_ms: 0.048333
  type: 'test'
  ...
# Subtest: Care-Value Index reproduces the six-domain formula
ok 2972 - Care-Value Index reproduces the six-domain formula
  ---
  duration_ms: 0.056292
  type: 'test'
  ...
# Subtest: substituting a new documentation score moves the index and can re-band
ok 2973 - substituting a new documentation score moves the index and can re-band
  ---
  duration_ms: 0.0545
  type: 'test'
  ...
# Subtest: band boundaries at 39/40, 54/55, 69/70, 84/85
ok 2974 - band boundaries at 39/40, 54/55, 69/70, 84/85
  ---
  duration_ms: 0.044875
  type: 'test'
  ...
# Subtest: no domain scores at all ⇒ index 0, not NaN
ok 2975 - no domain scores at all ⇒ index 0, not NaN
  ---
  duration_ms: 0.035084
  type: 'test'
  ...
# Subtest: the weights-version label is exact (PRD §2.8, §8.3)
ok 2976 - the weights-version label is exact (PRD §2.8, §8.3)
  ---
  duration_ms: 0.045917
  type: 'test'
  ...
# Subtest: preview: an unchanged candidate moves nothing
ok 2977 - preview: an unchanged candidate moves nothing
  ---
  duration_ms: 0.364375
  type: 'test'
  ...
# Subtest: preview: making a widely-missing field Critical moves the mean and reports movers
ok 2978 - preview: making a widely-missing field Critical moves the mean and reports movers
  ---
  duration_ms: 6.601583
  type: 'test'
  ...
# Subtest: preview: empty cohort yields zeroed stats, no throw (the OPD empty state)
ok 2979 - preview: empty cohort yields zeroed stats, no throw (the OPD empty state)
  ---
  duration_ms: 0.073834
  type: 'test'
  ...
# Subtest: preview: SD is population SD and a single row has SD 0
ok 2980 - preview: SD is population SD and a single row has SD 0
  ---
  duration_ms: 0.055792
  type: 'test'
  ...
# Subtest: missingPrevalence excludes `na` from the base, and reports a percentage
ok 2981 - missingPrevalence excludes `na` from the base, and reports a percentage
  ---
  duration_ms: 0.190625
  type: 'test'
  ...
# Subtest: systemic-defect warning fires only above 50% missing AND only at Critical; it never blocks
ok 2982 - systemic-defect warning fires only above 50% missing AND only at Critical; it never blocks
  ---
  duration_ms: 0.104125
  type: 'test'
  ...
# Subtest: the systemic-defect copy is verbatim per PRD §5.3
ok 2983 - the systemic-defect copy is verbatim per PRD §5.3
  ---
  duration_ms: 0.038042
  type: 'test'
  ...
# Subtest: scoreRow routes IPD and OPD to different index formulas
ok 2984 - scoreRow routes IPD and OPD to different index formulas
  ---
  duration_ms: 0.071208
  type: 'test'
  ...
# Subtest: the 21 discharge_summary fields match data/nabh-rubric.json EXACTLY (key, label, section)
ok 2985 - the 21 discharge_summary fields match data/nabh-rubric.json EXACTLY (key, label, section)
  ---
  duration_ms: 0.187666
  type: 'test'
  ...
# Subtest: cause_of_death is the ONE conditional key, read from the rubric
ok 2986 - cause_of_death is the ONE conditional key, read from the rubric
  ---
  duration_ms: 0.112875
  type: 'test'
  ...
# Subtest: the OPD label→key mapping covers every live-observed label (companion spec §4.7)
ok 2987 - the OPD label→key mapping covers every live-observed label (companion spec §4.7)
  ---
  duration_ms: 0.1615
  type: 'test'
  ...
# Subtest: the OPD engine's ACTUAL emitted keys are all in the catalogue (no orphan can appear)
ok 2988 - the OPD engine's ACTUAL emitted keys are all in the catalogue (no orphan can appear)
  ---
  duration_ms: 0.285292
  type: 'test'
  ...
# Subtest: the OPD structured emission is ADDITIVE: status/section added, present/mandatory preserved
ok 2989 - the OPD structured emission is ADDITIVE: status/section added, present/mandatory preserved
  ---
  duration_ms: 41.411333
  type: 'test'
  ...
# Subtest: the OPD engine emits the structured shape from BOTH completeness paths (GP and obstetric)
ok 2990 - the OPD engine emits the structured shape from BOTH completeness paths (GP and obstetric)
  ---
...
ok 2991 - A.1 THE FALLBACK RULE: a NULL completeness_items row keeps its stored scores, untouched
  ---
  duration_ms: 716.255875
  type: 'test'
  ...
# Subtest: A.1 a missing array is never read as 100 NOR as 0 — both directions
ok 2992 - A.1 a missing array is never read as 100 NOR as 0 — both directions
  ---
  duration_ms: 71.318083
  type: 'test'
  ...
# Subtest: A.1 a row WITH items is weighted, and weights_not_applicable flips to false
ok 2993 - A.1 a row WITH items is weighted, and weights_not_applicable flips to false
  ---
  duration_ms: 24.654667
  type: 'test'
  ...
# Subtest: A.1 continuity items are EXCLUDED from the OPD denominator (reproduces the engine's coverage)
ok 2994 - A.1 continuity items are EXCLUDED from the OPD denominator (reproduces the engine's coverage)
  ---
  duration_ms: 7.330333
  type: 'test'
  ...
# Subtest: A.1 applyOpdScoringPolicy never throws and handles an empty batch
ok 2995 - A.1 applyOpdScoringPolicy never throws and handles an empty batch
  ---
  duration_ms: 9.122958
  type: 'test'
  ...
# Subtest: A.1 parseOpdCompletenessItems drops malformed entries rather than throwing
ok 2996 - A.1 parseOpdCompletenessItems drops malformed entries rather than throwing
  ---
  duration_ms: 2.207417
  type: 'test'
  ...
# Subtest: A.1 the OPD write path persists the array, guarded by a column probe
ok 2997 - A.1 the OPD write path persists the array, guarded by a column probe
  ---
  duration_ms: 0.478542
  type: 'test'
  ...
# Subtest: A.1 the migration runner exists, is admin-guarded, and every statement is idempotent
ok 2998 - A.1 the migration runner exists, is admin-guarded, and every statement is idempotent
  ---
  duration_ms: 0.524375
  type: 'test'
  ...
# Subtest: A.1 the runner's inlined DDL matches the two .sql files it stands in for
ok 2999 - A.1 the runner's inlined DDL matches the two .sql files it stands in for
  ---
  duration_ms: 0.915125
  type: 'test'
  ...
# Subtest: A.1 no backfill: nothing in the build writes completeness_items to historical rows
ok 3000 - A.1 no backfill: nothing in the build writes completeness_items to historical rows
  ---
  duration_ms: 0.195375
  type: 'test'
  ...
# Subtest: the three continuity fields are EXCLUDED from the OPD weight vector (kickoff normative list)
ok 3001 - the three continuity fields are EXCLUDED from the OPD weight vector (kickoff normative list)
  ---
  duration_ms: 0.065625
  type: 'test'
  ...
# Subtest: the near-duplicate pairs are kept SEPARATE and flagged, not merged
ok 3002 - the near-duplicate pairs are kept SEPARATE and flagged, not merged
  ---
  duration_ms: 0.07275
  type: 'test'
  ...
# Subtest: labelToOpdKey: dynamic obstetric labels match by prefix; unknown returns null (never guesses)
ok 3003 - labelToOpdKey: dynamic obstetric labels match by prefix; unknown returns null (never guesses)
  ---
  duration_ms: 0.213125
  type: 'test'
  ...
# Subtest: every catalogued OPD label round-trips through the mapping
ok 3004 - every catalogued OPD label round-trips through the mapping
  ---
  duration_ms: 0.051708
  type: 'test'
  ...
# Subtest: fieldsFor / weightedKeysFor route by note type and never return an empty key space
ok 3005 - fieldsFor / weightedKeysFor route by note type and never return an empty key space
  ---
  duration_ms: 0.036167
  type: 'test'
  ...
# Subtest: diffVectors reports only real changes, with old → new tiers
ok 3006 - diffVectors reports only real changes, with old → new tiers
  ---
  duration_ms: 0.179
  type: 'test'
  ...
# Subtest: vectorsEqual treats absent as Standard (so a seeded v1 equals an empty draft)
ok 3007 - vectorsEqual treats absent as Standard (so a seeded v1 equals an empty draft)
  ---
  duration_ms: 0.051542
  type: 'test'
  ...
# Subtest: validateVector rejects non-objects but coerces unknown tiers rather than failing
ok 3008 - validateVector rejects non-objects but coerces unknown tiers rather than failing
  ---
  duration_ms: 0.111792
  type: 'test'
  ...
# Subtest: canonicalVectorJson is stable regardless of key insertion order
ok 3009 - canonicalVectorJson is stable regardless of key insertion order
  ---
  duration_ms: 0.05225
  type: 'test'
  ...
# Subtest: bySection groups and preserves first-seen order
ok 3010 - bySection groups and preserves first-seen order
  ---
  duration_ms: 0.086625
  type: 'test'
  ...
# Subtest: computeSignalHealth: FP rate, latest-per-doctor, top reasons, healable
ok 3011 - computeSignalHealth: FP rate, latest-per-doctor, top reasons, healable
  ---
  duration_ms: 0.701292
  type: 'test'
  ...
# Subtest: computeSignalHealth: ranks noisiest (audit_bug × rate) first
ok 3012 - computeSignalHealth: ranks noisiest (audit_bug × rate) first
  ---
  duration_ms: 0.150791
  type: 'test'
  ...
# Subtest: findingMatchesSuppression: type/scope/discriminator/active gates
ok 3013 - findingMatchesSuppression: type/scope/discriminator/active gates
  ---
  duration_ms: 0.109292
  type: 'test'
  ...
# Subtest: applySuppressions: drop removes, downgrade sets informational, no active = no-op
ok 3014 - applySuppressions: drop removes, downgrade sets informational, no active = no-op
  ---
  duration_ms: 0.177459
  type: 'test'
  ...
# Subtest: previewCollateral: dual-label invariant — refuses to remove a validated signal
ok 3015 - previewCollateral: dual-label invariant — refuses to remove a validated signal
  ---
  duration_ms: 0.104333
  type: 'test'
  ...
# Subtest: every ratified tier-2 kind maps to tier 2, not unlisted
ok 3016 - every ratified tier-2 kind maps to tier 2, not unlisted
  ---
  duration_ms: 1.110167
  type: 'test'
  ...
# Subtest: every ratified tier-3 kind maps to tier 3 — log only
ok 3017 - every ratified tier-3 kind maps to tier 3 — log only
  ---
  duration_ms: 0.133333
  type: 'test'
  ...
# Subtest: banned_fdc is ratified TIER 2 (not higher) and pregnancy_risk_verify is ratified TIER 3
ok 3018 - banned_fdc is ratified TIER 2 (not higher) and pregnancy_risk_verify is ratified TIER 3
  ---
  duration_ms: 0.580125
  type: 'test'
  ...
# Subtest: O1c: a model-invented kind lands in tier 2 and is flagged unlisted
ok 3019 - O1c: a model-invented kind lands in tier 2 and is flagged unlisted
  ---
  duration_ms: 0.060875
  type: 'test'
  ...
# Subtest: praise: *_high_value kinds and any high-value verdict (antibiotic_stewardship praise) are excluded and counted
ok 3020 - praise: *_high_value kinds and any high-value verdict (antibiotic_stewardship praise) are excluded and counted
  ---
  duration_ms: 0.048334
  type: 'test'
  ...
# Subtest: antibiotic_stewardship VIOLATION (low-value — antibiotic for a viral URTI) is tier 2
ok 3021 - antibiotic_stewardship VIOLATION (low-value — antibiotic for a viral URTI) is tier 2
  ---
...
ok 3022 - incomplete_dosing: missing strength / duration alone → tier 3 (ratified rows: findings 20, 24, 37, 41 + chronic continuation)
  ---
...
ok 3023 - incomplete_dosing: a missing frequency or route changes what the patient does → tier 2; unparseable → tier 2
  ---
  duration_ms: 0.07
  type: 'test'
  ...
# Subtest: E-1 (finding 36): a time-critical cardiac pattern in the finding text promotes to tier 1 — from any kind
ok 3024 - E-1 (finding 36): a time-critical cardiac pattern in the finding text promotes to tier 1 — from any kind
  ---
...
ok 3025 - E-2 (finding 49): persistent swelling ≥ 4 weeks with no follow-through promotes; with follow-through it does not
  ---
  duration_ms: 1.410416
  type: 'test'
  ...
# Subtest: praise never escalates: a high-value finding praising an appropriate ACS referral stays praise
ok 3026 - praise never escalates: a high-value finding praising an appropriate ACS referral stays praise
  ---
  duration_ms: 0.053625
  type: 'test'
  ...
# Subtest: bucketByTier: buckets are disjoint, complete, and count unlisted kinds
ok 3027 - bucketByTier: buckets are disjoint, complete, and count unlisted kinds
  ---
...
ok 3028 - dedupeTwins: same (finding_ref, doctor_uid, day) collapses with an occurrence count; different notes same day still collapse; different days do not
  ---
  duration_ms: 0.358875
  type: 'test'
  ...
# Subtest: dedupeTwins: unkeyable rows (no ref / no doctor / no date) never merge
ok 3029 - dedupeTwins: unkeyable rows (no ref / no doctor / no date) never merge
  ---
  duration_ms: 0.037625
  type: 'test'
  ...
# Subtest: allows SELECT / WITH and auto-adds LIMIT
ok 3030 - allows SELECT / WITH and auto-adds LIMIT
  ---
  duration_ms: 1.661292
  type: 'test'
  ...
# Subtest: rejects writes, DDL, multiple statements, non-SELECT, over-cap LIMIT, system fns
ok 3031 - rejects writes, DDL, multiple statements, non-SELECT, over-cap LIMIT, system fns
  ---
  duration_ms: 0.108625
  type: 'test'
  ...
# Subtest: blocks PHI-bearing relations anywhere in the query
ok 3032 - blocks PHI-bearing relations anywhere in the query
  ---
  duration_ms: 0.119208
  type: 'test'
  ...
# Subtest: honors a smaller caller cap
ok 3033 - honors a smaller caller cap
  ---
  duration_ms: 0.0635
  type: 'test'
  ...
...
ok 3034 - coverage floor (PRD §4): ≥5 dose-ceiling, ≥3 SOS, ≥3 banned-FDC, ≥4 interaction, ≥4 incomplete-dosing positives; exactly 6 negatives
  ---
  duration_ms: 0.629208
  type: 'test'
  ...
# Subtest: fixtures carry no PHI, no db13 uid, and banned-FDC fixtures use placeholder molecules only
ok 3035 - fixtures carry no PHI, no db13 uid, and banned-FDC fixtures use placeholder molecules only
  ---
...
ok 3036 - positive POS-DOSE-1 fires dose_ceiling_exceeded — paracetamol stacked across two products: 650 QID + 500 TDS = 4100 mg/day > 4000
  ---
...
ok 3037 - positive POS-DOSE-2 fires dose_ceiling_exceeded — ibuprofen 800 QID = 3200 mg/day > 2400 (single product)
  ---
  duration_ms: 0.03375
  type: 'test'
  ...
# Subtest: positive POS-DOSE-3 fires dose_ceiling_exceeded — diclofenac 75 TDS = 225 mg/day > 150
ok 3038 - positive POS-DOSE-3 fires dose_ceiling_exceeded — diclofenac 75 TDS = 225 mg/day > 150
  ---
...
ok 3039 - positive POS-DOSE-4 fires dose_ceiling_exceeded — etoricoxib 120 OD with NO documented gout > the 90 mg/day default ceiling (Decision 6)
  ---
  duration_ms: 0.029291
  type: 'test'
  ...
# Subtest: positive POS-DOSE-5 fires dose_ceiling_exceeded — mefenamic acid 500 QID = 2000 mg/day > 1500
ok 3040 - positive POS-DOSE-5 fires dose_ceiling_exceeded — mefenamic acid 500 QID = 2000 mg/day > 1500
  ---
...
ok 3041 - positive POS-SOS-1 fires dose_ceiling_sos — paracetamol 1000 TDS scheduled + 650 SOS uncapped (default cap 3) → 4950 potential > 4000
  ---
...
ok 3042 - positive POS-SOS-2 fires dose_ceiling_sos — etoricoxib 90 SOS with an EXPLICIT max 2/day → 180 potential > 90
  ---
...
ok 3043 - positive POS-SOS-3 fires dose_ceiling_sos — ibuprofen 600 grid 1-0-1 + 600 SOS uncapped → 3000 potential > 2400
  ---
  duration_ms: 0.271958
  type: 'test'
  ...
# Subtest: positive POS-FDC-1 fires banned_fdc — exact two-molecule banned set (placeholders mol-a + mol-b)
ok 3044 - positive POS-FDC-1 fires banned_fdc — exact two-molecule banned set (placeholders mol-a + mol-b)
  ---
...
ok 3045 - positive POS-FDC-2 fires banned_fdc — exact three-molecule banned set (placeholders mol-c/mol-d/mol-e)
  ---
...
ok 3046 - positive POS-FDC-3 fires banned_fdc — order-swapped banned pair (mol-b + mol-a) still matches the stored set
  ---
  duration_ms: 0.022708
  type: 'test'
  ...
# Subtest: positive POS-DDI-1 fires drug_interaction — warfarin + ibuprofen — anticoagulant + NSAID (major)
ok 3047 - positive POS-DDI-1 fires drug_interaction — warfarin + ibuprofen — anticoagulant + NSAID (major)
  ---
  duration_ms: 0.022084
  type: 'test'
  ...
# Subtest: positive POS-DDI-2 fires drug_interaction — atorvastatin + clarithromycin — statin + macrolide (major)
ok 3048 - positive POS-DDI-2 fires drug_interaction — atorvastatin + clarithromycin — statin + macrolide (major)
  ---
  duration_ms: 0.021916
  type: 'test'
  ...
# Subtest: positive POS-DDI-3 fires drug_interaction — sertraline + tramadol — two serotonergic drugs (major)
ok 3049 - positive POS-DDI-3 fires drug_interaction — sertraline + tramadol — two serotonergic drugs (major)
  ---
  duration_ms: 0.023292
  type: 'test'
  ...
# Subtest: positive POS-DDI-4 fires drug_interaction — telmisartan + spironolactone — ACE-I/ARB + potassium-sparing diuretic (major)
ok 3050 - positive POS-DDI-4 fires drug_interaction — telmisartan + spironolactone — ACE-I/ARB + potassium-sparing diuretic (major)
  ---
...
ok 3051 - positive POS-DOSING-1 fires incomplete_dosing — dose/strength blanked (no dose field, no strength, none in the name)
  ---
  duration_ms: 0.023875
  type: 'test'
  ...
# Subtest: positive POS-DOSING-2 fires incomplete_dosing — frequency blanked
ok 3052 - positive POS-DOSING-2 fires incomplete_dosing — frequency blanked
  ---
  duration_ms: 0.021584
  type: 'test'
  ...
# Subtest: positive POS-DOSING-3 fires incomplete_dosing — duration blanked
ok 3053 - positive POS-DOSING-3 fires incomplete_dosing — duration blanked
  ---
...
ok 3054 - positive POS-DOSING-4 fires incomplete_dosing — route blanked and not inferable (no dosage-form word anywhere on the line)
  ---
  duration_ms: 0.020792
  type: 'test'
  ...
# Subtest: negative NEG-1 stays silent — ibuprofen 800 TDS = exactly 2400 mg/day — AT the ceiling, not over it
ok 3055 - negative NEG-1 stays silent — ibuprofen 800 TDS = exactly 2400 mg/day — AT the ceiling, not over it
  ---
  duration_ms: 0.0365
  type: 'test'
  ...
...
ok 3056 - negative NEG-2 stays silent — etoricoxib 120 OD WITH a documented gout diagnosis — the conditional 120 ceiling applies
  ---
  duration_ms: 0.021667
  type: 'test'
  ...
# Subtest: negative NEG-3 stays silent — amoxicillin + paracetamol — just OUTSIDE every interaction pair (no shared mechanism tag)
ok 3057 - negative NEG-3 stays silent — amoxicillin + paracetamol — just OUTSIDE every interaction pair (no shared mechanism tag)
  ---
  duration_ms: 0.020208
  type: 'test'
  ...
# Subtest: negative NEG-4 stays silent — a COMPLETE prescription — dose, frequency, duration and route all present
ok 3058 - negative NEG-4 stays silent — a COMPLETE prescription — dose, frequency, duration and route all present
  ---
...
ok 3059 - negative NEG-5 stays silent — banned core + one extra molecule (mol-a + mol-b + mol-z) — the C5 superset boundary
  ---
...
ok 3060 - negative NEG-6 stays silent — paracetamol 500 SOS max 3/day = 1500 mg potential — well inside the 4000 ceiling
  ---
  duration_ms: 0.022291
  type: 'test'
  ...
# Subtest: recall_det = fired / planted, over the deterministic leg only (no LLM recall claim — PRD §6)
ok 3061 - recall_det = fired / planted, over the deterministic leg only (no LLM recall claim — PRD §6)
  ---
  duration_ms: 0.031958
  type: 'test'
  ...
# Subtest: 57 case 1 — production Vercel build with no key at all: MISSING
ok 3062 - 57 case 1 — production Vercel build with no key at all: MISSING
  ---
  duration_ms: 0.519917
  type: 'test'
  ...
# Subtest: 57 case 2 — production Vercel build with an unusable key (empty or whitespace): MISSING
ok 3063 - 57 case 2 — production Vercel build with an unusable key (empty or whitespace): MISSING
  ---
  duration_ms: 0.06775
  type: 'test'
  ...
# Subtest: 57 case 3 — production Vercel build with a usable key: NOT missing
ok 3064 - 57 case 3 — production Vercel build with a usable key: NOT missing
  ---
  duration_ms: 0.048042
  type: 'test'
  ...
# Subtest: 57 case 4 — a Vercel build that is not production: NOT missing, at any key value
ok 3065 - 57 case 4 — a Vercel build that is not production: NOT missing, at any key value
  ---
  duration_ms: 0.119458
  type: 'test'
  ...
# Subtest: 57 case 5 — not a Vercel build, even when the environment says production: NOT missing
ok 3066 - 57 case 5 — not a Vercel build, even when the environment says production: NOT missing
  ---
  duration_ms: 0.060583
  type: 'test'
  ...
# Subtest: 57 EXECUTED — importing the config in production with no key throws, and names the variable
ok 3067 - 57 EXECUTED — importing the config in production with no key throws, and names the variable
  ---
  duration_ms: 48.261541
  type: 'test'
  ...
# Subtest: 57 EXECUTED — a production import WITH a key succeeds
ok 3068 - 57 EXECUTED — a production import WITH a key succeeds
  ---
  duration_ms: 43.606792
  type: 'test'
  ...
# Subtest: 57 EXECUTED — a non-production import with no key succeeds
ok 3069 - 57 EXECUTED — a non-production import with no key succeeds
  ---
  duration_ms: 38.972583
  type: 'test'
  ...
# Subtest: 57 whole file — it parses, and holds EXACTLY three top-level statements in order
ok 3070 - 57 whole file — it parses, and holds EXACTLY three top-level statements in order
  ---
  duration_ms: 12.957209
  type: 'test'
  ...
# Subtest: 57 whole file — the declaration and the export are exactly what they must be
ok 3071 - 57 whole file — the declaration and the export are exactly what they must be
  ---
  duration_ms: 1.307917
  type: 'test'
  ...
# Subtest: 57 whole file — nothing executable outside the guard
ok 3072 - 57 whole file — nothing executable outside the guard
  ---
  duration_ms: 0.338208
  type: 'test'
  ...
# Subtest: 57 pin — each copy is exactly D8's three clauses, and there is no fourth of any kind
ok 3073 - 57 pin — each copy is exactly D8's three clauses, and there is no fourth of any kind
  ---
  duration_ms: 0.937042
  type: 'test'
  ...
# Subtest: 57 pin — next.config.mjs and telemetry-key-guard.ts express the SAME condition
ok 3074 - 57 pin — next.config.mjs and telemetry-key-guard.ts express the SAME condition
  ---
  duration_ms: 3.086417
  type: 'test'
  ...
# Subtest: 57 pin — the guard THROWS one Error, and the message a reader sees names the variable
ok 3075 - 57 pin — the guard THROWS one Error, and the message a reader sees names the variable
  ---
  duration_ms: 0.554958
  type: 'test'
  ...
# Subtest: no surface outside the allow-list SELECTs from the telemetry tables
ok 3076 - no surface outside the allow-list SELECTs from the telemetry tables
  ---
  duration_ms: 132.954209
  type: 'test'
  ...
# Subtest: the scan can actually fail — it is not passing because the matcher never matches
ok 3077 - the scan can actually fail — it is not passing because the matcher never matches
  ---
  duration_ms: 0.12675
  type: 'test'
  ...
# Subtest: the allow-list is by EXACT path, and every entry is one this build owns
ok 3078 - the allow-list is by EXACT path, and every entry is one this build owns
  ---
  duration_ms: 0.169584
  type: 'test'
  ...
# Subtest: no clinician-facing or patient-facing route names a telemetry table at all
ok 3079 - no clinician-facing or patient-facing route names a telemetry table at all
  ---
  duration_ms: 31.455542
  type: 'test'
  ...
# Subtest: GUARD 1 — admin: a set ADMIN_TOKEN with nothing presented is refused, and isAdminUnlocked is NOT consulted
ok 3080 - GUARD 1 — admin: a set ADMIN_TOKEN with nothing presented is refused, and isAdminUnlocked is NOT consulted
  ---
  duration_ms: 309.186916
  type: 'test'
  ...
# Subtest: GUARD 3 — preview: VERCEL_ENV=production is refused even with everything else correct
ok 3081 - GUARD 3 — preview: VERCEL_ENV=production is refused even with everything else correct
  ---
  duration_ms: 278.914959
  type: 'test'
  ...
# Subtest: GUARD 3 — preview: VERCEL_ENV=preview on a DIFFERENT branch is refused
ok 3082 - GUARD 3 — preview: VERCEL_ENV=preview on a DIFFERENT branch is refused
  ---
  duration_ms: 278.924083
  type: 'test'
  ...
# Subtest: GUARD 4 — arming: an unset CDMSS_OVERHEAD_MEASURE is refused
ok 3083 - GUARD 4 — arming: an unset CDMSS_OVERHEAD_MEASURE is refused
  ---
  duration_ms: 215.864375
  type: 'test'
  ...
# Subtest: GUARD 5 — THE ONE THAT MATTERS: a production endpoint id is refused
ok 3084 - GUARD 5 — THE ONE THAT MATTERS: a production endpoint id is refused
  ---
  duration_ms: 149.32675
  type: 'test'
  ...
# Subtest: GUARD 5 — an UNSET expectation refuses, it does not pass
ok 3085 - GUARD 5 — an UNSET expectation refuses, it does not pass
  ---
  duration_ms: 144.318583
  type: 'test'
  ...
# Subtest: GUARD 5 — an unparseable DATABASE_URL refuses
ok 3086 - GUARD 5 — an unparseable DATABASE_URL refuses
  ---
  duration_ms: 154.443459
  type: 'test'
  ...
# Subtest: GUARD 5 — a password containing @ cannot shift the parsed host
ok 3087 - GUARD 5 — a password containing @ cannot shift the parsed host
  ---
  duration_ms: 148.217208
  type: 'test'
  ...
# Subtest: GUARD 2 — expiry: past the hard UTC date every request is 410
ok 3088 - GUARD 2 — expiry: past the hard UTC date every request is 410
  ---
  duration_ms: 145.1435
  type: 'test'
  ...
# Subtest: GUARD 2 — before the expiry the route still runs
ok 3089 - GUARD 2 — before the expiry the route still runs
  ---
  duration_ms: 153.153917
  type: 'test'
  ...
# Subtest: ALL FIVE PASS — the route runs, writes route=script, and reports the first sample separately
ok 3090 - ALL FIVE PASS — the route runs, writes route=script, and reports the first sample separately
  ---
  duration_ms: 148.325875
  type: 'test'
  ...
# Subtest: FIX 2 + FIX 1 — a REAL 500, driven by an unparseable URL that still satisfies guard 5
ok 3091 - FIX 2 + FIX 1 — a REAL 500, driven by an unparseable URL that still satisfies guard 5
  ---
  duration_ms: 148.76225
  type: 'test'
  ...
# Subtest: v5 FIX 1 — two of the three leaking shapes are refused BEFORE the driver ever sees them
ok 3092 - v5 FIX 1 — two of the three leaking shapes are refused BEFORE the driver ever sees them
  ---
  duration_ms: 295.880167
  type: 'test'
  ...
# Subtest: FIX 3 — a query parameter cannot move the parsed host away from the one the driver uses
ok 3093 - FIX 3 — a query parameter cannot move the parsed host away from the one the driver uses
  ---
  duration_ms: 144.669083
  type: 'test'
  ...
# Subtest: FIX 4 — the denylist refuses production even when the expected value also names it
ok 3094 - FIX 4 — the denylist refuses production even when the expected value also names it
  ---
  duration_ms: 144.740416
  type: 'test'
  ...
# Subtest: FIX 4 — an ABSENT denylist refuses: a denylist that is not there is not a denylist
ok 3095 - FIX 4 — an ABSENT denylist refuses: a denylist that is not there is not a denylist
  ---
  duration_ms: 143.387917
  type: 'test'
  ...
# Subtest: FIX 5 — p95 and p99 are withheld below their floors, never a maximum in disguise
ok 3096 - FIX 5 — p95 and p99 are withheld below their floors, never a maximum in disguise
  ---
  duration_ms: 296.671416
  type: 'test'
  ...
# Subtest: FIX 5 — and p95 IS emitted once its floor is met, so the floor is not a blanket refusal
ok 3097 - FIX 5 — and p95 IS emitted once its floor is met, so the floor is not a blanket refusal
  ---
  duration_ms: 148.387958
  type: 'test'
  ...
# Subtest: FIX 6 — the invocation insert has its own cell
ok 3098 - FIX 6 — the invocation insert has its own cell
  ---
  duration_ms: 291.201209
  type: 'test'
  ...
# Subtest: FIX 7 — the shape is true per cell, and conc is gone
ok 3099 - FIX 7 — the shape is true per cell, and conc is gone
  ---
  duration_ms: 294.099459
  type: 'test'
  ...
# Subtest: FIX 8 — the real-audit arm refuses rather than silently becoming the null arm
ok 3100 - FIX 8 — the real-audit arm refuses rather than silently becoming the null arm
  ---
  duration_ms: 447.40575
  type: 'test'
  ...
# Subtest: v6 FIX 1 — the branch POOLED host passes against the bare expected id
ok 3101 - v6 FIX 1 — the branch POOLED host passes against the bare expected id
  ---
  duration_ms: 146.800708
  type: 'test'
  ...
# Subtest: v6 FIX 1 — the branch DIRECT host passes against the bare expected id
ok 3102 - v6 FIX 1 — the branch DIRECT host passes against the bare expected id
  ---
  duration_ms: 146.173583
  type: 'test'
  ...
# Subtest: v6 FIX 1 — the branch POOLED host passes against a POOLED expected id
ok 3103 - v6 FIX 1 — the branch POOLED host passes against a POOLED expected id
  ---
  duration_ms: 147.505875
  type: 'test'
  ...
# Subtest: v6 FIX 2 — production on its POOLED host refuses forbidden_endpoint, NOT endpoint_mismatch
ok 3104 - v6 FIX 2 — production on its POOLED host refuses forbidden_endpoint, NOT endpoint_mismatch
  ---
  duration_ms: 141.992542
  type: 'test'
  ...
# Subtest: v6 FIX 2 — production on its DIRECT host also refuses forbidden_endpoint
ok 3105 - v6 FIX 2 — production on its DIRECT host also refuses forbidden_endpoint
  ---
  duration_ms: 143.418667
  type: 'test'
  ...
# Subtest: v6 — `pooler` in the MIDDLE of a label is part of the id and is never truncated
ok 3106 - v6 — `pooler` in the MIDDLE of a label is part of the id and is never truncated
  ---
  duration_ms: 292.524208
  type: 'test'
  ...
# Subtest: v6 — a doubled `-pooler-pooler` strips exactly ONE, and that is the stated rule
ok 3107 - v6 — a doubled `-pooler-pooler` strips exactly ONE, and that is the stated rule
  ---
  duration_ms: 444.044833
  type: 'test'
  ...
# Subtest: v6 — normalisation cannot make two DIFFERENT endpoints compare equal
ok 3108 - v6 — normalisation cannot make two DIFFERENT endpoints compare equal
  ---
  duration_ms: 575.09725
  type: 'test'
  ...
# Subtest: NO OUTPUT ANYWHERE CARRIES A DATABASE_URL SUBSTRING — every response shape, stdout and stderr
ok 3109 - NO OUTPUT ANYWHERE CARRIES A DATABASE_URL SUBSTRING — every response shape, stdout and stderr
  ---
  duration_ms: 1593.745458
  type: 'test'
  ...
# Subtest: the route is POST-only, and carries its own expiry and deletion notice in source
ok 3110 - the route is POST-only, and carries its own expiry and deletion notice in source
  ---
...
ok 3111 - DEFAULT-OFF: unset ⇒ no cap (the shipped path is uncapped and must stay so)
  ---
  duration_ms: 1.166667
  type: 'test'
  ...
# Subtest: DEFAULT-OFF: 0, negative, junk and empty all mean "no cap", never a cap of 0
ok 3112 - DEFAULT-OFF: 0, negative, junk and empty all mean "no cap", never a cap of 0
  ---
  duration_ms: 0.2735
  type: 'test'
  ...
# Subtest: a set budget is honored, floored to an integer (the arms: 1647 / 823 / 128)
ok 3113 - a set budget is honored, floored to an integer (the arms: 1647 / 823 / 128)
  ---
  duration_ms: 0.6485
  type: 'test'
  ...
# Subtest: the cap rides the SL0-verified wire format (top-level google.thinking_config)
ok 3114 - the cap rides the SL0-verified wire format (top-level google.thinking_config)
  ---
  duration_ms: 1.992708
  type: 'test'
  ...
# Subtest: the cap is Gemini-only and cannot leak onto the Ollama fallback path
ok 3115 - the cap is Gemini-only and cannot leak onto the Ollama fallback path
  ---
  duration_ms: 0.244958
  type: 'test'
  ...
# Subtest: gen_params records the budget ONLY when capped — an uncapped trace is unchanged
ok 3116 - gen_params records the budget ONLY when capped — an uncapped trace is unchanged
  ---
  duration_ms: 0.153083
  type: 'test'
  ...
# Subtest: note-audit row → ClinicalFinding: verbatim vocab in the audit ext, valid core
ok 3117 - note-audit row → ClinicalFinding: verbatim vocab in the audit ext, valid core
  ---
  duration_ms: 1.57075
  type: 'test'
  ...
# Subtest: LOSSLESS: note-audit row round-trips byte-for-byte, incl. unmapped engine fields
ok 3118 - LOSSLESS: note-audit row round-trips byte-for-byte, incl. unmapped engine fields
  ---
  duration_ms: 0.47125
  type: 'test'
  ...
# Subtest: note-audit round-trip preserves absence: a minimal row gains no keys
ok 3119 - note-audit round-trip preserves absence: a minimal row gains no keys
  ---
  duration_ms: 0.116625
  type: 'test'
  ...
# Subtest: deterministic-source row maps to extractionMethod deterministic
ok 3120 - deterministic-source row maps to extractionMethod deterministic
  ---
  duration_ms: 0.052958
  type: 'test'
  ...
# Subtest: doc-audit AuditFinding → ClinicalFinding: verdict rides in ext.netValue (verbatim, separate slot)
ok 3121 - doc-audit AuditFinding → ClinicalFinding: verdict rides in ext.netValue (verbatim, separate slot)
  ---
  duration_ms: 0.432333
  type: 'test'
  ...
# Subtest: LOSSLESS: doc-audit AuditFinding round-trips byte-for-byte
ok 3122 - LOSSLESS: doc-audit AuditFinding round-trips byte-for-byte
  ---
  duration_ms: 0.155875
  type: 'test'
  ...
# Subtest: ExtractedCase → ClinicalState: clinical content in the core, metadata in surfaceExtras
ok 3123 - ExtractedCase → ClinicalState: clinical content in the core, metadata in surfaceExtras
  ---
  duration_ms: 0.635625
  type: 'test'
  ...
# Subtest: LOSSLESS: ExtractedCase round-trips byte-for-byte (full PX discharge shape)
ok 3124 - LOSSLESS: ExtractedCase round-trips byte-for-byte (full PX discharge shape)
  ---
  duration_ms: 0.131417
  type: 'test'
  ...
# Subtest: LOSSLESS: a sparse pre-PX ExtractedCase (no riskFactors/aftercare/completeness) round-trips
ok 3125 - LOSSLESS: a sparse pre-PX ExtractedCase (no riskFactors/aftercare/completeness) round-trips
  ---
...
ok 3126 - the traceless route stays traceless — no trace id reaches the rerank transport
  ---
  duration_ms: 1.013875
  type: 'test'
  ...
# Subtest: nothing in the change touches the outbound request object
ok 3127 - nothing in the change touches the outbound request object
  ---
  duration_ms: 0.290292
  type: 'test'
  ...
# Subtest: attachment is non-enumerable, so a serialized request or response is byte-identical
ok 3128 - attachment is non-enumerable, so a serialized request or response is byte-identical
  ---
  duration_ms: 0.403167
  type: 'test'
  ...
# Subtest: attachment returns the SAME object — it allocates nothing the caller could miss
ok 3129 - attachment returns the SAME object — it allocates nothing the caller could miss
  ---
  duration_ms: 0.050959
  type: 'test'
  ...
# Subtest: the ladder, its order and its terminal dispositions are untouched
ok 3130 - the ladder, its order and its terminal dispositions are untouched
  ---
  duration_ms: 0.229334
  type: 'test'
  ...
# Subtest: retry policy is unchanged — capture rides the existing callback and adds no try budget
ok 3131 - retry policy is unchanged — capture rides the existing callback and adds no try budget
  ---
  duration_ms: 0.374208
  type: 'test'
  ...
# Subtest: every one of the four return sites carries evidence — no silent unattributed path
ok 3132 - every one of the four return sites carries evidence — no silent unattributed path
  ---
  duration_ms: 0.276167
  type: 'test'
  ...
# Subtest: the local substitution reports the LOCAL model, never the requested cloud model (§6.2)
ok 3133 - the local substitution reports the LOCAL model, never the requested cloud model (§6.2)
  ---
  duration_ms: 0.228167
  type: 'test'
  ...
# Subtest: every name still resolves at its original path, so no existing importer moves
ok 3134 - every name still resolves at its original path, so no existing importer moves
  ---
  duration_ms: 0.360875
  type: 'test'
  ...
# Subtest: the attempts field is OPTIONAL, so tracedChat attributions stay valid unchanged
ok 3135 - the attempts field is OPTIONAL, so tracedChat attributions stay valid unchanged
  ---
  duration_ms: 1.255625
  type: 'test'
  ...
# Subtest: a hostile completion cannot break the transport
ok 3136 - a hostile completion cannot break the transport
  ---
  duration_ms: 0.330458
  type: 'test'
  ...
# Subtest: a 429 is distinguishable from every other failure class
ok 3137 - a 429 is distinguishable from every other failure class
  ---
  duration_ms: 0.127875
  type: 'test'
  ...
# Subtest: both tiers classify through the same function — a 429 cannot be tier-dependent
ok 3138 - both tiers classify through the same function — a 429 cannot be tier-dependent
  ---
  duration_ms: 0.456708
  type: 'test'
  ...
# Subtest: the attempt sequence is invocation-scoped, never module state (§4.1)
ok 3139 - the attempt sequence is invocation-scoped, never module state (§4.1)
  ---
  duration_ms: 0.553333
  type: 'test'
  ...
# Subtest: the evidence carries identifiers and enums only — no prompt, passage or query text
ok 3140 - the evidence carries identifiers and enums only — no prompt, passage or query text
  ---
...
ok 3141 - a total transport failure records no served provider, and says so explicitly
  ---
  duration_ms: 0.578666
  type: 'test'
  ...
# Subtest: a null attempt list means NOT COLLECTED, and is distinguishable from an empty one
ok 3142 - a null attempt list means NOT COLLECTED, and is distinguishable from an empty one
  ---
  duration_ms: 0.315417
  type: 'test'
  ...
# Subtest: every terminal phase is a stable NAME — never a message, never an interpolated value
ok 3143 - every terminal phase is a stable NAME — never a message, never an interpolated value
  ---
  duration_ms: 0.566083
  type: 'test'
  ...
# Subtest: the intended-local path records ONE attempt rather than an empty list
ok 3144 - the intended-local path records ONE attempt rather than an empty list
  ---
  duration_ms: 0.196291
  type: 'test'
  ...
# Subtest: both local arms record their attempt — the by-design one and the substitution
ok 3145 - both local arms record their attempt — the by-design one and the substitution
  ---
  duration_ms: 0.2225
  type: 'test'
  ...
# Subtest: the local success attempt is well-formed, and is at most one per invocation
ok 3146 - the local success attempt is well-formed, and is at most one per invocation
  ---
  duration_ms: 0.248792
  type: 'test'
  ...
# Subtest: classifyLocalAttempt reads what the SDK declared, and guesses nothing
ok 3147 - classifyLocalAttempt reads what the SDK declared, and guesses nothing
  ---
  duration_ms: 0.236542
  type: 'test'
  ...
# Subtest: the 429 rule has exactly one home — classifyLocalAttempt delegates, never duplicates
ok 3148 - the 429 rule has exactly one home — classifyLocalAttempt delegates, never duplicates
  ---
  duration_ms: 0.2615
  type: 'test'
  ...
# Subtest: the two terminal throws are BYTE-IDENTICAL to before this build
ok 3149 - the two terminal throws are BYTE-IDENTICAL to before this build
  ---
  duration_ms: 0.320541
  type: 'test'
  ...
# Subtest: the phase selector cannot drift from the throws it describes
ok 3150 - the phase selector cannot drift from the throws it describes
  ---
  duration_ms: 0.467708
  type: 'test'
  ...
# Subtest: the failure attach is a statement, not a control-flow change
ok 3151 - the failure attach is a statement, not a control-flow change
  ---
  duration_ms: 0.253792
  type: 'test'
  ...
# Subtest: nothing in the failure path touches the outbound request object
ok 3152 - nothing in the failure path touches the outbound request object
  ---
  duration_ms: 0.34625
  type: 'test'
  ...
# Subtest: the local calls are wrapped, and the SDK call expression itself is unchanged
ok 3153 - the local calls are wrapped, and the SDK call expression itself is unchanged
  ---
  duration_ms: 0.237959
  type: 'test'
  ...
# Subtest: failure evidence is a SEPARATE property, invisible to every success-attribution reader
ok 3154 - failure evidence is a SEPARATE property, invisible to every success-attribution reader
  ---
  duration_ms: 0.098125
  type: 'test'
  ...
# Subtest: the low-value-care judge reader is untouched — it reads two fields of the success shape
ok 3155 - the low-value-care judge reader is untouched — it reads two fields of the success shape
  ---
  duration_ms: 0.208625
  type: 'test'
  ...
# Subtest: tracedChat is not touched — D14 is scoped to the traceless arm
ok 3156 - tracedChat is not touched — D14 is scoped to the traceless arm
  ---
  duration_ms: 0.396459
  type: 'test'
  ...
# Subtest: every new name also resolves through lib/trace.ts, so no importer has to know the core path
ok 3157 - every new name also resolves through lib/trace.ts, so no importer has to know the core path
  ---
  duration_ms: 0.055833
  type: 'test'
  ...
# Subtest: failure evidence is IMMUTABLE — a later frame cannot rewrite what failed
ok 3158 - failure evidence is IMMUTABLE — a later frame cannot rewrite what failed
  ---
  duration_ms: 0.088292
  type: 'test'
  ...
# Subtest: a hostile or exotic error cannot break the transport
ok 3159 - a hostile or exotic error cannot break the transport
  ---
  duration_ms: 0.116542
  type: 'test'
  ...
# Subtest: failure evidence carries enums and counts only — no message, no body, no identifier
ok 3160 - failure evidence carries enums and counts only — no message, no body, no identifier
  ---
  duration_ms: 0.091917
  type: 'test'
  ...
# Subtest: the attempt shape admits the local provider, and nothing else new
ok 3161 - the attempt shape admits the local provider, and nothing else new
  ---
  duration_ms: 0.225375
  type: 'test'
  ...
# Subtest: a finish_reason defect is NOT retryable; an empty 200 still is
ok 3162 - a finish_reason defect is NOT retryable; an empty 200 still is
  ---
  duration_ms: 1.520167
  type: 'test'
  ...
# Subtest: THE 54 SECONDS: a truncating call is attempted ONCE, not three times
ok 3163 - THE 54 SECONDS: a truncating call is attempted ONCE, not three times
  ---
  duration_ms: 1.425333
  type: 'test'
  ...
# Subtest: …and the empty-200 retry budget is spent in full, exactly as before
ok 3164 - …and the empty-200 retry budget is spent in full, exactly as before
  ---
  duration_ms: 0.332542
  type: 'test'
  ...
# Subtest: the terminal error still names the truncation, so the sizing bug is readable
ok 3165 - the terminal error still names the truncation, so the sizing bug is readable
  ---
  duration_ms: 0.119625
  type: 'test'
  ...
# Subtest: transport failures are untouched by this rule — only BODY verdicts changed
ok 3166 - transport failures are untouched by this rule — only BODY verdicts changed
  ---
  duration_ms: 0.398458
  type: 'test'
  ...
# Subtest: the mini-sized cap is raised to a FLOOR on the bedrock path
ok 3167 - the mini-sized cap is raised to a FLOOR on the bedrock path
  ---
  duration_ms: 0.15375
  type: 'test'
  ...
# Subtest: ⚠️ BYTE-IDENTITY: the floor is the BEDROCK transport’s, and reaches no other provider
ok 3168 - ⚠️ BYTE-IDENTITY: the floor is the BEDROCK transport’s, and reaches no other provider
  ---
  duration_ms: 0.765292
  type: 'test'
  ...
# Subtest: a critique that never completed is recorded as UNAUDITED, not as clean
ok 3169 - a critique that never completed is recorded as UNAUDITED, not as clean
  ---
  duration_ms: 0.333666
  type: 'test'
  ...
# Subtest: the probe reducers carry critic_ran, so a lab row can tell the two apart
ok 3170 - the probe reducers carry critic_ran, so a lab row can tell the two apart
  ---
  duration_ms: 0.526208
  type: 'test'
  ...
# Subtest: unknown_finding + missing_critical + instability_input derive from a ClinicalState
ok 3171 - unknown_finding + missing_critical + instability_input derive from a ClinicalState
  ---
  duration_ms: 11.905667
  type: 'test'
  ...
# Subtest: med_contradiction derives from an open medication conflict (member stateRef)
ok 3172 - med_contradiction derives from an open medication conflict (member stateRef)
  ---
  duration_ms: 0.304708
  type: 'test'
  ...
# Subtest: med_contradiction: conflict on an episode HIGH-ALERT med is safety-critical
ok 3173 - med_contradiction: conflict on an episode HIGH-ALERT med is safety-critical
  ---
...
ok 3174 - med_contradiction also derives from reconciled status stopped/not_taking/unknown without a conflict row
  ---
  duration_ms: 0.547584
  type: 'test'
  ...
# Subtest: new_medication (B5): derives ONLY for meds absent from a NON-EMPTY snapshot med list
ok 3175 - new_medication (B5): derives ONLY for meds absent from a NON-EMPTY snapshot med list
  ---
  duration_ms: 0.395375
  type: 'test'
  ...
# Subtest: new_medication (B5): a high-alert episode med is skipped (it wins rank 0 anyway)
ok 3176 - new_medication (B5): a high-alert episode med is skipped (it wins rank 0 anyway)
  ---
  duration_ms: 0.084334
  type: 'test'
  ...
# Subtest: care_gap derives from a stale mapped-range abnormal (detail verbatim, severity mapped)
ok 3177 - care_gap derives from a stale mapped-range abnormal (detail verbatim, severity mapped)
  ---
  duration_ms: 0.373
  type: 'test'
  ...
# Subtest: followup_open derives from advice keywords; suppressed when a committed follow-up matches
ok 3178 - followup_open derives from advice keywords; suppressed when a committed follow-up matches
  ---
  duration_ms: 0.302041
  type: 'test'
  ...
# Subtest: allergy_unconfirmed only when the note allergy field is blank
ok 3179 - allergy_unconfirmed only when the note allergy field is blank
  ---
  duration_ms: 0.210792
  type: 'test'
  ...
# Subtest: snapshot absent ⇒ member-derived kinds simply absent (episode-only degradation, D14)
ok 3180 - snapshot absent ⇒ member-derived kinds simply absent (episode-only degradation, D14)
  ---
  duration_ms: 0.30825
  type: 'test'
  ...
# Subtest: determinism: identical inputs ⇒ deep-equal output (double run)
ok 3181 - determinism: identical inputs ⇒ deep-equal output (double run)
  ---
  duration_ms: 0.152417
  type: 'test'
  ...
# Subtest: every UnknownItem carries ≥1 sourceRef and a stateRef
ok 3182 - every UnknownItem carries ≥1 sourceRef and a stateRef
  ---
  duration_ms: 0.108291
  type: 'test'
  ...
# Subtest: stable ordering: safety before review before info, then kind, then subject
ok 3183 - stable ordering: safety before review before info, then kind, then subject
  ---
  duration_ms: 0.173417
  type: 'test'
  ...
# Subtest: bandFor thresholds
ok 3184 - bandFor thresholds
  ---
  duration_ms: 0.4375
  type: 'test'
  ...
# Subtest: findingPenalty scales with verdict severity and confidence
ok 3185 - findingPenalty scales with verdict severity and confidence
  ---
  duration_ms: 0.093209
  type: 'test'
  ...
# Subtest: a clean, complete episode scores high (band A)
ok 3186 - a clean, complete episode scores high (band A)
  ---
  duration_ms: 0.288542
  type: 'test'
  ...
# Subtest: domains route by tag; cost driven by low-value tariff spend; untagged → appropriateness
ok 3187 - domains route by tag; cost driven by low-value tariff spend; untagged → appropriateness
  ---
  duration_ms: 13.913375
  type: 'test'
  ...
# Subtest: estimated bed-day cost dents the cost domain even with no tariffed spend
ok 3188 - estimated bed-day cost dents the cost domain even with no tariffed spend
  ---
  duration_ms: 0.580459
  type: 'test'
  ...
# Subtest: weights are configurable and normalised
ok 3189 - weights are configurable and normalised
  ---
...
ok 3190 - cloudLadder: Vertex first, OpenRouter second — and GEMINI_VIA_OPENROUTER=1 inverts it
  ---
  duration_ms: 87.553291
  type: 'test'
  ...
# Subtest: cloudLadder: a second tier exists ONLY with a leg budget, and only when it can serve
ok 3191 - cloudLadder: a second tier exists ONLY with a leg budget, and only when it can serve
  ---
  duration_ms: 0.262333
  type: 'test'
  ...
# Subtest: the tier-2 slug derivation is the same google/ prefixing, flag or no flag
ok 3192 - the tier-2 slug derivation is the same google/ prefixing, flag or no flag
  ---
  duration_ms: 0.079208
  type: 'test'
  ...
# Subtest: the flag itself is NOT touched by this unit — one code read, no default, no write
ok 3193 - the flag itself is NOT touched by this unit — one code read, no default, no write
  ---
  duration_ms: 0.343041
  type: 'test'
  ...
# Subtest: tierCeilingMs: tier 1 gets the full budget, tier 2 the remainder, a spent leg gets 0
ok 3194 - tierCeilingMs: tier 1 gets the full budget, tier 2 the remainder, a spent leg gets 0
  ---
  duration_ms: 0.095416
  type: 'test'
  ...
# Subtest: a leg never exceeds its budget across both tiers — the naive sum would blow the box
ok 3195 - a leg never exceeds its budget across both tiers — the naive sum would blow the box
  ---
  duration_ms: 0.053834
  type: 'test'
  ...
# Subtest: ladderSkipError names the skipped tier and carries the earlier failure, capped
ok 3196 - ladderSkipError names the skipped tier and carries the earlier failure, capped
  ---
  duration_ms: 0.185208
  type: 'test'
  ...
# Subtest: both transports run the SAME ladder mechanics — no second budget idiom
ok 3197 - both transports run the SAME ladder mechanics — no second budget idiom
  ---
...
ok 3198 - F1: Vertex tier fails → the OpenRouter tier serves the SAME leg (the hop is real)
  ---
  duration_ms: 49.616875
  type: 'test'
  ...
# [provider-fallback] gemini gemini-2.5-pro failed → openrouter: {"provider":"gemini","label":"chatWithFallback","feature":null,"fellBackTo":"openrouter","intended_model":"gemini-2.5-pro","fallback_model":null,"region":"asia-south1","sa_identity":null,"inFlightAtError":1,"in_flight_by_provider":{"gemini":1},"http_status":null,"error_status":null,"error_code":null,"message":"GCP_SA_KEY is not valid JSON (or base64 JSON)","details":null}
# Subtest: F2: with NO leg budget there is NO second tier — the utility path is byte-identical
ok 3199 - F2: with NO leg budget there is NO second tier — the utility path is byte-identical
  ---
  duration_ms: 16.572458
  type: 'test'
  ...
...
ok 3200 - F3: noLocalFallback=true → both tiers failing THROWS; Ollama is not called
  ---
  duration_ms: 10.62375
  type: 'test'
  ...
# [provider-retry] openrouter google/gemini-2.5-pro attempt 1/1 http 500 — giving up: 500 "boom"
...
ok 3201 - F4: noLocalFallback absent → both tiers failing still falls back to Ollama (today's behaviour)
  ---
  duration_ms: 11.426
  type: 'test'
  ...
...
ok 3202 - F5: GEMINI_VIA_OPENROUTER=1 makes OpenRouter tier 1 — the inversion is live, not just typed
  ---
  duration_ms: 985.548
  type: 'test'
  ...
...
ok 3250 - a note with an ID absent from the vitals table is still no-vitals; one present is still covered
  ---
  duration_ms: 0.039708
  type: 'test'
  ...
# Subtest: the MEASURED window reproduces: 160 of 561 = 28.5%
ok 3251 - the MEASURED window reproduces: 160 of 561 = 28.5%
  ---
  duration_ms: 0.130709
  type: 'test'
  ...
# Subtest: rows outside the window are DROPPED — a boundary sliver is not a day
ok 3252 - rows outside the window are DROPPED — a boundary sliver is not a day
  ---
  duration_ms: 0.341708
  type: 'test'
  ...
# Subtest: Metabase type wobble is absorbed: string counts and ISO timestamps
ok 3253 - Metabase type wobble is absorbed: string counts and ISO timestamps
  ---
  duration_ms: 0.041167
  type: 'test'
  ...
# Subtest: junk never produces a number that looks real
ok 3254 - junk never produces a number that looks real
  ---
  duration_ms: 0.068083
  type: 'test'
  ...
# Subtest: the core is PURE and dependency-free — it must not reach the engine or any score
ok 3255 - the core is PURE and dependency-free — it must not reach the engine or any score
  ---
  duration_ms: 16.127
  type: 'test'
  ...
# Subtest: GATE 2 — flag OFF: all three fetch SQL strings are byte-identical to today's
ok 3256 - GATE 2 — flag OFF: all three fetch SQL strings are byte-identical to today's
  ---
  duration_ms: 0.859417
  type: 'test'
  ...
# Subtest: flag ON: vitals LEFT JOIN present, DISTINCT ON newest _update_time, scan bounded, quoted table
ok 3257 - flag ON: vitals LEFT JOIN present, DISTINCT ON newest _update_time, scan bounded, quoted table
  ---
  duration_ms: 0.288209
  type: 'test'
  ...
# Subtest: SWEEP-1 (D2) — the day fetch deduplicates by uid; the single/bulk uid fetches do NOT change
ok 3258 - SWEEP-1 (D2) — the day fetch deduplicates by uid; the single/bulk uid fetches do NOT change
  ---
  duration_ms: 0.098958
  type: 'test'
  ...
# Subtest: GATE 5 — no selected column ends in _tag, flag on or off (R-11: numbers, not judgments)
ok 3259 - GATE 5 — no selected column ends in _tag, flag on or off (R-11: numbers, not judgments)
  ---
  duration_ms: 0.113084
  type: 'test'
  ...
# Subtest: GATE 3 — synthetic control: a vitals row parses to the exact case shape
ok 3260 - GATE 3 — synthetic control: a vitals row parses to the exact case shape
  ---
  duration_ms: 0.918875
  type: 'test'
  ...
# Subtest: GATE 3 — no vitals row → vitalsRecorded false + vitals null (weight/height still mapped)
ok 3261 - GATE 3 — no vitals row → vitalsRecorded false + vitals null (weight/height still mapped)
  ---
...
ok 3262 - a record with every measurement blank is STILL vitalsRecorded true — a different finding from "no record"
  ---
  duration_ms: 0.09575
  type: 'test'
  ...
# Subtest: bp parse: null unless the string matches ^\\d+\\/\\d+$ (the raw string is kept as recorded)
ok 3263 - bp parse: null unless the string matches ^\\d+\\/\\d+$ (the raw string is kept as recorded)
  ---
  duration_ms: 0.105042
  type: 'test'
  ...
# Subtest: recordedAt: null when the note timestamp is missing (no wall clock ever leaks)
ok 3264 - recordedAt: null when the note timestamp is missing (no wall clock ever leaks)
  ---
...
ok 3265 - fail-safe: an error in the vitals leg resets to the safe state and leaves the rest of the case intact
  ---
  duration_ms: 0.360625
  type: 'test'
  ...
# Subtest: flag OFF: the A1 fields stay absent — every existing case literal and behaviour unchanged
ok 3266 - flag OFF: the A1 fields stay absent — every existing case literal and behaviour unchanged
  ---
  duration_ms: 0.079833
  type: 'test'
  ...
# Subtest: GATE 1 — opdCaseText is byte-identical with and without the vitals block (A1 is score-invariant)
ok 3267 - GATE 1 — opdCaseText is byte-identical with and without the vitals block (A1 is score-invariant)
  ---
  duration_ms: 0.556333
  type: 'test'
  ...
# Subtest: GATE 4 — OpdKeys carries no vitals field, no weight, no height
ok 3268 - GATE 4 — OpdKeys carries no vitals field, no weight, no height
  ---
...
# skipped 0
# todo 0
# duration_ms 19736.711
--- end output ---
EXIT: 0
ENDED: 2026-08-18T14:05:46Z
```
...
```text
COMMAND: cp lib/architecture/map.generated.ts /Users/vinaybhardwaj/cdmss-pass3-gate-18-aug-2026/gen1.ts
STARTED: 2026-08-18T14:06:25Z
--- output (stdout+stderr merged) ---
--- end output ---
EXIT: 0
...
```text
COMMAND: cmp lib/architecture/map.generated.ts /Users/vinaybhardwaj/cdmss-pass3-gate-18-aug-2026/gen1.ts
STARTED: 2026-08-18T14:06:26Z
--- output (stdout+stderr merged) ---
--- end output ---
EXIT: 0
ENDED: 2026-08-18T14:06:26Z
...
```text
GATE START 2026-08-18T14:05:26Z at commit 84040291ca9fcede333d0373c749d97ae58723e2
...
GATE END 2026-08-18T14:07:04Z
```

## Part 3 — evidence integrity

The EIGHT earlier evidence files, hashed at this pass — the six pinned by addendum v20 §6.1, the supplemental-3 file pinned
in commit f6e9188's message, and the supplemental-4 file pinned in commit 01c2375's message — every one must match:
...
All eight digests match their pinned values (v20 §6.1 for the first six; `f80c7591ad1cdd1df7dfaebed95c1c2575c0173d93282866742d494959c89b2a`
for the seventh, from commit f6e9188's message; `a5ff9ff20cfeb449c0b2fb58b07da4f872138e1d30c4fefad9ecb82ba6b7c3e9` for
the eighth, from commit 01c2375's message). No earlier evidence file, report, addendum or review was edited in this pass;
addendum v19 stands unchanged.
